...
{ 
  ...
  while (new_chain)
      ...
      else
	{
	  ...
	  cache_inserted++;
	  #ifdef SUPPORT_DNSROUTE
		/*find target address for the first reply which is the last one in the new_chain cache list*/		
		if( (new_chain->flags & (F_FORWARD|F_IPV4)) == (F_FORWARD|F_IPV4)){
			target = &new_chain->addr.addr.addr.addr4;
			//my_syslog(LOG_DEBUG, "target addr = %s\n", inet_ntoa(*target));
		}

	  if(target){
		addDnsRoute(target->s_addr, cache_get_name(new_chain));
	  }
	  #endif
	}
      new_chain = tmp;
