...
#ifdef SUPPORT_DNSROUTE /*ZyXEL, DNS ROUTE*/
DNSRoute_Entry_t DNSRoute[20];
...
int load_domain_name_table(void){
	...
	char dns_primary[4][16]={{0},{0},{0},{0}};	
	char dns_secondary[4][16]={{0},{0},{0},{0}};
	char dns_ifc[4][8]={{0},{0},{0},{0}};
	char gw[4][16]={{0},{0},{0},{0}};
	...
	while(fgets(line, sizeof(line), fp)!=NULL){
			...
			token = strtok(line, ":");
			memcpy(dns_ifc[i], token, 8);

			if(token = strtok(NULL, ":")){
				memcpy(gw[i], token, 16);
				if(token = strtok(NULL, ":")){
					second = strstr(token, ",");
					if(second!=NULL){
						*second = 0;
						...
						while((isspace(*second)) && (*second!=0))
								second++;
						memcpy(dns_secondary[i], second, 16);
					}
					memcpy(dns_primary[i], token, 16);
				}				
			}				
			memset(line, 0, 256);
			i++;
	...
	while(fgets(line, sizeof(line), fp2)!=NULL){
		...
		token2 = strtok(line, ":");
		memcpy(DNSRoute[i].domainName, token2, 128);		
		if(token2 = strtok(NULL, ":")){
			memcpy(DNSRoute[i].ifcName, token2, 16);			
			if(token2 = strtok(NULL, ":"))
				memcpy(DNSRoute[i].subnetMask, token2, 20);		
		}	
		memset(line, 0, 256);
		i++;
	}
	  ...
	  if (!iface)
		...
		struct timeval timeout;      
    		...
    		if (setsockopt (confd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,
                sizeof(timeout)) < 0){
		        my_syslog(LOG_ERR, ("setsockopt failed\n"));
    		}
