...
u_int32_t find_address_hash(u_int8_t *chaddr, int check_expired) 
{
	...
	seed = (rand()%(cur_iface->end - cur_iface->start))+1;
...
int check_ip(u_int32_t addr)
{
	...
	struct in_addr temp;
#ifdef MSTC_MODIFY  //__MSTC__,Lynn:sync from SinJia, TR-098 DHCP Conditional Serving Pool
...
void handleHostInfoUpdate(CmsMsgHeader *msg)
{
   ...
   if ((yiaddr & subnet_mask) == (cur_iface->server & subnet_mask))
   {
      if (leaseHost)
      {
         ...
         {
#ifdef BUILD_NORWAY_CUSTOMIZATION
            add_lease_MSTC(chaddr, yiaddr, server_config.offer_time, leaseHost->isStatic);
#else
