...
{
	int count = 0;
	char def_request_list[] = {DHCP_PARAM_REQ, 0, PARM_REQUESTS};
	char request_list[32]={0};
	
	def_request_list[OPT_LEN] = sizeof(def_request_list) - 2;
	count = sizeof(def_request_list) - 2;
	memcpy(request_list, def_request_list, sizeof(def_request_list));	
		
	if(ipv6rd_opt)
	{
		request_list[count+2] = DHCP_6RD_OPT;
		count++;
	}

	
	if(en_vendor_specific_info)
	{
		request_list[count+2] = DHCP_VSI;
		count++;
	}
	request_list[OPT_LEN] = count;
