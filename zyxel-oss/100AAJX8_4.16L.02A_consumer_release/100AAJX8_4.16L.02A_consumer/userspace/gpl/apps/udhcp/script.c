	...
	for(;;) {
		...
		case OPTION_IP:	/* Works regardless of host byte order. */
			...
			if (!strcmp(type_p->name, "dns")) {
             ...
             cmsUtl_strncat(dhcpc_opt.nameserver,sizeof(dhcpc_opt.nameserver), tmp);
			}
			if (!strcmp(type_p->name, "router"))
			    cmsUtl_strncpy(dhcpc_opt.gateway, tmp, sizeof(dhcpc_opt.gateway));
			if (!strcmp(type_p->name, "subnet"))
			    cmsUtl_strncpy(dhcpc_opt.mask, tmp, sizeof(dhcpc_opt.mask));
 			break;
		case OPTION_IP_PAIR:
