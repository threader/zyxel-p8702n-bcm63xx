...
#ifdef MSTC_MODIFY /* modify for DHCP Option */
	length = 1024;
#else
	length = 308;
#endif
...
#ifdef MSTC_MODIFY /* Enlarge timeout value of receive msg */
...
int createVIoption(int type, char *VIinfo)
{
  ...
  if (deviceOui[0] == '\0')
  {
     ...
     optionData[VENDOR_OPTION_CODE_OFFSET] = (char)VENDOR_IDENTIFYING_OPTION_CODE;
/* OPTION 125 */
#ifdef MSTC_MODIFY	/* The enterprise number should be 3561 (ADSL Forum) */
...
int saveVIoption(char *option, struct dhcpOfferedAddr *lease)
{
  char *optionPtr;
/* OPTION 125 */
#ifdef MSTC_MODIFY  /* The possible received VI info: OUI, serialNumber, productClass, modelName */
  int maxSubcode = 4;
#else
  int maxSubcode = 3;
#endif
...
#endif
  {
...
#endif
	{
     ...
     case VENDOR_GATEWAY_PRODUCT_CLASS_SUBCODE:
        ...
        break;
/* OPTION 125 */
#ifdef MSTC_MODIFY  /* Do nothing when parsing ModelName because it is not included in ManagementServer.ManageableDevice.{i}. */
