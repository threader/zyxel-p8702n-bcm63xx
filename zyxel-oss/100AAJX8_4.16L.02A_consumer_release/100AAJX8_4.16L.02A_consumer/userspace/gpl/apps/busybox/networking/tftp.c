   ...
   */
   ...
   if (glbCurPtr == NULL) 
   {
      ...
      if (cmsImg_isBcmTaggedImage(inBuf, &bcmImageSize))
      {
        ...
        /*In hostTools/bcmZyNandImageBuilder.c We add a empty blcok with size (TAG_BLOCK_LEN_NAND - TAG_LEN), MSTC_OBM_IMAGE_DEFAULT MSTC_ROM_D
          but bcmImageSize here just fs size.
          TODO: we should update bcmZyNandImageBuilder.c for totalImageLen
         */
         /*Workround: because can't get size of default configure, just reserved 128K here */
         allocSize += (/*TAG_BLOCK_LEN_NAND*/128*1024 + 128*1024);
#endif
