...
#ifdef MSTC_MODIFY //__MSTC__, Justin, Supoprt Tr143: DownloadDiagnostic
#include <time.h>
#include "cms_msg.h"
#include "cms_util.h"
#include "cms_log.h"
...
#ifdef MSTC_MODIFY //__MSTC__, Justin, Support TR143:DownloadDiagnstic
...
#ifdef MSTC_MODIFY//__MSTC__, Justin, Support TR143
void main_cleanup(void)
{
	char cmd[128];
...
//			bb_error_msg_and_die("bad redirection (no Location: header from server)");
    ...
    {
		getXSIDateTime(0, dateTimeBufBOM, sizeof(dateTimeBufBOM));
#ifdef MSTC_MODIFY//def WGET_DEBUG
		fprintf(stderr, "BOMTIME:%s\r\n", dateTimeBufBOM);
#endif
...
void catchIfStatistics(char *ifName, int *bytes)
{
		...
		while (fgets(buf, sizeof(buf), fp) != NULL) {
			...
			if ((ptr = strchr(buf, ':')) == NULL ||
			    (*ptr++ = 0, sscanf(buf, "%s", interfaceName) != 1)) {
