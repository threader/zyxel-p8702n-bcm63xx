/* Shared library add-on to ebtables to add policer support, MSTC Stan, 20100107 */
...
#ifdef MSTC_MODIFY//__MSTC__, Jones For compilation
...
#ifdef MSTC_MODIFY /* Init creditCap to check if the rule is new or not. __OBM__. MSTC, Stan Su, 20100611. */
	r->creditCap = 0;
#endif
                 ...
                 struct ebt_entry_match **match)
{
	...
	case '#':
		...
		if (ebt_check_inverse2(optarg)) {
			ebt_print_error2("Unexpected `!' after --mode");
		}
		if (strcmp(optarg, "tbf") == 0) {
			...
			ebt_print_error2("bad mode '%s'", optarg);
		}
		break;
	...
	case '%':
		if (r->policerMode == 0) {
			...
			if (ebt_check_inverse2(optarg)) {
				ebt_print_error2("Unexpected `!' after --policer");
			}
	...
	case '$':
		if (r->policerMode == 0) {
			...
			if (ebt_check_inverse2(optarg)) {
				ebt_print_error2("Unexpected `!' after --policer-burst");
			}
			str1 = optarg;
			r->burst = strtoul(str1, &remainder, 0);
			if (strcmp(remainder, "kb") !=0 || r->burst > 1000 || r->burst <= 0) {
				ebt_print_error2("bad --policer-burst `%s'", optarg);
			}
			break;
		}
		else {
	...
	case '1':
		if (r->policerMode == 1 || r->policerMode == 2) {
			...
			if (ebt_check_inverse2(optarg)) {
				ebt_print_error2("Unexpected `!' after --crate");
			}
			if (!parse_rate(optarg, &r->rate)) {
				ebt_print_error2("bad committed information rate '%s'", optarg);
			}
			break;
		}
		else {
	...
	case '2':
		if (r->policerMode == 1 || r->policerMode == 2) {
			...
			if (ebt_check_inverse2(optarg)) {
				ebt_print_error2("Unexpected `!' after --cbs-burst");
			}
			str1 = optarg;
			r->burst = strtoul(str1, &remainder, 0);
			if (strcmp(remainder, "kb") !=0 || r->burst > 1000 || r->burst <= 0) {
				ebt_print_error2("bad --cbs-burst `%s'", optarg);
			}
			break;
		}
		else {
	...
	case '3':
		if (r->policerMode == 2) {
			...
			if (ebt_check_inverse2(optarg)) {
				ebt_print_error2("Unexpected `!' after --prate");
			}
			if (!parse_rate(optarg, &r->pRate)) {
				ebt_print_error2("bad peak information rate '%s'", optarg);
			}
			if (r->rate > r->pRate) {
				ebt_print_error2("prate msut be equal or greater than crate");
			}
			break;
		}
		else {
	...
	case '5':
		if (r->policerMode == 1) {
			...
			if (ebt_check_inverse2(optarg)) {
				ebt_print_error2("Unexpected `!' after --ebs-burst");
			}
			str1 = optarg;
			r->pbsBurst = strtoul(str1, &remainder, 0);
			if (strcmp(remainder, "kb") !=0 || r->pbsBurst > 1000 || r->pbsBurst <= 0) {
				ebt_print_error2("bad --ebs-burst `%s'", optarg);
			}
			break;
		}
		else {
