...
#ifdef MSTC_MODIFY ////_MSTC_, Justin Lin, Support IPv6, Merge from Wuxi
/****************************************************************************
 Return true if a string could be an IPv4 or IPv6 address.
****************************************************************************/
uint32 interpret_addr_v4v6(struct sockaddr_storage *pss, const char *str)
{
	struct hostent *hp;
	uint32 res;

       if((hp=gethostbyname(str)) == NULL) {
            if((hp=gethostbyname2(str, AF_INET6)) == NULL) { /*check it is IPv6 or not*/
                return 0;
            }
       }
       if(hp->h_addrtype == AF_INET) {
            ((struct sockaddr_in *)pss)->sin_family=AF_INET;
            memcpy(&(((struct sockaddr_in *)pss)->sin_addr), hp->h_addr, hp->h_length);
       }else if(hp->h_addrtype == AF_INET6) {
            ((struct sockaddr_in6 *)pss)->sin6_family=AF_INET6;
            memcpy(&(((struct sockaddr_in6 *)pss)->sin6_addr), hp->h_addr, hp->h_length);
       }
...
BOOL is_myname_or_ipaddr(const char *s)
{
	...
	if (strequal(servername, "localhost")) 
		return True;
/*test by ming-hung    */
    if (strequal(servername, "192.168.1.1")) 
        return True;
