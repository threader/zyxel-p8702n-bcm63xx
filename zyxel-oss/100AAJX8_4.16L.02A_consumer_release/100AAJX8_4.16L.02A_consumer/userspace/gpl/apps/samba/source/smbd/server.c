	...
	if (lp_interfaces() && lp_bind_interfaces_only()) {
		...
		*/
		...
		for(i = 0; i < num_interfaces; i++) {
			...
			for (ptr=ports; next_token(&ptr, tok, " \t,", sizeof(tok)); ) {
				...
				s = fd_listenset[num_sockets] =
					open_socket_in_v4v6(SOCK_STREAM,
							port,
							num_sockets == 0 ? 0 : 2,
							ifss,
							true); 
		...
		for (sock_ptr=sock_addr;
				next_token(&sock_ptr, sock_tok, " \t,", sizeof(sock_tok)); ) {
			for (ptr=ports; next_token(&ptr, tok, " \t,", sizeof(tok)); ) {
				...
				/* open an incoming socket */
                		...
                		if (!interpret_addr_v4v6(&ss, sock_tok)) {
					continue;
				}
                        
				s = open_socket_in_v4v6(SOCK_STREAM,
						port,
						num_sockets == 0 ? 0 : 2,
						&ss,
						true);
