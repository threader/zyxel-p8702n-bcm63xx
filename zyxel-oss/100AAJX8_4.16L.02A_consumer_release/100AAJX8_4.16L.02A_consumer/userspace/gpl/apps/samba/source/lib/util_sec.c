...
void set_effective_uid(uid_t uid)
{
	...
	//DEBUG(0, ("assert_uid uid(%d) ",(int)uid));

	//assert_uid(-1, uid); //remove for testing 2009/04/22
}
...
#ifndef MSTC_MODIFY /*Eason, merge from samba 3.0.36, don't check gid*/
	assert_gid(-1, gid);
#endif
}
...
void restore_re_uid_fromroot(void)
{
	...
	DEBUG(0, ("resotre_re_uid assert_uid uid(%d,%d) ",(int)saved_ruid,saved_euid));
...
int set_re_uid(void)
{
	...
	DEBUG(0, ("resotre_re_uid assert_uid uid(%d,%d) ",(int)uid,uid));
	assert_uid(uid, uid);
	return 0;
}
...
void become_user_permanently(uid_t uid, gid_t gid)
{
	...
	DEBUG(0, ("become_user_permanentlyassert_uid uid(%d,%d) ",(int)uid,gid));
	assert_uid(uid, uid);
	assert_gid(gid, gid);
}
