...
#ifndef MSTC_MODIFY    /* __TO2__ , Hank , SAMBA can't work on MAC NB. */
	if(strlen(user_info->smb_name) == 0 || strlen(user_info->client_domain)==0  || strlen(user_info->internal_username)==0){
		DEBUG(0, ("check_ntlm_password:  userinfo ==NULL - cannot continue\n"));
		return NT_STATUS_LOGON_FAILURE;
	}
#endif
