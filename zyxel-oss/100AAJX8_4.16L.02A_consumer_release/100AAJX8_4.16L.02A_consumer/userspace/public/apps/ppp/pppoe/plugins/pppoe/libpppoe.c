...
#ifdef MSTC_MODIFY //__MSTC__, Jeff, support vlanautohunt
	if(vlanautohunt == 1 && ses->retransmits >= ses->retries)
	{
		printf("VLANAUTOHUNT: ppp timeout\n");
		sendMsgToVAH(CMS_MSG_VLANAUTOHUNT_TIMEOUT);
		kill(getpid(), SIGTERM);
		sleep(1000);
	}
#endif
