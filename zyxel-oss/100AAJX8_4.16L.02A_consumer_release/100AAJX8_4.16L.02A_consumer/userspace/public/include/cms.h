...
typedef enum
{
   ...
   CMSRET_DU_UPDATE_VERSION_EXISTED= 9032,/**< Invalid Deployment Unit Update, version already exists
                                                    * (associated with ChangeDUState:update)
                                                    */
#ifdef MSTC_MODIFY // __MSTC__, Nick Tseng, Login Privilege
...
#ifdef MSTC_MODIFY //__MSTC__, TengChang, for firmwareUpgrade.html
...
#ifdef MSTC_MODIFY //__MSTC__, Amy
typedef enum
{
   LSRET_PASS     = 0,
   LSRET_LOGINFAIL= 1,
...
#ifdef MSTC_MODIFY //__MSTC__, Dennis, 3 group privilege size
...
/* Some defines for selecting which address family (IPv4 or IPv6) we
 * want.  Note the values are chosen to be compatible with the isIPv4 flags
 * that are currently in use.
 */
#define CMS_AF_SELECT_IPV6       0
#define CMS_AF_SELECT_IPV4       1
#define CMS_AF_SELECT_IPVX       2

#define IS_CMS_AF_ALLOW_IPV4 (ipvx)  (((ipvx) == CMS_AF_SELECT_IPV4) || ((ipvx) == CMS_AF_SELECT_IPVX))
#define IS_CMS_AF_ALLOW_IPV6 (ipvx)  (((ipvx) == CMS_AF_SELECT_IPV6) || ((ipvx) == CMS_AF_SELECT_IPVX))
...
#ifdef MSTC_MODIFY //__MSTC__, DingRuei, Run time chage Web UI
/*Web UI Style Type*/
