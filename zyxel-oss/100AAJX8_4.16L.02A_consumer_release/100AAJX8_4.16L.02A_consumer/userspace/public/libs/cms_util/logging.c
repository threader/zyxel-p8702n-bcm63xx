   ...
   if (level <= logLevel)
   {
      ...
      if (logHeaderMask & CMSLOG_HDRMASK_APPNAME)
      {
         if (NULL != gAppName)
         {
            ...
            syslog_len = snprintf(syslog_buf, maxLen, "%s:", gAppName);
         }
         else
         {
            len = snprintf(buf, maxLen, "%d:", cmsEid_getPid());
            syslog_len = snprintf(syslog_buf, maxLen, "%d:", cmsEid_getPid());
         }
      }
...
#ifdef MSTC_MODIFY /*Support multi dest of cms log, __FTTB8-CHT__, MitraStar Peien, 20121030*/
