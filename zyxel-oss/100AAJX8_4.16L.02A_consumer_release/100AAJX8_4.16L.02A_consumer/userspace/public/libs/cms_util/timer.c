...
#ifdef MSTC_MODIFY //__MSTC__, Dennis sync from jhenyang
CmsRet cmsTmr_resetTimeByName(void *handle, UINT32 ms, const char *name)
{
   CmsTimerHandle *tmrHandle = (CmsTimerHandle *) handle;
   CmsTimerEvent *tmrEvent;
   void *ctxData;
   ...
   while ((tmrEvent != NULL) && (!found))
   {
      if (strcmp(tmrEvent->name, name)==0)
      {
         found = TRUE;
      }
      else
   ...
   if (found)
   {
      /* replace the one in list with this one. */
      ctxData = tmrEvent->ctxData;
      func = tmrEvent->func;
      cmsTmr_cancel(tmrHandle, func, tmrEvent->ctxData);
      return(cmsTmr_set(handle, func, ctxData, ms, name));   
   } /* found */
