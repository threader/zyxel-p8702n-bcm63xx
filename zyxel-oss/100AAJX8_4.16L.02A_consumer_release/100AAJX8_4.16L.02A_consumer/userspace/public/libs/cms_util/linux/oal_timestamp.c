...
CmsRet oalTms_getXSIDateTime(UINT32 t, char *buf, UINT32 bufLen)
{
   ...
   c = strftime(buf, bufLen, "%Y-%m-%dT%H:%M:%SZ", tmp);
   if ((c == 0) || (c+1 > bufLen))
   {
      /* buf was not long enough */
      return CMSRET_RESOURCE_EXCEEDED;
   }
#elif defined(SUPPORT_MSTC_SNTP) //__MSTC__, kenny, for SNTP
    ...
    FILE* fs = NULL;
	char timeZone[BUFLEN_8+1] = {0};
	if ((fs = fopen("/var/localtimezone", "r")) == NULL) {
		strcat ( buf, "+00:00" );
	}
	else {
		fgets(timeZone, BUFLEN_8, fs);
		strcat ( buf, timeZone );
	    fclose(fs);
	}
	buf[c+6] = '\0';
