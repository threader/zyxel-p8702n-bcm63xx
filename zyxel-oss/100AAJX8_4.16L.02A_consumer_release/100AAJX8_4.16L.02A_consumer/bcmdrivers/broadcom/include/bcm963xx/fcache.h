...
#define MDNAT_IN_INVALID            0x0F    /* Incarnation 0x0F is invalid    */
...
#define FCACHE_RTP_SEQ_GROUP_SIZE   2       /* # of flows in RTP seq group    */
#define FCACHE_RTP_HDR_TS_OFFSET    4       /* RTP time stamp offset          */
...
#define FCACHE_RTP_SEQ_MIN_SEQ      1       /* Min # of sequential packets    */
 ...
 *  - 16bit unused
 *  -  4bit incarnation id (avoid latent access)
 *  - 12bit entry id (index of entry in FDB cache table)
 *------------------------------------------------------------------------------
 */
typedef struct {
    union {
 ...
 *  -  4bit incarnation id (avoid latent access)
 *  - 12bit entry id (index of entry in flow cache table)
 *------------------------------------------------------------------------------
 */
typedef struct {
    union {
...
struct flow_t {
    ...
    union {
       uint8_t      flags;
       struct {
          uint8_t   rtp_seq_chk  :1;   /* mcast RTP Sequence check enabled    */
          uint8_t   new_flow     :1;   /* Flow is considered "new" if it is 
