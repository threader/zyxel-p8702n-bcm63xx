...
#ifndef MSTC_MODIFY //__MTSC__, Delon Yu
...
static void *retriedKmalloc(size_t size)
{
    ...
    if (!in_interrupt() ) {
      ...
      /* temparory disable the bug check for 63138, the flash update code disable_bh call
         somehow set the interrupt bit, need to fix it after root caused */
...
#if defined(CONFIG_BCM96362) || defined(CONFIG_BCM96328) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM96838) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM963381)
...
#ifdef MSTC_MODIFY /* __MSTC__, zongyue: to fix ImageDefault start postion and RootFS size if NAND FLASH have bad blocks */
void kerSysFlashBadBlockNumSet(int number)
{
    fInfo.flash_rootfs_bad_block_number = number;
}
#endif
#ifdef MSTC_MODIFY //__MSTC__, Dennis merge from Eleana
...
int otp_is_boot_secure(void)
{
    int boot_secure = (*((uint32_t *)(OTP_BASE + OTP_SHADOW_ADDR_BTRM_ENABLE_CUST_ROW)));
    boot_secure = (boot_secure & OTP_CUST_BTRM_BOOT_ENABLE_MASK) >> OTP_CUST_BTRM_BOOT_ENABLE_SHIFT;
    if (boot_secure)
    {
    ...
    int should_yield )
{
	 ...
	 * the file to be updated is secram.000 and not cferam.000
        ...
        if( 0 != ofs ) 
        {
            /* These targets can support pre/full secure boot, which means the "nvram" mtd device may be bigger than */
            /* just one block (block 0). Sw upgrade from within linux assumes migration from unsecure (no bootrom    */
            /* running) to presecure (bootrom runs and "nvram" partition is > 1 block) took place using cferam's     */
            /* writeWholeImage(). SW upgrade from Linux will not perform this migration. Therefore, check that the   */
            /* new image contains no headered cferoms if the "nvram" mtd device is only one block in size. Also check*/
            /* that the new sw image has a JFFS2_MAGIC_BITMASK at the appropriate spot if the "nvram" mtd is larger  */
            /* than one block (ie a pre/full secure target is being upgraded with a secure-capable sw image)         */
            struct mtd_info *mtd1 = get_mtd_device_nm("nvram");
            if( !IS_ERR_OR_NULL(mtd1) )
            {
                uint32_t *pHdr = (uint32_t *)image_ptr;
		int imageMatchesPartition = 1;
                pHdr += (mtd1->erasesize / 4); /* pHdr points to right after the headerless cferom */
                ...
                { 
                    ...
                    /* Assume the image does not match the partition table at first*/
		    if (imageMatchesPartition == 1)
		        imageMatchesPartition = 0;

                    if ((*pHdr       == (uint32_t)BTRM_SBI_UNAUTH_MGC_NUM_1) &&
                       (*(pHdr + 1) == (uint32_t)BTRM_SBI_UNAUTH_MGC_NUM_2))
                    {
                        /* This is good, image matches partition table */
		        imageMatchesPartition = 2;
                    }
                    pHdr += (mtd1->erasesize / 4);
                }
   
                if (imageMatchesPartition)
                {
                    /* So far so good */
...
#ifdef MSTC_MODIFY // __MSTC__, __MSTC__, Allen
