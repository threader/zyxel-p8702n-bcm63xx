...
Boolean SIMIO_Init(SIMIO_ID_t id, int vcc_polarity)
{
	...
	dev->sim_pedestal_id = PEDESTAL_AllocId();
        ...
        /*sim card pinmuxing*/
        if(BpGetSimInterfaces(&MuxSel) == BP_SUCCESS) 
        {   
...
//                set_pinmux(PINMUX_SIM_B_VCC_VOL_SEL_PIN, PINMUX_SIM_FUNC);
        ...
        {
            printk (KERN_EMERG "smart card err: can't pinmux!!! %s:%d\n",__FILE__, __LINE__ );
            return FALSE;
        }
...
int simio_activesim(SIMIO_ID_t id, UInt8 *data, int len)
{
    ...
    memcpy(data, dev->ATRRxBuffer, dev->ATRRxCount);
