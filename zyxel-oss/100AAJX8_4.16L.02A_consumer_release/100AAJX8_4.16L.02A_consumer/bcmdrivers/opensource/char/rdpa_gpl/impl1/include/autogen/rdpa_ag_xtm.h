 ...
 *
 * This handle should be passed to bdmf_new_and_set() function in
 * order to create xtmchannel object
 ...
 *
 * This handle should be passed to bdmf_new_and_set() function in
 * order to create xtmflow object
...
typedef enum {
    ...
    rdpa_xtmflow_attr_stat = 3, /* stat : R : aggregate/24 xtmflow_stat(rdpa_xtmflow_stat_t) : xtmflow statistics */
 ...
 * \return 0 or error code < 0
 */
static inline int rdpa_xtmflow_ptmBonding_get(bdmf_object_handle mo_, bdmf_number *ptmBonding)
{
    bdmf_number _nn_;
    int _rc_;
    _rc_ = bdmf_attr_get_as_num(mo_, rdpa_xtmflow_attr_ptmBonding, &_nn_);
    *ptmBonding = (bdmf_number)_nn_;
    return _rc_;
}
