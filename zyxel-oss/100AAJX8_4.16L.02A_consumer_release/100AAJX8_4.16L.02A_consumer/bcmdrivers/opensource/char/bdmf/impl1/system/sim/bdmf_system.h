...
/* Fast lock/unlock */
typedef bdmf_ta_mutex bdmf_fastlock;
typedef bdmf_ta_mutex bdmf_reent_fastlock;
#define DEFINE_BDMF_FASTLOCK(lock) bdmf_fastlock lock = __BDMF_TA_MUTEX_INITIALIZER(lock)
#define bdmf_fastlock_init(plock)  bdmf_ta_mutex_init(plock)
#define bdmf_fastlock_lock(plock)  bdmf_ta_mutex_lock(plock)
#define bdmf_fastlock_unlock(plock) bdmf_ta_mutex_unlock(plock)
#define bdmf_fastlock_lock_irq(plock, flags) \
    do { \
        bdmf_ta_mutex_lock(plock); \
        flags = 0; \
    } while (flags)
#define bdmf_fastlock_unlock_irq(plock, flags)  bdmf_ta_mutex_unlock(plock)
