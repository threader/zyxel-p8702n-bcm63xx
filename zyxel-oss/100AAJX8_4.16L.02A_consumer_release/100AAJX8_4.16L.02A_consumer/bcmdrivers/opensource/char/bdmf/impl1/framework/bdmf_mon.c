                          ...
                          const bdmfmon_cmd_parm_t parm[],  uint16_t n_parms)
{
    ...
    bdmf_subsystem_t local_subs = bdmf_subsystem();
    while((drv=bdmf_type_get_next(drv)))
    {
        if (!drv->po)
        {
            if ((drv->subsystem != local_subs) && (drv->subsystem != BDMF_SUBSYSTEM_LOCAL))
            {
                char name[32];
                snprintf(name, sizeof(name), "%s@%s", drv->name, bdmf_subsystem_name(drv->subsystem));
                bdmf_session_print(session, "%-16s %s\n", name, drv->description);
            }
            else
                bdmf_session_print(session, "%-16s %s\n", drv->name, drv->description);
                            ...
                            const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    if (rc || !drv)
        return rc;
    if (drv->subsystem != BDMF_SUBSYSTEM_LOCAL)
        bdmf_session_print(session, "Objects of type %s@%s (%s)",
                    drv->name, bdmf_subsystem_name(drv->subsystem), drv->description);
    else
...
/* Connect to remote susbsystem
    BDMFMON_MAKE_PARM_ENUM("subsystem", "Remote subsystem", subsystem_type_table, 0),
*/
static int bdmf_mon_rpc_connect(bdmf_session_handle session,
                               const bdmfmon_cmd_parm_t parm[],  uint16_t n_parms)
{
    bdmf_subsystem_t subs = (bdmf_subsystem_t)parm[0].value.number;
    return bdmfproxy_connect(subs);
}

/* Disconnect from remote susbsystem
    BDMFMON_MAKE_PARM_ENUM("subsystem", "Remote subsystem", subsystem_type_table, 0),
*/
static int bdmf_mon_rpc_disconnect(bdmf_session_handle session,
                               const bdmfmon_cmd_parm_t parm[],  uint16_t n_parms)
{
    bdmf_subsystem_t subs = (bdmf_subsystem_t)parm[0].value.number;
    return bdmfproxy_disconnect(subs);
}

#endif /* #ifdef BDMF_RPC */
...
bdmfmon_handle_t bdmf_flow_mon_init(bdmfmon_handle_t root_dir)
{
...
#ifdef BDMF_RPC
    rpc_dir = bdmfmon_dir_add(bdmf_dir, "remoting",
                             "Remoting support",
                             BDMF_ACCESS_ADMIN, NULL);

    {
        ...
        bdmfmon_cmd_add(rpc_dir, "connect", bdmf_mon_rpc_connect,
                      "Connect to remote susbsystem",
                      BDMF_ACCESS_ADMIN, NULL, parms);
    }
    {
        ...
        bdmfmon_cmd_add(rpc_dir, "disconnect", bdmf_mon_rpc_disconnect,
                      "Disconnect from remote susbsystem",
                      BDMF_ACCESS_ADMIN, NULL, parms);
    }
#endif
