        ...
        bdmf_index *index, const char *val, int is_add)
{
    ...
    if (rc > 0)
        rc = 0;
    /* Reduce object reference incremented by a_to_val */
    if (attr->type == bdmf_attr_object)
    {
        struct bdmf_object *ref_obj = *(struct bdmf_object **)int_val;
        if (ref_obj)
            bdmf_put(ref_obj);
    }
    return rc;
...
#ifdef BDMF_RPC
    if (mo->drv->subsystem != BDMF_SUBSYSTEM_LOCAL)
    return bdmfproxy_configure(mo, set);
#endif
