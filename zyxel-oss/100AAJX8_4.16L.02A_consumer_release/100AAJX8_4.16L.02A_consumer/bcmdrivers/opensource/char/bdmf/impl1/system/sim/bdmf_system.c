...
static int bdmf_subs_pid[BDMF_SUBSYSTEM__NUMBER_OF];
static int bdmf_subs_stale_pid[BDMF_SUBSYSTEM__NUMBER_OF];
int bdmf_lastpid;

static int bdmf_get_subs_pid(int subs)
{
    FILE *f;
    char fname[32];
    int pid;

    if (bdmf_subs_pid[subs])
        return bdmf_subs_pid[subs];
    ...
    fscanf(f, "%d", &pid);
    fclose(f);
    if (pid == bdmf_subs_stale_pid[subs])
    {
        char sysstr[64];
        snprintf(sysstr, sizeof(sysstr), "rm -f %s", fname);
        system(sysstr);
        return 0;   /* stale pid file */
    }
    bdmf_subs_pid[subs] = pid;
    return pid;
}

void bdmf_irq_raise(int subs, int irq)
{
    int pid = bdmf_get_subs_pid(subs);
    union sigval value;
    int rc;

    if (!pid)
        bdmf_print_error("Failed to raise irq%d on subs %s. No target pid. Waiting..\n",
            irq, bdmf_subsystem_name(subs));

again:
    /* wait for another process to start */
    while(!(pid = bdmf_get_subs_pid(subs)))
        usleep(1);
    value.sival_int = irq;
    ...
    if (rc)
    {
        ...
        if (rc == ESRCH)
        {
            bdmf_subs_stale_pid[subs] = pid;
            bdmf_subs_pid[subs] = 0;
            goto again;
        }
    }
}
