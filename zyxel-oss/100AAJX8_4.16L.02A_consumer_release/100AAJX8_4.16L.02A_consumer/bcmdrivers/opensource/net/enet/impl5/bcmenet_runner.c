...
static rdpa_filter lan_filters[] = { RDPA_FILTER_ETYPE_ARP, RDPA_FILTER_BCAST, RDPA_FILTER_IP_FRAG, RDPA_FILTER_IGMP
#ifdef CONFIG_BCM_PTP_1588
    ,RDPA_FILTER_ETYPE_PTP_1588, RDPA_FILTER_L4_PTP_1588
#endif
};
static rdpa_filter gbe_filters[] = { RDPA_FILTER_ETYPE_ARP, RDPA_FILTER_IP_FRAG };
...
static int bcme_create_rdpa_wan_gbe_port(void)
{
    ...
    rdpa_port_emac_cfg_set(rdpa_port_attrs, &port_emac_cfg);
...
int bcmeapi_create_vport(struct net_device *dev)
{
    ...
    if ((init_cfg.wan_type == rdpa_wan_gbe || init_cfg.wan_type == rdpa_wan_active_enet)
...
#if defined(NETDEV_CPU_HI_RX_QUEUE_ID)
        && reason_cfg->queue != NETDEV_CPU_HI_RX_QUEUE_ID
#endif /* NETDEV_CPU_HI_RX_QUEUE_ID */
        )
    {
        return 1;
    }
...
#endif
#if !defined(CONFIG_BCM963138) && !defined(CONFIG_BCM963148)
    case rdpa_wan_active_enet:
        wan_port_id = AE_PORT_ID;
    break;
#endif
    case rdpa_wan_gbe:
...
#if defined(NETDEV_CPU_HI_RX_QUEUE_ID)
    else if (rdpa_cpu_queue_not_empty(rdpa_cpu_host,NETDEV_CPU_HI_RX_QUEUE_ID))
    {
        rdpa_cpu_int_disable(rdpa_cpu_host, NETDEV_CPU_HI_RX_QUEUE_ID);
        rdpa_cpu_int_clear(rdpa_cpu_host, NETDEV_CPU_HI_RX_QUEUE_ID);
        rdpa_cpu_int_disable(rdpa_cpu_host, NETDEV_CPU_RX_QUEUE_ID);
...
int bcmeapi_fttdp_init_cfg(int physical_port)
{
    ...
    rdpa_port_emac_cfg_t lag_port_emac_cfg = {};
    ...
    rdpa_port_emac_cfg_get(lag_port_obj, &lag_port_emac_cfg);
    lag_port_emac_cfg.enable = 1;
    lag_port_emac_cfg.mode = rdpa_emac_mode_rgmii; /* XXX: other ports ???? */
    lag_port_emac_cfg.emac_param.rate = rdpa_emac_rate_1g;
    rdpa_port_emac_cfg_set(lag_port_obj, &lag_port_emac_cfg);
