...
void ethsw_port_based_vlan(int port_map, int wan_port_map, int txSoftSwitchingMap)
{
    ...
    for (port = 0; port < TOTAL_SWITCH_PORTS; port++)
    {
        ...
        if (port_map & (1 << port))
        {
            ...
            {
                value = DEFAULT_PBVLAN_MAP & (~wan_port_map) & (~txSoftSwitchingMap);
            }
...
#endif
    value |= PBMAP_MIPS;
    value |= txSoftSwitchingMap;
    ethsw_wreg(PAGE_CONTROL, REG_DISABLE_LEARNING, (uint8 *)&value, 2);

    ageMap = wan_port_map | txSoftSwitchingMap;
    for(port = 0; port < TOTAL_SWITCH_PORTS; port++)
    {
        if((ageMap >> port) & 0x1)
        {
            fast_age_port(port, 0);
        }
    }
...
void ethsw_set_wanoe_portmap(uint16 wan_port_map, int txSoftSwitchingMap)
{
    ...
    map &= ( 1 << SAR_PORT_ID );
    map |= PBMAP_MIPS | wan_port_map | txSoftSwitchingMap;
#else
    map = PBMAP_MIPS | wan_port_map | txSoftSwitchingMap;
#endif
...
void bcmeapi_ethsw_init_hw(int unit, uint32_t portMap,  int wanPortMap)
{
...
#endif
#ifdef MSTC_MODIFY //__MSTC__, DelonYu, merge patch from CSP#448131
	uint16 x16;//leo
#endif
#if !defined(SUPPORT_SWMDK)
    ...
    if (unit == 1)  /* External switch is present - decided at run time */
    {
        ...
        {
            ethsw_set_wanoe_portmap(GET_PORTMAP_FROM_LOGICAL_PORTMAP(wanPortMap, 0), 0);
        }
#endif
    }
    else
...
#if defined (MSTC_MODIFY) && !defined(BUILD_MSTC_DSL_2492HNAU_B1BC) //__MSTC__, DelonYu, merge patch from CSP#448131 for DSL-491GNU-B1D C3 board
	//internal GPHY (EthernetWAN)

	x16 = 0x0200;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(3), 0x9, &x16); // Enable 1000M capability for internal GPHY by default
	x16 = 0x8077;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(3), 0x18, &x16); // Enable Ethernet@wirespeed for internal GPHY by default

	ethsw_phy_rreg(ETHSW_PHY_GET_PHYID(3), 0, &x16);
	x16 = x16|0x1800;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(3), 0, &x16 );
#endif
...
#endif
#ifdef MSTC_MODIFY //__MSTC__, DelonYu, Reset PHY on RGMII1 interface for HPNA bug CSP#522628
	x16 = 0x8000;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(4), 0x0, &x16);
#endif
