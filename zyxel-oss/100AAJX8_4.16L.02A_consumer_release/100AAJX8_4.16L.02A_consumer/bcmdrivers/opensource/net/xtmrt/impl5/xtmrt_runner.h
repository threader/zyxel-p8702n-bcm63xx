...
static inline void bcmxapi_napi_post(UINT32 mask);
...
/*---------------------------------------------------------------------------
 * void bcmxapi_napi_post(UINT32 mask)
 * Description:
...
static inline void bcmxapi_napi_post(UINT32 mask)
{
/* Since bcmxtmrt_rxtask service each cpu queue until empty,
 * the condition stated below should not exist.
 */
#if 0
   rdpa_cpu_rx_stat_t cpu_rx_hi;
   rdpa_cpu_rx_stat_t cpu_rx_lo;
    ...
    */
   rdpa_cpu_rxq_stat_get(rdpa_cpu_obj, RDPA_XTM_CPU_HI_RX_QUEUE_ID, &cpu_rx_hi);
   rdpa_cpu_rxq_stat_get(rdpa_cpu_obj, RDPA_XTM_CPU_LO_RX_QUEUE_ID, &cpu_rx_lo);
   
   if (cpu_rx_hi.queued == NETDEV_CPU_RX_QUEUE_SIZE ||
       cpu_rx_lo.queued == NETDEV_CPU_RX_QUEUE_SIZE)
   {
