...
#define SAR_TX_QUEUE_SIZE_EXTENSION 128   //JJC: temp
...
static int __is_dsl_mode(void)
{
    ...
    if(!rc)
    {
        rc = rdpa_system_init_cfg_get(system_obj, &system_init_cfg);
        if(!rc)
        {
            rc = system_init_cfg.wan_type == rdpa_wan_dsl ? 1 : 0;
        }
    }
...
int bcmxapi_module_init(void)
{
   ...
   if (!__is_dsl_mode())
   {
       printk("%s: ETH WAN Detected\n", __FUNCTION__);

       return rc;
   }

   printk("%s: DSL WAN Detected\n", __FUNCTION__);
...
static void cpu_rxq_isr(int queue_id)
{
   ...
   /* schedule napi for each device that is open */
 ...
 * Description:
 *    xtm receive task called by napi poll function.
 * Returns:
...
UINT32 bcmxapi_rxtask(UINT32 ulBudget, UINT32 *pulMoreToDo)
{
   ...
   */
   do
   {
      ...
      rc = rdpa_cpu_packet_get(rdpa_cpu_host, RDPA_XTM_CPU_HI_RX_QUEUE_ID,
                               (bdmf_sysb *)&skb, &info);
      if (rc == BDMF_ERR_NO_MORE)
      {
         /* Receive packet from low priority cpu rx queue */
         rc = rdpa_cpu_packet_get(rdpa_cpu_host, RDPA_XTM_CPU_LO_RX_QUEUE_ID,
                                  (bdmf_sysb *)&skb, &info);
         if (rc == BDMF_ERR_NO_MORE)
         {
...
#ifdef MSTC_MODIFY //MSTC_VCAUTOHUNT
