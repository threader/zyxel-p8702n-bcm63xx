		 ...
		 enum nf_nat_manip_type maniptype)
{
	...
	*tuple = *orig_tuple;
	...
	if (!(range->flags & NF_NAT_RANGE_PROTO_RANDOM)) {
		if (range->flags & NF_NAT_RANGE_PROTO_SPECIFIED) {
#ifdef CONFIG_MSTC_MODIFY /* Support one-to-one port mapping. __TELEFONICA__, MSTC Stan Su, 20120308. */
			...
			if (( !range->mappingFlag && proto->in_range(tuple, maniptype, &range->min,
					    &range->max) ) &&
#else			
