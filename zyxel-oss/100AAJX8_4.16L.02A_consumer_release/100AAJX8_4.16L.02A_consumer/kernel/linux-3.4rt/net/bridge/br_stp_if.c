...
bool br_stp_recalculate_bridge_id(struct net_bridge *br)
{
	...
	list_for_each_entry(p, &br->port_list, list) {
#ifdef CONFIG_MSTC_MODIFY // __MSTC__, Paul Ho
		if (((br->dev->name[0] == 'b' && br->dev->name[1] == 'r' && br->dev->name[2] != '0') || 
			strncmp(p->dev->name, "wl0.", 4) != 0) &&
		    (addr == br_mac_zero || memcmp(p->dev->dev_addr, addr, ETH_ALEN) < 0))
#else	
		if (addr == br_mac_zero ||
