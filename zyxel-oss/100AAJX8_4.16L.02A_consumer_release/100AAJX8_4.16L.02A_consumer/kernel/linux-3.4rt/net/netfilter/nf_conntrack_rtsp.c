...
int look_existing_port(struct nf_conn *ct, enum ip_conntrack_info ctinfo,struct sk_buff *skb)
{
	int ret = 0;
	unsigned int i =0;
	...
	//printk(KERN_INFO"look_existing_port: dir=%d\r\n",dir);
	//printk(KERN_INFO"[look_existing_port ct] %u.%u.%u.%u:%u-%u.%u.%u.%u:%u  %u.%u.%u.%u:%u-%u.%u.%u.%u:%u	  \n",
				//NIPQUAD( ct->tuplehash[0].tuple.src.u3.ip), ntohs( ct->tuplehash[0].tuple.src.u.udp.port),
				//NIPQUAD( ct->tuplehash[0].tuple.dst.u3.ip), ntohs( ct->tuplehash[0].tuple.dst.u.udp.port),	
				//NIPQUAD( ct->tuplehash[1].tuple.src.u3.ip), ntohs( ct->tuplehash[1].tuple.src.u.udp.port),
				//NIPQUAD( ct->tuplehash[1].tuple.dst.u3.ip), ntohs( ct->tuplehash[1].tuple.dst.u.udp.port)); 

	for (; i < net->ct.htable_size; i++) {	
	    hlist_nulls_for_each_entry(h, n, &net->ct.hash[i], hnnode) {
			ct1 = nf_ct_tuplehash_to_ctrack(h);

			//printk(KERN_INFO"[look_existing_port 1] %u.%u.%u.%u:%u-%u.%u.%u.%u:%u  %u.%u.%u.%u:%u-%u.%u.%u.%u:%u	  \n",
			//	NIPQUAD( ct1->tuplehash[0].tuple.src.u3.ip), ntohs( ct1->tuplehash[0].tuple.src.u.udp.port),
			//	NIPQUAD( ct1->tuplehash[0].tuple.dst.u3.ip), ntohs( ct1->tuplehash[0].tuple.dst.u.udp.port),	
			//	NIPQUAD( ct1->tuplehash[1].tuple.src.u3.ip), ntohs( ct1->tuplehash[1].tuple.src.u.udp.port),
			//	NIPQUAD( ct1->tuplehash[1].tuple.dst.u3.ip), ntohs( ct1->tuplehash[1].tuple.dst.u.udp.port)); 
		
			if ((rtsp_new_port ==ct1->tuplehash[0].tuple.dst.u.udp.port) ||(rtsp_new_port ==ct1->tuplehash[1].tuple.src.u.udp.port)){
				printk(KERN_INFO"[look_existing_port 1] Found exist port: [%hu]",rtsp_new_port);
				ret = 1;
				return ret;
			}
            }
...
int get_available_port(struct nf_conn *ct,enum ip_conntrack_info ctinfo, int portRangIndex ,struct sk_buff *skb)
{
...
again:
	...
	for (rtsp_new_port=rtsp_old_port; rtsp_new_port<=rtsp_max_port; rtsp_new_port++) {	
		...
		if(look_existing_port(ct, ctinfo,skb)==1){
			count+=1;
			if(count <((rtsp_max_port-rtsp_min_port)/4+1)){
				goto again;
			}else{
				found = 1;
				printk(KERN_INFO"get_available_port: The RTSP port has already been used up! {6970~7000} \n");
				return found;
			}
		}
		if(found == 0){
...
#ifdef CONFIG_MSTC_TELENOR_NORWAY_PORT_CHANGE
static void refresh_UDP_timer(struct nf_conn *ct, struct sk_buff *skb)
{
	struct nf_conn *child;
	...
	/* write_lock_bh(&nf_conntrack_lock); */ 
	list_for_each_entry(child, &ct->derived_connections, derived_list) {
		printk(KERN_INFO"[refresh_UDP_timer] %u.%u.%u.%u:%u-%u.%u.%u.%u:%u  %u.%u.%u.%u:%u-%u.%u.%u.%u:%u	 timeout=%d \n",
				NIPQUAD( child->tuplehash[0].tuple.src.u3.ip), ntohs( child->tuplehash[0].tuple.src.u.udp.port),
				NIPQUAD( child->tuplehash[0].tuple.dst.u3.ip), ntohs( child->tuplehash[0].tuple.dst.u.udp.port),	
				NIPQUAD( child->tuplehash[1].tuple.src.u3.ip), ntohs( child->tuplehash[1].tuple.src.u.udp.port),
				NIPQUAD( child->tuplehash[1].tuple.dst.u3.ip), ntohs( child->tuplehash[1].tuple.dst.u.udp.port), child->derived_timeout ); 

		child->derived_timeout = 180*HZ;	
		nf_ct_refresh(child, skb, 180*HZ);
		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while(get_next_message(tcpdata, tcpdatalen, &msgoff, &msglen,
		...
		if (memcmp(&ct->tuplehash[dir].tuple.src.u3,
			...
			if(memcmp(tcpdata+msgoff, "TEARDOWN ", 9) == 0) {
				pr_debug("nf_ct_rtsp: TEARDOWN\n");
                                hlp->help.ct_rtsp_info.teardown = 1;
				...
				hlp->help.ct_rtsp_info.teardown = 0;
			} 
#ifdef CONFIG_MSTC_TELENOR_NORWAY_PORT_CHANGE
			if(memcmp(tcpdata+msgoff, "GET_PARAMETER ", 14) == 0) {
				refresh_expect_timer(ct);
				refresh_UDP_timer(ct, skb);
		...
		/* Reply message that's from WAN side. */
			...
			if(hlp->help.ct_rtsp_info.teardown == 1){
				printk(KERN_INFO"TEARDOWN Flag enable!\n");
			//if (teardown_flag == 1) {
		...
		while(get_next_client_port(tcpdata, tpoff, tplen,
			...
			if (memcmp(&ct->tuplehash[dir].tuple.src.u3,
				...
				if(get_available_port(ct,ctinfo,portRangIndex,skb)==1){
					printk(KERN_INFO"get_available_port Fail\n");
					break;
				}else{
					printk(KERN_INFO"get_available_port =%d \n", portRangIndex );		
				}
#endif
