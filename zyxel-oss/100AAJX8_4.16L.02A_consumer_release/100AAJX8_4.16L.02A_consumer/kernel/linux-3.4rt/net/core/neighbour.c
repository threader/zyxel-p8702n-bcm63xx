...
#ifdef BUILD_NORWAY_CUSTOMIZATION
#define HBUFFERLEN 30   //MAC buffer LEN ( 00:19:cb:00:00:01 )

void notify_ssk(const char *ip, const char *mac, int family)
{
	char cmd[64];
	memset(cmd, 0, sizeof(cmd));
	snprintf(cmd, sizeof(cmd), "%s|%s|%s", (family==AF_INET)?"v4":"v6", mac, ip);
	kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_IP_UPDATE, cmd, strlen(cmd)+1);
	return;
}

int notify_pre(const unsigned char *pkey, const unsigned char *ha, int family)
{
	int k, j;
  	char tbuf[40];
	...
	if(family == AF_INET)
		sprintf(tbuf, "%pI4", pkey);
//ip
	else
		sprintf(tbuf, "%pI6", pkey);
//ip

	notify_ssk(tbuf, hbuffer, family);
...
#ifdef BUILD_NORWAY_CUSTOMIZATION
		/* for new case */
		if( !memcmp( dev->name, "br0", 3)){
			notify_pre(neigh->primary_key, neigh->ha, neigh->tbl->family);
		}
#endif		
	}
	if (new == old)
