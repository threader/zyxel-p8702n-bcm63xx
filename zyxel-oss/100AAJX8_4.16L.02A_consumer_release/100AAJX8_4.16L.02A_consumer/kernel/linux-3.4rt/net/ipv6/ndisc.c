...
static inline int accept_ra(struct inet6_dev *in6_dev)
{
	...
	if (((in6_dev->cnf.forwarding) && 
		...
		!strstr(in6_dev->dev->name, "atm") && 
		!strstr(in6_dev->dev->name, "ppp"))))
...
static void ndisc_router_discovery(struct sk_buff *skb)
{
	...
	in6_dev->if_flags = (in6_dev->if_flags & ~(IF_RA_MANAGED |
				...
				(ra_msg->icmph.icmp6_addrconf_other ?
					IF_RA_OTHERCONF : 0);
#ifdef CONFIG_MSTC_MODIFY // __MSTC__, DingRuei  /* accept_ra_defrtr =  Learn default router in Router Advertisement*/

		lifetime = ntohs(ra_msg->icmph.icmp6_rt_lifetime);
	  /* JustinLin Note :																					  *
	  *  Send event MSG when router lifetime > 0 & don't accept RA default route   *
	  *  TODO: the RA accept function should be modify! 									 */
		if (!in6_dev->cnf.accept_ra_defrtr && lifetime > 0)
		{
			char cmd[64];
			char defrtaddr[44];
	
			sprintf(defrtaddr, "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
					ipv6_hdr(skb)->saddr.s6_addr16[0], ipv6_hdr(skb)->saddr.s6_addr16[1],
					ipv6_hdr(skb)->saddr.s6_addr16[2], ipv6_hdr(skb)->saddr.s6_addr16[3],
					ipv6_hdr(skb)->saddr.s6_addr16[4], ipv6_hdr(skb)->saddr.s6_addr16[5],
					ipv6_hdr(skb)->saddr.s6_addr16[6], ipv6_hdr(skb)->saddr.s6_addr16[7]);
			snprintf(cmd, sizeof(cmd), "%s|%s", skb->dev->name, defrtaddr);
			kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_IPV6_DEFAULT_ROUTER, cmd, strlen(cmd)+1);
			goto skip_defrtr;
		}
#else
