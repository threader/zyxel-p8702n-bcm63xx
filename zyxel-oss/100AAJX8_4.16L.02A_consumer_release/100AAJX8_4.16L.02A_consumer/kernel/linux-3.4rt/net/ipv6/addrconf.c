...
#ifdef CONFIG_MSTC_MODIFY // __MSTC__, KuanJung
...
#ifdef CONFIG_MSTC_MODIFY //__MSTC__, Dennis, increase dad_transmits to 7 in order to prevent dad fail while boot up.
		ndev->cnf.dad_transmits = 7;
#else		
...
#ifdef CONFIG_MSTC_MODIFY // __MSTC__, DingRuei
		if (!(__ipv6_addr_type(&ifp->addr) & IPV6_ADDR_LINKLOCAL) &&
		    idev->cnf.accept_dad < 1)
		{
			char cmd[64];
			char ipv6addr[44];

			sprintf(ipv6addr, "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
			        ifp->addr.s6_addr16[0], ifp->addr.s6_addr16[1],
			        ifp->addr.s6_addr16[2], ifp->addr.s6_addr16[3],
			        ifp->addr.s6_addr16[4], ifp->addr.s6_addr16[5],
			        ifp->addr.s6_addr16[6], ifp->addr.s6_addr16[7],
			        ifp->prefix_len);
			snprintf(cmd, sizeof(cmd), "%s|%s", ifp->idev->dev->name, ipv6addr);
			kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_IPV6_SLAAC_ADDIP, cmd, strlen(cmd)+1);
		}
...
#ifdef CONFIG_SYSCTL
#ifdef CONFIG_MSTC_MODIFY //__MSTC__, SinJia, Numbered mode and unnumber mode for ipv6 support
static  
int addrconf_sysctl_autoconf(ctl_table *ctl, int write,  
...
#if 1 ////__MSTC__, for 63381                
      ifp= list_first_entry(&idev->addr_list,
                           struct inet6_ifaddr, if_list);
#else
      ifp = (struct inet6_ifaddr *)(&(idev->addr_list));
#endif

      if(ifp == NULL || ifp->idev==NULL || ifp->idev->dev==NULL)
         return ret;  
...
#if 0
static int addrconf_sysctl_autoconf_strategy(struct ctl_table *table,
...
static struct addrconf_sysctl_table
{
	...
	.addrconf_vars = {
		...
		},
		{
			...
			.mode		= 0644,
#ifdef CONFIG_MSTC_MODIFY //__MSTC__, SinJia, Numbered mode and unnumber mode for ipv6 support
	         .proc_handler   = addrconf_sysctl_autoconf, 
	         //.strategy	=	addrconf_sysctl_autoconf_strategy,
#else			
			.proc_handler	= proc_dointvec,
#endif			
		},
