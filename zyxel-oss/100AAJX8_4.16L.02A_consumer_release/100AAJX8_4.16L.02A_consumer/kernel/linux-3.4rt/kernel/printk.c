...
asmlinkage int printk(const char *fmt, ...)
{
	...
	if(oops_file != NULL) {
		if(oops_file->f_op && oops_file->f_op->write) {
			mm_segment_t fs;
			fs = get_fs();
			set_fs(KERNEL_DS);
			oops_file->f_op->write(oops_file, (char __user *)oops_buf, (size_t)oops_len, &oops_file->f_pos);
			set_fs(fs);
		}
	}
#endif
...
#ifdef CONFIG_MSTC_RECORD_CALL_TRACE
	if(oops_file != NULL) {
		memcpy(oops_buf, printk_buf, printed_len);
		oops_len = printed_len;
	}
#endif
