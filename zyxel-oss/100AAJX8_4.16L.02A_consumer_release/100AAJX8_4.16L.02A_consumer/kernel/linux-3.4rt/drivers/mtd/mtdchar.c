	...
	uint32_t __user *retp)
{
	...
	uint32_t retlen;
	int ret = 0;
#ifdef CONFIG_MSTC_MODIFY /* __MSTC__, zongyue: merge BRCM NAND bad block detect patch */
	memset(&ops, 0x00, sizeof(ops));
#endif
	...
	uint32_t __user *retp)
{
	...
	ops.datbuf = NULL;
	ops.mode = (mfi->mode == MTD_FILE_MODE_RAW) ? MTD_OPS_RAW :
#if 0//defined(CONFIG_BCM_KF_MTD_OOB_AUTO) /* __MSTC__, zongyue: merge BRCM NAND bad block detect patch */
	MTD_OPS_AUTO_OOB;
#else
        MTD_OPS_PLACE_OOB;
#endif
