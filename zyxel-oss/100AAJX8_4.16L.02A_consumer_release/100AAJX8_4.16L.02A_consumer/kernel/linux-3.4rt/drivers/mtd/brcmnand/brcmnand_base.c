...
#endif
#if defined(CONFIG_MSTC_MODIFY) /*__MSTC__, FuChia*/
	{	/* 39 */
		.chipId = TOSHIBA_TC58NVM9S3ETA00,
		.mafId = FLASHTYPE_TOSHIBA,
		.chipIdStr = "TOSHIBA TC58NVM9S3ETA00",
		.options = NAND_BBT_USE_FLASH, 
		.idOptions = 0,
		.timing1 = 0, 
		.timing2 = 0,
...
/*__MSTC__, michael.zhang@mitrastar.cn:  Use Hardware Semaphore, prevent access conflict between mtd and nandflash driver*/
#if defined(CONFIG_BRCMNAND_USE_HW_SEMAPHORE)  
#define BCHP_NAND_SEM_MTD_ID	0x55		/* value should be different from the one in nandflash driver */
#define BCHP_NAND_SEM_MTD_MAX_SCHEDULE_CNT	3000	/* max schedule count for dead lock check */
#define BCHP_NAND_SEM_MTD_SCHEDULE_TIMEOUT	(HZ/100)	/* sleep 10ms in every retry */
#endif
...
static int brcmnand_from_flash_memcpy32(struct brcmnand_chip* chip, void* dest, loff_t offset, int len)
{
	...
	/* this is ugly.. the NAND data buf is big endian in MIPS chip and little end in ARM chip  
           ...
           cpu_to_be/le32 */
			...
			if ((UtilGetChipId() == 0x63138) && (UtilGetChipRev() == 0xA0)) {
				pDest[0] = pSrc[3];
				pDest[1] = pSrc[2];
				pDest[2] = pSrc[1];
				pDest[3] = pSrc[0];
...
{
	struct brcmnand_chip * chip = mtd->priv;
	#if defined(CONFIG_BRCMNAND_USE_HW_SEMAPHORE) /*__MSTC__ michael.zhang@mitrastar.cn:  Use Hardware Semaphore, prevent access conflict between mtd and nandflash driver*/
	unsigned int cnt = 0;
	#endif
	...
	if (chip) {
		...
		*/
			chip->ctrl_write(BCHP_NAND_SEMAPHORE,BCHP_NAND_SEM_MTD_ID);
			while(chip->ctrl_read(BCHP_NAND_SEMAPHORE)!=BCHP_NAND_SEM_MTD_ID)
			{
				if (!in_atomic() && !irqs_disabled() && !in_interrupt())
				{
					if(cnt++ == BCHP_NAND_SEM_MTD_MAX_SCHEDULE_CNT)
					{
						printk("MTD: #### Can't get NAND Flash hardware semaphore after %u times schedule. Must be in dead lock ####\n",BCHP_NAND_SEM_MTD_MAX_SCHEDULE_CNT);
						chip->ctrl_write(BCHP_NAND_SEMAPHORE,0);
					}
					else
					{
						set_current_state(TASK_UNINTERRUPTIBLE);
						schedule_timeout(BCHP_NAND_SEM_MTD_SCHEDULE_TIMEOUT);
					}
				}
				else	/* oops! should not enter here */
				{
					printk("MTD: %s: Bug!!!, Can't get NAND Flash hardware semaphore and can't yield CPU \n", __FUNCTION__);
					chip->ctrl_write(BCHP_NAND_SEMAPHORE,0);
				}
...
static void brcmnand_release_device(struct mtd_info *mtd)
{
	...
	/* __MSTC__, michael.zhang@mitrastar.cn:  Use Hardware Semaphore, prevent access conflict between mtd and nandflash driver*/
#if defined(CONFIG_BRCMNAND_USE_HW_SEMAPHORE)
	/* code in this macro must be put before code "wake_up(&chip->ctrl->wq);" 
	 otherwise "get" and "clear" semaphore maybe not one by one.
	 because, after that code, schedule maybe occurs and other task can get the semaphore again without release*/
