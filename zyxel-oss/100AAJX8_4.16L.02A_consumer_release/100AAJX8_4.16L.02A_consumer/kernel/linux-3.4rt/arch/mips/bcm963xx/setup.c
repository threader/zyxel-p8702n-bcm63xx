...
static BOOL acb_write_wait_end(uint32 addr, uint8 dataw)
{
    int i=0, timeout=1000, retval=1;
    uint32 *acb_instruction = ioremap_nocache(0x80b00004,4); /* PLCPHY.PLCPHY_REGISTERS.QBUS2ACB_BRIDGE.ACB_INSTRUCTION */
    uint32 *acb_status = ioremap_nocache(0x80b00008,4); /* PLCPHY.PLCPHY_REGISTERS.QBUS2ACB_BRIDGE.ACB_STATUS */
...
{
    uint32 afe_pplo_lock;
    uint32 *afe_status = ioremap_nocache(0x80300074,4); /* PLCPHY.PLCPHY_REGISTERS.PLCPHY_SYS_CTRL.AFE_STATUS_REG */
    uint32 *acb_config = ioremap_nocache(0x80b00000,4); /* PLCPHY.PLCPHY_REGISTERS.QBUS2ACB_BRIDGE.ACB_CONFIG. */
    uint32 *rom_id = ioremap_nocache(PLC_ROM_CHECK_ADDR, 4);
	
    *acb_config = 0xa;
    /* PLL Configuration, (to enable 25MHz XTAL, 4.8GHz DCO, 2.4GHz PI clock, 100MHz PCIe, 400MHz PI update clock) */
    acb_write_wait_end(0x500, 0x2); acb_write_wait_end(0x501, 0x04); acb_write_wait_end(0x502, 0x7F);
    acb_write_wait_end(0x503, 0x02); acb_write_wait_end(0x504, 0x7d); acb_write_wait_end(0x505, 0xb1);
    acb_write_wait_end(0x506, 0x07); acb_write_wait_end(0x593, 0x06); acb_write_wait_end(0x592, 0x00);
    acb_write_wait_end(0x591, 0x00); acb_write_wait_end(0x590, 0x00); acb_write_wait_end(0x58c, 0x10);
    acb_write_wait_end(0x58b, 0x10); acb_write_wait_end(0x58a, 0x10); acb_write_wait_end(0x589, 0x06);
    acb_write_wait_end(0x588, 0x18); acb_write_wait_end(0x587, 0x06); acb_write_wait_end(0x586, 0x4);
    ...
    acb_write_wait_end(0x583, 0x31);
    acb_write_wait_end(0x582, 0x88); acb_write_wait_end(0x581, 0x00); acb_write_wait_end(0x5C6, 8);
    ...
    if (rom_id[0] == PLC_ROM_ID_A0)
    {
        acb_write_wait_end(0x5C3, 16);
        acb_write_wait_end(0x543, 16);
    }
    else
    {
        acb_write_wait_end(0x5C3, 24);
        acb_write_wait_end(0x543, 0x10);
    }
    acb_write_wait_end(0x5c4, 48);   acb_write_wait_end(0x544, 12);
    acb_write_wait_end(0x60a, 1<<6);

    acb_write_wait_end(0x612, 0x10); acb_write_wait_end(0x712, 0x10);
    udelay(1000);
    acb_write_wait_end(0x612, 0x30); acb_write_wait_end(0x712, 0x30);
    acb_write_wait_end(0x106, 0x03); acb_write_wait_end(0x306, 0x03);
    acb_write_wait_end(0x021, 42);   acb_write_wait_end(0x020, 3);
    acb_write_wait_end(0x221, 42);   acb_write_wait_end(0x220, 3);
    acb_write_wait_end(0x000, 0x3);  acb_write_wait_end(0x200, 0x3);

    acb_write_wait_end(0x421, 0x01);
    udelay(1000);
    acb_write_wait_end(0x421, 0x03); acb_write_wait_end(0x412, 0xa0);
    iounmap(afe_status);
    iounmap(acb_config);
    iounmap(rom_id);
}
...
static int __init bcm6328_hw_init(void)
{
    ...
    if( BpGetGPIOverlays(&GPIOOverlays) == BP_SUCCESS ) {
		...
		printk(KERN_ERR "\r\n  set up GPIO 1: 0x%x, 0x%x,%x,\n",  (int)GPIO->GPIOio,(int)GPIO->GPIODir,(int)GPIO->GPIOMode);
...
#ifdef CONFIG_MSTC_MODIFY /*__MSTC__, DelonYu: Support DSL-491GNU-B1D C3 board */
		GPIO->LEDCtrl |= (1 << LED_INET_ACT);
		GPIO->LEDCtrl |= (1 << LED_GPHY0_ACT);
#endif		
...
#ifdef CONFIG_MSTC_MODIFY//__MSTC__, RaynorChung: Support DSL-491HNU-B1B v2 board
				GPIO->GPIOBaseMode |= (GPIO_BASE_VDSL_PHY_OVERRIDE_0);
#else									
...
{ 
    ...
    bcm_set_pinmux(96, 1);/* USB_PWRFLT */    
    bcm_set_pinmux(97, 1);/* USB_PWRON  */   
