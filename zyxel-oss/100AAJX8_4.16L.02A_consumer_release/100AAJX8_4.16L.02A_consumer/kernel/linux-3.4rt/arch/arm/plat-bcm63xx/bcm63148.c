...
static int core_set_freq(void *p)
{
	...
	if (shift != 0) {
		B15CTRL->cpu_ctrl.rac_cfg0 = 0; // disable read-ahead
		dsb();
		...
		B15CTRL->cpu_ctrl.rac_cfg0 = 0xdddddddd; // enable read-ahead
		dsb();
	}
...
void __init soc_add_devices(void)
{
	...
	// FIXME! the following is debug code for printing out RAC and MCP
	// can be removed once we verify it
	printk("%s:%d:CPU_CREDIT_REG = 0x%08x\n", __func__, __LINE__,
		(unsigned int)B15CTRL->cpu_ctrl.credit);
	printk("%s:%d:RAC_CONFIG0_REG = 0x%08x\n", __func__, __LINE__,
		(unsigned int)B15CTRL->cpu_ctrl.rac_cfg0);
	printk("%s:%d:RAC_CONFIG1_REG = 0x%08x\n", __func__, __LINE__,
		(unsigned int)B15CTRL->cpu_ctrl.rac_cfg1);
}
