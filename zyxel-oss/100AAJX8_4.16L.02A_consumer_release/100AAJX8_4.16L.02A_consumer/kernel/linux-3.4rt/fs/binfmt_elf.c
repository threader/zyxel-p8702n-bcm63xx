...
#if CONFIG_MSTC_DEC_AND_GZIP_CORE
...
static int elf_core_dump(struct coredump_params *cprm)
{
	...
	#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
	gzfile = kernel_gz_open(cprm->file, cprm->file->f_dentry->d_name.name, "wb9 ", 0);
	if( gzfile == NULL ){
...
#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
	if (size > cprm->limit || !dump_write(gzfile, elf, sizeof(*elf)))
#else
	...
	if (size > cprm->limit
#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
		|| !dump_write(gzfile, phdr4note, sizeof(*phdr4note)))
#else		
	...
	for (vma = first_vma(current, gate_vma); vma != NULL;
		...
		if (size > cprm->limit
#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
			|| !dump_write(gzfile, &phdr, sizeof(phdr)))
#else			
...
#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
	if (elf_coredump_extra_notes_write(gzfile, &foffset))
#else
...
#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
	if (!dump_seek(gzfile, dataoff - foffset))
#else	
