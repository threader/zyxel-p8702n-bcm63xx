...
#ifdef CONFIG_MSTC_MODIFY//__MSTC__, RaynorChung: Patch form SVN 4.06L03
...
#endif
#define FLASH_IMAGE_DOWNLOAD_SIZE       (33*ONEK*ONEK)  

#else//CONFIG_MSTC_MODIFY
...
#define DEFAULT_FEATUREBITS0     "MS"
...
#ifdef MSTC_TE_ATTE
#define ATTE_FEATUREBITS_IDX		7
#endif
#ifdef MSTC_TE_ATMC/* Merge from P880 trunk : implement ATMC,  __TELEFONICA__,  JhihShan, 20120515. */
#define ATMC_FEATUREBITS_IDX     7
#endif
#endif


#if defined (TMPC1_AUTOGEN_WPAPSK) || defined (BUILD_NORWAY_CUSTOMIZATION) || defined (BUILD_TMPC2_COMMON_CUSTOMIZATION)
#define NVRAM_WPAPSK_LEN	13
#define DEFAULT_WPAPSK	"1234567890ABC"
#endif
...
#ifdef CONFIG_MSTC_MODIFY //__MSTC__
...
#ifdef CONFIG_SIK_PARTITION_FEATURE
#define NAND_CUSTOMER_SIZE_KB   15360
#define NAND_DATA_SIZE_KB       4096
#else
#define NAND_DATA_SIZE_KB       (16 * 1024)
#endif
...
typedef struct
{
    ...
    char szBootline[NVRAM_BOOTLINE_LEN];  // 256   260
    char szBoardId[NVRAM_BOARD_ID_STRING_LEN]; // 16 276 
    unsigned int ulMainTpNum;  // 280  
    unsigned int ulPsiSize;      // 284
    unsigned int ulNumMacAddrs;  // 288
    unsigned char ucaBaseMacAddr[NVRAM_MAC_ADDRESS_LEN]; //  6  294  
    char pad;  // 295
    char backupPsi;  /**< if 0x01, allocate space for a backup PSI */  // 296
    unsigned int ulCheckSumV4;  // 300
    char gponSerialNumber[NVRAM_GPON_SERIAL_NUMBER_LEN]; // 13 313 
    char gponPassword[NVRAM_GPON_PASSWORD_LEN];   // 11 324
    char wpsDevicePin[NVRAM_WPS_DEVICE_PIN_LEN];  //  8   332
    char wlanParams[NVRAM_WLAN_PARAMS_LEN];       // 256     588
    unsigned int ulSyslogSize; /**< number of KB to allocate for persistent syslog */  // 592
 ...
 //   unsigned char ucUnused3[2];
    ...
    char partitionNum;
#if defined (TMPC1_AUTOGEN_WPAPSK) || defined (BUILD_NORWAY_CUSTOMIZATION) || defined (BUILD_TMPC2_COMMON_CUSTOMIZATION)
	char AutoWpaPsk[NVRAM_WPAPSK_LEN+1];
	char chUnused[111];
#endif
#else	
    char chUnused[344]; /* Adjust chUnused such that everything above + chUnused[] + ulCheckSum = 1k */
#endif
union {
...
#if (INC_BTRM_BOOT==1) && defined(_BCM963268_)
    unsigned int ulCheckSum_1k;    /// 2048
    char signature[NVRAM_BOOTLDR_SIG_LEN];  //256  300
    char mfgCvKey[NVRAM_CV_KEY_LEN];  //514   
    char opCvKey[NVRAM_CV_KEY_LEN];  //514
    char bek[NVRAM_ENC_KEY_LEN];   //32
    char iek[NVRAM_ENC_KEY_LEN];  // 32
    char biv[NVRAM_ENC_IV_LEN];  //32
    char iiv[NVRAM_ENC_IV_LEN];  //32
    /* Add any secure boot related elements here */
    char chSecUnused[632];    
#endif

    unsigned int ulCheckSum;
#ifdef CONFIG_MSTC_MODIFY //_MSTC__, zongyue: to make sure compiler pack with byte and do not re-aligned/re-organized */
