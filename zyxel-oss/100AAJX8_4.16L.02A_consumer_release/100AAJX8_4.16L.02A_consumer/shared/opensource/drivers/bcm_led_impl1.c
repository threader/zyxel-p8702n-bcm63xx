...
void bcm_common_led_init(void) {
    ...
    if( ((UtilGetChipId() == 0x63138) || (UtilGetChipId() == 0x63381)) && ((UtilGetChipRev()&0xf0) == 0xa0) ) {
        old_led_ctrl = 1;
        LedData = &LED->mask;
    } else {
        old_led_ctrl = 0;
    ...
    if ( old_led_ctrl == 0 && UtilGetChipId() == 0x63138 ) 
        LED->HwPolarity = 0xb8000; // bit 15 and 19 for adsl activity AH, bit 16 and 17 PWR LED AH  
