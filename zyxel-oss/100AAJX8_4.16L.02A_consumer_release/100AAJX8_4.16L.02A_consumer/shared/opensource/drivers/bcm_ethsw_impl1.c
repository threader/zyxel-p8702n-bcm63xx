...
#if you_ever_need_this
uint16 bcm_ethsw_phy_read_c45_reg(int phy_id, int page, int reg)
{
    bcm_ethsw_phy_write_reg(phy_id, 0xd, page);
    bcm_ethsw_phy_write_reg(phy_id, 0xe, reg);
    bcm_ethsw_phy_write_reg(phy_id, 0xd, 0x4000|page);
    return bcm_ethsw_phy_read_reg(phy_id, 0xe);
}
#endif

void bcm_ethsw_phy_write_c45_reg(int phy_id, int page, int reg, uint16 data)
{
    bcm_ethsw_phy_write_reg(phy_id, 0xd, page);
    bcm_ethsw_phy_write_reg(phy_id, 0xe, reg);
    bcm_ethsw_phy_write_reg(phy_id, 0xd, 0x4000|page);
    bcm_ethsw_phy_write_reg(phy_id, 0xe, data);
    return;
}
...
static void phy_adjust_afe(unsigned int phy_id)
{
    ...
    // Configure EEE, this first step must be done soon after reset
    bcm_ethsw_phy_write_c45_reg(phy_id, 7, 0x803d, 0xc000);
}
...
static void phy_adjust_afe(unsigned int phy_id)
{
    ...
    //AFE_VDAC_ICTRL_0
    bcm_ethsw_phy_write_misc_reg(phy_id, 0x39, 0x1, 0xa7aa);
    //AFE_VDAC_OTHERS_0
...
static void phy_adjust_misc(unsigned int phy_id)
{
    ...
    bcm_ethsw_phy_write_misc_reg(phy_id, 0xa, 0x0, 0x111);
...
void bcm_ethsw_init(void)
{
    ...
    if ( (UtilGetChipId() == 0x63148) || ((UtilGetChipId() == 0x63138) && (UtilGetChipRev() != 0xa0)) ) {
