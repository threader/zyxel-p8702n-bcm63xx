...
static  S_DPI_CFG	DpiBasicConfig = {DRV_BBH_GPON,1536,0,0,0,0};
static	int			RdpBlockUnreset = 0;
static  const ETHERNET_MAC_INFO* 	pBoardInfo;

#ifdef G9991
static int g999_debug_port_idx;
#endif
...
static void f_initialize_dma_sdma(void)
{
    ...
    if (UtilGetChipRev() > 1  )
    {
      if(pDpiCfg->wan_bbh!=DRV_BBH_EMAC_4)
      {
     ...
     else
     {
          ...
          /*override the default dma_mem_sel*/
          dma_mem_sel_reg = 0x10;
    ...
    if ( pDpiCfg->wan_bbh == DRV_BBH_EMAC_5 )
    {
        /* DMA */
        fi_dma_configure_memory_allocation ( BB_MODULE_DMA ,
                                             DMA_PERIPHERAL_EMAC_AE_GPON_EPON ,
    ...
    else if (pDpiCfg->wan_bbh==DRV_BBH_GPON)
    {
        /* DMA*/
        fi_dma_configure_memory_allocation ( BB_MODULE_DMA ,
        									 DMA_PERIPHERAL_EMAC_AE_GPON_EPON ,
    ...
    else if (pDpiCfg->wan_bbh==DRV_BBH_EPON)
    {
        /* DMA*/
        fi_dma_configure_memory_allocation ( BB_MODULE_DMA ,
        									 DMA_PERIPHERAL_EMAC_AE_GPON_EPON ,
...
static E_DPI_RC f_initialize_bbh_dma_sdma_related_arrays(void)
{
    ...
    uint32_t wanEmac  = pDpiCfg->wan_bbh;
      /* handle emac ports */
...
static uint8_t f_bbh_rx_runner_task_normal_queue(DRV_IH_RUNNER_CLUSTER runner_cluster, DRV_BBH_PORT_INDEX port_index)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#ifdef G9991
                if (port_index == g999_debug_port_idx)
...
static void f_initialize_bbh_of_emac_port(DRV_BBH_PORT_INDEX port_index)
{
    ...
    if(UtilGetChipRev() > 1)
    {
    	if(port_index == DRV_BBH_EMAC_4)
    	{
		if(BBH_PORT_IS_WAN(port_index))
		{
			lan_fifo_size  = BBH_RX_DMA_FIFOS_SIZE_WAN_GPON_B0;
			lan_exc_thresh = BBH_RX_DMA_EXCLUSIVE_THRESHOLD_WAN_BBH_B0;
		}
    		else
		{
			lan_fifo_size  = BBH_RX_DMA_FIFOS_SIZE_RGMII_LAN_BBH_B0;
			lan_exc_thresh = BBH_RX_DMA_EXCLUSIVE_THRESHOLD_RGMII_LAN_BBH_B0;
		}
    	}
    	else
    	{
    		lan_fifo_size  = BBH_RX_DMA_FIFOS_SIZE_LAN_BBH_B0;
    		lan_exc_thresh = BBH_RX_DMA_EXCLUSIVE_THRESHOLD_LAN_BBH_B0;
    	}
...
static int fi_ih_configure_ingress_queues(void)
{
    ...
    if (!eth_wan)
    {
        ...
        f_update_bbh_ih_ingress_buffers_bitmask(pDpiCfg->wan_bbh, ingress_queue_config.base_location,
...
#define BPM_WAN_LAN_GROUP(emac) ((BBH_PORT_IS_WAN( emac)) ? \
    ...
    bpm_ug_configuration.ug_arr[DRV_BPM_USER_GROUP_1].threshold = 5120;
    bpm_ug_configuration.ug_arr[DRV_BPM_USER_GROUP_1].exclusive_threshold = 5120;
...
static uint32_t f_ih_calculate_forward(DRV_IH_TARGET_MATRIX_SOURCE_PORT source_port, DRV_IH_TARGET_MATRIX_DESTINATION_PORT destination_port)
{
    ...
    {
        printk("enable IH Wan-Wan forwarding...\n");
        return 1;
    }
...
#ifdef G9991
/* Config IH to parse after G9991 TCI on ports marked in BP for G9991 */
static void g9991_fix_logical_ports_config(DRV_IH_LOGICAL_PORTS_CONFIG *ih_lpc)
{
    int bp_port_idx = 0;
    const ETHERNET_SW_INFO *sw = &pBoardInfo[0].sw;

    for (; bp_port_idx < BP_MAX_SWITCH_PORTS; ++bp_port_idx)
    {
        if (!(sw->port_map & 1 << bp_port_idx))
            continue;

        if (sw->port_flags[bp_port_idx] & PORT_FLAG_ATTACHED)
        {
            (&(ih_lpc->eth0_config) + bp_port_idx)->proprietary_tag_size =
                DRV_IH_PROPRIETARY_TAG_SIZE_4;
        }
    }
}    
#endif
...
static void f_ih_init(void)
{
    ...
    runners_load_thresholds.runner_a_high_congestion_threshold 		= pDpiCfg->wan_bbh == DRV_BBH_GPON ? \
        IH_GPON_MODE_DS_RUNNER_A_HIGH_CONGESTION_THRESHOLD :  pDpiCfg->wan_bbh == DRV_BBH_EPON ? \
    ...
    runners_load_thresholds.runner_a_exclusive_congestion_threshold = pDpiCfg->wan_bbh == DRV_BBH_GPON ? IH_GPON_MODE_DS_RUNNER_A_EXCLUSIVE_CONGESTION_THRESHOLD : DRV_RDD_IH_RUNNER_0_MAXIMAL_NUMBER_OF_BUFFERS;
...
#ifdef G9991
    g9991_fix_logical_ports_config(&logical_ports_config);
#endif
    ...
    if ( pDpiCfg->wan_bbh == DRV_BBH_GPON ||
    	 pDpiCfg->wan_bbh == DRV_BBH_EPON ||
    	 pDpiCfg->wan_bbh == DRV_BBH_EMAC_5)
    {
...
#ifdef G9991
int get_g9991_debug_port_bp(void)
{
    int port;

    for (port = DRV_BBH_EMAC_0; port <= DRV_BBH_EMAC_4; port++)
    {
        if (pBoardInfo[0].sw.port_map & 1 << port &&
            pBoardInfo[0].sw.port_flags[port] & PORT_FLAG_MGMT)
        {
            printk("Found g9991 debug port in BP - emac%d\n", port);
            g999_debug_port_idx = port;
            return 0;
        }
    }
...
uint32_t oren_data_path_init( S_DPI_CFG  *pCfg)
{
...
#ifdef G9991
    if (get_g9991_debug_port_bp())
    {
        printk("ERROR: g9991 debug port not set in BoardParams\n");
        return -1;
    }
#endif
	...
	else if (pDpiCfg->wan_bbh == DRV_BBH_EMAC_5)
	{
...
uint32_t oren_data_path_go(void)
{
	...
	if ( initDone != 1 || BpGetBoardId( boardIdStr ) == BP_BOARD_ID_NOT_SET)
	{
	...
	PhyReset(pBoardInfo[0].sw.port_map);
	...
	if (pDpiCfg->wan_bbh == DRV_BBH_EMAC_5)
	{
	    mac_hwapi_init_emac(DRV_BBH_EMAC_5);
            mac_hwapi_set_unimac_cfg(DRV_BBH_EMAC_5);
	...
	if (xmii_port)
	{
	    if (IsRgmiiDirect(pBoardInfo[0].sw.phy_id[4]) || IsExtPhyId(pBoardInfo[0].sw.phy_id[4]))
	    {
	        rgmii_out_reg = EGPHY_RGMII_OUT_PORT_MODE_RGMII | (EGPHY_RGMII_OUT_REF_50_MHZ << EGPHY_RGMII_OUT_REF_OFFSET);
                /* Do not set internal delay on 968380FTTDPS and 968380DP2*/
                if (strcmp(boardIdStr, "968380FTTDPS") && strcmp(boardIdStr, "968380DP2"))
