...
static S_PhyCache 	phyAddrCache[BP_MAX_SWITCH_PORTS] = {[0 ... (BP_MAX_SWITCH_PORTS-1)] = {0,0,0} };
...
void  PhyAutoEnable(uint32_t macId)
{
	...
	if(mode == MAC_IF_SERDES)
	{
		...
		if (mdio_write_c22_register(mdioType ,phyId, MII_PTEST1_RDBRW, mdio_data))
			printk("failed MDIO_AE write reg=0x%x data=0x%x\n ",MII_PTEST1_RDBRW,mdio_data);
			mdio_data = 0x1140;
		if (mdio_write_c22_register(mdioType ,phyId, MII_BMCR, mdio_data))
				printk("failed MDIO_AE write reg=0x%x data=0x%x\n ",MII_BMCR,mdio_data);
			mdio_data = 0x01a0;
		if (mdio_write_c22_register(mdioType ,phyId, MII_ADVERTISE, mdio_data))
				printk("failed MDIO_AE write reg=0x%x data=0x%x\n ",MII_ADVERTISE,mdio_data);
		mdio_data = 0x4101;
		if (mdio_write_c22_register(mdioType ,phyId, MII_XCTL, mdio_data))
			printk("failed MDIO_AE write reg=0x%x data=0x%x\n ",MII_XCTL,mdio_data);
			return;
	}
