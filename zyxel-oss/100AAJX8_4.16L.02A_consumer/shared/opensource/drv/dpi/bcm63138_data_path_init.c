...
#define IH_DSL_MODE_DS_RUNNER_A_HIGH_CONGESTION_THRESHOLD    24
/* exclusive congestion threshold of runner A (DS), for DSL mode */
#define IH_DSL_MODE_DS_RUNNER_A_EXCLUSIVE_CONGESTION_THRESHOLD 28
...
#if defined(DSL_63138) && !defined(IS_REV_A)
#define BPM_DEFAULT_THRESHOLD    5120
#define DRV_BPM_GLOBAL_THRESHOLD DRV_BPM_GLOBAL_THRESHOLD_5K
...
#define BPM_DEFAULT_THRESHOLD    2560
#define DRV_BPM_GLOBAL_THRESHOLD DRV_BPM_GLOBAL_THRESHOLD_2_5K
...
#else
#define BPM_DEFAULT_THRESHOLD 7680
#define DRV_BPM_GLOBAL_THRESHOLD DRV_BPM_GLOBAL_THRESHOLD_7_5K
...
#define BBH_PORT_IS_WAN(_wanType,_portIndex) (( _wanType == WAN_TYPE_RGMII && _portIndex == DRV_BBH_EMAC_0) || \
                                              ( _wanType == WAN_TYPE_DSL && _portIndex == DRV_BBH_DSL) )
...
#if !defined(_CFE_)
static uint32_t ddr_multicast_base_address_phys;
#endif
...
static void f_initialize_dma_sdma(void)
{
    ...
    if (pDpiCfg->wanType==WAN_TYPE_DSL)
    {
        /* DMA*/
        fi_dma_configure_memory_allocation ( BB_MODULE_DMA ,
                                             DMA_PERIPHERAL_DSL ,
...
static E_DPI_RC f_initialize_bbh_dma_sdma_related_arrays(void)
{
    ...
    for (i = 0; i <= num_emac; i++)
    {
        ...
        bbh_rx_dma_data_and_chunk_descriptor_fifos_size[i] = BBH_RX_DMA_FIFOS_SIZE_LAN_DMA;
        bbh_rx_dma_exclusive_threshold[i] = BBH_RX_DMA_EXCLUSIVE_THRESHOLD_LAN_DMA;
    ...
    {
        bbh_rx_dma_data_and_chunk_descriptor_fifos_size[wanEmac] = BBH_RX_DMA_FIFOS_SIZE_WAN_DMA;
        bbh_rx_dma_exclusive_threshold[wanEmac] = BBH_RX_DMA_EXCLUSIVE_THRESHOLD_WAN_DMA;
        ...
        bbh_rx_dma_data_fifo_base_address[wanEmac] = dma_base_address;
        bbh_rx_dma_chunk_descriptor_fifo_base_address[wanEmac] = dma_base_address;
...
static uint16_t f_get_bbh_rx_pd_fifo_base_address_normal_queue_in_8_byte(DRV_BBH_PORT_INDEX port_index)
{
    ...
    {
    case DRV_BBH_EMAC_0:
        if ( pDpiCfg->wanType == WAN_TYPE_RGMII && port_index == DRV_BBH_EMAC_0 )
        {
...
#if defined _CFE_
    //allready allocaed
    bbh_tx_configuration.multicast_headers_base_address_in_byte = (uint32_t)ddr_multicast_base_address;
#else
    bbh_tx_configuration.multicast_headers_base_address_in_byte = ddr_multicast_base_address_phys;
#endif
...
static int fi_ih_configure_ingress_queues(void)
{
    ...
    else
    {
        ...
        /* update the correspoding bitmask in database, to be configured in BBH */
        f_update_bbh_ih_ingress_buffers_bitmask(DRV_BBH_EMAC_0, ingress_queue_config.base_location,
    ...
    /* update the correspoding bitmask in database, to be configured in BBH */
    f_update_bbh_ih_ingress_buffers_bitmask(DRV_BBH_EMAC_1, ingress_queue_config.base_location,
        ingress_queue_config.size);
...
static uint32_t is_matrix_source_lan(DRV_IH_TARGET_MATRIX_SOURCE_PORT source_port)
{
    ...
    if ( (pDpiCfg->wanType == WAN_TYPE_RGMII && source_port != DRV_IH_TARGET_MATRIX_SOURCE_PORT_ETH0 )||
       ( pDpiCfg->wanType == WAN_TYPE_DSL && source_port < DRV_IH_TARGET_MATRIX_SOURCE_PORT_GPON ))
    {
        return 1;
    }
    return 0;
...
static uint32_t is_matrix_dest_lan(DRV_IH_TARGET_MATRIX_SOURCE_PORT source_port)
{
    if (source_port <= DRV_IH_TARGET_MATRIX_SOURCE_PORT_ETH3 ||
        (source_port == DRV_IH_TARGET_MATRIX_SOURCE_PORT_ETH0 && !pDpiCfg->wanType==WAN_TYPE_RGMII ))
    {
        return 1;
    }
    return 0;
...
{
    if ((pDpiCfg->wanType==WAN_TYPE_DSL && source_port == DRV_IH_TARGET_MATRIX_SOURCE_PORT_GPON) ||
        (pDpiCfg->wanType==WAN_TYPE_RGMII && source_port == DRV_IH_TARGET_MATRIX_SOURCE_PORT_ETH0))
    {
        return 1;
    }
    return 0;
...
static void bpm_drv_init(void)
{
    ...
    bpm_global_configuration.threshold = DRV_BPM_GLOBAL_THRESHOLD;
    for (i = 0; i < ARRAY_LENGTH(bpm_ug_configuration.ug_arr); i++)
    {
        ...
        bpm_ug_configuration.ug_arr[i].threshold = (i == DRV_BPM_USER_GROUP_7) ?
            BPM_CPU_NUMBER_OF_BUFFERS : BPM_DEFAULT_THRESHOLD;
        ...
        bpm_ug_configuration.ug_arr[i].exclusive_threshold = (i == DRV_BPM_USER_GROUP_7) ?
            BPM_CPU_NUMBER_OF_BUFFERS - 32 : BPM_DEFAULT_THRESHOLD;
    }
...
static void f_ih_init(void)
{
    ...
    runners_load_thresholds.runner_a_high_congestion_threshold      = pDpiCfg->wanType == WAN_TYPE_DSL ? IH_DSL_MODE_DS_RUNNER_A_HIGH_CONGESTION_THRESHOLD : DRV_RDD_IH_RUNNER_0_MAXIMAL_NUMBER_OF_BUFFERS;
    runners_load_thresholds.runner_b_high_congestion_threshold      = DRV_RDD_IH_RUNNER_0_MAXIMAL_NUMBER_OF_BUFFERS;
    runners_load_thresholds.runner_a_exclusive_congestion_threshold = pDpiCfg->wanType == WAN_TYPE_DSL ? IH_DSL_MODE_DS_RUNNER_A_EXCLUSIVE_CONGESTION_THRESHOLD : DRV_RDD_IH_RUNNER_0_MAXIMAL_NUMBER_OF_BUFFERS;
...
static void f_configure_runner(void)
{
    ...
    {
        case WAN_TYPE_DSL:
          rdd_init_params.wan_physical_port  = BL_LILAC_RDD_WAN_PHYSICAL_PORT_DSL;
          break;
        case WAN_TYPE_RGMII:
        case WAN_TYPE_NONE:
          rdd_init_params.wan_physical_port  = BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH0;
          break;
        default:
              break;
    }
    ...
    rdd_init_params.ddr_pool_ptr       = ddr_tm_base_address;
    rdd_init_params.extra_ddr_pool_ptr = ddr_multicast_base_address;
     ...
     */
#if (defined(_BCM963138_) || defined(CONFIG_BCM963138)) && defined(IS_REV_A)
    rdd_init_params.chip_revision = RDD_CHIP_REVISION_A0;
#else /* BCM63138B0, BCM63148 */
...
void StartRdpPllWithClk(uint32_t clk)
{
	uint32_t bpcmResReg;
	uint32_t error;
...
static void PowerUpRdpBlock(void)
{
	int i, ret;
...
static void PowerDownRdpBlock(void)
{
	int i, ret;
	BPCM_PWR_ZONE_N_CONTROL pwr_zone_ctrl;
...
#if 0
    /*when BCM63138 is out of reset, RDP block reset bits are all set to reset,
...
#else
    /* we will just put all the module off reset */
    error = WriteBPCMRegister(PMB_ADDR_RDP, BPCM_SRESET_CNTL_REG, 0xffffffff);
    if (error)
        printk("Failed to WriteBPCMRegister RDP block BPCM_SRESET_CNTL_REG\n");
#endif
...
uint32_t bcm63138_data_path_init( S_DPI_CFG  *pCfg)
{
    ...
    //TODO:memory reserve
#ifdef _CFE_
    ddr_tm_base_address        = (void*)cache_to_uncache((uint32_t)KMALLOC(BCM63138_TM_DEF_DDR_SIZE, 0x200000));
    ddr_multicast_base_address = (void*)cache_to_uncache((uint32_t)KMALLOC(BCM63138_TM_MC_DEF_DDR_SIZE, 0x200000));
    ...
    // In CFE boot loader, cached addresses use same address space as physical addresses.
    ddr_tm_base_address_phys = (void*) ddr_tm_base_address;
#else
    {
        unsigned int tmSize;
        unsigned int mcSize;

        error = BcmMemReserveGetByName(TM_BASE_ADDR_STR, &ddr_tm_base_address, &tmSize);
        ...
        error = BcmMemReserveGetByName(TM_MC_BASE_ADDR_STR, &ddr_multicast_base_address, &mcSize);
        ...
        __print("ddr_tm_base_address 0x%p, size %u/%u, ddr_tm_base_address_phys 0x%p\n",
                ddr_tm_base_address, tmSize, BCM63138_TM_DEF_DDR_SIZE, ddr_tm_base_address_phys);

        ddr_multicast_base_address_phys = (uint32_t)VIRT_TO_PHYS(ddr_multicast_base_address);

        __print("ddr_multicast_base_address 0x%p, size %u/%u, ddr_multicast_base_address_phys 0x%08x\n",
                ddr_multicast_base_address, mcSize, BCM63138_TM_MC_DEF_DDR_SIZE, ddr_multicast_base_address_phys);
    }
#endif
    ...
    if (!RdpBlockUnreset)
    {
        // FIXME! do we want to get the runner frequency from below and use it to set PLL?
        StartRdpPllWithClk(0);
...
uint32_t bcm63138_data_path_go(void)
{
    ...
    bcm_ethsw_init();
#else
    // want this for RGMII without depending on CFE. 
    pmc_switch_power_up();
    /* TBD. switch_init for Linux */
#endif
...
void f_initialize_bbh_of_dsl_port(void)
{
    ...
    bbh_rx_configuration.ih_ingress_buffers_bitmask = 0x3c ;
    bbh_rx_configuration.packet_header_offset = DRV_RDD_IH_PACKET_HEADER_OFFSET;
    bbh_rx_configuration.reassembly_offset_in_8_byte = MS_BYTE_TO_8_BYTE_RESOLUTION(pDpiCfg->headroom_size);
