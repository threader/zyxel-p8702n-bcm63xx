...
#define ID_GD5F1GQ4U        0xD1
#define ID_GD5F2GQ4U        0xD2
#define ID_GD5F4GQ4U        0xD4
...
#define SPI_NAND_DEVICES                                                \
    {{SPI_MAKE_ID(GIGADEVPART, ID_GD5F1GQ4),  "GigaDevice GD5F1GQ4"},   \
     {SPI_MAKE_ID(GIGADEVPART, ID_GD5F1GQ4U), "GigaDevice GD5F1GQ4U"},  \
     {SPI_MAKE_ID(GIGADEVPART, ID_GD5F2GQ4U), "GigaDevice GD5F2GQ4U"},  \
     {SPI_MAKE_ID(GIGADEVPART, ID_GD5F4GQ4U), "GigaDevice GD5F4GQ4U"},  \
...
int spi_nand_write_page(unsigned long page_addr, unsigned int page_offset, unsigned char *buffer, int len);
...
int spi_nand_write_jffs2_clean_marker(int addr);
...
static flash_device_info_t flash_spi_nand_dev =
    {
        ...
        FLASH_IFC_SPINAND,
        "",
        spi_nand_sector_erase_blk, // flash_sector_erase_int in flash_api.c
        spi_nand_read_buf,         // flash_read_buf in flash_api.c
        spi_nand_write_buf,        // flash_write_buf in flash_api.c
        spi_nand_get_numsectors,   // flash_get_numsectors in flash_api.c
        spi_nand_get_sector_size,  // flash_get_sector_size in flash_api.c
        spi_nand_get_memptr,       // flash_get_memptr in flash_api.c
        spi_nand_get_blk,          // flash_get_blk in flash_api.c
        spi_nand_get_total_size    // flash_get_total_size in flash_api.c
    };
...
static struct nand_ecclayout spinand_oob_64_bch4 =
    {
    .eccbytes = 17,
...
static int spi_nand_read_cfg(PCFE_SPI_NAND_CHIP pchip)
{
    ...
    /* currently all the supported chips have the same page size, block size and spare settings,
       over write it in the switch case if new chip has a different size */
    pchip->chip_page_size = 2048;
    pchip->chip_page_shift = 11;
    pchip->chip_block_size = 64 * pchip->chip_page_size;   // 64 pages per block
    ...
    case SPI_MAKE_ID(GIGADEVPART, ID_GD5F1GQ4U):
        pchip->chip_num_blocks = 1024;       //1024 blocks total
        pchip->chip_num_plane = 1;
        pchip->chip_ecc_offset = 0x840; // location of ECC bytes
        break;

    case SPI_MAKE_ID(GIGADEVPART, ID_GD5F2GQ4U):
        pchip->chip_num_blocks = 2048;       //2048 blocks total
        pchip->chip_num_plane = 1;
        pchip->chip_ecc_offset = 0x840; // location of ECC bytes
        break;

    case SPI_MAKE_ID(GIGADEVPART, ID_GD5F4GQ4U):
        pchip->chip_num_blocks = 4096;       //4096 blocks total
        pchip->chip_num_plane = 1;
        pchip->chip_ecc_offset = 0x840; // location of ECC bytes
        break;
...
#if defined(CFG_RAMAPP)
    pchip->ecclayout = &spinand_oob_64_bch4;
#endif
    return ret;
}
...
#if defined(CFG_RAMAPP)
    bool flash_features; // turn off ECC
...
int spi_nand_block_erase(unsigned short blk);
int spi_nand_block_erase(unsigned short blk)
{
    PCFE_SPI_NAND_CHIP pchip = &g_spinand_chip;
    unsigned char buf[11];
    ...
    DBG_PRINTF("spi_nand_block_erase block 0x%x\n", blk);
...
int spi_nand_write_page(unsigned long page_addr, unsigned int page_offset, unsigned char *buffer, int len)
{
    ...
    if (verify == 2)
        memcpy(xfer_buf + pchip->chip_page_size + 1, jffs2_clean_marker, sizeof(jffs2_clean_marker));
    ...
    if (len)
    {
        ...
        do
        {
            ...
            { // mark block bad
                ...
                spi_nand_write_page(block_addr, pchip->chip_page_size, (unsigned char *) "\0", 1 | FLASH_FEATURES);
                return(FLASH_API_ERROR);
            }
...
static int spi_nand_is_blk_bad(PCFE_SPI_NAND_CHIP pchip, unsigned short blk)
{
    ...
    spi_nand_read_page(pchip->chip_block_size * blk, pchip->chip_page_size, &buf, 1);
...
int spi_nand_write_jffs2_clean_marker(int page_addr)
{ // we need to worry about the position of useable OOB data
    ...
    return(spi_nand_write_page(page_addr, pchip->chip_page_size, buf, pchip->chip_spare_size));
}
