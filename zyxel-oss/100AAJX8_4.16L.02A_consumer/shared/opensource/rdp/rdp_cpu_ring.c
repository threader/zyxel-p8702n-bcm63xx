                             ...
                             void  *ic_cfg_voidp) 
{
	...
	pDescriptor->base = (RING_DESC_UNION*)alignedAlloc(sizeof(RING_DESC_UNION) * entries, &phy_addr);
	...
	rdd_ring_init(ring_id, (uint32_t*)phy_addr, entries, sizeof(*pTravel), ring_id );
...
#ifdef __KERNEL__
    {
#if defined(CONFIG_BCM963138) || defined(_BCM963138_) || defined(CONFIG_BCM963148) || defined(_BCM963148_)|| defined(CONFIG_BCM96838) || defined(_BCM96838_)
        rdpa_cpu_rxq_ic_cfg_t *ic_cfg_p = (rdpa_cpu_rxq_ic_cfg_t *)ic_cfg_voidp;
        if (ic_cfg_p && ic_cfg_p->ic_enable)
        {      
            rdd_cpu_rx_interrupt_coalescing_config(ring_id, ic_cfg_p->ic_timeout_us, ic_cfg_p->ic_max_pktcnt);
            printk("Interrupt coalescing enabled for ring %d. ic_timeout %d ic_maxpktcnt %d\n", ring_id,
                    ic_cfg_p->ic_timeout_us, ic_cfg_p->ic_max_pktcnt);
        }
        else
#else
        {
            printk("Interrupt coalescing disabled for ring %d\n", ring_id);
        }
#endif
