...
#ifdef BRCM_FTTDP
#define G9991_DEBUG_RDPA_PORT rdpa_if_lan21 /* Hardcoded in FW */
...
#endif
#if defined(BUILD_MSTC_DSL_2492GNAU_B3BC) 
uint8 swLEDLedEn = 0xff;
#endif
...
static int create_vport(void)
{
    ...
    for ( vport_id = 1, logical_port = 0 ; vport_id < vport_cnt + 1; vport_id++, logical_port++, (*map_p) /= 2)
    {
		...
		phyport_to_phyid[logical_port]=phy_id; 	
#endif
#if defined(CONFIG_BCM_SWITCH_PORT_TRUNK_SUPPORT)
...
#if !defined(BUILD_MSTC_DSL_2492GNAU_B1BC_MLD) && !defined(BUILD_MSTC_DSL_2492GNAU_B3BC)  // MitraStar Elina, Reverse vport <--> phyport mapping
		/* phyport	-> logical_port:  port1| port2| port3| port4		   */
		/* vport	  -> vport_id:  eth1 | eth2 | eth3 | eth0		   */
...
#ifdef BUILD_MSTC_DSL_2492GNAU_B1BC_MLD //__MSTC__, Eason
		printk(KERN_ERR "%s, %s, vport_id = %d, logical_port = %d, phy_id = 0x%lx\n",__FUNCTION__, dev->name, vport_id, logical_port, phy_id);
#endif				
        printk("%s: <%s sw port: %d> <Logical : %02d> PHY_ID <0x%08lx : 0x%02lx> MAC : %02X:%02X:%02X:%02X:%02X:%02X\n",
               dev->name,
               (LOGICAL_PORT_TO_UNIT_NUMBER(logical_port)?"Ext":"Int"),LOGICAL_PORT_TO_PHYSICAL_PORT(logical_port), logical_port,
                (unsigned long)phy_id, (unsigned long)phy_id&BCM_PHY_ID_M,
    ...
    if ((priv->linkState & mask) != linkMask) {
        ...
        if (linkstatus) {
            ...
            else
            {
				...
				sprintf(buf,"%x",swLEDLedEn);
				kerSysSendtoMonitorTask(MSG_EXTSW_10MSWLED_En, buf, sizeof(buf));
#endif	
            }
...
static uint32 bcm63xx_rx(void *ptr, uint32 budget)
{
    ...
    for(bcmeapi_prepare_rx(); budget > 0; dev = NULL, pBuf = NULL, skb = NULL)
    {
        ...
        ret = bcmeapi_rx_pkt(pDevCtrl, &pBuf, &pFkb, &len, &gemid, &phy_port_id, &is_wifi_port, &dev, &rxpktgood, &rxContext1);
...
static int bcm_set_soft_switching(int swPort, int type)
{
    ...
    {
#if defined(CONFIG_BCM96328) || defined(CONFIG_BCM96362) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM96828) || defined(CONFIG_BCM96318) || defined(CONFIG_BCM963381)
        ethsw_set_wanoe_portmap(GET_PORTMAP_FROM_LOGICAL_PORTMAP(pDevCtrl->wanPort,0), pDevCtrl->txSoftSwitchingMap);
#else
        ethsw_port_based_vlan(pDevCtrl->EnetInfo[0].sw.port_map, pDevCtrl->wanPort, pDevCtrl->txSoftSwitchingMap);
#endif
    }
...
#define MGMT_SID_LINK 4
    ...
    rdpa_vport_to_dev[rdpa_if_lan0 + 17] = rdpa_vport_to_dev[rdpa_if_lan0 + MGMT_SID];
    rdpa_vport_to_dev[rdpa_if_lan0 + 18] = rdpa_vport_to_dev[rdpa_if_lan0 + MGMT_SID];
    rdpa_vport_to_dev[rdpa_if_lan0 + 19] = rdpa_vport_to_dev[rdpa_if_lan0 + MGMT_SID];
    rdpa_vport_to_dev[rdpa_if_lan0 + 20] = rdpa_vport_to_dev[rdpa_if_lan0 + MGMT_SID];
#endif
    create_rdpa_vports();
#endif
...
static int bcm63xx_enet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    switch (cmd)
    {
        ...
        case SIOCGWANPORT:
            ...
            {
                if ((pDevCtrl->wanPort >> i) & 0x1) // wanPort mask is by logical port.
                {
                    if (LOGICAL_PORT_TO_VPORT(i) > 0)
                    {
                        int vport_idx = LOGICAL_PORT_TO_VPORT(i);
                        struct net_device *dev = vnet_dev[vport_idx];
                        BcmEnet_devctrl *cb_priv = netdev_priv(dev);
                        int cb_port = cb_priv->cb_ext_port;
                        do {
                        ...
                        if ((cum_len + len + 1) < MAX_WAN_IFNAMES_LEN)
                        {
                            ...
                            if (cb_port != BP_CROSSBAR_NOT_DEFINED) { /* Check more cb_ports */
                                vport_idx++;
                                dev = vnet_dev[vport_idx];
                                cb_port = BP_CROSSBAR_NOT_DEFINED;
                                if (dev) {
                                    cb_priv = netdev_priv(dev);
                                    cb_port = cb_priv->cb_ext_port;
                                    if (cb_priv->sw_port_id != i) {
                                        break;
                                    }
                                }
                            }
...
#if defined(CONFIG_BCM96328) || defined(CONFIG_BCM96362) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM96828) || defined(CONFIG_BCM96318) || defined(CONFIG_BCM63381)
