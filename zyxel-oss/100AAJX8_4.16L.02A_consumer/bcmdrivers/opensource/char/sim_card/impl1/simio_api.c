...
int sim_active(UInt8 *data, int len) 
{
    ...
    SIMIO_SetParam(SIMIO_ID_0, SIMIO_PARAM_VOLTAGE, SIM_3V);
    SIMIO_set_frequency(SIMIO_ID_0, SIMIO_CLK_3P12HZ); 
    SIMIO_SetParam(SIMIO_ID_0, SIMIO_PARAM_T0_WWT, 0xffff);

    return (simio_activesim(SIMIO_ID_0, data, len));
}

int sim_online(void)
{
    int detection_status;
    SIMIO_read_detection_status(SIMIO_ID_0, &detection_status); 
...
int sim_set_baud(int F, int D)
{
    ...
    return (SIMIO_set_pps(SIMIO_ID_0, val, divisor));
}
...
int sim_card_command(UInt16 command, UInt8 *data, int len)
{
    ...
    return SIMIO_command(SIMIO_ID_0, command, data, len);
} 
...
int sim_write(UInt8 *data, int len)
{
    ...
    return (sim_card_command(1, data, len));
}
...
int sim_read(UInt8 *data, int len)
{
    ...
    return (sim_card_command(0, data, len));
}
...
int sim_card_protocol(PROTOCOL_t protocol)
{
    ...
    {
        SIMIO_SetPrefProtocol(SIMIO_ID_0, protocol); 
        ret = 0;
    }
    else
...
int sim_card_control(int control)
{
    ...
    if (control == 0)
        SIMIO_DeactiveCard(SIMIO_ID_0);
    else if (control == 1)
        SIMIO_activeCard(SIMIO_ID_0);
    else
        return SIM_CARD_INPUT_IS_INVALID;
...
int sim_card_reset(int reset_mode, SimVoltageLevel_t activation_voltage, SIMIO_DIVISOR_t frequency)
{
    ...
    ret = SIMIO_set_frequency(SIMIO_ID_0, frequency); 

    SIMIO_SetParam(SIMIO_ID_0, SIMIO_PARAM_VOLTAGE, activation_voltage);

    if (reset_mode == 0)
        SIMIO_ResetCard(SIMIO_ID_0);
    else
        SIMIO_WarmResetCard(SIMIO_ID_0);
