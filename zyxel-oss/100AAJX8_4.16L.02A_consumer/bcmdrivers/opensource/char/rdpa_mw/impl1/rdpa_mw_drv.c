...
static int blog_rule_mapping_get(blogRule_t *blog_rule, blogRuleCommand_t req_cmd)
{
    int i;
    ...
    for (i = 0; i < blog_rule->actionCount; i++)
    {
        ...
        switch (cmd)
        {
            case BLOG_RULE_CMD_SET_SKB_MARK_PORT:
                return blog_rule->action[i].skbMarkPort;
            case BLOG_RULE_CMD_SET_SKB_MARK_QUEUE:
                return blog_rule->action[i].skbMarkQueue;
            case BLOG_RULE_CMD_SET_DSCP:
                return blog_rule->action[i].dscp;
            case BLOG_RULE_CMD_DROP:
                return 0;
            case BLOG_RULE_CMD_SET_DEI:
                return blog_rule->action[i].dei == 1 ? RDPA_IC_DEI_SET : RDPA_IC_DEI_CLEAR;
            default:
                break;
        }
    }
    ...
    uint32_t *flow_idx_p, rdpa_if *egress_port)
{
    ...
    is_drop_rule = !blog_rule_mapping_get(blog_rule, BLOG_RULE_CMD_DROP);
    ...
    if (blog_rule_mapping_get(blog_rule, BLOG_RULE_CMD_SET_DEI) != -1)
    {
        flow.result.dei_command = blog_rule_mapping_get(blog_rule, BLOG_RULE_CMD_SET_DEI); 
    }
    else
        flow.result.dei_command = RDPA_IC_DEI_COPY;

    if (blog->tx.info.phyHdrType == BLOG_GPONPHY || blog->tx.info.phyHdrType == BLOG_EPONPHY) /* XXX: check EPON LLID */
        flow.result.wan_flow = blog_rule_mapping_get(blog_rule, BLOG_RULE_CMD_SET_SKB_MARK_PORT);
    ...
    if (blog_rule_mapping_get(blog_rule, BLOG_RULE_CMD_SET_SKB_MARK_QUEUE) != -1)
    {
        blog_queue_id = blog_rule_mapping_get(blog_rule, BLOG_RULE_CMD_SET_SKB_MARK_QUEUE);
    }
        ...
        if (rdpa_mw_packet_based_forward && dir == rdpa_dir_ds)
        {
            ...
            for (lan_port = rdpa_if_lan0; lan_port <= rdpa_if_ssid0; ++lan_port)
            {
                ...
                rc = configure_action_per_port(ingress_class_obj, flow_idx, lan_port,
                    this_rule_port ? vlan_action_obj : NULL, this_rule_port ? is_drop_rule : 1);
                if (rc)
                    return rc;
            }
        }
...
static void rdpa_bind_blog(int enabled)
{
    BlogBind_t hook_info;

    /* Register for all possible calls from blog.
     * XXX: For each call, when invoked, decide separately and update this init
     * appropriately. */
    hook_info.hook_info = 0xFF;
    if (enabled)
    {
        blog_bind_config((BlogScHook_t)rdpa_config_fp,
            (BlogSdHook_t)rdpa_deconf_fp, BlogClient_runner, hook_info);
    }
    else
        blog_bind_config(NULL, NULL, BlogClient_runner, hook_info);

    protoInfo("Blog rules reflection to Runner %s.", enabled ? "enabled" : "disabled");
}
