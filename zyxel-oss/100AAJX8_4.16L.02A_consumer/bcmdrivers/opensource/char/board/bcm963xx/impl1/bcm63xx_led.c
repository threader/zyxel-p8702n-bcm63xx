...
static BP_LED_INFO bpLedInfo[] =
{
    ...
    {kLedWlanEn, BpGetWlanEnLedGpio, NULL},
#endif
    {kLedEnd, NULL, NULL}
};
...
#ifdef MSTC_MODIFY //__MSTC__, Leo, when LED is turned off by auto-scheduling, some LEDs will turn on after reboot. 
...
#if defined(MSTC_MODIFY)  && !defined(CONFIG_BCM96328) //__MSTC__, Eason
...
#elif !defined(CONFIG_BCM963138) && !defined(CONFIG_BCM960333) && !defined(CONFIG_BCM963148) && !defined(CONFIG_BCM963381)
    if (led_gpio & BP_GPIO_SERIAL) {
...
#else
#if !defined(CONFIG_BCM963138) && !defined(CONFIG_BCM960333) && !defined(CONFIG_BCM963148) && !defined(CONFIG_BCM963381)
    if (led_gpio & BP_GPIO_SERIAL) {
        while (GPIO->SerialLedCtrl & SER_LED_BUSY);
        GPIO->SerialLed ^= GPIO_NUM_TO_MASK(led_gpio);
    }
...
static void ledTimerExpire(void)
{
    ...
    for (i = 0, pCurLed = gLedCtrl; i < kLedEnd; i++, pCurLed++)
    {
        ...
        case kLedStateFail:
            ...
            break;
#ifdef MSTC_MODIFY /* LED , __TELEFONICA__, Mitrastar Curtis, 20120106 */
		case ALLLedStateOn:
		case ALLLedStateOff:
			spin_unlock_irqrestore(&brcm_ledlock, flags);
			break;
