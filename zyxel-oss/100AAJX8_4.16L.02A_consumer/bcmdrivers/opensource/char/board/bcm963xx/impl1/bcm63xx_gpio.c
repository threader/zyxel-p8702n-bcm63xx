...
void kerSysSetGpioStateLocked(unsigned short bpGpio, GPIO_STATE_t state)
{
    ...
    /* DUNA TODO */
    /* WHEN DOING THE DUNA TODO, IMPLEMENT gpio_set_data in shared/ and keep it consistent with 6838/63138 */
#else
...
void kerSysSetGpioDirLocked(unsigned short bpGpio)
{
    ...
    /* DUNA TODO */
    /* DUNA TODO -- ditto about using shared */
#else
...
{
#if   !defined(CONFIG_BCM96838) && !defined(CONFIG_BCM963138) && !defined(CONFIG_BCM963381) &&!defined(CONFIG_BCM963148)
    unsigned long flags;
