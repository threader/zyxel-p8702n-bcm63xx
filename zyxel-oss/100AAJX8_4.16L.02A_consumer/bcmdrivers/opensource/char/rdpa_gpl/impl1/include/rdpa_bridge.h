...
typedef struct {
    ...
    bdmf_boolean auto_forward;     /**< Update forwarding matrix automatically when ports/VLANs are connected or disconnected */
    bdmf_boolean auto_aggregate;   /**< Update aggregation table automatically when ports/VLANs are connected or disconnected */
} rdpa_bridge_cfg_t;
...
typedef struct {
    ...
    uint16_t entry;                 /**< Output parameter: Entry handle that is recieved after adding new MAC to the FDB. Can be used for optimizations */
} rdpa_fdb_key_t;
