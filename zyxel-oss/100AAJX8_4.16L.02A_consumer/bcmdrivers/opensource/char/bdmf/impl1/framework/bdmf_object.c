             ...
             bdmf_object_handle *pmo)
{
    ...
    if (drv->po && !owner && drv->subsystem==BDMF_SUBSYSTEM_LOCAL &&
        bdmf_find_get(drv->po, NULL, attr, &owner))
    {
    ...
    {
        mo->comm_handle = bdmf_rpc_ptr_store(mo);
        if (!mo->comm_handle)
            goto out;
    }
#endif
...
#ifdef BDMF_RPC
    if (drv->subsystem != BDMF_SUBSYSTEM_LOCAL)
        rc = rc ? rc : bdmfproxy_new(mo, attr);
    else
#endif
...
#ifdef BDMF_RPC
    if ((mo->drv->subsystem == BDMF_SUBSYSTEM_LOCAL) &&  mo->comm_handle)
    {
        bdmf_rpc_handle_remove(mo->comm_handle);
        mo->comm_handle = 0;
    }
#endif
                                       ...
                                       const char *type, bdmf_object_handle mo)
{
    ...
    BUG_ON(!owner);
#ifdef BDMF_RPC
    if (owner->drv->subsystem != BDMF_SUBSYSTEM_LOCAL)
        return bdmfproxy_get_next_child(owner, type, mo);
#endif
