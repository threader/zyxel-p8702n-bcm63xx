...
int bdmf_type_register_ext(struct bdmf_type *root, struct bdmf_type *drv)
{
    ...
    /* ignore registration of types that are not valid on this subsystem */
    if (drv->subs_valid && (drv->subs_valid != (1<<BDMF_SUBSYSTEM_ANY)) &&
        (drv->subs_valid & (1<<bdmf_subsystem()))==0)
    ...
    {
        drv->comm_handle = bdmf_rpc_ptr_store(drv);
        if (drv->comm_handle < 0)
        {
            rc = (int)drv->comm_handle;
            goto cleanup;
        }
    }
#endif
...
#ifdef BDMF_RPC
    if (drv->subsystem == BDMF_SUBSYSTEM_LOCAL)
        bdmf_rpc_handle_remove(drv->comm_handle);
#endif
...
static inline int bdmf_type_check_visibility(struct bdmf_type *drv)
{
    ...
    uint32_t local_mask = (1<<local_subs);
    if (drv->subs_visible &&
        (drv->subs_visible != (1<<BDMF_SUBSYSTEM_ANY)) &&
        (drv->subs_visible & local_mask)==0)
        return 0;
    return 1;
}
...
int bdmf_type_find_get(const char *name, bdmf_type_handle *pdrv)
{
    ...
    char type_name[20];
    bdmf_subsystem_t subs = BDMF_SUBSYSTEM_ANY;
    bdmf_subsystem_t local_subs = bdmf_subsystem();

    if (!name)
        return BDMF_ERR_PARM;
    /* Does name contain "@subsystem" ? */
    if ((subs_name = strchr(name, '@')))
    {
        strncpy(type_name, name, subs_name-name);
        type_name[subs_name-name] = 0;
        pname = type_name;
        ++subs_name;
        /* Identify subsystem */
        if (bdmf_subsystem_by_name(subs_name, &subs))
            return BDMF_ERR_NO_SUBS;
    }
    while((drv=bdmf_type_get_next(drv)))
    {
        if ((subs==BDMF_SUBSYSTEM_ANY || (subs==drv->subsystem) ||
              (subs==local_subs && drv->subsystem==BDMF_SUBSYSTEM_LOCAL)
            ) && !strcmp(drv->name, pname))
        {
            *pdrv = drv;
            return 0;
        }
...
int bdmf_type_info(bdmf_type_handle drv, bdmf_type_info_t *info)
{
    ...
    info->location = (drv->subsystem==BDMF_SUBSYSTEM_LOCAL) ? bdmf_subsystem() : drv->subsystem;
    return 0;
}
