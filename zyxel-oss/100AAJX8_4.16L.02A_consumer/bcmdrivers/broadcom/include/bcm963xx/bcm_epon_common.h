...
typedef enum 
    ...
    BCMEPONCfgLink   = 6,
    BCMEPONCfgQueue  = 7,
    BCMEPONGetMpcpStatus  = 8,
    BCMEPONCfgFec  = 9,
    BCMEPONCfgProtParm = 10,
    BCMEPONCfgTxPower = 11,
    BCMEPONDumpStats = 12,
    BCMEPONGather = 13,
    BCMEPONSilence = 14,
    BCMEPONBCap = 15,
    ...
    BCMEPONCtcPer = 22,
    BCMEPONCtcStats = 23,
    BCMEPONCtcDeallocSilence = 24,
    BCMEPONHoldover = 25,
    BCMEPONLosTime = 26,
    BCMEPONSetPid = 27,
    BCMEPONLoadPers = 28,
    BCMEPONCfgL2PonState = 29,
    BCMEPONCfgLaserEn = 30,
    BCMEPONCfgAssignMcast = 31,
    BCMEPONCfgUserTraffic = 32,
    BCMEPONCfgByteLimit = 33,
    BCMEPONCfgOamOui = 34,
    BCMEPONCfgLoopback = 35,
    BCMEPONCfgRogueOnuDet = 36,
    BCMEPONCfgClktrans = 37,
    BCM_EPON_IOCTL_MAX
    } EponioctlCmd_e;
...
typedef enum
    {
    HoldoverNoFlags     = 0,
    HoldoverReRange     = (1 << 0)  // Send register request to re-range ONU
    } PACK HoldoverFlags;

typedef struct
    {
    U16             time;    // Unit is 1ms
    HoldoverFlags   flags;
    } PACK HoldoverConfig;
...
typedef enum
    {
    EponCfgItemPortUpRateMap            = 0,
    EponCfgItemPortDnRateMap            = 1,
...
typedef struct 
    ...
    U8 link_num;
    U8 link_q_num[TkOnuNumBiDirLlids];
...
typedef enum
    {
    LlidUnregistered,           // Not registered (and not trying to)
    
    LlidRegistering,            // enqueue reg_req and not registered yet
    
    LlidWaitingReg,             // wait register from OLT
                                
    LlidInService,              // Fully registered (MPCP - we are
                                // receiving gates from the OLT)
    LlidWaitingForGates,        // Have received a register from the OLT, and
                                // enqueued a register ack, waiting for normal
                                // gates to start
    LlidStoppedForDebug = 0x7f
    } LlidState_e;
typedef U8 LlidState;
