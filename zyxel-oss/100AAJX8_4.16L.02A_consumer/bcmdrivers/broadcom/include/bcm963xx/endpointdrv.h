...
#endif
#ifdef MSTC_MODIFY	/* aircheng add for RTP statistics */
...
#include <globalCfg.h>
#define MAXQ_PACKETS  (GLOBAL_CFG_MAX_CNX * 7)
/* Queue for storing the packets received by the DSP before passing to user space. */
typedef struct PACKETQUEUE
{
   ENDPOINTDRV_PACKET packet[MAXQ_PACKETS];
   UINT32 head;
   UINT32 tail;
   UINT32 length; /* The length can be calculated knowing the head, tail
                  ** and size of the queue, but having a separate field length
                  ** eliminates the calculation. */
} PACKETQUEUE;
#endif
