...
#define BUZZZ_ASSERT_STATUS_DISABLED()                                         \
	if (buzzz_g.status != BUZZZ_STATUS_DISABLED) {                             \
		printk(CLRerr "WARN: %s tool already enabled" CLRnl, __FUNCTION__);   \
		return BUZZZ_ERROR;                                                    \
	}
               ...
               uint32_t log_size, buzzz_dump_log_fn_t dump_log_fn)
{
	...
	/* FIXME: Simplify the following options */
	if (limit != BUZZZ_INVALID) {   /* dump limited last few logs */
		...
		if (total > part1) {        /* some events prior to wrap */
			...
			while (part2--) {   /* from end-xxx to end : part2 */
		...
		while (total--) {   /* from cur-xxx to cur : part1 */
			dump_log_fn(buzzz_g.page, log);
			printk("%s", buzzz_g.page);
			log = (void*)((uint32_t)log + log_size);
		}
...
buzzz_pmon_dump(void)
{
	...
	if (buzzz_g.status != BUZZZ_STATUS_DISABLED)
		printk(CLRerr "PMon not disabled" CLRnl);
...
buzzz_kevt_dump_log(char * p, buzzz_kevt_log_t * log)
{
	...
	static uint32_t nsecs[NR_CPUS + 1];
...
buzzz_kevt_config(uint32_t config_evt)
{
	...
	if ((config_evt == BUZZZ_KEVT_GROUP_RESET) ||
		...
		printk(CLRerr "Invalid event group<%u>" CLRnl, config_evt);
		return BUZZZ_ERROR;
	}
...
buzzz_start_now(buzzz_t * buzzz_p)
{
	switch (buzzz_g.tool) {
		...
		default:
			printk(CLRerr "Unsupported start for tool %s" CLRnl,
				str_buzzz_tool(buzzz_g.tool));
			return BUZZZ_ERROR;
	}
...
buzzz_start(uint32_t after)
{
	...
	if (buzzz_g.timer.function != NULL) {
		printk(CLRerr "Timer already in use, overwriting" CLRnl);
...
buzzz_stop_now(buzzz_t * buzzz_p)
{
	switch (buzzz_g.tool) {
		...
		default:
			printk(CLRerr "Unsupported stop for tool %s" CLRnl,
				str_buzzz_tool(buzzz_g.tool));
			return BUZZZ_ERROR;
	}
	buzzz_p->timer.function = NULL;
...
buzzz_audit(void)
{
	printk(CLRerr "Unsupported audit capability" CLRnl);
	return BUZZZ_ERROR;
}
...
buzzz_dump(uint32_t items)
{
	switch (buzzz_g.tool) {
		...
		default:
			printk(CLRerr "Unsupported dump for tool %s" CLRnl,
				str_buzzz_tool(buzzz_g.tool));
			return BUZZZ_ERROR;
	}
	...
	switch (buzzz_g.tool) {
		...
		case BUZZZ_TOOL_PMON:
			if (mode == BUZZZ_MODE_WRAPOVER)
				...
				printk(CLRerr "Unsupported mode %s for %s" CLRnl,
...
buzzz_klog_reg(uint32_t klog_id, char * klog_fmt)
{
	if (klog_id < BUZZZ_KLOG_MAXIMUM)
		...
		printk(CLRerr "WARN: Too many events id<%u>" CLRnl, klog_id);
}
...
__init __init_buzzz(void)
{
	...
	/* Create a character driver and register */
	if (register_chrdev(BUZZZ_DRV_MAJOR, BUZZZ_DRV_NAME, &buzzz_fops) )
    {
        printk( CLRerr "%s Unable to get major number <%d>" CLRnl,
                  __FUNCTION__, BUZZZ_DRV_MAJOR);
        return BUZZZ_ERROR;
	} else {
		printk(CLRb "Registered device %s" BUZZZ_VER_FMTS " <%d,%d>" CLRnl,
			BUZZZ_DEV_PATH, BUZZZ_VER_FMT(BUZZZ_DEV_VERSION),
			BUZZZ_DRV_MAJOR, BUZZZ_DRV_MINOR);
	}
...
fail_dev_dereg:
    ...
    unregister_chrdev(BUZZZ_DRV_MAJOR, BUZZZ_DRV_NAME);
