...
int oops_file_num = CONFIG_MSTC_OOPS_FILE_NUM;
...
/* Open kenel oops dump file */
{
	...
	oops_file = filp_open(oops_name,
				 O_CREAT | 2 | O_NOFOLLOW | O_LARGEFILE,
				 0600);
	if (IS_ERR(oops_file)) {
		oops_file = NULL;
		printk(KERN_EMERG "Oops Log Error: No space left on device... Stop logging ---\n");
	}
}
#endif
	...
	if(oops_file != NULL) {
		if(oops_file->f_op && oops_file->f_op->fsync) {
			oops_file->f_op->fsync(oops_file, 0, LLONG_MAX, 1);
		}
