...
#ifdef CONFIG_MSTC_MODIFY //__MSTC__, Dennis, For compressing duplicated log message 
	sb_add(m, "LEN=%u TOS=0x%02X PREC=0x%02X TTL=%u ",
		   ntohs(ih->tot_len), ih->tos & IPTOS_TOS_MASK,
		   ih->tos & IPTOS_PREC_MASK, ih->ttl);
#else
		  ...
		  const char *prefix)
{
	...
	unsigned int mtsSecLog = 0;
#endif

#ifdef CONFIG_MSTC_MODIFY //__MSTC__, Dennis 
	if (strstr(prefix, "Attack") != NULL)
	{
	  mtsSecLog = 1;
	  printk("category:\"%s\" detail:\"%s " , KERN_FAC_ATTACK, prefix);
	}
	else if (strstr(prefix, "ACL") != NULL)
	{
	  mtsSecLog = 1;
	  printk("category:\"%s\" detail:\"%s " , KERN_FAC_FIREWALL, prefix);
	}
	if (mtsSecLog == 1)
	{
	  printk("IN=%s OUT=%s ",
