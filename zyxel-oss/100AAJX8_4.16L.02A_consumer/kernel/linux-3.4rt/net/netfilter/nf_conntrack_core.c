...
#if defined(CONFIG_MSTC_MODIFY)

/*MSTC, Nils, Session control by local address.*/

unsigned int nf_session_ctl_max __read_mostly ;	 /*setup with /proc/sys/net/netfilter/ */
...
#if 0 //defined(CONFIG_MSTC_MODIFY) /*ZyXEL, Nils, Cone NAT supported*/ /* Clean all session tables */ 	
...
static int nf_conntrack_init_init_net(void)
{
	...
	if (!nf_conntrack_htable_size) {
#if defined(CONFIG_MSTC_MODIFY) //__MTSC__, Delon Yu, Bugfix: port triggering can only handle 16 entries
