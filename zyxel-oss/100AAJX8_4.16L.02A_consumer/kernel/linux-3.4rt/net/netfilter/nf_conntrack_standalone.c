...
#ifdef CONFIG_MSTC_NORWAY_VOIP_CUSTOMIZATION
int
m_print_tuple(struct seq_file *s, const struct nf_conntrack_tuple *tuple,
            const struct nf_conntrack_l3proto *l3proto,
            const struct nf_conntrack_l4proto *l4proto)
{
	//return l3proto->print_tuple(s, tuple) || l4proto->print_tuple(s, tuple);
	seq_printf(s, "%pI4:%hu\n", &tuple->src.u3.ip,ntohs(tuple->src.u.udp.port));
	return seq_printf(s, "%pI4:%hu\n", &tuple->dst.u3.ip,ntohs(tuple->dst.u.udp.port));
}
EXPORT_SYMBOL_GPL(m_print_tuple);
#endif
...
/* return 0 on success, 1 in case of error */
static int udp_port_ct_seq_show(struct seq_file *s, void *v)
{
	struct nf_conntrack_tuple_hash *hash = v;
	struct nf_conn *ct = nf_ct_tuplehash_to_ctrack(hash);
	const struct nf_conntrack_l3proto *l3proto;
	...
	ret = -ENOSPC;
	if(!strcmp(l4proto->name,"udp")){
		//only record udp.		

		if (l4proto->print_conntrack && l4proto->print_conntrack(s, ct))
			goto release;

		if (m_print_tuple(s, &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple,
				l3proto, l4proto))
			goto release;
		...
		if (m_print_tuple(s, &ct->tuplehash[IP_CT_DIR_REPLY].tuple,
				l3proto, l4proto))
			goto release;
...
static const struct seq_operations udp_port_ct_seq_ops = {
	.start = ct_seq_start,
	.next  = ct_seq_next,
	.stop  = ct_seq_stop,
	.show  = udp_port_ct_seq_show
};

static int udp_port_ct_open(struct inode *inode, struct file *file)
{
	struct seq_file *seq;
	...
	if (st == NULL)
		return -ENOMEM;
	ret = seq_open(file, &udp_port_ct_seq_ops);
	if (ret)
		goto out_free;
...
static const struct file_operations udp_port_ct_file_ops = {
	.owner   = THIS_MODULE,
	.open    = udp_port_ct_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = seq_release_net,
};
...
#if 0
	while (is_a_nulls(head)) {
		if (likely(get_nulls_value(head) == &net->ct.session_control_table[st->bucket])) {
			if (++st->bucket >= net->ct.htable_size)
				return NULL;
		}
...
static ctl_table nf_ct_sysctl_table[] = {
	...
	/*MSTC, Nils, Session control by local address.*/	 
	{
			//.ctl_name	= NET_NF_SESSION_CTL_MAX,
...
/*MSTC, Nils, Session control by local address.*/
	...
	{
#ifdef CONFIG_MSTC_NORWAY_VOIP_CUSTOMIZATION
		struct proc_dir_entry *udp_port_proc;
#endif	
		struct proc_dir_entry *proc_stat;
			...
			if (!proc_stat)
				goto err_3;
#ifdef CONFIG_MSTC_NORWAY_VOIP_CUSTOMIZATION
			udp_port_proc = proc_net_fops_create(net, "nf_udp_port", 0440, &udp_port_ct_file_ops);
			if (!udp_port_proc) goto out_unregister_netfilter;
			
			proc_stat = proc_create("nf_udp_port", S_IRUGO, net->proc_net_stat, &ct_cpu_seq_fops);
			if (!proc_stat)
				goto err_4; 	
#endif				
	}
...
#ifdef CONFIG_MSTC_NORWAY_VOIP_CUSTOMIZATION
err_4:
	proc_net_remove(net, "nf_udp_port");
#endif
...
#endif
#ifdef CONFIG_MSTC_NORWAY_VOIP_CUSTOMIZATION
	remove_proc_entry("nf_udp_port", net->proc_net_stat);
	proc_net_remove(net, "nf_udp_port");
#endif	
	kfree(table);
}
