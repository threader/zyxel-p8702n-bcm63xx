...
#if (defined(CONFIG_MSTC_DEC_AND_GZIP_CORE) || defined(CONFIG_MSTC_RECORD_CALL_TRACE))
...
#define ITEM_FILE_NAME	"/tmp/coredump_rule.txt"
...
int core_pre_threshold = CONFIG_MSTC_CORE_SIZE_PRE_THRESHOLD;
int core_post_threshold = CONFIG_MSTC_CORE_SIZE_POST_THRESHOLD;


extern long sys_mkdir(const char *pathname, umode_t mode);
...
static int parseConf(char *fileName)
{
	...
	len = fp->f_op->read(fp, buf, BUFFER_SIZE, &fp->f_pos);
	if(len == BUFFER_SIZE) {
...
static int check_permitted_cores(char *corename, const char *path)
{
	...
	do {
		...
		list_for_each(list, &file->f_dentry->d_subdirs) {
...
void do_coredump(long signr, int exit_code, struct pt_regs *regs)
{
	...
	#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
	//unsigned long core_limit = current->signal->rlim[RLIMIT_CORE].rlim_cur = RLIM_INFINITY;
	#endif	
...
#ifdef CONFIG_MSTC_DEC_AND_GZIP_CORE
		{
			sprintf(&cn.corename[strlen(cn.corename)], ".gz");
