...
#ifdef CONFIG_MSTC_MODIFY /* __MSTC__, zongyue: to fix ImageDefault start position and RootFS size if NAND FLASH have bad blocks */
#include <board.h>
	...
	int bits, int offs, int reserved_block_code)
{
	...
	unsigned int rootfs_start_addr, rootfs_end_addr;
	int flash_rootfs_bad_block_number = 0;
	PFILE_TAG pTag = kerSysImageTagGet();

	rootfs_start_addr = ((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET) - FLASH_BASE;
	rootfs_end_addr = ((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET + 
									 (unsigned int)simple_strtoul(pTag->rootfsLen, NULL, 10)) - FLASH_BASE;
	PRINTK(KERN_DEBUG "%s: ROOTFS start at 0x%08x, end at 0x%08x\n", __FUNCTION__, rootfs_start_addr, rootfs_end_addr);
#endif
		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < 8; j += bits, act += 2) {
				...
				printk (KERN_DEBUG "nand_read_bbt: Bad block at 0x%08x\n",
					((offs << 2) + (act >> 1)) << this->bbt_erase_shift);
#ifdef CONFIG_MSTC_MODIFY /* __MSTC__, zongyue: to fix ImageDefault start postion and RootFS size if NAND FLASH have bad blocks */
				if ((rootfs_start_addr <= (((offs << 2) + (act >> 1)) << this->bbt_erase_shift) &&
					(((offs << 2) + (act >> 1)) << this->bbt_erase_shift) <= rootfs_end_addr)) {
					flash_rootfs_bad_block_number++;
				}
#endif				
...
#ifdef CONFIG_MSTC_MODIFY /* __MSTC__, zongyue: to fix ImageDefault start postion and RootFS size if NAND FLASH have bad blocks */
	kerSysFlashBadBlockNumSet(flash_rootfs_bad_block_number);
#endif
	return 0;
}
...
#ifdef CONFIG_MSTC_MODIFY /* __MSTC__, zongyue: to fix ImageDefault start position and RootFS size if NAND FLASH have bad blocks */
	int rootfs_start_block, rootfs_end_block;
	int flash_rootfs_bad_block_number = 0;
	PFILE_TAG pTag = kerSysImageTagGet();
#endif
...
#ifdef CONFIG_MSTC_MODIFY // __MSTC__, zongyue: to fix ImageDefault start position and RootFS size if NAND FLASH have bad blocks
	rootfs_start_block = getNandBlock(((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET), 1);
	rootfs_end_block = getNandBlock(((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET + 
									 (unsigned int)simple_strtoul(pTag->rootfsLen, NULL, 10)), 1);
#endif
...
PRINTK("$$$$$$$$$$$$$$$ Bad eraseblock %d at 0x%08x, ret=%d\n",
			...
			mtd->ecc_stats.badblocks++;
#ifdef CONFIG_MSTC_MODIFY // __MSTC__, zongyue: to fix ImageDefault start position and RootFS size if NAND FLASH have bad blocks
			if ((rootfs_start_block <= (i >> 1))&&((i >> 1) <= rootfs_end_block)) {
				flash_rootfs_bad_block_number++;
				/* rootfs_end_block++; */
				kerSysFlashBadBlockNumSet(flash_rootfs_bad_block_number);
			}
			PRINTK(KERN_WARNING "%s: bad block number = %d\n", __FUNCTION__, flash_rootfs_bad_block_number);
#endif			
		}
