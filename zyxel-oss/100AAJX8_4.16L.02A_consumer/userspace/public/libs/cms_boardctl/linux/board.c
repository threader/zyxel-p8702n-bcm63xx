           ...
           if (ret == CMSRET_SUCCESS)
           {
              if ((boardIoctl == BOARD_IOCTL_GET_PSI_SIZE) ||
                  ...
                  (boardIoctl == BOARD_IOCTL_SET_USBTEST) ||
					  ...
					  (boardIoctl == BOARD_IOCTL_REINJECTION) ||
#endif					  	
...
int sysGetNVRAMFromFlash( PNVRAM_DATA nvramData )
{
	...
	ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, NVRAM, (char*) nvramData, sizeof(NVRAM_DATA), 0, NULL);
   	return ret;
}
...
int sysSetNVRAMToFlash( PNVRAM_DATA nvramData )
{
	...
	ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_WRITE, NVRAM,  (char*) nvramData, sizeof(NVRAM_DATA), 0, NULL);
   	return ret;
}
...
int sysGetTAGFromFlash( PFILE_TAG tagData )
{
	...
	ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, FILETAG,  (char*) tagData, sizeof(FILE_TAG), 0, &imageNumber);
   	return ret;
}

#ifdef MSTC_OBM_IMAGE_DEFAULT //__MSTC__, Dennis ZyXEL ImageDefault feature, zongyue
int sysGetIMAGETAGFromFlash( PIMAGE_TAG tagData )
{
	CmsRet ret = CMSRET_SUCCESS;

	ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, IMAGETAG,  (char*) tagData, sizeof(IMAGE_TAG), 0, NULL);
   	return ret;
}
#endif
...
int sysUSBTestSet( int value )
{
	...
	ret = devCtl_boardIoctl(BOARD_IOCTL_SET_USBTEST, 0, NULL, value, 1, NULL);
   	return ret;
}
...
int sysUSBTestUnSet( int value )
{
	...
	ret = devCtl_boardIoctl(BOARD_IOCTL_SET_USBTEST, 0, NULL, value, 0, NULL);
   	return ret;
}
#endif

#ifdef MSTC_MODIFY // __MSTC__, Paul Ho, for Wireless country code
