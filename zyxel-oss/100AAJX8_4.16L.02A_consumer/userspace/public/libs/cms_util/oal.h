...
extern int oal_readLogPartial_MSTC(int ptr, char* buffer, int logType);
extern int oal_readLogFromFile(char* filename, int offset, char* buffer);
#endif
...
extern CmsRet oal_getIfLinkLocalAddr6(const char *ifname, char *ipAddr);
#endif
#ifdef MSTC_MODIFY //__MSTC__,Lynn,DHCP
extern CmsRet oal_getPortNameFromMac(const char *ifName, const unsigned char *macAddr, char *portName);
extern CmsRet oal_getMacFromIpAddr(const struct in_addr ipaddr, const char *brName, unsigned char *macaddr);
#endif
