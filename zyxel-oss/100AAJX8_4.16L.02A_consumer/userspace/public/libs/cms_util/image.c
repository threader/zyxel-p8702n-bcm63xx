...
{
#ifdef MSTC_MODIFY //__COMMON__, jchang for model id check
...
#ifdef MSTC_MODIFY /* If can't find the MSTC FW ID, should reject the FW */
...
CmsRet verifyBroadcomFileTag(FILE_TAG *pTag)
{
    ...
    crc = CRC_INITIAL_VALUE;
#ifdef MSTC_MODIFY // __MSTC__, zongyue
...
CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, UINT32 opts)
{
    ...
    int cfeSize, rootfsSize, kernelSize, noReboot;
#endif
    unsigned long cfeAddr, rootfsAddr;
	...
	if( needReboot == FALSE )
	{
		...
		// upgrade to current partition or others, all need reboot.
		needReboot = TRUE;
	}else{
...
void cmsImg_sendLoadfailMsg(void *msgHandle)
{
   CmsMsgHeader msg = EMPTY_MSG_HEADER;

   msg.type = CMS_MSG_LOAD_IMAGE_fail;
...
#ifdef MSTC_ROM_D  //__COMMON__, jchang, ROM-D      
...
#if 0
	if(result == CMS_IMAGE_FORMAT_INVALID ){
		cmsImg_sendLoadfailMsg(msgHandle);
	}
#endif
...
static void saveLog(void *msgHandle)
{
	...
	if( cmsMsg_sendAndGetReply(msgHandle, &msg) == CMSRET_SUCCESS ){
		...
		msg2.flags_request = 1;
		cmsMsg_sendAndGetReplyWithTimeout(msgHandle, &msg2,10*1000);
	}
}
