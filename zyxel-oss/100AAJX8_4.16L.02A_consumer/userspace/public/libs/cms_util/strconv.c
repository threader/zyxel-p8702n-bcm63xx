...
UBOOL8 cmsUtl_isValidIpAddressRange(const char* min_input , const char* max_input)
{
   ...
   UINT32 i = 0, num_min = 0, num_max = 0;
   ...
   strtok_r(buf_min, ".", &pLast_min);
   strtok_r(buf_max, ".", &pLast_max);
   for (i = 0; i < 3; i++) {
...
#else
      cmsLog_debug("null pointer reference src=0x%x dest=0x%x", src, dest);
#endif
#ifdef MSTC_MODIFY //__MSTC__, TengChang, MTS LOG parameters, migrate from Common_406
      if ( src == NULL && dest != NULL ) {
         strcpy(dest, "");
      }
...
#ifdef MSTC_MODIFY //__MSTC__,Lynn
...
#ifdef MSTC_MODIFY // __MSTC__, Richard Hunag
...
UBOOL8 cmsUtl_isCgiUnsafeString(const char *compareChar)
{
   SINT32 i, j, data_len;
   char unsafeString[] = {'\"', '<', '>', '%', '\\', '^', '[', ']', 
                              '`', '+', '$', '=', '\'', '#', '&', '\t', 
   ...
   //Do not check the string if the string is empty.
   if ( IS_EMPTY_STRING(compareChar))      
      return FALSE;   
...
UBOOL8 cmsUtl_isValidDialString(const char *compareChar)
{
   SINT32 i, j, data_len;
   char unsafeString[] = {'\"', '<', '>', '%', '\\', '^', '[', ']', 
                              '`', '+', '$', '=', '\'', '&', '\t', 
...
#ifdef MSTC_MODIFY //__MSTC__, Steven
...
#ifdef MSTC_MODIFY // __MSTC__, Axel, merge cli bonding state command form P-873HNUP-51B
/*
