...
#ifdef INDEX_REMAPPING_FOR_43602
#define WL_CHIP_UNKNOW     0
#define WL_CHIP_4360       1
#define WL_CHIP_43602      2
#endif
...
#ifdef INDEX_REMAPPING_FOR_43602
void convertIfName(const char *oriIfName, char *newIfName );
#endif
...
#ifdef INDEX_REMAPPING_FOR_43602
int Wlan_getDeviceId(int idx)
{
	int  found=0;
	unsigned int deviceid;
	char cmd[80];
	FILE *fp = NULL;
	char match_str[]="deviceid";
...
int Wlan_getChipType(void)
{
	int wl0DevId = Wlan_getDeviceId(0);
	int wl1DevId = Wlan_getDeviceId(1);

	if (wl0DevId == 0x435f)
		return WL_CHIP_4360;
	else if (wl1DevId == 0x435f)
		return WL_CHIP_43602;

	return WL_CHIP_UNKNOW;
}


void convertIfName(const char *oriIfName, char *newIfName )
{
	char tmpIfName[BUFLEN_32] = {0};
	char tmpIfNameNum[BUFLEN_4] = {0};
	
	sprintf(tmpIfName, "%s", oriIfName);
	
	if (strstr(oriIfName, "wl") != NULL){

		int chipType = Wlan_getChipType();
		
		if (chipType == WL_CHIP_43602){
			sprintf(tmpIfNameNum, "%d", !atoi(&oriIfName[2]));
			tmpIfName[2] = tmpIfNameNum[0];
		}
	}

	sprintf(newIfName, "%s", tmpIfName);
	return;
...
{	
#ifdef INDEX_REMAPPING_FOR_43602
    char ifnameStr[BUFLEN_128];
	convertIfName(ifname, ifnameStr);
	return (oal_isInterfaceUp(ifnameStr));
#else
...
UBOOL8 cmsNet_isInterfaceLinkUp(const char *ifname)
{
	...
	return (oal_isInterfaceLinkUp(ifnameStr));
#else
                         ...
                         char *ipAddr, UINT32 *ifIndex, UINT32 *prefixLen, UINT32 *scope, UINT32 *ifaFlags)
{
	...
	cmsLog_debug("%s->%s", ifname, ifnameStr);
	return oal_getIfAddr6(ifnameStr, addrIdx, ipAddr, ifIndex, prefixLen, scope, ifaFlags);
#else
...
#ifdef MSTC_MODIFY //__MSTC__, SinJia, Numbered mode and unnumber mode for ipv6 support
/* Extract 64 from 2222:2222:2222:2112:219:cbff:fe33:2288/64 */
int cmsNet_getPrefixLength(const char *prefix_addr){
   char tmp[CMS_IPADDR_LENGTH];   
   char *token;

   strcpy(tmp, prefix_addr);
...
CmsRet cmsNet_getPortNameFromMac(const char *ifName, const unsigned char *macAddr, char *portName)
{
    return oal_getPortNameFromMac(ifName, macAddr, portName);
}

CmsRet cmsNet_getMacFromIpAddr(const struct in_addr ipaddr, const char *brName, unsigned char *macaddr)
{
    return oal_getMacFromIpAddr(ipaddr, brName, macaddr);
}
#endif

#ifdef DMP_X_5067F0_IPV6_1 // __MSTC__, Richard Huang
CmsRet cmsNet_getIfLinkLocalAddr6(const char *ifname, char *ipAddr)
{
    return oal_getIfLinkLocalAddr6(ifname, ipAddr);
}
#endif
