...
MODRET authfile_auth(cmd_rec *cmd) {
  ...
  char strSubnet[16] = {0};
  char strRouter[16] = {0};
  char strClnaddr[16] = {0};  
  struct in_addr clntAddr, inAddr, inMask;
  pr_netaddr_t *remote_addr = pr_netaddr_get_sess_remote_addr();
  int isOnLanSide = 0;
  if (remote_addr)
  {
    strncpy(strClnaddr, pr_netaddr_get_ipstr(remote_addr), sizeof(strClnaddr)-1);
  }	
  if ((strClnaddr != NULL) && (strchr(strClnaddr, ':') == NULL))
  {    
    clntAddr.s_addr = inet_addr(strClnaddr);
    fp = fopen("/etc/udhcpd.conf","r");
    if (fp != NULL)
    {
      while (fgets(buf, sizeof(buf), fp) != NULL) 
      {
        if (strstr(buf, "interface br0"))
        {
          found = 1;
        }
        if (found && strstr(buf,"option subnet"))
        {
          strncpy(strSubnet, buf+14, sizeof(strSubnet)-1);
          found ++;        
        }
        if (found && strstr(buf,"option router"))
        {
          strncpy(strRouter, buf+14, sizeof(strRouter)-1);
          found ++;        
        }
        if (found > 2)
          break;
    ...
    if ((strSubnet != NULL) && (strRouter != NULL))	
    {
      inMask.s_addr = inet_addr(strSubnet);
      inAddr.s_addr = inet_addr(strRouter);	
  ...
  if ((isOnLanSide == 1 && strcmp(name, "Kung")) ||
	 (isOnLanSide == 0 && strcmp(name, "Kundservice")))
  {  
    pr_log_auth(PR_LOG_NOTICE, "%s is not allowed to access", name);
    return PR_DECLINED(cmd);
  }
#endif
