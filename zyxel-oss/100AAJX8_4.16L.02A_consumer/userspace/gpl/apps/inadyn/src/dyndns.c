...
static RC_TYPE do_update_alias_table(DYN_DNS_CLIENT *p_self)
{
	int i;
	RC_TYPE rc = RC_OK;
	int isServerUpdate = 0; /* Support DDNS status page, __MSTC__, yic */
	FILE *fp;
	time_t now;
	time(&now);
	do 
		...
		for (i = 0; i < p_self->alias_info.count; ++i)
		{
			...
			/*build dyndns transaction*/
			{
				...
				if (rc == RC_OK)
				{
					...
					if ((fp = fopen("/var/ddns_status.log", "w+")) != NULL ) /* Support DDNS status page, __MSTC__, yic */
					{
						fprintf(fp,"address=%s",p_self->info.my_ip_address.name);
						if (update_ok==1)
						{
							if (strstr(p_self->info.dyndns_server_name.name, DYNDNS_SYSTEM_DYNAMIC) != NULL)
							{
#ifdef MSTC_MODIFY							
								if (strstr(http_tr.p_rsp, "127.0.0.1") != NULL)
								{
									fprintf(fp,"/is_success=%d",-1); /*updated is fail.*/
								}
								else if (strstr(http_tr.p_rsp, DYNDNS_OK_RESPONSE) != NULL)
#else
								if (strstr(http_tr.p_rsp, DYNDNS_OK_RESPONSE) != NULL)
#endif
								{
									fprintf(fp,"/is_success=%d",1); /*updated on server is success.*/
									isServerUpdate = 1;
								}
								else if (strstr(http_tr.p_rsp, DYNDNS_OK_NOCHANGE) != NULL)
								{
									fprintf(fp,"/is_success=%d",2); /*updated on server is success but the ip is no chang*/
								}
								else
								{
									fprintf(fp,"/is_success=%d",-1); /*updated is fail.*/
							...
							else if (strstr(p_self->info.dyndns_server_name.name, NO_IP_SYSTEM_DYNAMIC) != NULL)
							{
								if (strstr(http_tr.p_rsp, DYNDNS_OK_RESPONSE) != NULL)
								{
									fprintf(fp,"/is_success=%d",1); /*updated on server is success.*/
							...
							else if (strstr(p_self->info.dyndns_server_name.name, FREEDNS_SYSTEM_DYNAMIC) != NULL)
							{
								if (strstr(http_tr.p_rsp, FREEDNS_UPDATED) != NULL)
								{
									fprintf(fp,"/is_success=%d",1); /*updated on server is success.*/
									isServerUpdate = 1;
								}
								...
								{
									fprintf(fp,"/is_success=%d",3); /*print the error message from freedns server.*/
								}
							}
							else
							{
						...
						fprintf(fp,"/last_date=%s",ctime(&now));
						fclose( fp );
						if (isServerUpdate == 1)
						{
							if ((fp = fopen("/data/log/ddns_status_time.log", "w+")) != NULL )
							{
...
#ifdef MSTC_MODIFY
	FILE *fs;
	if ((fs = fopen("/var/ddns_status.log", "w")) != NULL) {
		fprintf(fs,"%s","enter//");
		fclose(fs);
	}
#endif
...
#else
			FILE *fs; /* Support DDNS status page, __MSTC__, yic */
			if ((fs = fopen("/var/ddns_status.log", "w+")) != NULL )
			{
			   fprintf(fs,"%s","//");
			   fclose(fs);
