...
#ifdef MSTC_MODIFY //__MSTC__, TengChang
#include "cms_log.h"
#include "cms_mem.h"
...
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
static int logpritmp = 0;
#endif
...
#ifdef MSTC_MODIFY //__MSTC__, TengChang, compress duplicated logs
#include <regex.h>
...
#endif
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
struct shbuf_ds *phoneBuf = NULL;
#endif
int copyProtect = 0;/*for the copy log to flash*/  
...
int VARlogfd = -1;
unsigned VARlogFileSize = 300 * 1024;
unsigned VARcurFileSize;
smallint VARisRegular;
unsigned VARlogFileRotate = 1;
#endif
...
#define MACADDRESS_STR 		 "MAC Address"
...
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
	nextHead = (char*)secBuf + (G.shm_size/LOG_NUM);
	phoneBuf = nextHead;
	phoneBuf->size = (G.shm_size/LOG_NUM) - offsetof(struct shbuf_ds, data) - 1;
#endif
#else
...
/* Print a message to the /var/log/syslog.log file. */

static void VAR_LOG_Syslog(time_t now, char *msg)
{
	int len = strlen(msg);
	
    /*VARlogfd=open(VAR_LOG_SYSLOGFILE, O_WRONLY | O_CREAT
 ...
 reopen:
		...
		if (VARlogfd < 0) {
			...
			int fd2 = device_open(DEV_CONSOLE, O_WRONLY | O_NOCTTY | O_NONBLOCK);
			if (fd2 < 0)
				fd2 = 2; /* then stderr, dammit */
			full_write(fd2, msg, len);
			if (fd2 != 2)
				close(fd2);
			return;
		}
/*ROTATE_LOGFILE*/
		{
			struct stat statf;
			VARisRegular = (fstat(VARlogfd, &statf) == 0 && S_ISREG(statf.st_mode));
			/* bug (mostly harmless): can wrap around if file > 4gb */
			VARcurFileSize = statf.st_size;
		}
	}

if (VARlogFileSize && VARisRegular && VARcurFileSize > (VARlogFileSize - 1024)) {
	
		if (VARlogFileRotate) { /* always 0..99 */
			int i = strlen(VAR_LOG_SYSLOGFILE) + 3 + 1;			
			char oldFile[i];
			char newFile[i];
			i = VARlogFileRotate - 1;
			/* rename: f.8 -> f.9; f.7 -> f.8; ... */
			while (1) {
				sprintf(newFile, "%s.%d", VAR_LOG_SYSLOGFILE, i);
				if (i == 0) break;
				sprintf(oldFile, "%s.%d", VAR_LOG_SYSLOGFILE, --i);
				/* ignore errors - file might be missing */
				rename(oldFile, newFile);
			}
			/* newFile == "f.0" now */
			rename(VAR_LOG_SYSLOGFILE, newFile);

			close(VARlogfd);
			goto reopen;
		}
		ftruncate(VARlogfd, 0);
	}
...
/*ROTATE_LOGFILE*/
	...
	if(!strcmp(msg,""))
	{
		...
		VARcurFileSize += full_write(VARlogfd, repeatLog, len);		
	}	
	else if(repeatcount < 1)
	{
		...
		len = strlen(msg);		
		VARcurFileSize += full_write(VARlogfd, msg, len);		
	}         
...
#endif
{
	...
	int len = strlen(msg);
	char str[256];
	const char *tempLogFilePath=NULL;
...
#endif
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
		else if(logType == PHONE_TYPE)
			log_to_shmem(msg, len, phoneBuf);
 ...
 reopen:
	...
	if (G.logFileSize && G.isRegular && G.curFileSize > (G.logFileSize - 1024)) {
		if (G.logFileRotate) {
			...
			if(copyProtect !=0) {/*do not save log into flash*/	
...
#ifdef MSTC_MODIFY //__MSTC__, TengChang, Modify format of timestamp
	struct tm *tmdata;
...
#ifdef MSTC_MODIFY //__MSTC__, TengChang, Category
...
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
	if(!strncmp(facility,"VoIP", strlen("VoIP"))){
		phoneLog = 1;
		logType = PHONE_TYPE;
	}
...
*/
	...
	if(phoneLog){
		tempLogFilePath = G.logFilePath;
		tempFlashLogFilePath = flashLogFilePath;
		G.logFilePath = PHONE_LOG_FILE;
		flashLogFilePath = PHONE_FLASH_LOG_FILE;
	}
...
#ifdef BUILD_NORWAY_CUSTOMIZATION
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
    if(logType==SYSTEM_TYPE || logType==SECURITY_TYPE || logType==PHONE_TYPE ){
#else
	if(logType==SYSTEM_TYPE || logType==SECURITY_TYPE){
#endif
		    VAR_LOG_Syslog(now, G.printbuf);
			}
#endif
...
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
	if((fp = fopen(PHONE_LOG_FILE, "r"))==NULL)
	{
		return ;
	}
	while(fgets(line,2048,fp)){
		log_to_shmem(line, strlen(line), phoneBuf);
	}
	fclose(fp);
	...
	if (fp == NULL) {
		...
		if (fp != NULL) {
			...
			if (!sysGetNVRAMFromFlash(&nvramData)) {
				...
				// MAC Address
				memset(tempStr, 0, sizeof(tempStr));
				if (nvramData.ucaBaseMacAddr != NULL){
					sprintf(tempStr ,"%02X%02X%02X%02X%02X%02X\n", (UINT8)nvramData.ucaBaseMacAddr[0],
					   (UINT8)nvramData.ucaBaseMacAddr[1],(UINT8)nvramData.ucaBaseMacAddr[2],
					   (UINT8)nvramData.ucaBaseMacAddr[3], (UINT8)nvramData.ucaBaseMacAddr[4],
					   (UINT8)nvramData.ucaBaseMacAddr[5]);
				}else{
...
static void syslogd_prefix(char *input, int size, const struct in_addr *serverIP)
{
	...
	char macAddr[32] = {'\0'};
	char serialNum[32] = {'\0'};
#ifndef BUILD_NORWAY_CUSTOMIZATION
	char modelName[32] = {'\0'};
#endif
...
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION// for syslog
    snprintf(input, size, "[%s] [MAC %s] [SN %s] [SYS-UPTIME %s]",
			ipAddr, macAddr, serialNum, sysupTime);
#else
#ifdef BUILD_NORWAY_CUSTOMIZATION
	syslogd_nvram_read(macAddr, MACADDRESS_STR, sizeof(macAddr));
	snprintf(input, size, "[%s] [MAC %s] [SN %s] [%s] [SYS-UPTIME %s]",
				ipAddr, macAddr, serialNum, "P8702N", sysupTime);
#else
...
// brcm end
		...
		if(isLogFull && !isLogFullAlarmSent){
			...
			if ((now - last_check_mailsend) >= CHECK_MAILSEND_INTERVAL){
				...
				if(mailResult && fgets(result,2,mailResult) && atoi(result)){
					...
					fullAlarmSentCount++;
					printf("Send log capacity full alarm by mail\n");
					last_check_mailsend = now - last_check_mailsend -1;
				}
                if(mailResult) {
                    fclose(mailResult);                
...
#ifdef MSTC_MODIFY //__MSTC__, TengChang, read log from sock_fd to recvbuf
 ...
 read_again:
         ...
         */
         ...
         while(1)
         	{
					if (str[strlentemp] != '\0' && str[strlentemp] != '\n'){
						strlentemp++;
					}
					else{
						break;
         ...
         {  
           ...
           if(!strcmp(facility,"Crash"))
              logpritmp = 4;
            if(match||!strcmp(facility,"Crash")){
			 ...
			 * so that DNS resolution and connect is retried? */
				...
				char timetmp[17] = {'\0'};
				char *categorytmp =NULL;
				char *modelnametmp[32] = {'\0'};
				char detailtmp[MAX_READ] = {'\0'};
				if(logType == BRCM_TYPE)
					strcpy(facility,"kernel");
				
				ptr3 = strstr(recvbuf, ">");
               
				strncpy(timetmp, ptr3, 16);
				categorytmp = facility;
				strncpy(detailtmp,str,strlentemp);
				syslogd_nvram_read(modelnametmp, MODELNAME_STR, sizeof(modelnametmp));
	            
				if(strcmp(categorytmp,"kernel") == 0)
					strncpy(temp1, recvbuf, 3);
				else
					strncpy(temp1, recvbuf, 4);
				syslogd_prefix(temp2, sizeof(temp2), &(rh->remoteAddr->u.sin.sin_addr));
				snprintf(temp3, sizeof(temp3), "%s%s %s %s:[%s] [%d] %s", temp1, timetmp+1, temp2, categorytmp, modelnametmp, logpritmp, detailtmp);
				if(strcmp(categorytmp,"kernel") == 0)
				sendto(rh->remoteFD, temp3, strlen(temp3), MSG_DONTWAIT,
				&(rh->remoteAddr->u.sa), rh->remoteAddr->len);
				else
				sendto(rh->remoteFD, temp3, strlen(temp3)-1, MSG_DONTWAIT,
				&(rh->remoteAddr->u.sa), rh->remoteAddr->len);	
				// __MSTC__, yenling end
   			}
			}/* __MSTC__, Klose,DSL-2492GNU-B1B_Eircom [SPR 121023887], Fix logsetting remote filter problem, 20121115 */
...
#endif
#endif
#ifndef BUILD_TMPC2_COMMON_CUSTOMIZATION
		if (/*!ENABLE_FEATURE_REMOTE_LOG ||*/ (option_mask32 & OPT_locallog)) {
			recvbuf[sz] = '\0'; /* ensure it *is* NUL terminated */
			split_escape_and_log(recvbuf, sz);
		}
...
#endif
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
						else if(msg->wordData == PHONE_TYPE)
						{
							memset(phoneBuf, 0, phoneBuf->size);
							phoneBuf->size = (G.shm_size/LOG_NUM) - offsetof(struct shbuf_ds, data) - 1;
						}
#endif
...
#endif
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION
						copyLogToFlash(PHONE_LOG_FILE, PHONE_FLASH_LOG_FILE, G.logFileSize, G.logFileRotate);
						checkIsLogFull(PHONE_FLASH_LOG_FILE);
#endif
...
{
   printf("\nsyslog received signal %d", signum);
...
#ifdef MSTC_MODIFY //__MSTC__, TengChang, Log Category
