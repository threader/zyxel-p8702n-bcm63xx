...
{
	/* save current system time for next boot up time */
	time_t cur_time = time(NULL);
	char output_time[32];
	struct tm *tmp_time;

	/* start to save system time */   
	tmp_time = localtime(&cur_time);
	if(tmp_time == NULL){
	  printf("Get localtime fail");
	}
	
	if(strftime(output_time, sizeof(output_time), "%Y%m%d%H%M.%S", tmp_time) == 0){
	  printf("Get strftime fail");
	}
	
	FILE *fp=NULL;
	if( NULL == ( fp = fopen("/data/currentTime","w+") ))
	{
		printf("could not open /data/currentTime");
	}else{
		fwrite(output_time, 1, 32, fp);
		fclose(fp);
	}
   
	printf("reboot time: %s", output_time);
	/* end to save system time */ 
...
void rotateLog(char *flashLogFilePath, int logFileRotate)
{
	...
	for(i=logFileRotate-1;i>0;i--) {
		sprintf(oldFile, "%s%d.log", flashLogFilePath, i-1);
		sprintf(newFile, "%s%d.log", flashLogFilePath, i);
...
void copyLogToFlash(char *logFilePath, char *flashLogFilePath, unsigned int logFileSize, int logFileRotate)
{
	...
	FILE *dummyFP=NULL,*dummyFP2=NULL;
	char log_0_file[128];
	sprintf(log_0_file, "%s%d.log", flashLogFilePath, 0);
	if((dummyFP = fopen(logFilePath,"r")) == NULL){
		return;
	}
	else if((dummyFP2 = fopen(log_0_file,"r")) == NULL){
		fclose(dummyFP);
		printf("move %s to flash\n", logFilePath);
		sprintf(cmd,"cp %s %s%d.log", logFilePath, flashLogFilePath, 0);
		system(cmd);
		sleep(1);
		sprintf(cmd,"rm %s", logFilePath);
		system(cmd);
	}
	else{
		fclose(dummyFP);
		...
		sprintf(cmd,"cp %s%d.log %s", flashLogFilePath, 0, tempFile);
		system(cmd);
		printf("save flash done\n");
		...
		sprintf(cmd,"cp %s %s%d.log", tempFile, flashLogFilePath, 0);
		system(cmd);
...
#ifdef MSTC_SAVE_LOG_TO_FLASH //__MSTC__, TengChang
	if ( strcmp(m, "reboot") == 0 ) {
		copyLogToFlash(LOG_FILE, FLASH_LOG_FILE, 100*1024, 5);
		copyLogToFlash(SECURITY_LOG_FILE, SECURITY_FLASH_LOG_FILE, 100*1024, 5);
	}
#endif //__MSTC__, TengChang
...
int init_main(int argc UNUSED_PARAM, char **argv)
{
	...
	putenv((char *) "LD_LIBRARY_PATH=/lib/public:/lib/private:/lib/gpl:/lib");
#endif
