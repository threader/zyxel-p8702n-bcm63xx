...
#ifdef MSTC_LOG //__MSTC__, TengChng
        zyLog_app(LOG_LEVEL_WARNING, LOG_FAC_ACCOUNT, "User %s login from FTP failed", user);
#endif //__MSTC__, TengChng
		return 1;
    }
...
#else
   if(userinfo.pw_uid == 0 || (accessMode == NETWORK_ACCESS_DISABLED))
#endif  
#else
    if ((accessMode == NETWORK_ACCESS_DISABLED) ||
...
#endif //__MSTC__, Dennis
    {
       ...
       control_printf(SL_FAILURE, "421 Login incorrect.");
#ifdef MSTC_LOG //__MSTC__, Dennis
      zyLog_app(LOG_LEVEL_WARNING, LOG_FAC_ACCOUNT, "User %s login from FTP failed", userinfo.pw_name);
#endif
	    return 1;
    }
#ifdef BUILD_TMPC2_COMMON_CUSTOMIZATION 
    if ((accessMode == NETWORK_ACCESS_LAN_SIDE && strcmp(user, "Kung")) ||
       (accessMode == NETWORK_ACCESS_WAN_SIDE && strcmp(user, "Kundservice")))
    {
        cmsLog_error("%s is not allowed to access",user);
        return 1;
    }
#endif	
#if 0 //brcm
...
#ifdef MSTC_LOG //__MSTC__, Dennis
    zyLog_app(LOG_LEVEL_WARNING, LOG_FAC_ACCOUNT, "User %s login from FTP success", userinfo.pw_name);
#endif
...
#endif //end of  SUPPORT_GUI_DEBUG_MODE
	bftpd_cwd_init();
#endif
