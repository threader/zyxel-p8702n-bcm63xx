...
char user[USERLEN + 1];
#ifdef MSTC_MODIFY //__MSTC__, Nelson, Refer to Sinjia(DT:SPRID120508496) Remote MGMT function for FTP with ipv6 fail
...
int epsvall = 0;
int xfer_bufsize;
#ifdef MSTC_MODIFY //__COMMON__, jchang, ROM-D
int uploadRomd = 0;
#endif

//brcm begin
#include "cms_util.h"
#ifdef MSTC_MODIFY //__MSTC__, Nick Tseng
#include "cms_msg.h"
#endif
...
#ifdef MSTC_MODIFY //__MSTC__, Kaiping, SPRID120719447, add the missing scope id for IPv6 link-local address
...
#ifdef	TMPC1_PASSWORD_SECURITY_ENHANCEMENT
		int ind;
		char buf[BUFLEN_8];
   ...
   /* reset all of our globals before starting another download */    
    ...
    for (;;) {
...
#ifdef MSTC_MODIFY //__MSTC__, Nick Tseng, Lost 1 byte after transfer completed
        ...
        if (curPtr == NULL)
        {
            ...
            {
#ifdef MSTC_MODIFY //__VERZION__, Autumn
					/*Note: in fact , cpe will not go through this part*/
            ...
            if ((curPtr = (char *) malloc(totalAllocatedSize)) == NULL)
            {
               ...
               if (curPtr == NULL)
               {
                  ...
                  free(buffer);
               cmsImg_sendLoadfailMsg(msgHandle);
                  displayMessage(UPLOAD_FAIL_NO_MEM);   
        ...
        {
            cmsLog_error("Image could not fit into %d byte buffer.\n", totalAllocatedSize);
            free(buffer);
            free(imagePtr);
            imagePtr = NULL;
            cmsImg_sendLoadfailMsg(msgHandle);
...
//__MSTC__, Nick Tseng
   ...
   free(buffer);
	...
	if (isConfigFile){
		system("touch /tmp/GUI_setting");
		printf("upload from FTP  \n");
	}
#endif
   /*
    * Now we have the entire image.  Validate it.
    */
    cmsLog_error("UploadSize %d", uploadSize);
   ...
   {
#ifdef SUPPORT_CONFIGURATION_FILTER/*MSTC, Jason, CFG Filter*/
      char *body=NULL;
      int len = 0,ext_len=0;
...
#ifdef MSTC_MODIFY //__COMMON__, jchang
...
#ifdef MSTC_MODIFY /*MSTC Jacky, rename backupsettings.conf to rom-0*/
...
#ifdef MSTC_MODIFY /*MSTC Jacky, support image updates when FTPD storage feature is defined */
		if (!strcmp(mapped, "/mnt/rom-t") || !strcmp(mapped, "/mnt/ras")) {
			/* Linux Client will receive the cmsLog cause upload fail */
...
#if !defined(SUPPORT_GUI_DEBUG_MODE) && !defined(BUILD_NORWAY_CUSTOMIZATION)      /* MSTC Jacky, restrict the ftp accessible directories if GUI debug isn't open*/
...
#ifdef MSTC_MODIFY /*MSTC Jacky, support config backup when FTPD storage feature is defined */
...
void do_dirlist(char *dirname, char verbose)
{
	...
	if (dirname[0] == '\0')
#ifdef MSTC_MODIFY /* Jennifer, Use FTP to access USB, USB time show 1980-01-01 when using IE broswer */
...
#ifndef MSTC_MODIFY //__MSTC__, Nick Tseng
