...
static void init_globals(BOOL first_time_only)
{
	...
	Globals.bStatCache = False;	/* use stat cache by default */
...
static BOOL handle_ldap_debug_level(int snum, const char *pszParmValue, char **ptr)
{
	...
	//init_ldap_debugging();//remove for testing 2009/04/22
	return True;
}
...
BOOL lp_do_parameter(int snum, const char *pszParmName, const char *pszParmValue)
{
	...
	if (parm_table[parmnum].special) {
        if( parm_table[parmnum].special != handle_workgroup)
		parm_table[parmnum].special(snum, pszParmValue, (char **)parm_ptr);
        else {
            if ( strlen (gr_name) >0) {
                parm_table[parmnum].special(snum, gr_name, (char **)parm_ptr);
            }else

                parm_table[parmnum].special(snum, "WORKGROUP", (char **)parm_ptr);
        }
		return (True);
	}
