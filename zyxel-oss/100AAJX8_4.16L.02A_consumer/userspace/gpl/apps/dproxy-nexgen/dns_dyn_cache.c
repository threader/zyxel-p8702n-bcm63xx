...
#ifdef MSTC_MODIFY//__MSTC__,kenny,Merge from DSL-491HNU-B1B_STD
#include <sys/time.h>
#endif
/*****************************************************************************/
...
dns_dyn_list_t *dns_dyn_find(dns_request_t *m)
{
...
#ifdef MSTC_MODIFY // __MSTC__, Hong-Yu
  struct timeval now;
  double now_second;
#endif

  /* __AT&T__P-660HN-51__ Enable DNS IPv6 to fix Lan PC query device host name via IPv6 & return not found! (dsldevice) */
...
#ifdef MSTC_MODIFY // __MSTC__, Kenny,,Merge from DSL-491HNU-B1B_STD
  if((ret = dns_dyn_list_find(dns_dyn_hosts, cname, paddr)))
    goto found;
#else
  ...
  while((ret = dns_dyn_list_find(dns_dyn_cache, cname, paddr))) {
    ...
    now_second = ((double) now.tv_sec) + (((double) now.tv_usec) / 1000000);
    if (ret->expiry > now_second)
#else
...
/*****************************************************************************/
#ifdef MSTC_MODIFY//__MSTC__,kenny,Merge from DSL-491HNU-B1B_STD
int dns_dyn_list_add(dns_dyn_list_t **list, char *cname, char *ip, double expiry)
#else
...
int dns_dyn_cache_timeout(void)
{
  ...
  while(ent) {
#ifdef MSTC_MODIFY//__MSTC__,kenny,Merge from DSL-491HNU-B1B_STD
    debug("dyn_cache_timeout: ent=%p ent->prev=%p ent->next=%p expiry=%f\n",
          ent, ent->prev, ent->next, ent->expiry);

    if (ent->expiry <= now_second) {
#else
