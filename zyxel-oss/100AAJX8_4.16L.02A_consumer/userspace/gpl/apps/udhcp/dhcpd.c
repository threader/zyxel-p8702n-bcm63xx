...
#ifdef MSTC_MODIFY //__MSTC__, Steven: TR111 can't work
...
#endif
#ifdef MSTC_MODIFY //__MSTC__,Lynn, DHCP:sync from SinJia, TR-098 DHCP Conditional Serving Po
...
#ifdef MSTC_MODIFY //__MSTC__,Lynn, DHCP:sync from Autumn prevent response
...
/* __MSTC__, klose, QoS classifier rule DHCP option exclude support, 20121207 */
            ...
            case CMS_MSG_QOS_DHCP_OPT77_COMMAND:
               ...
               break;
#ifdef MSTC_MODIFY /* Jennifer, support QoS option 61 & option 125 */
...
#ifdef MSTC_MODIFY //__MSTC__,Lynn, DHCP	/* move hostname block here to keep it without flushing by prevoius lease message, __TELEFONICA__, MSTC Lloyd Wu, 20100301 */					
					if (hostname) {
						bytes = hostname[-1];
						if (bytes >= (int) sizeof(tmpHostName))
							bytes = sizeof(tmpHostName) - 1;
