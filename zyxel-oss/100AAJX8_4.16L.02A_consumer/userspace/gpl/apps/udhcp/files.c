...
static source_port_t * read_sourcePortRules(char *line)
{
	...
	source_port_t *new = NULL, *header = NULL, *now = NULL;
...
{
	char *token, tmp[BUFLEN_256];
	vendor_id_t *new = NULL, *header = NULL , *now = NULL;
...
{
	char *token, tmp[BUFLEN_256];
	mac_t *new = NULL, *header = NULL, *now = NULL;
...
{
	char *token, tmp[BUFLEN_256];
	clnt_id_t *new = NULL, *header = NULL, *now = NULL;
...
{
	char *token, tmp[BUFLEN_256];
	vsi_t *new = NULL, *header = NULL, *now = NULL;
...
static int read_opt(char *line, void *arg)
{
	...
	do {
#ifdef MSTC_MODIFY //__MSTC__,Lynn,DHCP
 /* Fix option does not support "," issue, __TELEFONICA__, MSTC YungAn, 20100927. */
                val = strtok(NULL, " \t");		
#else
		val = strtok(NULL, ", \t");
#endif
...
#ifdef MSTC_MODIFY //__MSTC__,Lynn,DHCP	
/* For DHCP Option 60 Vendor ID Mode, __TELEFONICA__, MSTC Eva, 20100211. */
...
#ifdef MSTC_MODIFY //__MSTC__,Lynn,DHCP: sync from SinJia, TR-098 DHCP Conditional Serving Pool
/* Support LAN port match criteria, __TELEFONICA__, MSTC YungAn, 20090819. */
...
#ifdef MSTC_MODIFY //__MSTC__,Lynn,DHCP 
/*MSTC Autumn static DHCP will course error*/
...
#ifdef MSTC_MODIFY //__MSTC__,Lynn,DHCP:sync from SinJia, TR-098 DHCP Conditional Serving Pool
/* TR-098 DHCP Conditional Serving Pool, __TELEFONICA__, MSTC YungAn, 20090817. */
      char poolDelim[] = "_";
      char *poolInterface = cmsMem_strdup(cur_iface->interface);
      char *poolSocketIf;
      if( strstr(cur_iface->interface, "condservpool") ){
...
#ifndef MSTC_MODIFY //__MSTC__,Lynn,DHCP 
...
void getArpInfo(void)
{
   ...
   struct arpmacEntry *temp_entry=NULL;   
   struct arpmacEntry *cur_arpmac=NULL;
   struct arpmacEntry *arpmac_head=NULL;
   int mac_counter=0;
   u_int8_t mac_chaddr[16];

   if (fsArp != NULL)
   {
		arpmac_head=cur_arpmac = NULL;
      while (fgets(line, sizeof(line), fsArp))
      {
         if ( count++ < 1 ) continue;

         sscanf(line, "%s %s %s %s %s %s", col[0], col[1], col[2], col[3], col[4], col[5]);
		 temp_entry = (struct arpmacEntry *) malloc (sizeof(struct arpmacEntry));
         memset(temp_entry->chaddr, 0, sizeof(temp_entry->chaddr));
         cmsUtl_macStrToNum(col[3], temp_entry->chaddr);
         temp_entry->next = NULL;
         if (arpmac_head == NULL)
         {
            arpmac_head = cur_arpmac = temp_entry;
         }
         else
         {
            cur_arpmac->next = temp_entry;
            cur_arpmac = temp_entry;
         }
      }
	  fclose(fsArp);
   }

   fsArp = fopen("/proc/net/arp", "r");
   count = 0;
#endif
   ...
   if (fsArp != NULL)
   {
      ...
      while (fgets(line, sizeof(line), fsArp))
      {
         ...
         sscanf(line, "%s %s %s %s %s %s", col[0], col[1], col[2], col[3], col[4], col[5]);
#ifdef MSTC_MODIFY //__COMMON__, Steven: Support interface group
...
#ifdef BUILD_MSTC_WLAN_CLIENT_MODE
	mac_counter=0;
	memset(mac_chaddr, 0, sizeof(mac_chaddr));
	cmsUtl_macStrToNum(col[3], mac_chaddr);

      for (cur_arpmac = arpmac_head ; cur_arpmac != NULL ; cur_arpmac = cur_arpmac->next)
      {

         if (!memcmp(cur_arpmac->chaddr, mac_chaddr, 16)) {
				mac_counter++;
         }
      }

	  if(mac_counter > 1) continue;
