...
{//Break
	nat_return_t                         ret_val=NAT_RETURN_SUCCESS;
    stun_message_st     stun_msg;
    nat_error_t         ecode = 0;
    nat_uint8_t			context_id = 0;
    nat_string_st	password = {0};
    nat_recv_udp_pkt_ind_st *p_recv_msg = 
		(nat_recv_udp_pkt_ind_st *)p_glb_nat_pdb->p_recv_msg->p_msg->p_msg_data;

    nat_port_memset(&stun_msg,0,nat_port_sizeof(stun_message_st));

    if(NAT_RETURN_SUCCESS != stun_decode_message_header(
            &(p_recv_msg->octets), &stun_msg.msg_hdr,ecode))
    {
    ...
    {
        nat_uint128_t transaction_id = stun_msg.msg_hdr.transaction_id;
        ...
        else if(NAT_RETURN_SUCCESS != stun_decode_message(
            &(p_recv_msg->octets), password,&stun_msg,&ecode)) //himanshu
        {
            /* ignore the message, could be some stray message 
            p_glb_nat_pdb->p_glb_data->p_stun_context_map[context_id]*/
            return NAT_RETURN_SUCCESS;
        ...
        else
        {
			...
			if((NAT_TRUE == p_glb_nat_pdb->p_glb_data->message_integrity) &&
			( BindResponseMsg == stun_msg.msg_hdr.msg_type) && 
			(0 != nat_port_strcmp(p_glb_nat_pdb->p_glb_data->\
				p_stun_context_map[context_id].message_integrity,
				stun_msg.message_integrity.hash))) //himanshu
		    {
				stun_binding_resp.result = NAT_RESULT_FAILURE;
                stun_binding_resp.error = NAT_ERR_MSG_INTEGRITY_FAIL;
			}
            else if(BindResponseMsg == stun_msg.msg_hdr.msg_type)//himanshu
            {
				nat_boolean_t validation_pass = NAT_TRUE;

				if(NAT_FALSE == stun_msg.hasMappedAddress)
				{
					validation_pass = NAT_FALSE;
				}
				else if(NAT_FALSE == stun_msg.hasSourceAddress)
				{
					validation_pass = NAT_FALSE;
				}
				else if(NAT_FALSE == stun_msg.hasChangedAddress)
				{
					validation_pass = NAT_FALSE;
				}


				if(NAT_TRUE == validation_pass)
				{
                ...
                nat_port_memcpy( &(stun_binding_resp.mapped_addr),
					 &(stun_msg.mapped_address.addr),
                     nat_port_sizeof(nat_ip_port_address_st));
		
                nat_port_memcpy( &(stun_binding_resp.changed_addr),
					   &(stun_msg.changed_address.addr),
                       nat_port_sizeof(nat_ip_port_address_st));
				}
				else
				{
            ...
            else if(BindErrorResponseMsg == stun_msg.msg_hdr.msg_type)
            {
                stun_binding_resp.result = NAT_RESULT_FAILURE;
	...
	/*
		1. Invoke stun_decode_message_header()to decode the header of received UDP packet.
		...
		3. else 
			...
			c) Decode the message , with function stun_decode_message() 
        ...
        INOUT nat_glb_pdb_st             *p_glb_nat_pdb)
{
	...
	/*
		1. Invoke stun_decode_message_header()to decode the header of received TCP packet.
		2. If decode message header failed then
			a)Ignore this message.
		3. else 
			...
			c) Else Decode the message , with function stun_decode_message() 
