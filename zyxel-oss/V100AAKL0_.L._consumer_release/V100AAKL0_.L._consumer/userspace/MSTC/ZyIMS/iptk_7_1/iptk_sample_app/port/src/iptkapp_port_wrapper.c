...
iptkapp_return_t iptkapp_iptk_ipc_send(icf_void_t *p_msg,icf_uint32_t buf_len, iptkapp_error_t *p_error) {
...
/* ZyXEL porting*/
	...
	while( packet_length ) {
		...
		bytes_written = sendto(g_fd,(icf_void_t*)p_buf,packet_length,0,(struct sockaddr*)&addr,sizeof(struct sockaddr_un));
