        ...
        INOUT icf_cta_pdb_st     *p_pdb)
{
    ...
    {
        /*Swap both the call-context pointers to retain the
         * definition that REFER is sent to party C*/
        /*Transferred party call context*/
        p_pdb->p_cta_ctx->p_call_ctx_b = p_pdb->p_cta_ctx->p_call_ctx_c;

        /*Update transferred party ID*/
        p_pdb->p_cta_ctx->transferred_id = 
            p_pdb->p_cta_ctx->p_call_ctx_b->call_id;

        /*Party to which REFER is sent*/
        ...
        INOUT  icf_cta_pdb_st   *p_pdb)
{
    icf_return_t    ret_val = ICF_SUCCESS;
    icf_glb_pdb_st  *p_glb_pdb = ICF_NULL;
    icf_call_ctx_st  *p_check_ctxt = ICF_NULL;

    p_glb_pdb = p_pdb->p_internal_msg->msg_hdr.p_glb_pdb;
    
    ICF_FUNCTION_ENTER(p_glb_pdb)
    if (p_pdb->p_cta_ctx->p_call_ctx_b->call_id == p_pdb->p_cta_ctx->transferred_id)
        p_check_ctxt = p_pdb->p_cta_ctx->p_call_ctx_c;
    else
        p_check_ctxt = p_pdb->p_cta_ctx->p_call_ctx_b;


    /*If signaling ongoing in any other module*/
    if (ICF_INT_MODULE_INVALID != 
                        ICF_CTA_GET_SIG_ONGOING(p_check_ctxt))
    {
        /*
    ...
    icf_cta_pdb_st   *p_cta_pdb)
{
    ...
    icf_error_t    *p_ecode = &(p_cta_pdb->p_internal_msg->msg_hdr.ecode);
