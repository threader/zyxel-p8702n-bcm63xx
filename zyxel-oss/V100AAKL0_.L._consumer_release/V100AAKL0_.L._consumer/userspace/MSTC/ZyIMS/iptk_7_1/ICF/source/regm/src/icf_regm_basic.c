 ...
 *                                               enhancement.                       * 7-Sep-2007   Deepti Goyal    SPR 16019       Enhancement and Correction in
...
ICF_CHECK_IMS_END
    ...
    if (ICF_SUCCESS == ret_val)                
    {
		...
		{
			if(0 == ((icf_rgm_glb_data_st *)(p_crm_pdb->p_rgm_glb_data))->ongoing_reg_counter)
			{
					...
					while (ICF_NULL != p_user_db)
					{
						   ...
						   if (ICF_SUCCESS == icf_rgm_get_ctxt_for_pub_uri(p_crm_pdb->p_glb_pdb,
							  ...
							  if(!(ICF_REG_REQ_TO_PRESENT & p_recv_msg_payload->bit_mask))
							    {
                                 ...
                                 p_rgm_context->rgm_list_of_lines[0] = p_recv_msg_payload->line_id;
                                 p_rgm_context->p_app_data_list[register_response.app_id - 1].line_id =
                                         p_recv_msg_payload->line_id;
								 register_response.options_flag |= ICF_RGM_RESPONSE_CFM;
                            ...
                            if(ICF_DUMMY_APP_ID != p_crm_pdb->p_glb_pdb->p_recv_msg->hdr.app_id)
                            {
						        ...
						        ((icf_rgm_glb_data_st *)(p_crm_pdb->p_rgm_glb_data))->ongoing_reg_counter++;
                            }
...
{
    icf_return_t                 ret_val = ICF_SUCCESS;
    icf_sic_notify_msg_st        *p_notify; 
    icf_uint32_t                 count,index_regn_blk;
    icf_glb_pdb_st               *p_glb_pdb = ICF_NULL;
    icf_msg_body_st				*p_msg_body = ICF_NULL;
    icf_int32_t					old_version = ICF_NULL;
    icf_rgm_reginfo_st			recd_reg_info;
	icf_reg_event_subs_ctx_st	*p_reg_event_subs_ctx = ICF_NULL;
    icf_rgm_regn_block_st		*p_regn_blk;
...
????ICF_NULL != p_crm_pdb->p_config_data))
  ...
  /* get the pointer to RGM global data */
    ...
    ICF_ASSERT(p_glb_pdb, (ICF_NULL != p_notify))
...
/* SPR: 9068: fix ends */
    ...
    ICF_ASSERT(p_glb_pdb, (ICF_NULL != p_msg_body))
    ...
    if (p_msg_body && p_msg_body->length)
    {
...
/* SPR: 9044: fix end */
		...
		{	if (recd_reg_info.registration_list.p_regn_block)
			{
						...
						index_regn_blk++)
			{
				...
				{
					if(ICF_SUCCESS == icf_rgm_fetch_ctxt_for_uri(
						p_crm_pdb->p_glb_pdb,
						p_regn_blk->p_aor,
		...
		if(ICF_NULL != ((icf_rgm_glb_data_st *)p_crm_pdb->p_rgm_glb_data)->p_reg_event_subs_ctx)
		{
