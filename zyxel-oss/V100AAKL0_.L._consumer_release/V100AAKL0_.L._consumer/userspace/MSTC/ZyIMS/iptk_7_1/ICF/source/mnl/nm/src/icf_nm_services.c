			...
			INOUT	icf_nm_pdb_st						*p_nm_pdb)
{
	...
	ICF_STATIC_MEMGET(
		...
		sizeof(icf_nm_non_ims_qos_map_st),
       	p_nm_pdb->p_nm_glb_data->p_non_ims_qos_map,
		ICF_RET_ON_FAILURE,
        p_nm_pdb->p_ecode,
        ret_val);

	p_non_ims_qos_map = p_nm_pdb->p_nm_glb_data->p_non_ims_qos_map;
	...
	/* ICF_DUMMY_ID_FOR_NON_IMS is used temporarily for non ims */
  	p_qos_res_req->id = ICF_DUMMY_ID_FOR_NON_IMS;
			...
			INOUT	icf_nm_pdb_st					*p_nm_pdb)
{
	...
	if ((ICF_NULL != p_non_ims_qos_map) && 
            (p_non_ims_qos_release->call_id == p_non_ims_qos_map->call_id))
	{
		id = p_non_ims_qos_map->id;
	}
        ...
        INOUT icf_nm_pdb_st		*p_nm_pdb)
{
	...
	cntxt_id = (icf_uint16_t)(qos_res_non_ims_resp_st.id);

	p_non_ims_qos_map = p_nm_pdb->p_nm_glb_data->p_non_ims_qos_map;

	result=(icf_uint8_t)((ICF_PORT_PLTFM_GPRS_NOERR==\
