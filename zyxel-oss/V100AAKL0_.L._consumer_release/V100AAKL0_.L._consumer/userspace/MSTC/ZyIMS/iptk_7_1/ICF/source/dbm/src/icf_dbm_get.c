 ...
 * 12-Jan-2007  Amit Sikka      SPR 17945   Support for PUBLISH/MESSAGE
 * 13-Aug-09	Aman Aggarwal	SPR 19926
...
#endif
#ifdef ICF_ERROR_ENABLE
    icf_error_data_st               error_data= {0,{0},0,{{0,"\0"}},{0,"\0"} };
#endif
    icf_return_t                    ret_val = ICF_SUCCESS;
                ...
                rgn_cntxt_blk_next_used_id) && (ICF_FAILURE == ret_val))
        {
            p_rgn_blk_addr= 
             ...
             /* Compare Addresses*/ 
                ...
                {
                    for(i = 0;i<p_rgn_blk_addr->num_of_lines;i++)
                    {
    ...
    OUT   icf_error_t    *p_ecode)
{
    ...
    if (ICF_SUCCESS == icf_cfg_fetch_app_conf(
                            ...
                            p_ecode))
    {
        if (ICF_TRUE == p_app_conf->app_profile.icf_simul_call_line_arbitration)
        {
        ...
        OUT   icf_error_t *p_ecode)
{
    ...
    if(ICF_INVALID_LINE_ID != line_id)
    {
	    ...
	    if (ICF_NULL < p_line_data->num_call_waiting_calls)
	    {
	    	/*
        ...
        OUT   icf_address_st *p_e164)
{
   icf_return_t ret_val = ICF_FALSE;
    ...
    while(counter<p_address->addr_val.str_len 
...
icf_return_t icf_dbm_chk_if_dom_is_local_ip(
        ...
        IN    icf_address_st *p_address)
{
    icf_uint8_t domain[ICF_MAX_STR_LEN];
    icf_uint8_t *p_temp = ICF_NULL;
    icf_uint8_t str_tok[ICF_MAX_STR_LEN];
    icf_transport_address_st ip_address;
    icf_config_data_st *p_config_data = ICF_NULL;
    icf_return_t ret_val = ICF_SUCCESS;
    icf_error_t ecode;
        ...
        OUT   icf_error_t *p_ecode)
{
    ...
    if (!icf_port_strcmp(p_transport_val, (icf_uint8_t *)"tcp"))
   ...
   /* Get line data */
          ...
          p_ecode))
      {
          ...
          if(ICF_NULL == *p_p_curr_node)
          {
              ...
              (*p_p_curr_node)->p_assoc_uri_list = ICF_NULL;
              (*p_p_curr_node)->p_contact_uri_list = ICF_NULL;
		...
		p_ecode))
	{
		...
		if(ICF_SUCCESS == ret_val)
		{
			...
			{					
				...
				if(ICF_SUCCESS != ret_val)
				{
					/* If public uri does not match, check the associated uris */
					if( 0 != (ICF_USER_DB_USE_ASSOCIATED_URIS & search_bitmask))
					{
			...
			OUT		icf_boolean_t		*p_match_found)
{
	...
	icf_address_st						user_info_full_1;
	icf_address_st						user_info_full_2;
	icf_address_st						user_info_full_derived_1;
