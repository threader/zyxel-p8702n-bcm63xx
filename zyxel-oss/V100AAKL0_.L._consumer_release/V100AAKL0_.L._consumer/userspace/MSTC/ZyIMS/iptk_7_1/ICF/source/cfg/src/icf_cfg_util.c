 ...
 * 10-Nov-2005  Aman Aggarwal                   Changes regarding the self_mode                                                 
 change
 ...
 * 07-Nov-2008  Anurag Khare       SPR 19168    Modified Function icf_cfg_set_call_prms_in_db
 *  
 * 07-Jan-2009  Vinay Jindal       SPR 19250    Updated 'icf_cfg_set_srvc_config_prm'
...
/*#ifndef ICF_FEATURE_ABANDON*/
	case ICF_CFG_TRACE_SRVC_PRMS_REQ_RECD:
        ...
        case ICF_CFG_TRACE_PROXY_ADDRESS_SET:
            ...
            if(ICF_TRANSPORT_ADDRESS_IPV4_ADDR == p_crm_pdb->
...
ICF_CHECK_IMS_END
#endif
        p_crm_pdb->p_line_data[line_id]->reg_mode = ICF_TRANSPORT_MODE_BOTH;
	 ...
	 * SPR 13978 Aman 06-01-07
        ...
        {
                p_crm_pdb->p_line_data[line_id]->server_index[i] = ICF_MAX_NUM_OF_SERVERS;
        }
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
    ...
    icf_transport_mode_t	        effective_self_mode = ICF_TRANSPORT_MODE_BOTH;
    ...
    *failure*/
    ...
    if(ICF_NULL != (ICF_SET_TRP_REQ_PROXY_ADDR &
			    p_recv_msg_payload->bit_mask))
    {
        /* Check that the port number given is valid */
		if ((ICF_NULL != 
				   p_recv_msg_payload->proxy_addr.transport_addr.port_num) &&
		   (ICF_CFG_MIN_PORT_VALUE >
                   p_recv_msg_payload->proxy_addr.transport_addr.port_num) && 
            (0 != p_recv_msg_payload->proxy_addr.transport_addr.port_num) &&
           (ICF_CONFIG_ACTION_DELETE != p_recv_msg_payload->proxy_addr.action))
        {
        ...
        else 
        {
            ...
            if(ICF_CONFIG_ACTION_ADD == p_recv_msg_payload->
                    proxy_addr.action) 
            {
               /*If again proxy is trying to add,then ret Failure*/             
               if(ICF_SUCCESS == icf_cfg_check_line_configured(p_crm_pdb, (icf_boolean_t)ICF_FALSE))
                {
                  for(line_id = 0; line_id < 
                          p_crm_pdb->p_config_data->max_lines; line_id++)
                  {
                    if ((ICF_TRUE == p_crm_pdb->lines_present[line_id]) &&
                        (ICF_NULL != (ICF_CFG_LINE_DATA_PROXY_RECD 
                        & p_crm_pdb->p_line_data[line_id]->line_data_received)))                      {
                        ICF_CFG_SET_VALID_PARAM(p_crm_pdb, ICF_FALSE)
                        ICF_CFG_SET_RESPONSE(p_crm_pdb,
                                            ICF_FAILURE,
                ...
                if(ICF_FAILURE == icf_cfg_check_line_configured(p_crm_pdb,
                            (icf_boolean_t)ICF_TRUE))
                {   
                    /* Response has been set in the above function */
                    ICF_CFG_SET_VALID_PARAM(p_crm_pdb, ICF_FALSE)
                    ICF_FUNCTION_EXIT(p_crm_pdb->p_glb_pdb)
                    return ret_val;
                }
            ...
            {
                /* Action is delete or modify, only fetch line pointers */
                icf_cfg_check_line_configured(p_crm_pdb, 
                        (icf_boolean_t)ICF_FALSE);
            }
        ...
        IN      icf_line_id_t    line_id)
{
    ...
    icf_port_memcpy(
            (icf_void_t*) &(p_crm_pdb->p_line_data[line_id]->proxy_addr),
            (icf_void_t*) &(p_recv_msg_payload->proxy_addr.
                               transport_addr),
            (icf_uint32_t)ICF_PORT_SIZEOF(icf_transport_address_st));
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
    ...
    if(ICF_ALL_LINES == p_recv_msg_payload->proxy_addr.line_id)
    {
         ...
         * either proxy or registrar or atleast one user. If there is 
         * no such line then all lines upto max_lines in config data 
         * will be configured.
         */
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
                            ...
                            p_recv_msg_payload->bit_mask))
    {
        p_crm_pdb->p_config_data->default_settings = 
            (icf_uint8_t) p_recv_msg_payload->default_settings;
...
/*                internal_msg.msg_hdr.ecode = ICF_ERROR_NONE;*/
    ...
    else
    {
        ...
        {
        if((p_recv_msg_payload->update_reqd == ICF_UPDATE_WHN_OFFER_SDP_WITH_MULTIPLE_CODEC) || 
           (p_recv_msg_payload->update_reqd == ICF_NO_UPDATE_WHN_ANSWER_SDP_WITH_SINGLE_CODEC) ||
           (p_recv_msg_payload->update_reqd == ICF_NO_UPDATE))
          {
   ...
   /* Check for the presence of DNS Min Retransmission Timer */
    ...
    if (ICF_NULL != (ICF_SET_PORT_IN_FROM_HDR &
                            p_recv_msg_payload->bit_mask))
    {
          p_crm_pdb->p_config_data->port_to_be_sent_in_from_header = p_recv_msg_payload->port_to_be_sent_in_from_hdr;
    }
...
/*******************************************************************************
 *
 * FUNCTION:        icf_cfg_compare_trnspt_addr
 *
 * DESCRIPTION:     This function compares two transport addresses
 *
 * RETURNS:         ICF_SUCCESS if the addresses match
 *                      
 ******************************************************************************/

icf_return_t icf_cfg_compare_trnspt_addr(
        INOUT   icf_crm_pdb_st           *p_crm_pdb,
        IN  icf_transport_address_st     *p_addr_a,
		IN  icf_transport_address_st     *p_addr_b)
{
    icf_return_t                         ret_val = ICF_SUCCESS;
    ...
    /* compare the port numbers and address type */
    if (
            (p_addr_a->port_num != p_addr_b->port_num) ||
            (p_addr_a->addr.addr_type != p_addr_a->addr.addr_type)
       )
    {
        ret_val = ICF_FAILURE;
    }

    /* Now we also need to compare the addresses based on the address type */
    /* domain name */
    else if (ICF_TRANSPORT_ADDRESS_DOMAIN_NAME == p_addr_a->addr.addr_type)
    {
        if (
                (p_addr_a->addr.addr.domain.str_len != 
                 p_addr_b->addr.addr.domain.str_len) ||
                (0 != (icf_port_strcmp(
                                    p_addr_a->addr.addr.domain.str,
                                    p_addr_b->addr.addr.domain.str)))
           )
        {
            ret_val = ICF_FAILURE;
        }
    ...
    /* IPv4 address */
    else if (ICF_TRANSPORT_ADDRESS_IPV4_ADDR == p_addr_a->addr.addr_type)
    {
        if (
                (p_addr_a->addr.addr.ipv4_addr.octet_1 != 
                 p_addr_b->addr.addr.ipv4_addr.octet_1) ||
                (p_addr_a->addr.addr.ipv4_addr.octet_2 != 
                 p_addr_b->addr.addr.ipv4_addr.octet_2) ||
                (p_addr_a->addr.addr.ipv4_addr.octet_3 != 
                 p_addr_b->addr.addr.ipv4_addr.octet_3) ||
                (p_addr_a->addr.addr.ipv4_addr.octet_4 != 
                 p_addr_b->addr.addr.ipv4_addr.octet_4)
                )
        {
            ret_val = ICF_FAILURE;
        }
        ...
        IN      icf_line_id_t    line_id)
{
    ...
    {
	    if((p_recv_msg_payload->reg_mode != self_mode)||
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
		...
		else if (ICF_CONFIG_ACTION_DELETE == p_recv_msg_payload->action)
		{
            ...
            {	
                    ...
                    &p_line_data, p_crm_pdb->p_ecode))
                {
                    ...
                    ret_val = icf_cfg_validate_del_regm_prms(p_crm_pdb, line_index);
                    if (ICF_FALSE == p_crm_pdb->is_valid_param)
                    {
                        ICF_FUNCTION_EXIT(p_crm_pdb->p_glb_pdb)
                            return ret_val;
    ...
    else
    {
        ...
        /* No validation in case of action = delete */
    
        else if (ICF_CONFIG_ACTION_DELETE == p_recv_msg_payload->action)
		{
			ret_val = icf_cfg_validate_del_regm_prms(p_crm_pdb, line_id);
			if (p_crm_pdb->is_valid_param == ICF_FALSE)
			{
				ICF_FUNCTION_EXIT(p_crm_pdb->p_glb_pdb)
				return ret_val;
			}
        ...
        IN      icf_line_id_t    line_id)
{
	...
	if(ICF_FAILURE == icf_dbm_fetch_line_cntxt_blk(
        p_crm_pdb->p_glb_pdb, line_id,
        &p_line_data, p_crm_pdb->p_ecode))	/* no get_line needed */
    {
	...
	/*If the line is not configured and delete request is there, then FAILURE*/
	else  
	{
	/* Success by this function means that the line exists and 
	 * pointer to the line is returned further check if the registar is not
	 * present then the delete registrar request is not valid
	 */ 
        ...
        IN      icf_line_id_t    line_id)
{
    ...
    icf_port_memset(
            (icf_void_t*)&(p_crm_pdb->p_line_data[line_id]->registrar_addr),
            (icf_uint8_t)0,
            (icf_uint32_t)ICF_PORT_SIZEOF(icf_transport_address_st));
            ...
            if(ICF_CONFIG_ACTION_ADD == p_recv_msg_payload->action)
            {
                 if(ICF_TRANSPORT_MODE_BOTH == p_crm_pdb->p_config_data->self_mode)
                 {
                  ...
                  else
                  {
                        ...
                        {
                             if(p_recv_msg_payload->reg_mode !=
                                p_crm_pdb->p_config_data->self_mode)
                             {
                                   ICF_CFG_TRACE(p_crm_pdb,ICF_TRACE_PARAM_VALIDITY)
                                   ICF_CFG_SET_RESPONSE(p_crm_pdb, ICF_FAILURE,
        ...
        IN      icf_line_id_t    line_id)
{
    ...
    icf_port_memcpy(
            (icf_void_t*)&(p_crm_pdb->p_line_data[line_id]->registrar_addr),
            (icf_void_t*)&(p_recv_msg_payload->registrar_addr),
            (icf_uint32_t)ICF_PORT_SIZEOF(icf_transport_address_st));
            ...
            p_recv_msg_payload->bit_mask))
    {
        ...
        if(ICF_ALL_LINES == p_recv_msg_payload->call_hold_option.line_id)
        ...
        else
         {
           ...
           if(ICF_TRUE == p_crm_pdb->requested_lines[line_id])
            {
              p_crm_pdb->p_line_data[line_id]->hold_option =
              p_recv_msg_payload->call_hold_option.hold_option;
              p_crm_pdb->line_id = line_id;
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
    ...
    for (line_id = 0; line_id < p_crm_pdb->p_config_data->max_lines; line_id++)
    {
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
    ...
    sic_msg.msg_hdr.msg_id = ICF_FORCEFUL_CALL_CLEAR_IND;
		...
		&& (ICF_SUCCESS == ret_val));p_ch++)
	{
		...
		case ICF_PARSE_STATE_IN_STRING:
            /* traverse and store the string till an ending double 
               ...
               operator */
			{
				for(i=0;state == ICF_PARSE_STATE_IN_STRING;i++,p_ch++)
				{
					if(*p_ch == '\0') 
						...
						IN		icf_line_id_t		line_id)
{
	...
	for(icf_line_count=0;icf_line_count<p_rgm_context->num_of_lines;icf_line_count++)
	{
		if(line_id == p_rgm_context->rgm_list_of_lines[icf_line_count])
		{
			for(icf_temp_count = icf_line_count;
			icf_temp_count<(p_rgm_context->num_of_lines)-1;icf_temp_count++)
			{
              ...
              icf_crm_pdb_st    *p_crm_pdb)
{
    ...
    p_user_db = p_crm_pdb->p_line_data[p_crm_pdb->line_id]->p_user_db;
    ...
    while (ICF_NULL != p_user_db)
    {
                  ...
                  p_crm_pdb->p_ecode))
        {
            ...
            /*p_crm_pdb->p_rgm_context was NULL here ,so
            assigned p_rgm_context to it*/
            p_crm_pdb->p_rgm_context = p_rgm_context;
            if(1 == p_rgm_context->num_of_lines)
            {
                /* SPR 16019: Set the bit mask in rgm_context to indicate that
