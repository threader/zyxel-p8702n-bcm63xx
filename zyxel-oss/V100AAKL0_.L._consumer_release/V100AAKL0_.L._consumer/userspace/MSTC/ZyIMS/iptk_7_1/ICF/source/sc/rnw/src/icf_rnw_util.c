...
* 01-Sep-200r65  Amit Sharma   SPR:12710
* 01-Sep-200r65  Amit Sharma   SPR:12728
* 11-Dec-2008    Vinay Jindal  SPR 19234     when validating incoming refer, 
*                                            icf_dbm_chk_if_local_usr 
*                                            check is deleted 
* 15-Feb-2010  Rohit Aggarwal	SPR 20294	Check configuration for 
*											hiding identity in case of
*											incoming call.
*
        ...
        INOUT  icf_call_ctx_st   *p_call_ctx)
{
	...
	/* Fix for CSR 1-8269232: SPR 20294 */
    ...
    if (ICF_SUCCESS == ret_val)
    {
    	...
    	if ((ICF_NULL != (ICF_CALL_DIR & p_glb_pdb->p_call_ctx->common_bitmask)) &&
        	(ICF_NULL == (ICF_SETTINGS_CONFIG_PRESENTATION_IND &
        ...
        INOUT icf_trace_data_st   *p_trace_data)
{
    switch(event)
    {
        ...
        case ICF_SC_INTERNAL_RNW_REJ:
            icf_port_strcpy(p_trace_data->string_val[0].str,
                    (icf_uint8_t *)"ICF_SC_INTERNAL_RNW_REJ");
            break;
        ...
        INOUT  icf_rnw_pdb_st    *p_pdb)
{
   ...
   if ((ICF_NULL == icf_port_strcmp(p_refer_to_addr->addr_val.str,
                p_glb_pdb->p_call_ctx->remote_party_addr.addr_val.str))
            && (p_refer_to_addr->addr_type ==
    ...
    if (ICF_TRUE == icf_cmn_check_if_param_present(
                &(p_refer_to_addr->addr_val), (icf_uint8_t * )"transport", param_val, ICF_NULL, ICF_NULL))  
    {
        icf_config_data_st   *p_glb_cfg_data = ICF_NULL;
        icf_line_data_st     *p_line_data = ICF_NULL;
        icf_transport_mode_t transport_mode;

        if (icf_port_strcmp(param_val, (icf_uint8_t * )"tcp"))
                    ...
                    (icf_void_t **)&(p_glb_cfg_data), p_ecode))
        {
            result = ICF_FALSE;
                            ...
                            p_ecode))
        {
            /*If proxy routed call then compare with transport_mode on 
             * line data*/
