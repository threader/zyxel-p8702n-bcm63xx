 ...
 ** 28/01/09	 Pavneet							 SPR 19272-Added support for session refresh by IPTK on 
 **													 recieving 200 Ok without session timer header.
 ** 18-Sep-2009  Anurag Khare   SPR 20062            Fix for CSR 1-7771967
...
#endif
{
    ...
    for(dIndex=0; dIndex<SipHdrTypeAny; dIndex++)
    {
        ...
        for (dLIndex = 0; dLIndex < dCount; dLIndex++)
        {
            ...
            {
                Sdf_ty_s8bit *pTempHeaderString;
                ...
                if (Sdf_co_null != pTempHeaderString)
                {
                    ...
                    Sdf_mc_strcat(trace, pTempHeaderString);
                    sdf_fn_trace(Sdf_en_allTraceLevels, \
...
#endif
	{
		...
		if (dCompleteMatch == Sdf_co_true)
		{
			...
			for (dIndex = 0; dIndex < dOuterParamCount; dIndex++)
			{
				...
				(void)sdf_fn_uaConvertHexEncodedString(pName1, dHexConvertedName1,\
					Sdf_co_smallBufferSize);
				(void)sdf_fn_uaConvertHexEncodedString(pValue1, dHexConvertedValue1,\
					Sdf_co_smallBufferSize);
				/*
				...
				for (dJindex = 0; dJindex < dInerParamCount; dJindex++)
				{
					...
					(void)sdf_fn_uaConvertHexEncodedString(pName2, dHexConvertedName2,\
						Sdf_co_smallBufferSize);
						...
						dHexConvertedName2) == 0)
					{
						...
						(void)sdf_fn_uaConvertHexEncodedString(pValue2, \
...
#endif
{
	...
	else
	{
		if(pExistingTag != Sdf_co_null)
		{
			 ...
			 */
                        /* Fix for CSR 1-7273951: SPR 19738 */
			if (((pNewHeader->dType == SipHdrTypeTo) && \
                             (pExistHeader->dType == SipHdrTypeTo)))
                        {
                            if(dIsToTagCmp == Sdf_co_true)                              
...
#endif
{
	...
	(void)sdf_fn_uaConvertHexEncodedString(pUrl1->pHost, dHost1,Sdf_co_smallBufferSize);
	(void)sdf_fn_uaConvertHexEncodedString(pUrl2->pHost, dHost2,Sdf_co_smallBufferSize);
	...
	(void)sdf_fn_uaConvertHexEncodedString(pUrl1->pUser, dUser1,Sdf_co_smallBufferSize);
	(void)sdf_fn_uaConvertHexEncodedString(pUrl2->pUser, dUser2,Sdf_co_smallBufferSize);
	...
	for (dIndex = 0; dIndex < dParamCount1; dIndex++)
	{
		...
		for (dLoop = 0; dLoop < dParamCount1; dLoop++)
		{
			...
			if (Sdf_mc_strcasecmp(dHexConvertedName1,dHexConvertedName2) == 0)
			{
				...
				(void)sdf_fn_uaConvertHexEncodedString(pValue2, dHexConvertedValue2,\
					Sdf_co_smallBufferSize);
...
#endif
{
	...
	while (dIndex < size)
	{
		...
		{
			if(Sdf_mc_strcmp(pParam->pName,"branch")==0)
			{
				SIP_S8bit *pBranch;
...
#endif
{
...
#else
    Sdf_ty_s8bit dTempArray[Sdf_co_characterBufferSize];
#endif    
	...
	Sdf_mc_strcpy(dTempArray,pTempViaSentBy);
...
/* CSR_5155164 Change End */
	...
	if (dMsgType==SipMessageRequest)
	{
		...
		else
		{
			...
			if (pCallObj->pSessionTimer->dMinSe != 0)
				pCallObj->pSessionTimer->dMinSe = 0;
		}
	}
	 ...
	 */
	/*CSR 6802051_START*/
	/* CSR_1-5736356 As part of this CSR the dStatusCode is checked for all
		...
		(dStatusCode < 300))
	{
				...
				&dTempHeader,(SipError*)&(pErr->stkErrCode)))
		{
			...
			/*
			 * Session Timer negotiation is done
			 */
...
#else
        Sdf_ty_s8bit  pTempString[Sdf_co_characterBufferSize];
#endif        
...
#if SDF_TLS
		if (Sdf_en_protoTls == pOverlapTransInfo->pDestInfo->dScheme)
		{
...
#endif
{
	...
	if( pSipUrl != Sdf_co_null )
	{
		...
		{
			do{
				Sdf_ty_s8bit 	pActualContact[Sdf_co_mediumBufferSize];
				en_HeaderType 	dHdrType;
				Sdf_ty_u32bit 	dSize = 0;
				Sdf_ty_u32bit 	dIndex = 0;
#ifdef SDF_LINT
...
#else
                Sdf_ty_s8bit 	pTempConact[Sdf_co_mediumBufferSize];
#endif                
				...
				if(Sdf_mc_strstr(pTempConact,"=")!=Sdf_co_null)
				{
					...
					Sdf_mc_strcpy(pActualContact, pHeaderName);
