 ...
 * Alok Tiwari        16-Jan-2008    SPR-17945        Application level trace
 *                                                    added for 
 *                                                    PUBLISH/MESSAGE support
 * Puneet Sharma      20-Aug-2008    CSR 1-6179301
 * Anurag Khare       07-Nov-2008    SPR 19168      Modified function
 *                                                  icf_sic_process_options_resp           
 *                                                  icf_sic_process_message_resp
 *                                                  icf_sic_process_publish_resp
 * Alok Tiwari        09-Apr-2010    SPR 20362      Modified Function
 *                                                  icf_sic_process_options_req
 *
 ******************************************************************************/
        ...
        INOUT icf_internal_msg_st        *p_internal_msg)
{
    ...
    /* Copy the ecode in the pdb */
    sic_pdb.p_ecode = &(p_internal_msg->msg_hdr.ecode);
	sic_pdb.resp_code = ICF_NULL;
    
    sic_pdb.event = ICF_SIC_EVENT_INVALID;
    sic_pdb.valid_event = ICF_TRUE;
	sic_pdb.send_htb_list = ICF_TRUE;
    sic_pdb.event_package.str[0] = ICF_NULL;
	
	sic_pdb.best_effort_unsub = ICF_FALSE;    

    /* Initialize the pointers in the pdb to NULL */
    sic_pdb.p_sic_ctx = ICF_NULL;
   
	sic_pdb.dest_addr.addr_type = 0;
    sic_pdb.dest_addr.addr_val.str_len = 0;
	sic_pdb.dest_addr.addr_val.str[0] = 0;

    sic_pdb.p_config_data = ICF_NULL;
    sic_pdb.p_msg = ICF_NULL;
    
    for (line_id = 0; line_id < ICF_MAX_NUM_OF_LINES; line_id++)
    {
        sic_pdb.p_line_data[line_id] = ICF_NULL;
    }
	...
	if( p_internal_msg->msg_hdr.msg_id == ICF_NW_INC_MESSAGE_IND)
	{
...
/*NOTIFY_OUT_OF_DIALOG_START*/
                            ...
                            p_sic_pdb->p_sic_ctx->timer_type))
                {
                    ...
                    /*SPR 18430 For out of dialog Notify when PA does not send
                      ...
                      inside sic ctx was set to null but sic context inside ssa
                      ctx was not set to null immediately. In between other OOD 
                      notify is received by IPTK and system allocates same mem 
                      again for new sic context. Now timer expiry for first SSA
                      ctx comes which still has the sic context pointer, however
                      now belongs to new sic context therefore inside FreeAppDat
                      the new SSA ctx to null and later crash occurs when access
                      pointer for new SSA ctx. Therefore to just initilaize the
                      sic ctx pointer inside SSA ctx after sending notify resp
                      we used below flag which is set true for only pointer to
...

{
    ...
    for(line_id = 0; line_id < p_sic_pdb->p_config_data->max_lines; line_id++)
    {
...
ICF_CHECK_IMS_END
            ...
            else
            {
                ...
                {
                /* mark in CC context that we have send the In-dialog option successfully */
                    p_sic_pdb->p_sic_ctx->p_call_ctx->common_bitmask |= ICF_IN_DLG_OPTIONS_SEND;
                }
            }
		...
		else
		{
			...
			{
				p_sic_pdb->p_sic_ctx->error_code = ICF_CAUSE_IN_DIALOG_OTG_OPTIONS_RECEIVED;
			}
			else
			{
...
/*  Fix fpr CSR 1-6456839/ SPR 19168 */
/*  SCENARIO : If Application sends OPTIONS req
 *             OPTIONS retransmits and remote does not respond
 ...
 */
    ...
    /*  Fix for SPR 19168/ CSR 1-6456839 */
    p_sic_pdb->p_internal_msg->msg_hdr.msg_id =  
...
/*    icf_line_id_t				line_id = 0;*/
	...
	icf_sic_msg_ind_st			msg_ind;
	icf_return_t					dummy_retval = ICF_SUCCESS;
    
    ICF_FUNCTION_ENTER(p_sic_pdb->p_glb_pdb)


	p_msg_ind = (icf_nw_inc_msg_ind_st *)
                    (p_sic_pdb->p_msg);
...
/*    ICF_SIC_TRACE(p_sic_pdb,ICF_TRACE_SIC_EVENT_PKG_RECD)*/
	...
	/*Send the message response to SSA*/
	...
	msg_ind.call_id = ICF_INVALID_CALL_ID;
...
ICF_CHECK_IMS_START(p_sic_pdb->p_glb_pdb,ICF_FALSE)
	...
	{
        /*CSR 1-6179301: When we send an CFM for OOD NOTIFY
          the context is being deallocated, so the SSA ctxt 
