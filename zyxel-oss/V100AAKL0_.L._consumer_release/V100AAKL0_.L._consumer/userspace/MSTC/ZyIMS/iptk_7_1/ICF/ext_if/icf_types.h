 ...
 * 
 *---------------------------------------------------------------------------*/
typedef icf_uint8_t icf_mode_t;        
...
/*------------------------------------------------------------------------------
 *
 * Type Name   :  ippstk_ipsec_init_params_st
 * Description :  IPSec configuration parameters read from icf.cfg
 *
 *----------------------------------------------------------------------------*/
typedef struct 
{
...
typedef struct 
{
	...
	/* seventh bit indicates the type of authentication required,if true then Non-IMS 
	   authentication is performed, if not then IMS authentication is done */
    #define ICF_PLAIN_DIGEST_AUTH_REQUIRED 0x00000040
	/*  eighth bit indicates whether client nonce string is present */
    #define ICF_SIP_AUTH_CNONCE 0x00000080
	/*  ninth bit indicates whether SIGCOMP is required */
    #define ICF_PACK_PERIOD_PRESENT 0x00000100
	/*  tenth bit indicates whether SIGCOMP is required */
    #define ICF_SIGCOMP_REQUIRED 0x00000200
	/*  eleventh bit indicates whether network trace is enabled */
    #define  ICF_IPSEC_REQUIRED 0x00000400
...
#ifdef ICF_NW_ACTIVATE
    icf_boolean_t         auto_activate; 
#endif
    icf_boolean_t	      auto_register;
#ifdef ICF_NW_ACTIVATE
	/* bitmask indicating the type of access network */
    #define ICF_ACCESS_NET_TYPE_GPRS 0x01
	icf_uint8_t           access_net_type;
#endif

	icf_string_st         auth_password; /* password obtained from configuration file */
    icf_string_st         c_nonce_string;
	...
	*/
    ...
    icf_ipsec_init_params_st    *p_ipsec_init_data;
