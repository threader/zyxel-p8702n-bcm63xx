	...
	SIP_S8bit **ppNewCurrentPos, SdpMessage *pSdpMsg, SipError *pErr)
{
	...
	if (SIP_NULL == pToken)
	{
		...
		else /* SDP line terminated by LF alone */
			increment = 1;
	}
	else
	{
	...
	if ((pCurrentPos == pToken) || \
		(SipFail == sip_atoi(pCurrentPos, pToken - 1, &temp)))
	{
		/* 
	...
	SIP_S8bit **ppNewCurrentPos, SipList *pBwList, SipError *pErr)
{
	...
	pBwInfo = sip_nStrdup(DECODE_MEM_ID, pCurrentPos, pToken - pCurrentPos, pErr);
#ifndef SIP_NO_CHECK	
	if (SIP_NULL == pBwInfo)
	{
		return SipFail;
	...
	SIP_S8bit **ppNewCurrentPos, SipList *pRepList, SipError *pErr)
{
	...
	pRep = sip_nStrdup(DECODE_MEM_ID, pCurrentPos, pToken - pCurrentPos, pErr);
	if (SIP_NULL == pRep)
	{
		return SipFail;
	}
	...
	SIP_S8bit **ppNewCurrentPos, SipList *pAttrList, SipError *pErr)
{
	...
	pToken = sip_tokenize_sdp_attr(pCurrentPos, pEnd, '\r');
	if (SIP_NULL == pToken)
	{
	...
	pTmpToken = sip_tokenize_sdp_attr(pCurrentPos, pToken-1, ':');
	...
	SIP_S8bit **ppNewCurrentPos, SIP_S8bit **ppKey, SipError *pErr)
{
	...
	*ppKey = sip_nStrdup(DECODE_MEM_ID, pCurrentPos, pToken - pCurrentPos, pErr);
	...
	SIP_S8bit **ppNewCurrentPos, SipList *pEmailList, SipError *pErr)
{
	...
	pEmail = sip_nStrdup(DECODE_MEM_ID, pCurrentPos, pToken - pCurrentPos, pErr);
#ifndef SIP_NO_CHECK
	if (SIP_NULL == pEmail)
	{
		*pErr = E_PARSER_ERROR;
