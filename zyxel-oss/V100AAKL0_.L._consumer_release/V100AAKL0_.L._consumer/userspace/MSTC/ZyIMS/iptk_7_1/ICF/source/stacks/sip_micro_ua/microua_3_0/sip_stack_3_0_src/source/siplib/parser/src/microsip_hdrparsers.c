	...
	SipWarningHeader **ppWarningHdr, SipError *pErr)
{
	...
	if (SIP_NULL == pToken)
	{
		sip_error(SIP_Minor, "Syntax error in parsing warning value");
		*pErr = E_PARSER_ERROR;
		return SipFail;
	}

	/*
	 * Validate and store warn-code
	 */
	...
	/* end SPR1_0_3 */
	{
		sip_error(SIP_Minor, "Non-numeric warn code present"); 
		*pErr = E_PARSER_ERROR;
		return SipFail;
	}
	/* begin SPR1_0_3 */
	(*ppWarningHdr)->dIntVar1 = (SIP_U16bit) tempCodeNum;
	/* end SPR1_0_3 */

	/* 
	 * Check for warn-code being 3-DIGIT 
	 */

	if (((*ppWarningHdr)->dIntVar1 > 999) || ((*ppWarningHdr)->dIntVar1 < 100))
	{
		/*
		 * warn-code is not 3-DIGIT; raise parser error
		 */
	
		sip_error(SIP_Minor, "Syntax error in warn-code: warn-code is not \
			3-digit");	
		*pErr = E_PARSER_ERROR;
		return SipFail;
	}
	...
	{
		/*
		 * Warn-agent not present;
		 * raise parser error.
		 */

		sip_error(SIP_Minor, "Warn agent not present");	
		*pErr = E_PARSER_ERROR;
		return SipFail;
	}
	
	/* 
	 * Warn-agent found. Skip leading and trailing LWS
	 * and store into header.
	 */

	/*pCurrentPos = sip_stripLeadingLWS(pCurrentPos, pToken - 1);
	pTemp = sip_stripTrailingLWS(pCurrentPos, pToken - 1);*/
	pTemp = pToken - 1;
	...
	{
		/*
		 * No warn-text present;
		 * Raise parser error.
		 */

		sip_error(SIP_Minor, "warn text not present");
		*pErr = E_PARSER_ERROR;
		return SipFail;
	}
	
	/* 
	 * Warn-text found. Store into header.
	 */

	(*ppWarningHdr)->pStr2 = sip_nStrdup(DECODE_MEM_ID, pCurrentPos, \
		pTemp - pCurrentPos + 1, pErr); 

#ifndef SIP_NO_CHECK
	if (SIP_NULL == (*ppWarningHdr)->pStr2)
	{
		return SipFail;
	}
#endif
	SIPDEBUGFN("Exiting from sip_parseWarningValue");
	return SipSuccess;
			...
			SipHeader *pOutHdr,  en_HeaderType dType, SipError *pErr)
{
	...
	if (SipHdrTypeRetryAfterDate == dType)
	{
		...
		if (SipFail == sip_parseSipDate(pSecBegin, pSecEnd, \
			&pDateStruct, pErr))
		{
			*pErr = E_PARSER_ERROR;
			return SipFail;
