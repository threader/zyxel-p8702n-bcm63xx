             ...
             if(ICF_SUCCESS == dummy_ret_val)
             {
                 ...
                 if(ICF_SUCCESS == dummy_ret_val)
                 {
                    dummy_ret_val = icf_cmn_add_node_to_list(
									p_mmi_pdb->p_glb_pdb,
									&p_mmi_msg_payload->p_local_stream_capab_list,
									p_node);
                    if(ICF_FAILURE == dummy_ret_val)
                    {
                        /* MEMFREE p_node */
	...
	else if(ICF_MMI_MMS_MOD_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
	{
			...
			stream_count++)
		{
			...
			if(ICF_STREAM_TYPE_TBCP  != stream_type)
			{
				...
				if((bitmask & ICF_REMOTE_SDP_CODEC_CHANGE)||(bitmask & \
				...
				if(bitmask & ICF_LOCAL_ANSWER_REVERT_CODEC_CHANGE)
				{
					/* Fill codecs from last_commited SDP in the request 
					 * payload for this stream */
					dummy_ret_val = icf_mmi_get_sdp_for_stream_id(p_mmi_pdb,
	...
	else if(ICF_MMI_MMS_ADD_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
	{
			...
			stream_count++)
		{
					...
					p_mms_resp->added_stream_resp[stream_count].status_success))
			{
				...
				{
					/* Case for Outgoing Re-INVITE for adding a T.38 stream when
					 * no profile recvd in app_add or Re-INVITE */
		...
		for(stream_count = 0; stream_count < p_cms_resp->count; stream_count++)
		{
				...
				p_cms_resp->stream_status[stream_count].status_success)
			{
				...
				if(ICF_SUCCESS == dummy_ret_val && ICF_NULL != p_node)
				{
					...
					dummy_ret_val = icf_cmn_add_node_to_list(
										p_mmi_pdb->p_glb_pdb,
										&p_mmi_msg_payload->p_channel_list,
										p_node);
		...
		for(stream_count = 0; stream_count < p_cms_resp->count; stream_count++)
		{
					...
					p_cms_resp->stream_status[stream_count].status_success)
			{
				...
				if(ICF_SUCCESS == dummy_ret_val)
				{
					dummy_ret_val = icf_cmn_add_node_to_list(
										p_mmi_pdb->p_glb_pdb,
										&p_mmi_msg_payload->p_stream_list_data,
										p_node);
          ...
          (ICF_LOCAL_MEDIA_CONNECT_MODE_CHANGE & bit_mask))
			...
			if(ICF_SUCCESS == dummy_ret_val)
			{
				dummy_ret_val = icf_cmn_add_node_to_list(p_mmi_pdb->p_glb_pdb,
							&p_mmi_msg_payload->p_stream_data_list, p_node);
						...
						if(ICF_SUCCESS == dummy_ret_val)
						{
							dummy_ret_val = 
								icf_cmn_add_node_to_list(
									p_mmi_pdb->p_glb_pdb,
									&p_mmi_msg_payload->p_stream_commit_list,
									p_node);
        ...
        INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
	...
	if(ICF_MMI_MMS_DEL_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
	{
		...
		for(stream_count = 0; stream_count< p_mms_req->p_delete_media->count;
			stream_count++)
		{
...
ICF_CHECK_QOS_SUPPORT_END
		 ...
		 * action of MM_req */
			for(stream_count = 0; stream_count < 
						p_mms_req->p_delete_media->count;
						stream_count++)
			{
				stream_id = 
					p_mms_req->p_delete_media->stream_id[stream_count];
		 ...
		 * MM_req */
			for(stream_count = 0; 
							stream_count < p_mms_resp->add_stream_count; 
						stream_count++)
			{
				if(ICF_FALSE == 
				p_mms_resp->added_stream_resp[stream_count].status_success)
...
)
{
	icf_return_t						ret_val = ICF_FAILURE,
										dummy_ret_val = ICF_SUCCESS;
	icf_close_media_channel_req_st		*p_mmi_msg_payload = ICF_NULL;
    icf_msg_st                          *p_mmi_msg = ICF_NULL;
	icf_uint8_t							api_len = ICF_NULL;
	icf_mmi_mms_request_st				*p_mms_req = ICF_NULL;
	icf_mmi_internal_mms_resp_st		*p_mms_resp = ICF_NULL;
	...
	/* Fill the payload */
    p_mmi_msg_payload = (icf_close_media_channel_req_st *)(p_mmi_msg->payload);	
	...
	if(p_mmi_pdb->bitmask & ICF_MMI_PDB_ALL_STREAMS)
	{
		...
		else if(ICF_MMI_MMS_DEL_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
		{
			...
			for(stream_count = 0; 
					stream_count < p_mms_req->p_delete_media->count;
					stream_count++)
			{
				/* MEMGET for p_node as type icf_channel_st */
				ICF_MEMGET (p_mmi_pdb->p_glb_pdb,
        ...
        INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
	...
	for(stream_count = 0; stream_count < p_mmi_msg_payload->count; 
		stream_count++)
	{
	...
	while(ICF_NULL != p_sdp)
	{
		switch(api_id)
		{
			...
			case ICF_CODEC_COMMIT_RESP:
            {
				...
				for(codec_count = 0; codec_count < p_mmi_pdb->num_of_codecs;
										codec_count++)
				{   
        ...
        IN icf_trace_id_t        trace_id)
{
    ...
    /*Fix for UMR */
    icf_port_memset(&trace_data,0,sizeof(icf_trace_data_st));
	...
	case ICF_TRACE_RTP_ADDR_ALLOCATE:
        {
            ...
            if( ICF_SUCCESS == p_mmi_pdb->result)
            {
                ...
                icf_port_sprintf((icf_int8_t *)port,":%d",p_mmi_pdb->stream_addr.port_num);
...
)
{
	...
	ICF_PRINT(("[MMI]:Entered icf_mmi_handle_mm_init_comp\n"));
	...
	{
		ICF_PRINT(("[MMI]:MemAlloc failure\n"));
		ret_val = ICF_FAILURE;
	}

	ICF_PRINT(("[MMI]: icf_handle_codec_cfg_resp\n"));
	return ret_val;
}
...
)
{
	...
	ICF_PRINT(("[MMI]:Entered icf_send_ringer_tones_req_to_mm\n"));
	...
	ICF_PRINT(("[MMI]: icf_send_ringer_tones_req_to_mm\n"));
	return ret_val;
}
...
)
{
    ...
    {
		/* add a trace here */
        ICF_PRINT(("[MMI]: ERROR: Could not open configuration file %s \n",
			filename));
    }
	else
	{
	...
	for (count =0; count< conf_file.count; count++)
	{
			...
			(icf_uint8_t*)conf_file.line[count].data) == 0)
		{
            ...
            {
                ICF_PRINT(("NOTE: CODEC MODE - Supported Input Values [1],[2] \n"));
                ICF_PRINT(
			...
			(icf_uint8_t*)conf_file.line[count].data ) == 0)
		{
            ...
            {
                ICF_PRINT(("NOTE: CRC ENABLED - Supported Input Values [1/Y],[0/N] \n"));
                ICF_PRINT(
    ...
    ICF_PRINT(("INFO: CODEC_MODE: %d \n", 
		p_mmi_data->mm_init_params.codec_mode)); 
    ICF_PRINT(("INFO: CRC_ENABLED: %d \n", 
		p_mmi_data->mm_init_params.crc_enabled)); 
    ICF_PRINT(("INFO: APP_RTCP_PORT_MUX: %d \n", 
		p_mmi_data->mm_init_params.app_rtcp_port_mux)); 
	ICF_PRINT(("INFO: PACKETIZATION_PERIOD: %d \n", 
		p_mmi_data->mm_init_params.pkt_period ));
	ICF_PRINT(("INFO: RTP_START_PORT: %d \n", 
		p_mmi_data->mm_init_params.rtp_start_port ));
	ICF_PRINT(("INFO: RTP_END_PORT: %d \n", 
...
)
{
	...
	if (ICF_NULL != p_msg)
	{
       ...
       p_cfg_codec_info_req->codec_supp[1].bitmask &= ICF_CODEC_PARAM_PRESENT;
		...
		if(ICF_SUCCESS == ret_val)
	    {
			ICF_PRINT(("[MMI]: Codec Configure Request sent to MM successfully\n"));
			ret_val = ICF_SUCCESS;
		}
		else
		{
			ICF_PRINT(("[MMI]: Codec Configure Request to MM failed\n"));
			ret_val = ICF_FAILURE;
		}
	...
	ICF_PRINT(("[MMI]: icf_mmi_handle_mm_init_comp\n"));
	return ret_val;
}
...
ICF_CHECK_IMS_START(p_mmi_pdb->p_glb_pdb,ICF_TRUE)
	ret_val = icf_mmi_send_sec_pdp_activ_req(p_mmi_pdb);
ICF_CHECK_IMS_END
#endif
#endif
...
ICF_CHECK_IMS_START(p_mmi_pdb->p_glb_pdb,ICF_FALSE)
	...
	ret_val = icf_mmi_send_qos_reserve_for_non_ims_req(p_mmi_pdb);

ICF_CHECK_IMS_END
#endif

ICF_MMI_SET_BIT(p_mmi_pdb,ICF_QOS_RESERVATION_ATTEMPTED)
...
ICF_CHECK_IMS_START(p_mmi_pdb->p_glb_pdb,ICF_FALSE)
		...
		ret_val = icf_mmi_send_qos_release_for_non_ims_req(p_mmi_pdb);
...
/******************************************************************************
 *
 * FUNCTION:        icf_mmi_send_qos_release_for_non_ims_req
 *
 * DESCRIPTION:                        
 *                  
 * RETURNS:         ICF_SUCCESS in case of success and ICF_FAILURE in 
...
icf_return_t icf_mmi_send_qos_release_for_non_ims_req(
		INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
...
/******************************************************************************
 *
 * FUNCTION:        icf_mmi_send_qos_reserve_for_non_ims_req
 *
 * DESCRIPTION:                       
 *                  
 * RETURNS:         ICF_SUCCESS in case of success and ICF_FAILURE in 
...
icf_return_t icf_mmi_send_qos_reserve_for_non_ims_req(
		INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
	icf_return_t					ret_val = ICF_SUCCESS;
	icf_internal_msg_st				*p_nm_internal_msg = ICF_NULL;
...
)
{
	...
	for(stream_count = 0; stream_count < p_src->count; stream_count++)
	{
...
)
{
	...
	for(stream_count = 0; stream_count < p_ctx->stream_count; stream_count++)
	{
	...
	while(ICF_NULL != p_src)
	{
		...
		if(p_data->p_remote_received_sdp)
		{
			...
			if(ICF_SUCCESS == ret_val)
			{
				...
				p_node->stream_active = (icf_uint8_t)
						(ICF_MEDIA_STREAM_ACTIVE & p_data->stream_status);
...
)
{
	...
	if(ICF_SUCCESS == mms_resp.result)
	{
		...
		for(stream_count = 0; stream_count  < mms_resp.add_stream_count; 
			stream_count++)
		{
		...
		for(stream_count = 0; stream_count  < mms_resp.mod_stream_count; 
			stream_count++)
		{
...
)
{
	...
	if(ICF_SUCCESS == cms_resp.result)
	{
		...
		for(stream_count = 0; stream_count < cms_resp.count; stream_count++)
		{
...
)
{
	...
	if(ICF_SUCCESS == ret_val)
	{
		...
		for(count =0; count < total_count; count++)
		{
			...
			if(p_reject_list)
			{
				for(loc_count = 0; loc_count < p_reject_list->count; loc_count++)
				{
			...
			if(0 != p_app_amr_params->amr_mode_count)
			{
				if(0 != p_amr_params->amr_mode_count)
				{
					...
					for(app_mode_index = 0; app_mode_index < p_app_amr_params->\
						amr_mode_count; app_mode_index++)
					{
						for(recv_mode_index = 0; recv_mode_index < p_amr_params->\
							amr_mode_count; recv_mode_index++)
						{
            ...
            icf_list_st         **p_p_codec_info_list)
{
    ...
    for (cnt = 0; cnt < p_mmi_pdb->p_mmi_ctx->stream_count; cnt++)
    {
        if (stream_id == p_mmi_pdb->p_mmi_ctx->codec_for_commit[cnt].stream_id)
        {
            for (codec_cnt = 0; 
                    codec_cnt < p_mmi_pdb->p_mmi_ctx->codec_for_commit[cnt].codec_count;
                    codec_cnt++)
            {
                ICF_MEMGET (p_mmi_pdb->p_glb_pdb,
					ICF_PORT_SIZEOF(icf_codec_attrib_st),
...
)
{
	...
	for(stream_count = 0; stream_count < p_mmi_pdb->p_mmi_ctx->stream_count;
		stream_count++)
	{
		if(stream_id == 
			p_mmi_pdb->p_mmi_ctx->codec_for_commit[stream_count].stream_id)
		{
				...
				p_mmi_pdb->p_mmi_ctx->codec_for_commit[stream_count].codec_count)
			{
                /*This function is triggered to ignore codec mismatch if the 
                  difference is there due to presence of dtmf codecs*/
                if(ICF_FAILURE == icf_mmi_check_dtmf_codec(p_mmi_pdb, p_codec_info_list,
...
)
{
	...
	else if(ICF_MMI_MMS_MOD_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
	{
		...
		for(stream_count = 0; stream_count < p_mms_resp->mod_stream_count; stream_count++)
		{
 ...
 *                 DTMF codec.
 *****************************************************************************/
icf_return_t    icf_mmi_check_dtmf_codec
(
    IN      icf_mmi_pdb_st          *p_mmi_pdb,
    IN      icf_list_st             *p_codec_list,
...
)
{
    ...
    if(ICF_FAILURE == ret_val)
    {
        for(codec_count = ICF_NULL; codec_count < p_codec_for_commit->codec_count;
               codec_count++)
        {
            if(ICF_NULL == icf_port_strcmp((icf_uint8_t *) "telephone-event",
