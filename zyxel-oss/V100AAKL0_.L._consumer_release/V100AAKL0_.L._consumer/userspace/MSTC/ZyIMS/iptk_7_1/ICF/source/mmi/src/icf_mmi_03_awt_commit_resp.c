...
)
{
	...
	{
		for(loc_count1 = 0; loc_count1 < p_mms_resp->mod_stream_count; loc_count1++)
		{
			for(loc_count2 = 0; loc_count2 < p_mms_resp->mod_stream_count;
				loc_count2++)
			{
	...
	if(ICF_MMI_MMS_MOD_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
	{
				...
				ICF_CODEC_COMMIT_RESP_REJ_STREAM_LIST_PRESENT)
		{
			...
			{
				for(stream_count = 0; 
						stream_count < p_mms_resp->mod_stream_count;
						stream_count++)
				{
					stream_id = 
...
ICF_CHECK_QOS_SUPPORT_START(p_mmi_pdb->p_glb_pdb,ICF_TRUE)
		...
		{	
			/* Send Secondary PDP Context Activation Request */
			ret_val = icf_mmi_send_qos_reserve_req(p_mmi_pdb);
...
)
{
	...
	while(ICF_NULL != p_list)
	{
		...
		for(codec_count = 0; codec_count < p_data->codec_count; codec_count++)
		{
