...
* 07-Jan-2008  Amit Sikka           SPR 17945   PUBLISH-MESSAGE Support
* 24-Sep-2009  Anurag Khare         SPR 20071   Fix for CSR 1-7798174
* 25-Jan-2010     Pavneet Bajaj       SPR 20278   Comment added for ICF_MAX_STR_LEN  
...
#define ICF_NETWORK_API_BASE                    ICF_APP_SETUP_API_END + 1  /* 6 */
...
#define ICF_MPH_API_BASE						ICF_NETWORK_API_END + 1  /* 11 */
...
#define ICF_CFG_API_BASE                        ICF_MPH_API_END + 1  /* 22 */
...
#define ICF_CONTROL_PLANE_API_BASE              ICF_CFG_API_END + 1  /* 38 */
...
#define ICF_CONTROL_PLANE_API_END         ICF_CONTROL_PLANE_API_BASE + 84

#define ICF_SM_API_START                  ICF_CONTROL_PLANE_API_END + 1  /* 123 */
...
#define ICF_SM_API_END          ICF_SM_API_START + 19

/****************************************************************************
  These API's are used for ICF Testing. Not to be used by external apps.
  **************************************************************************/

#define ICF_TIMER_MSG                                           ICF_SM_API_END + 1  /* 143 */
...
#define ICF_MISC_API_START						(ICF_NW_API_CLOSE_CONN_IND + 2000)  /* 2148 */
...
#define ICF_PORT_PLTFM_API_START				ICF_MSG_FRM_PLTFM /* 2168 */ 			
...
#define ICF_PORT_PLTFM_PDP_ACT_START			(ICF_PORT_PLTFM_IPSEC_END + 1) /* 2181 */  
...
#define ICF_PORT_PLTFM_MISC_START					(ICF_PORT_PLTFM_PDP_ACT_END + 1) /* 2194 */ 
...
#define ICF_TRACE_NW_BUFF_IND						(ICF_PORT_PLTFM_API_END+1) /* 2212 */  
/* End of misc APIs */

/* API to send logs to Application */
#define ICF_LOGS_IND		        				(ICF_TRACE_NW_BUFF_IND+1) /* 2213 */ 

#ifdef  ICF_NAT_MEDIA_TRAVERSAL

#define ICF_API_NAT_START                   (ICF_LOGS_IND + 1) /* 2214 */ 
...
#define ICF_MAX_NUM_OF_LINES						2

/* Maximum number of registration supported */
#define ICF_MAX_NUM_OF_REGISTRATION 				2
...
/* Default values */
#define ICF_DEFAULT_MAX_LINES                       2

#define ICF_DEFAULT_MAX_APPLICATIONS                2

#define ICF_DEFAULT_MAX_REGISTRATIONS               2

#define ICF_DEFAULT_MAX_AUDIO_CALLS                       10
#define ICF_DEFAULT_MAX_VIDEO_CALLS                       10
#define ICF_DEFAULT_MAX_SERVICE_CALLS                     10
...
/* This parameter defines the maximum length of a SIP Header supported by IPTK and IPTK is tested 
 * with the default specified value only. 
 * 
 * 
 * Note:: 
 * 1. This parameter has internal dependencies on other internal parameters like SIP_MAX_HDR_SIZE
 *    should be less than  SIP_MAX_MSG_SIZE. Therefore, changing this value, requires 
 *    thorough review/testing. These parameters are defined in the following file: 
 ...
 *    Tuning: ICF_SSA_MAX_HEADER_VALUE_LEN <= SIP_MAX_HDR_SIZE
 * 
 * 2. Changing the default value of this parameter shall also effect the functionality 
 *    when IPTK has to send the correspoding header to application (Application has requested the header).
 *    The parameter ICF_MAX_STR_LEN defines the maximum length of header sent to be sent to Application if 
 *    requested.
 *    The Application can tune ICF_MAX_STR_LEN in the following manner:
 *        ICF_MAX_STR_LEN < ICF_SSA_MAX_HEADER_VALUE_LEN + 2
 *				 - truncated (if it is larger) header is given to application (if requested)
 *		  ICF_MAX_STR_LEN = ICF_SSA_MAX_HEADER_VALUE_LEN + 2
 *				 - complete header is given to application (if requested)
 * 		  ICF_MAX_STR_LEN > ICF_SSA_MAX_HEADER_VALUE_LEN + 2
 *				 - complete header is given to application (if requested)
...
#define ICF_ADDRESS_PLAN_SIPS_URI                     6 /* SIP URI */
...
#define ICF_CALL_CLEAR_REASON_PROXY_AUTH_FAILURE        32
...
#define ICF_CALL_CLEAR_REASON_CODEC_NOT_AVAILABLE       50


#define ICF_CALL_CLEAR_REASON_END			51
...
#define ICF_DISP_USER_DEREGISTERED						3	
#define ICF_DISP_CALL_MUTE_FAILURE						4
#define ICF_DISP_CALL_UNMUTE_FAILURE					5
/*ICF_HANDLE_REINVITE_START*/
