       ...
       INOUT icf_cfw_pdb_st      *p_cfw_pdb)
{
    ...
    else
    {
         ...
         */
        for(line_id = 0; line_id < (p_config_data->max_lines); line_id++)
        {
        ...
        {
            if(ICF_NULL == p_line_data->addr_for_cfd.addr_val.str_len)
            {
                p_cfw_pdb->is_valid_req = ICF_FALSE;
            }
            else
        ...
        {        
            if(ICF_NULL == p_line_data->addr_for_cfb.addr_val.str_len)
            {
                p_cfw_pdb->is_valid_req = ICF_FALSE;
            }
            else
                ...
                (ICF_TRUE == p_cfw_pdb->is_valid_req))
        {
            ...
            else
            {
                            ...
                            no_of_diversion_data); div_index++)
                {
                    ...
                    for(line_id = 0; line_id < p_config_data->max_lines; 
                            line_id++)
                    {
        ...
        INOUT icf_cfw_pdb_st     *p_cfw_pdb)
{
    ...
    else
    {
        ...
        if(ICF_NULL != p_line_data->user_name.str_len)
        {
            icf_port_memcpy(
