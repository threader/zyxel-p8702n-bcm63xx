	...
	const Sdf_ty_s8bit * fmt, ...)
{
        ...
        _vsnprintf(s,dSize,fmt,ap);

#else

         vsnprintf(s,dSize,fmt,ap);
...
#if !defined(SDF_SUPER) && (!defined(SDF_LINUX) || !defined(ICF_PORT_LINUX))
/******************************************************************************
	...
	*ppDest=Sdf_co_null;
...
#ifdef SDF_DEBUG
	sdf_fn_debug((Sdf_ty_s8bit *)"SDF_DEBUG - Entering sdf_fn_uaGetHostIp");
#endif	
	...
	if (pError == Sdf_co_null)
	{
		...
		sdf_fn_uaDisplayFnError((Sdf_ty_s8bit*)"SDF_ERROR - "
			"sdf_fn_uaGetHostIp( ): Invalid Error parameter");
#endif
		return Sdf_co_fail;
	}
	if (pHost == Sdf_co_null)
	{
		...
		sdf_fn_setError(Sdf_en_minorErrors,Sdf_en_invalidParamError,\
			(Sdf_ty_s8bit *)"sdf_fn_uaGetHostIp( ): "
...
#ifdef SDF_DEBUG
	sdf_fn_debug((Sdf_ty_s8bit *)"SDF_DEBUG - Exiting sdf_fn_uaGetHostIp");
#endif	
...
#endif
{
	...
	if (pSocket->dProtocol == Sdf_en_protoTls)
	{
		...
		pSslCtx = (SSL_CTX *)(((Sdf_st_sslData *)pExtraArgs)->pData);
...
#ifdef SDF_LINT
	(void)SSL_set_fd(pSsl, pSocket->dSockfd);
#else
 	SSL_set_fd(pSsl, pSocket->dSockfd);
#endif
 ...
 **
 *****************************************************************************/
#if defined(SDF_LINUX) || defined(ICF_PORT_SYMBIAN)|| defined(ICF_PORT_WINCE)
                             ...
                             Sdf_st_error *pErr)
{
    ...
    if(ICF_NULL == pStr)
    {
       ICF_PRINT(((icf_uint8_t *)"pStr is null"));
...
#if 0
   Sdf_ty_s8bit *pTimeStr = Sdf_co_null ;

#if defined ICF_PORT_WINCE
	Sdf_mc_printf("SDF_TRACE: Generated At ");         
	sdf_fn_uaGetTimeString(&pTimeStr);                               
	Sdf_mc_printf("%s",pTimeStr);      
	sdf_memfree(Sdf_mc_accessorMemId,(Sdf_ty_pvoid*)&pTimeStr,pErr); 
	Sdf_mc_printf("::::<>::::");                                     
	Sdf_mc_printf("%s",pStr);                        
	Sdf_mc_printf("\n\n");        
...
/*
	...
	_write(2,(Sdf_ty_s8bit *)"SDF_TRACE: Generated At ",24);         

	sdf_fn_uaGetTimeString(&pTimeStr);                               

	_write(2,(Sdf_ty_s8bit *)pTimeStr,Sdf_mc_strlen(pTimeStr));      
	...
	if(dLen == 0)                                                    
		...
		_write(2, pStr, Sdf_mc_strlen(pStr));                        
...
*/
   ...
   /*sdf_fn_uaTestCancel(Sdf_co_null);      */ 
	(void)write(2,(Sdf_ty_s8bit *)"SDF_TRACE: Generated At ",24);          
	sdf_fn_uaGetTimeString(&pTimeStr);                               
	write(2,(Sdf_ty_s8bit *)pTimeStr,Sdf_mc_strlen(pTimeStr));       
	(void)sdf_memfree(Sdf_mc_accessorMemId,(Sdf_ty_pvoid*)&pTimeStr,pErr); 
	(void)write(2, "::::<>::::", 10);                                      
	if(dLen == 0)                                                    
		(void)write(2, pStr, Sdf_mc_strlen(pStr));                         
	else                                                             
		(void)write(2, pStr, dLen);                                        
	(void)write(2, "\n\n", 2);                                             
/*	sdf_fn_uaTestCancel(Sdf_co_null);*/
#endif	/* end of Windows flag */
#endif /* end of WIN CE Flag */
#endif 
}
...
Sdf_ty_retVal sdf_fn_uaInitSslData(Sdf_ty_s8bit *pEGDRandFile)
{
	 ...
	 */
    /*w534*/ (void)SSLeay_add_ssl_algorithms();
...
#endif
{
	...
	if(pSslCtx == Sdf_co_null)
    {
		...
		sdf_fn_setError(Sdf_en_majorErrors,Sdf_en_noMemoryError, \
			...
			"Failed to create SSL_CTX structure",pErr);
#endif
    ...
    if (pTempSslParams->dCertFormat == Sdf_en_fileFormatPEM)
		dCertFormat = SSL_FILETYPE_PEM;
	else
		dCertFormat = SSL_FILETYPE_ASN1;

    if (SSL_CTX_use_certificate_file(pSslCtx, pTempSslParams->pCertFile, \
				dCertFormat ) != 1)
    {
#ifdef SDF_ERROR
	...
	SSL_CTX_set_default_passwd_cb(pSslCtx, sdf_cbk_uaGetPrivateKeyPassword);

	/* 
	 * Set the password in the userdata. The same userdata will be returned
	 * when the callback is issued.
	 */
	SSL_CTX_set_default_passwd_cb_userdata(pSslCtx, \
			(Sdf_ty_pvoid)(pTempSslParams->pPrivateKeyPasswd));

	if (SSL_CTX_use_PrivateKey_file(pSslCtx, pTempSslParams->pPrivateKeyFile, \
				dCertFormat) != 1)
    {
#ifdef SDF_ERROR
		sdf_fn_setError(Sdf_en_majorErrors, \
	...
	/* setting CAPath */
	/* CSR_1-5036521*/
    if (SSL_CTX_load_verify_locations(pSslCtx, pTempSslParams->pCACertFile, \
	...
	SSL_CTX_set_verify(pSslCtx, pTempSslParams->dCertVerifyMode, \
	...
	if(SSL_CTX_set_cipher_list(pSslCtx, pCipherList) != 1)
	{
#ifdef SDF_ERROR
		sdf_fn_setError(Sdf_en_majorErrors, \
	...
	if (pTempSslParams->pDHParamFile != Sdf_co_null)
	{
		...
		{
			dh = PEM_read_DHparams(pDHParamfp, Sdf_co_null, Sdf_co_null, \
					Sdf_co_null);
		}
		else
		{
			/* FORMAT_ASN1 case */
			dh = d2i_DHparams_fp(pDHParamfp, Sdf_co_null);
		}
		...
		if(Sdf_co_null != dh)
		{
			/*w534*/ (void)SSL_CTX_set_tmp_dh(pSslCtx, dh);
			/*w534*/ (void)SSL_CTX_set_options(pSslCtx, SSL_OP_SINGLE_DH_USE);
			DH_free(dh);
		}
		else
		{
