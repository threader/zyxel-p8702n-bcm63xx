...
ICF_CHECK_QOS_SUPPORT_START(p_cc_pdb->p_glb_pdb,ICF_TRUE)
	 ...
	 * response for UPDATE for Qos is received and now 180 ringing
	 * can be processed
	 */

	p_cc_pdb->p_call_ctx->resource_reservation_completed = ICF_TRUE;
	
	ret_val = icf_cc_send_pa_media_connect(p_cc_pdb);
        ...
        INOUT icf_cc_pdb_st *p_cc_pdb)
{
     ...
     * management. If icf_simul_call_line_arbitration is TRUE, then ICF is 
     * internally managing the lines and hence PA cannot request this */
    if ((ICF_CALL_CLEAR_REASON_ATTEMPT_ON_ALTERNATE_LINE == 
            p_cc_pdb->p_call_ctx->release_cause) &&
        (ICF_FALSE == icf_simul_call_line_arbitration))
    {
        if(ICF_FAILURE == 
        ...
        INOUT icf_cc_pdb_st *p_cc_pdb)
{
   ...
   p_user_alerted = ((icf_pa_cc_user_alerted_req_st *)
	...
	else  
	{
	    ...
	    if (ICF_HDR_LIST_PRESENT & p_user_alerted->bit_mask)
	    {
		    ret_val = icf_cmn_clone_header_list(
					    p_cc_pdb->p_glb_pdb,
					    &(p_user_alerted->header_list),
	    ...
	    if ((ICF_SUCCESS == ret_val) && 
		    (ICF_TAG_LIST_PRESENT & p_user_alerted->bit_mask))
	    {
		    ret_val = icf_cmn_clone_tag_list(
					    p_cc_pdb->p_glb_pdb,
					    &(p_user_alerted->tag_list),
	    ...
	    if ((ICF_SUCCESS == ret_val) && 
		    (ICF_BODY_LIST_PRESENT & p_user_alerted->bit_mask))
	    {
		    ret_val = icf_cmn_clone_body_list(
					    p_cc_pdb->p_glb_pdb,
					    &(p_user_alerted->body_list),
        ...
        INOUT icf_cc_pdb_st *p_cc_pdb)
{
    ...
    /* clone the header list into call ctx, if received from app */
    if (ICF_HDR_LIST_PRESENT & p_media_connect->bit_mask)
    {
        ret_val = icf_cmn_clone_header_list(
                p_cc_pdb->p_glb_pdb,
                &(p_media_connect->header_list),
                &(p_cc_pdb->p_call_ctx->header_list),
                p_cc_pdb->p_ecode);
    }
    ...
    /* clone the tag list into call ctx, if received from app */
    if ((ICF_SUCCESS == ret_val) && 
            (ICF_TAG_LIST_PRESENT & p_media_connect->bit_mask))
    {
        ret_val = icf_cmn_clone_tag_list(
                p_cc_pdb->p_glb_pdb,
                &(p_media_connect->tag_list),
                &(p_cc_pdb->p_call_ctx->tag_list),
                p_cc_pdb->p_ecode);
    }
    ...
    /* clone the body list into call ctx, if received from app */
    if ((ICF_SUCCESS == ret_val) && 
            (ICF_BODY_LIST_PRESENT & p_media_connect->bit_mask))
    {
        ret_val = icf_cmn_clone_body_list(
                p_cc_pdb->p_glb_pdb,
                &(p_media_connect->body_list),
                &(p_cc_pdb->p_call_ctx->body_list),
                p_cc_pdb->p_ecode);
    }
