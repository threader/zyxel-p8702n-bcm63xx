 ...
 *
 * 16-Jan-2008  Alok Tiwari                         Added traces for 
	...
	INOUT icf_glb_data_st *p_glb_data)
{
  if(ICF_NULL == p_glb_data)
    ...
    p_glb_data->app_trace_data[ICF_TRACE_DEREG_REQ].p_trace_string =
        (icf_uint8_t *)"DeRegistration Req Recvd from App";
    ...
    p_glb_data->app_trace_data[ICF_TRACE_NOTIFY_CFM].p_trace_string =
        (icf_uint8_t *)"Out of Dialog Notify Cfm Recvd from App";
	...
	/*************************************************************************
     ...
     p_glb_data->app_trace_data[ICF_TRACE_CALL_MODIFY_RESP].p_trace_string =
            (icf_uint8_t *)"Call Modifty Response";
     ...
     p_glb_data->app_trace_data[ICF_TRACE_CALL_WAITING_IND].p_trace_string =
            (icf_uint8_t *)"Call Waiting Indication to APP.(icf_uint8_t *)";
         ...
         icf_error_t *p_ecode)
{
   ...
   if (ICF_TRACE_NW_BUFF_IND == p_msg->hdr.api_id)
   {
       ICF_MEMGET(
               p_glb_pdb,
...
/*
	case ICF_TRACE_NW_BUFF_IND:
		p_sprintf_count=icf_cmn_trace_nw_buff_ind(p_msg,p_sprintf_count);
...

{
   ...
   case ICF_MGMT_PLANE_API_PROC_FAILURE        :p_sprintf_count+=ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\n api name = ICF_MGMT_PLANE_API_PROC_FAILURE\n")); break;
	...
	case ICF_TRACE_NW_BUFF_IND                 :p_sprintf_count+=ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\n api name = ICF_TRACE_NW_BUFF_IND"));break; 
...

{
    ...
    p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,
                            "bit_mask = %d \ndefault_settings = %d, qos_mark = %d, \nsetup_timer = %d, alerting_timer = %d,\nrelease_timer = %d, \nmodify_media_timer = %d, registration_resp_timer = %d,\nregistration_retry_timer = %d  ",
                            p_set_call->bit_mask,p_set_call->default_settings, p_set_call->qos_mark,p_set_call->setup_timer, p_set_call->alerting_timer,
	                    p_set_call->release_timer,p_set_call->modify_media_timer,p_set_call->registration_resp_timer, p_set_call->registration_retry_timer
                            ))
    ...
    {
    	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\noptions_timer = %d",p_set_call->options_timer));
    }
	...
	{
		p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"%c",p_set_call->phone_info.str[i]));
	}
    }
    ...
    {
    	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\nsession_timer = %d",p_set_call->session_timer))

    }

    if(p_set_call->bit_mask & ICF_SET_CALL_PRM_MIN_SESSION_TIMER_INTERVAL)
    {
    	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\nmin_se = %d",p_set_call->min_se))
    }
#endif
   /* if(ICF_TRUE==p_set_call->multiple_answer_enable)
    ...
    {
	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\ndns_min_retrans = %d",p_set_call->dns_min_retrans));
    }
    if( ICF_SET_CALL_PRM_DNS_MAX_TIMER & p_set_call->bit_mask )
    {
	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\ndns_max_retrans = %d",p_set_call->dns_max_retrans));
    }
...
/* Header LIST  */    if (p_con_ind->bit_mask & ICF_HDR_LIST_PRESENT) {
	...
	while(p_con_ind->p_accepted_stream_list!=ICF_NULL)
	{
		...
		p_sprintf_count = icf_cmn_trace_modify_media_change_req(p_con_ind->p_accepted_stream_list->p_data,p_sprintf_count);
...
/* FIX FOR SPR 17944 */	
	...
	{	
    	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,
            "\nrt_info.len = %d", p_get_line_stg_req->routing_info.len));

    	p_sprintf_count = icf_cmn_trace_transport_addr(
                          ((icf_transport_addr_st*)&(rt_info->ipc.udp.ip)),
                            p_sprintf_count);
...

{
	...
	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,
        "\nrt_info.len = %d", p_get_sys_stg_req->routing_info.len));

	p_sprintf_count = icf_cmn_trace_transport_addr(((icf_transport_addr_st*)&(rt_info->ipc.udp.ip)),p_sprintf_count);
    p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,
	    "\nrt_info.port = %d\n", rt_info->ipc.udp.port));
...

{
    ...
    p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,
        "\nrt_info.port = %d", rt_info->ipc.udp.port));
...
/* Tag LIST  */    if (p_reinvite_cfm->bit_mask & ICF_TAG_LIST_PRESENT) {
	...
	p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\n Rejected stream count %d:",p_reinvite_cfm->count));
                ...
                for(i=0;i<p_reinvite_cfm->count;i++)
                {
                        p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\n stream id %d:",
...

{
	...
	while(p_options_resp->p_remote_stream_list!=ICF_NULL)
	{
		...
		p_sprintf_count = icf_cmn_trace_stream_capab(p_options_resp->p_remote_stream_list->p_data,p_sprintf_count);
...
icf_uint8_t* icf_cmn_trace_trace_data(icf_trace_data_st* p_trace_data, icf_uint8_t* p_sprintf_count)
{
    ...
    for(i=0; i<p_trace_data->num_of_integer_values; i++)
    {
...
icf_uint8_t* icf_cmn_trace_error_data(icf_error_data_st* p_error_data, icf_uint8_t* p_sprintf_count)
{
    ...
    for(i=0; i<p_error_data->num_of_integer_values; i++)
    {
...
icf_uint8_t* icf_cmn_trace_misc_stats(icf_misc_stats_st* p_misc_stat, icf_uint8_t* p_sprintf_count)
{
    ...
    p_sprintf_count += ICF_SPRINTF(((icf_int8_t *)p_sprintf_count,"\n No of SRV DNS Query Failure Response: %d",
        p_misc_stat->stats_SRV_DNS_QUERY_FAILURE_RESP));
