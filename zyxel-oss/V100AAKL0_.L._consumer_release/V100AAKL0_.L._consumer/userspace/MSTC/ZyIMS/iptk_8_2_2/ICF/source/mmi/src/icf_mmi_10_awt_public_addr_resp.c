	...
	else
	{
        ...
        if(ICF_SUCCESS == ret_val)
        {
           ...
           icf_mmi_convert_nat_addr_to_trans_addr(&(p_recv_msg->public_addr),\
                    &public_addr);
#ifdef ICF_LOAD_STAT
           /* SPR 19825 : If the public address received is NULL,fill
              the same with hardcoded values.
           */ 
           if(ICF_NULL == public_addr.addr.addr.ipv4_addr.octet_1)
           {
               public_addr.addr.addr_type = 2;
               public_addr.addr.addr.ipv4_addr.octet_1 = 172;
               public_addr.addr.addr.ipv4_addr.octet_2 = 16;
               public_addr.addr.addr.ipv4_addr.octet_3 = 105;
               public_addr.addr.addr.ipv4_addr.octet_4 = 35;
               public_addr.port_num = 20043;
           }
#endif
