 ...
 * 17-Jan-2008  Abhishek Dhammawat ICF 8.0      Precondition Addition
 * 21-Jan-2007  Shruti Thakral                  Enhancements for Rel 8.0
 * 25-Jan-2008  Tarun Gupta    IPTK Rel 8.0 LLD  Added fields in ssa context st
 * 01-May-2008  Amit Sharma    SPR:18390       New bit and parameter in SSA PDB
 * 06-May-2008  Alok Tiwari    SPR:18503    New field  p_pending_pdu added in
 *                                          structure icf_ssa_tcpm_conn_data_st
 * 02-Jun-2008  Tarun Gupta    SPR 18585    Modified icf_ssa_ctxt_st                                         
 ...
 * 10-Jul-2008 Abhishek Dhammawat SPR 18630    Modified SSA PDB
 * 11-Nov-2008  Tarun Gupta       SPR 19189    Merged Rel 7.1.2 SPR 18872
 * 02-Mar-2009     Alok Tiwari      IPTK Rel8.1 IPTK open source replacement:
 *                                              fields related to ares has
 *                                              been removed from structure
 *                                              icf_ssa_ctx_st.
 * 18-Mar-2009     Rajiv Kumar     IPTK Rel8.1  UT Defect Fix 
 * 07-Apr-2009     Tarun Gupta     Rel 8.2      REFER/NOTIFY Enhancements
 ...
 * 24-Jul-2009     Anuradha Gupta    SPR 19097      bitmask name modified in SSA Ctx
 * 04-Aug-2009 Abhishek Dhammawat  SPR 19886     Added invite_being_sent in
 *                                               SSA context
 ...
 * ------------------------------------------------------------------------*/
   typedef union{
    ...
    icf_nw_inc_generic_msg_ind_st nw_inc_generic_msg_ind;
    icf_nw_inc_generic_msg_resp_st nw_inc_generic_msg_resp_st;
...
typedef struct icf_dns_buffer_tg{
    ...
    icf_string_st       srv_fqdn;
    icf_string_st       naptr_fqdn;
    icf_string_st       queryA_fqdn;

    /* CSR 1-6669377: SPR 19245 */
	/* Contains the TTL value received in DNS response for this IP */
	icf_uint32_t		ttl;
...
#define ICF_SSA_AUTH_IN_PROGRESS    0x40000000
...
#define ICF_MANDATORY_HDR_MISSING           0x08
#define ICF_REQ_URI_TOO_LONG                 0x10
#define ICF_SSA_OUT_OF_DIALOG_OPTIONS_CALL_WAITING    0x20
/*  This bit will be set when secondary proxy is active */
#define ICF_SEC_PROXY_SERVER_ACTIVE         0x40
/*  This bit will be set when secondary registrar is active */
#define ICF_SEC_REGISTRAR_ACTIVE            0x80
/*  This bit will be set when secondary DNS Server is active */
#define ICF_SEC_DNS_SERVER_ACTIVE           0x100
/*This bit will be set when request is routed through proxy */
#define ICF_SSA_REQ_ROUTED_THRU_PROXY       0x200
/* This bit will be set when request is routed through registrar */
#define ICF_SSA_REQ_ROUTED_THRU_REG        0x400
/*This bit will be set when inactive remote address (rem_addr_inactive) 
 * is requested*/
#define ICF_SSA_SEC_SERVER_ADDRESS_IS_USED  0x800
/* Flag to indicate if request is routed through proxy or not */
#define ICF_SSA_REQ_ROUTED_THROUGH_PROXY    0X1000
/*Start: Rel8.0 Added for precondition support */
#ifdef ICF_QOS_SUPPORT
#define ICF_SSA_PRECONDITION_NONE_PRESENT       0x4000
#define ICF_SSA_PRECONDITION_OPTIONAL_PRESENT   0x8000
#define ICF_SSA_PRECONDITION_FAIL_E2E        0x10000
#define ICF_SSA_PRECONDITION_FAIL_UNKNOWN    0x20000
#define ICF_SSA_PRECONDITION_PROV_WITHOUT_100REL 0x040000
#endif
/* End: Rel8.0 Added for precondition support */
/* SPR 18147: New bitmasks to identify the API for sending UPDATE.
 ...
 */
#define ICF_SSA_MEDIA_MODIFY_FOR_CALL_MODIFY                0x080000
#define ICF_SSA_MEDIA_MODIFY_FOR_CALL_HOLD                  0x100000
#define ICF_SSA_MEDIA_MODIFY_FOR_CALL_RESUME                0x200000
/* Bit mask to inform ssa that the timer expired is of UATK and IPTK
 * needs to take corrective action based on call back function.
 * Currently this is used to distinguish between a transaction and its
 * retransmission */
#define ICF_SSA_UATK_TIMER_TYPE_EXPIRE                        0x400000

/* Bitmask used for marking that there is an ongoing DNS query on the 
 * ssa context
 */
#define ICF_SSA_DNS_QUERY_ONGOING                             0x800000

/* this bitmask is to indicate that message body has been received in call
 * termination response from application and the same has to be inserted in
 * BYE message if challenged.
 */ 
#define ICF_SSA_MSG_BODY_RECVD_FOR_BYE                      0x01000000
/* Fix for SPR: 19999 Merged (CSR 1-7413863: SPR-19883)*/
/* This bitmask indicates whether the route header contains the 
 * registrar address.
 */
#define ICF_SSA_REGISTRAR_ADDR_PRESENT_IN_ROUTE_HEADER      0x02000000 
...
ICF_SIC_END
        ...
        /* This parameter defines the number of times 491 shall be handled
         * for session refresh request
		 */ 	
		icf_uint8_t         num_491_retries_allowed;
#endif
...
/*Following bitmask is used in case of QOS validation failure
 * for incoming resposnes*/
#define ICF_SSA_QOS_VALIDATION_FAILURE_CANCEL_CALL    0x40000
        ...
        */
        ...
        /* This variable keeps a count of SRV  records returned 
		 * in the SRV query. Removed as there will be only one SRV 
         * query at a time*/
         /* icf_uint8_t 		dns_srv_count; */

        /* Active server address (proxy/registrar) */
        icf_transport_address_st    *p_rem_addr_active;

        /* Inactive server address (proxy/registrar) */
        icf_transport_address_st    *p_rem_addr_inactive;

        /* Number of DNS Server addresses already tried */
        icf_uint8_t                 dns_servers_tried;

        /* Number of Proxy/Registrar addresses already tried */
        icf_uint8_t                 rem_servers_tried;

        /* Pointer to currently active DNS Server data structure */
        icf_dns_server_data_st      *p_current_dns_server;

        /* FQDN list received in DNS-SRV response.
         * This will be populated from Ares_cbk_uaQueryResult.
         * ICF_SSA_MAX_FQDN_ENTRIES will be be the number of 
         * FQDNs stored as per priority*/
        icf_string_st               fqdn_list[ICF_SSA_MAX_FQDN_ENTRIES];
...
/* Is an array of pointers for those pdus which have not been sent due to connection unavailability */
    ...
    /* These fields will store the auth data received from network and auth data
     * sent after processing on the basis of the challenge algo
     * received(like AKA).
     */
...
#endif
    /* buffer to store the internal message ID in case DNS query is 
     * ongoing, Scope: the message ID is set when internal message is stored
     * on ongoing DNS query and reset when the message is processed after query
     * response or call clear is triggered in case of any failures.
     */
    icf_uint16_t               internal_msg_id;

    /* pointer to store the internal message of in case DNS query is ongoing
     * and another message is issued on the same context.
     * Scope: the internal message payload is allocated when internal message is stored
     * on ongoing DNS query and reset when the message is processed after query
     * response or call clear is triggered in case of any failures. 
     */
      icf_void_t  *p_dns_query_internal_msg;
      
      /* Fix for SPR:19593*/
      /* This variable is used to store the dns event context pointer in ssa
         conetxt. We are using this variable only for validating the response
         received from DNS CLIENT. Never used this variable for updating or
         reterving the any value for DNS event context.*/
      icf_void_t  *p_dns_event_context;
    /* CSR_1_7113139 Merged SPR 19672 Start */
    /* SDP o-line version number received in last SDP from peer */
    icf_uint32_t        remote_sdp_o_version;

    /* Boolean that indicates whether peer has changed the o-line version
     * in SDP as compared to the previous value it had sent in last SDP.
     * If this flag is TRUE, it indicats peer has sent same version in
     * o-line and this is a session refresh request with SDP from peer.
     */
    icf_boolean_t		session_refresh_sdp_with_same_version;
    /* CSR_1_7113139 Merged SPR 19672 End */
    /*SPR 19886 Fix for CSR 1-7208970 This field shall be used to
     know that we are in process of sending INVITE, default shall be ICF_FALSE*/
    icf_boolean_t       invite_being_sent;

    /*Fix for SPR: 19999 (Merged CSR 1-7586127 : SPR-19988)*/
    /* pointer to store the concatenated value of sentby and branch parameter
     * in via header.
     */ 
    icf_uint8_t			 *p_via_hash_key;    

    /* Fix for SPR: 19999(Merged CSR 1-7605218: SPR-19997 fixed)*/
    icf_address_st      req_uri;
...
#endif
    /*This pointer is used in case of error message incomplete during decode
      of network message.Memory allocated to the pointer if error code is
      of type message incomplete and it will free when either the connection
      is closed or decode is successful*/

    icf_int8_t          *p_pending_pdu;

   /*This variable contains the length of data in p_pending_pdu buffer*/
    icf_uint16_t          pending_pdu_length;
    /*Fix for CSR 7694021 - SPR 20007*/
    /*New Boolean conn_not_to_close is added to track whether
      the connection needs to be closed or not on UATK callback trigger
     */

    icf_boolean_t 	conn_not_to_close;
...
#define ICF_SSA_USE_REM_SERVER_INACTIVE            0x04000000

/* CSR 1-5385147 Merge */
#define ICF_SSA_STATS_ALREADY_UPDATED              0x08000000

/*This bit will be set when Callobject reference is taken in SSA PDB
 *this will happen when SSA tries to send request on secondary server*/
#define ICF_SSA_DEALLOCATE_CALL_OBJECT              0x10000000
        ...
        /*Temporary store call context pointer, which will be used in sending
         *indication to CC in case of DNS failure*/
        icf_call_ctx_st              *p_call_ctxt;

        /*Temporary store SIC context pointer, which will be used in sending
         *indication to SIC in case of DNS failure*/
        icf_sic_ctx_st               *p_sic_ctxt;
	...
	icf_uint32_t	hash_index;
     ...
     /*Start: Rel8.0 Added for precondition support */
     /*To identify precondition is present in require hdr of incoming
       INVITE/RE-INVITE */
     icf_boolean_t    precondition_in_require_hdr;
    
     /*To identify precondition is present in supported hdr of incoming
       INVITE */
     icf_boolean_t    precondition_in_supp_hdr;
     /*End: Rel8.0 Added for precondition support */

     icf_uint8_t      rem_srv_cnt;

     /*Store Context pointer of call context,regm context or sic context*/
     icf_void_t       *p_app_ctxt;
     
     /*Pointer to store the invalid assoc uri received in the 200 ok response 
      *to register request.This invalid assoc uri is send to application in 
      *report_trace_ind*/ 
     icf_uint8_t      *p_invalid_assoc_uri;
     /* 
      * The boolean would identify if the message body needs to be 
      * copied into the message formed with authentication parameters
      * Default value would be true.
      */
      icf_boolean_t    copy_msg_body_from_persitent_data;

      /* 
       * Identifies the failure reason code that should be sent to
       * other modules in case of failure responses.
       */ 
        icf_uint16_t      reason_code_1;
  
      icf_boolean_t    auth_params_pres_in_chal;
      
      /*Reference to call object which should be freed in SSA return path
       *as primary server is not responding and secondary server is tried
       *with new call object*/
      Sdf_st_callObject   *p_call_obj_to_dealloc;

      /*parameter to store m-line information. This is set with call type
       *if specific stream received is with same transport type as configured
       *by application as preferred media transport type*/
      icf_call_type_t     mline_rcvd_w_app_profile;

      /*This boolean will be set in sdf_cbk_uaNewCallReceived and will be
        during SDP parsing in SSA to indicate that we are handling the
        incoming INVITE.*/
      icf_boolean_t    qos_call_check;
      icf_boolean_t    prack_reqd_for_qos;
      /*SPR 18630: qos_update_succ_resp_recv will be set to TRUE
        when 200OK for QOS UPDATE is received from remote */
      icf_boolean_t    qos_update_succ_resp_recv;
      
      /* CSR_1_6763883 Merged SPR 19672 Start */
#ifdef ICF_ERROR_ENABLE
     /* Contains the error string to be sent to APP */
	icf_uint8_t			err_str[ICF_MAX_STR_LEN];
#endif
