 ...
 * 17-Jan-2008  Abhishek Dhammawat ICF Rel8.0   Modifications for precondition
 *                                              in icf_cc_00_pa_setup_req
 *                                              function
 * 18-Jan-2008  Kamini Gangwani  Rel 8.0        Copy the media_transport_type in call context.
 ...
 * 1-Apr-2008   Sumant Gupta     IPTK Rel 8.0   Rel 7.1 Windows Porting Merging
 ...
 * 16-Jun-2008  Tarun Gupta       SPR 18585     Modified icf_cc_00_pa_setup_req
 * 27-Jun-2008  Amit Sharma       SPR 18734     Call released when user not configured on line
 * 01-Dec-2008  Rajiv Kumar       SPR 19215     CSR-1-6212448 Merged
 * 09-Feb-2009  Tarun Gupta       Rel 8.1 LLD   SDP Tunnelling Enhancements
 * 13-Feb-2009  Ashutosh Mohan    Rel8.1        Handling for 415/488 added
 * 17-Feb-2009  Tarun Gupta       Rel 8.1 LLD   SDP Tunnelling Enhancements:
 *                                              Modified icf_cc_00_ssa_setup_req
 * 04-Mar-2009  Tarun Gupta       Rel 8.1       Modified icf_cc_00_ssa_setup_req
 * 14-Mar-2009  Abhishek Dhammawat Rel8.1       Modifications for 415/488
 *                                              in icf_cc_00_ssa_setup_req
 * 15-Mar-2009  Anuradha Gupta    Rel 8.1       Modified icf_cc_00_pa_setup_req for session timer
 * 17-Mar-2009 Saurabh Gupta     SPR 19349      Klocworks fix
 * 27-Mar-2009 Ashutosh Mohan    Rel 8.2        Modified icf_cc_00_pa_setup_req
 * 30-Mar-2009  Anurag Khare      Rel 8.2       changes done for PRACK Enh
 * 30-Mar-2009  Anuradha Gupta    Rel 8.2       Emergency Call Enh
 * 21-May-2009  Kamal Ashraf      SPR 19672     CSR_1_7116253 Merged
 * 26-May-2009  Anuradha Gupta    SPR 19672     CSR 1-6727350 Merged
 * 08-Jun-2009  Kamal Ashraf      SPR 19590     Modified icf_cc_00_pa_setup_req 
 * 10-Jun-2009  Abhishek Dhammawat  SPR 19590   Modified icf_cc_00_pa_setup_req 
 * 11-Jun-2009 Ashutosh Mohan     SPR 19737     Changes done for GCC 4.1 warnings removal
 * 19-Jun-2009  Abhishek Dhammawat  SPR 19590   Modified icf_cc_00_pa_setup_req 
 ...
 * 30-Oct-2009  Anuradha Gupta    SPR 20202     Modified icf_cc_00_ssa_setup_req
 *
        ...
        INOUT icf_cc_pdb_st *p_cc_pdb)
{
    ...
    icf_boolean_t       request_precondition_reqd = ICF_FALSE;
    ...
    else if(ICF_NULL == p_cc_pdb->p_config_data)
    {
        ICF_FUNCTION_EXIT(p_cc_pdb->p_glb_pdb)
        return ICF_FAILURE;
    }
			...
			p_cc_pdb->p_call_ctx->replace_initiated_call_id)
	{
        ...
        if(ICF_NULL != p_cc_pdb->p_call_ctx->p_stream_sdp_info)
        {
            ...
            /* Now freeing the memory for the tunnelled SDP */
            if((ICF_NULL != p_stream_sdp) &&
                    (ICF_NULL != p_stream_sdp->p_remote_received_sdp) &&
                    (ICF_NULL != p_stream_sdp->p_remote_received_sdp->p_tunnelled_sdp))
            {
                ICF_MEMFREE(p_cc_pdb->p_glb_pdb,
                        p_stream_sdp->p_remote_received_sdp->p_tunnelled_sdp,
                        ICF_MEM_COMMON,
                        p_cc_pdb->p_ecode,
                        ret_val)

                    p_stream_sdp->p_remote_received_sdp->p_tunnelled_sdp = ICF_NULL;    
            }
        }
	}
    ...
    {
       icf_pa_cc_create_call_req_st *p_pa_create_call_req = ICF_NULL;
       p_pa_create_call_req =  (icf_pa_cc_create_call_req_st *) \
       ...
       if(ICF_HDR_LIST_PRESENT &
                      (p_pa_create_call_req->bit_mask))
       {    
            p_temp_hdr_list = &(p_pa_create_call_req->header_list);
       }
       if(ICF_BODY_LIST_PRESENT &
                      (p_pa_create_call_req->bit_mask))
       {    
            p_temp_body_list = &(p_pa_create_call_req->body_list);
       }
       if(ICF_TAG_LIST_PRESENT &
                      (p_pa_create_call_req->bit_mask))
       {    
            p_temp_tag_list = &(p_pa_create_call_req->tag_list);
       }
       /*SPR 19590 code changes end */


   /*Rel8.1 redirect_support_reqd is being set in the call context for future use when
        *3xx response is received from the network. If redirect_support_reqd is set to TRUE 
        *IPTK shall support all the 3xx responses, apart from 380, as 302 (IPTK automatically
        *initiate new INVITE), if set to FALSE IPTK shall give final failure 
        *indication (in ICF_TERMINATE_CALL_IND API) to application and clear the call*/
 
       if(ICF_CREATE_CALL_REDIRECT_SUPPORT_REQD == 
               (ICF_CREATE_CALL_REDIRECT_SUPPORT_REQD & request_bitmask))
        {
            p_cc_pdb->p_call_ctx->redirect_support_reqd = ((icf_pa_cc_create_call_req_st *) \
               (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->redirect_support_reqd;
        }

/*Start: Rel8.0 Added for precondition support */
#ifdef ICF_QOS_SUPPORT       
       if(ICF_PA_CC_CREATE_CALL_REQ_PRECONDITION_REQD &
				request_bitmask)
       {   
          request_precondition_reqd = ((icf_pa_cc_create_call_req_st *) \
           (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->precondition_required;
       }   
#endif     
/*End: Rel8.0 Added for precondition support */
       ...
       if(ICF_CREATE_CALL_TUNNEL_MODE_OPERATION_PRESENT ==
               (ICF_CREATE_CALL_TUNNEL_MODE_OPERATION_PRESENT & request_bitmask))
       {
           p_cc_pdb->p_call_ctx->tunnel_mode = ((icf_pa_cc_create_call_req_st *) \
               (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->tunnel_mode;
       }
       /* end of determining tunnel mode */
       /* Rel 8.2 changes start */
       /* If prack value is received then store it in call ctx */
       if(ICF_CREATE_CALL_PRACK_VALUE_PRESENT  == 
             (((icf_pa_cc_create_call_req_st *) \
             (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->bit_mask &
               ICF_CREATE_CALL_PRACK_VALUE_PRESENT))
      {
           /* If precondition required is true and prack
            * value is ICF_NO_PRACK then reject it and clear
            * the call.
            */
           if((ICF_NO_PRACK  == 
                  ((icf_pa_cc_create_call_req_st *) \
                  (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->
                         prack_value) && 
              (ICF_TRUE == request_precondition_reqd))
           {
               ...
               ICF_CC_SET_RELEASE_CAUSE(p_cc_pdb,
                   ICF_CALL_CLEAR_REASON_PRECONDITION_CALL_NOT_ALLOWED)
      ...
      /*  Rel 8.2 changes End */
       ...
       /* Determining if it is an Emergency Call in the call context. */
       if(ICF_CREATE_CALL_SETTINGS_PRESENT ==
               (ICF_CREATE_CALL_SETTINGS_PRESENT & request_bitmask))
       {
           if(ICF_EMERGENCY_CALL & (((icf_pa_cc_create_call_req_st *) \
                (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->call_settings))
           {
               ICF_CC_SET_COMMON_BITMASK_2(p_cc_pdb,ICF_CALL_TYPE_EMERGENCY)
           }
       }


       /* In tunnel mode do not read the srtp fallback setting field */
ICF_CHECK_TUNNEL_MODE_START(p_cc_pdb->p_glb_pdb,ICF_FALSE)
        ...
        *create call request otherwise copy it from global config data*/
       if (ICF_SRTP_FALLBACK_FLAG_PRESENT & request_bitmask)
       {
           p_cc_pdb->p_call_ctx->srtp_fallback_reqd = ((icf_pa_cc_create_call_req_st*)
               (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->srtp_fallback_reqd;
       }
       else
       {
           p_cc_pdb->p_call_ctx->srtp_fallback_reqd = p_cfg_data->srtp_fallback_reqd;
       }
ICF_CHECK_TUNNEL_MODE_END
    ...
    else
    {
       ...
       if(ICF_CC_EXTERNAL_CONFERENCE_CALL &
            p_cc_pdb->p_call_ctx->common_bitmask_2)
       {
         icf_pa_cc_create_ext_xconf_req_st  *p_pa_create_xconf_req = ICF_NULL;
         p_pa_create_xconf_req =   (icf_pa_cc_create_ext_xconf_req_st *) \
           (p_cc_pdb->p_glb_pdb->p_recv_msg->payload);
         ...
         request_toggle_pres_ind =((icf_pa_cc_create_ext_xconf_req_st *) \
          ...
          /*SPR 19590 code changes start */
         if(ICF_HDR_LIST_PRESENT &
                      (p_pa_create_xconf_req->bitmask))
         {    
            p_temp_hdr_list = &(p_pa_create_xconf_req->header_list);
         }
         if(ICF_TAG_LIST_PRESENT &
                      (p_pa_create_xconf_req->bitmask))
         {    
            p_temp_tag_list = &(p_pa_create_xconf_req->tag_list);
         }
         /*SPR 19590 code changes end */
/*Start: Rel8.0 Added for precondition support */
#ifdef ICF_QOS_SUPPORT       
         if(ICF_CREATE_XCONF_PRECONDITION_REQD_PRESENT & request_bitmask)
         {   
            request_precondition_reqd =((icf_pa_cc_create_ext_xconf_req_st *) \
           (p_cc_pdb->p_glb_pdb->p_recv_msg->payload))->precondition_required; 
         }   
#endif     
/*End: Rel8.0 Added for precondition support */       
     ...
     /*If no user is configured on line even wildcard user
     ...
     /*SPR 13978 Aman 03-01-07*/
     else if(ICF_FAILURE == icf_cmn_validate_open_nw_server(p_cc_pdb->p_glb_pdb,line_id))
...
ICF_CHECK_TUNNEL_MODE_START(p_cc_pdb->p_glb_pdb,ICF_TRUE)
    /* In tunnel mode, we are not using the preferred media
       profile. Thus freeing its memory.
    */
...
ICF_CHECK_TUNNEL_MODE_END
	...
	if( ICF_FAILURE == icf_cmn_validate_sip_addr(
        ...
        if(ICF_SUCCESS == ret_val)
        {
            ...
            /* For User Param Support, if ICF_REQUEST_URI_PARAM_PRESENT is
             * set, copy the request_uri_param to p_req_uri_param
             * in call context */
            if(ICF_REQUEST_URI_PARAM_PRESENT & p_call_setup->bit_mask)
            {
                    ...
                    {
                        icf_port_strcpy(
                                    p_cc_pdb->p_call_ctx->p_req_uri_param->str,
                                    p_call_setup->request_uri_param.str);
                        p_cc_pdb->p_call_ctx->p_req_uri_param->str_len = 
                        icf_port_strlen(
                                p_cc_pdb->p_call_ctx->p_req_uri_param->str);
                    }
            }
...
/*Start: Rel8.0 Modified for precondition support */
#ifdef ICF_QOS_SUPPORT
		if (ICF_SUCCESS == ret_val)
		{
			/* check if user has requested precondition support in 
               create_call request then set bitmask for precondition
               For outgoing call system wide flag will not be used only
               per call flag has the precendence.*/ 
			   if(ICF_TRUE == request_precondition_reqd)
        		{
					/* set the bitmask in call context */
					ICF_CC_SET_COMMON_BITMASK_2(p_cc_pdb,ICF_PRECONDITION_REQD)
                    ...
                    {
                      /* When setting the bitmask for the precondition
                         required also set prack_reqd in the configuration*/
...
#endif
/*End: Rel8.0 Modified for precondition support */
    }
     ...
     if(ICF_CREATE_CALL_SESSION_TIMER_ENABLE_PRESENT ==
               (ICF_CREATE_CALL_SESSION_TIMER_ENABLE_PRESENT & request_bitmask))
     {
         p_cc_pdb->p_call_ctx->session_timer_enable = ((icf_pa_cc_create_call_req_st *) \
...
ICF_SESSION_CONTROL_END
            ...
            /* In tunnel mode, we will not read the media transport type. */
ICF_CHECK_TUNNEL_MODE_START(p_cc_pdb->p_glb_pdb,ICF_FALSE)

            /*Set the media transport type in call context.*/
             if(ICF_MEDIA_TYPE_REQD_FLAG_PRESENT &
                                        p_call_setup->bit_mask)
             {
                  p_cc_pdb->p_call_ctx->media_transport_type =
                                     p_call_setup->media_transport_type; 
             }
ICF_CHECK_TUNNEL_MODE_END
...
ICF_CHECK_IMS_START(p_cc_pdb->p_glb_pdb,ICF_TRUE)
			...
			if (ICF_PREF_IDENTITY_PRESENT & p_call_setup->bit_mask)
            {
                ...
                /* Fix For SPR:20106*/    
                /* While validating the preferred identity we will now search
                 * public uri filled (one configured by application using
                 * ICF_SET_SELF_ID  API) along with assoc uri list (one received
                 * in assoc uri list of 200 OK response to REGISTER request.)*/	
                ret_val = icf_dbm_get_public_uri_node_from_uri_for_line(
                        p_cc_pdb->p_glb_pdb,
                        p_call_setup->line_id,
                ...
                /* Fix For SPR:20106*/
                /* For the IPTK release 8.2 we are not mandating the registration of
                 * URI filled by application in ICF_CREATE_CALL_REQ API ie for
                 * preferred identity and calling party address in IMS mode.If
                 * user not found in PUBLIC and ASSOCIATED URIS rejetct the
                 * request with release cause as ICF_CALL_CLEAR_REASON_USER_NOT_REGISTERED*/
                if (ICF_FAILURE == ret_val)
...
ICF_SESSION_CONTROL_END
	    	...
	    	if(ICF_CREATE_CALL_SETUP_TIMER_PRESENT & p_call_setup->bit_mask)
		    {
    			p_cc_pdb->setup_timer = p_call_setup->setup_timer;
	    	} 
        }
    }
	else if(ICF_TRUE == icf_cmn_check_if_param_present(
	...
	if (ICF_SUCCESS == ret_val)
    {
         ...
         /*The code change is moved in generic leg
           so that both create call and create xconf have lists
           populated*/
         if((ICF_NULL != p_temp_hdr_list) ||
            (ICF_NULL != p_temp_body_list) ||
            (ICF_NULL != p_temp_tag_list))
         {
               /*increment the txn id so that unique txn id 
                 gets associated with newly created node*/
	...
	else
	{
	   ...
	   p_cc_pdb->p_call_ctx->p_trace_criteria_data->bitmask |=
        ...
        {
            /* local sdp will not be populated in tunnel mode */
ICF_CHECK_TUNNEL_MODE_START(p_cc_pdb->p_glb_pdb,ICF_FALSE)
            ret_val = icf_cc_populate_local_sdp(p_cc_pdb);
ICF_CHECK_TUNNEL_MODE_END

           /* This function will create a single node in the call context of
              type icf_stream_sdp_info. This will be called only in tunnel
              mode.
           */
...
ICF_CHECK_TUNNEL_MODE_END
            ...
            /* CSR 1-6212448:Send create call trace to application after updating
             * the call type in call context.
             */
#ifdef ICF_TRACE_ENABLE
...
ICF_CHECK_TUNNEL_MODE_START(p_cc_pdb->p_glb_pdb,ICF_FALSE)
           ...
           /* The following code snippet is not required in tunnel mode as we do 
              not understand stream status, mode etc.
           */   
...
ICF_CHECK_TUNNEL_MODE_START(p_cc_pdb->p_glb_pdb,ICF_TRUE)
                ...
                ICF_CC_APP_TRACE(p_cc_pdb,ICF_TRACE_TUNNELLED_CMS_REQ_SENT)  
ICF_CHECK_TUNNEL_MODE_END
#endif
        ...
        INOUT icf_cc_pdb_st *p_cc_pdb)
{
    ...
    if((ICF_FAILURE != ret_val) && (ICF_TRUE != p_cc_pdb->p_call_ctx->call_waiting_call))
    {
        ...
        /* If the application has not set the field early_incoming_session_ind_reqd 
           through the API ICF_SET_CALL_PARAMS_REQ, it implies that the 
           application wants to operate in a non-tunnel mode. In such a case
           we will free the memory allocated for the tunnelled SDP in the
           remote received SDP of the call context. This memory was allocated by SSA
           module on receipt of an incoming INVITE in the function 
           sdf_cbk_uaNewCallReceived.This will be done only in the case of
           a fast start call. For a non fast start call, no memory is allocated
           by SSA for the stream SDP info structure.
         */
        if((ICF_FAILURE != ret_val) &&
           (ICF_NULL != ICF_CC_CHECK_COMMON_BITMASK(p_cc_pdb,
								   ICF_CALL_MEDIA_SETUP_TYPE)) &&
           (ICF_FALSE == p_config_data->early_incoming_session_ind_reqd))
        {
            icf_internal_list_st	*p_stream_sdp_node = ICF_NULL;
	        icf_stream_sdp_info_st	*p_stream_sdp = ICF_NULL;
            ...
            {
                /* set ret_val to failure so that following code 
                   snippet is not excecuted.
                */   
                ret_val = ICF_FAILURE;
                /* If any of the above pointers are NULL, we have to 
                   clear the call and send a 5xx response to the peer.
                */   
                ICF_CC_SET_RELEASE_CAUSE(p_cc_pdb,
...
/*Change Start Rel 8.1 415/488 handling*/
        /* If application does not set the early_session_ind_required field in
           set call params req API, indicating non tunnel mode operation then 
           IPTK will send 415/488 response to the network.*/
        if((ICF_FAILURE != ret_val) &&
           (ICF_FALSE == p_config_data->early_incoming_session_ind_reqd) && 
           (p_cc_pdb->p_call_ctx->common_bitmask_2 & ICF_FAILURE_RESPONSE_CODE_PRESENT))
        {
            ...
            /* SRP 20202: In case the reject response is sent then the
             * execution should return from here and no 100 trying should 
             * be attempted as it will for sure fail.
             */
             return ret_val;
        }
/*Change End Rel 8.1 415/488 handling*/
        ...
        /* If application has configured to receive early call indication, 
           send the same to the application. This flag will over-ride the
           flag early_inc_ind_for_all_users and is applicable in tunnel
           mode only.
        */
        else if((ICF_TRUE == p_config_data->early_incoming_session_ind_reqd) ||
    ...
    /* Check for early incoming call indication is made while populating the
     * session timer data in call context.Here we first get the line data
     * based on line id in call context .From the line and config data we populate the
     * session timer data in call context .However in case of  early incoming
     * call indication the line id at this point will be invalid and thus the
     * function retrun failure from here .The population of session timer data
     * in case when early incoming call indication is TRUE is done in function 
     * icf_cc_20_early_ind_cfm ,when early call cfm is received from
     * application .*/
    
    if ((ret_val != ICF_FAILURE) && \
        (ICF_TRUE == p_cc_pdb->p_call_ctx->session_timer_enable) &&
        (ICF_TRUE != early_indication))
    {
                 if (ICF_NULL != p_session_timer_data)
                 {
    ...
    if(ICF_SUCCESS == ret_val)
    {
        ...
        /* The following code snippet is not required in tunnel
           mode.At this moment, tunnel mode is determined by the flag
           early_incoming_session_ind_reqd in the config data set 
           through API ICF_SET_CALL_PARAMS_REQ.
        */
        if( (ICF_NULL != p_config_data) && (ICF_FALSE == p_config_data->early_incoming_session_ind_reqd))
        {
        ...
        INOUT icf_cc_pdb_st *p_cc_pdb)
{
    ...
    /* Fix for SPR 19995 
     * If MM interface is not yet cleared, send DMS request
     * to MM. This will be forceful DMS since call context
     * is anyways being deallocated below so we need not 
     * wait for DMS response.
     */
    if (ICF_NULL == ICF_CC_CHECK_CALL_CLR_STATUS(p_cc_pdb,
                ICF_CALL_CLEAR_STATUS_MM_CLEARED))
    {
    ...
    /*Call decrease_call_waiting and detach function only 
     *when release_cause is LINE_NOT_CONFIGURED, as this 
     *is set when IPTK could not fetch line_data for given
     *line and both function fetches line_data*/
    if (ICF_CALL_CLEAR_REASON_LINE_NOT_CONFIGURED !=
