 ...
 * 24-Jan-2008  Tarun Gupta     IPTK Rel 8.0 LLD Added constants for AKA.
 * 26-Jun-2008  Amit Sharma     SPR:18725   Define warning header codes
 * 04-Nov-2008  Tarun Gupta     SPR 19189   Merged Rel 7.1.2 SPR 18880
 * 09-Feb-2009  Anurag Khare    ICF Rel 8.1 Enhanced for the new state 
 *                                          ICF_SIC_04_AWT_EARLY_SESSION_CFM
 * 28-May-2009  Tarun Gupta     Rel 8.2     Async NOTIFY Support
 * 31-May-2009  Preksha         Rel 8.2     Asynchronus Message Enhancements
 * 14-Jun-2009 Abhishek Dhammawat SPR19590 modified ICF_CMN_MAX_API_LENGTH
 * 14-Jul-2009 Abhishek Dhammawat SPR19785 added ICF_CMN_MAX_TRACE_LIST_NODES
 * 18-Jul-2009 Rajiv Kumar      Rel 8.2     Vxwork 5.4 Porting
 * 03-Sep-2009 Abhishek Dhammawat SPR20016 modified ICF_CMN_MAX_API_LENGTH
 * 08-Oct-2009 Abhishek Dhammawat SPR20063 Merged CSR 1-7407479
 *
...
/* If any new timer type is defined, pls also update the 
   corresponding Timer Validation code in the function 
   icf_cc_is_valid_timer_blk
*/     
/* SPR 18880 */
#define ICF_TIMER_TYPE_INVALID       0xff
/* SPR 18880 End */
...
/* New timer type for support of asynchronous NOTIFY */
#define ICF_TIMER_TYPE_NOTIFY_CFM    26    

/* ICF_6_1_OPTIONS_SUPPORT end */

/* ASYNCHRONUS_MESSAGE TIMER*/
#define ICF_SIC_MESSAGE_TIMER 27
...
typedef enum
{
    ...
    /* New state 05 for support of asynchronous NOTIFY */
 ...
 * Enum enhanced for having a new state ICF_SIC_04_AWT_EARLY_SESSION_CFM
 *
 *---------------------------------------------------------------------------*/
typedef enum
{
...
/*----------------------------------------------------------------------------
 *
 * Enum for the MESSAGE fsm states
 * Support for ASYNCHRONUS_MESSAGE 
 *---------------------------------------------------------------------------*/
typedef enum
{
	ICF_SIC_00_MESSAGE_IDLE=0,
	ICF_SIC_01_AWAIT_MESSAGE_CFM,
	ICF_SIC_MAX_MESSAGE_STATES
} icf_sic_message_states_et;
...
/**************ICF 8.0*************/
/*Subscribe duration for the reg event package*/
#define ICF_REG_EVENT_SUBS_DURATION             600000
...
/*SPR 19590 increased the size for long header issue.
 To avoid IPTK crash when sending in trace the large number
 of long headers*/
/*SPR 20016 For avoiding crash if the single body comes of large
  size (20K) we have increased the trace buffer to 30000*/
#define ICF_CMN_MAX_API_LENGTH      30000

/*SPR 19785 we shall be communicating only limited headers
  tags, body, URI and other list nodes as per the below
  define ICF_CMN_MAX_TRACE_LIST_NODES. This is to avoid 
  exceeding the array limits*/
#define ICF_CMN_MAX_TRACE_LIST_NODES      10
#endif

/* These are to indicate the challenge algo received from the network.
 * Accordingly the data type icf_challenge_algo_t is assigned and is stored
 * in regm_context_st/ssa_ctxt_st.
 */
#define ICF_CHALLENGE_ALGO_INVALID 0
#define ICF_CHALLENGE_ALGO_MD5     1
#define ICF_CHALLENGE_ALGO_AKA     2

/*SPR 19590 header list overwrite enh*/
#define ICF_INVALID_TXN_ID 0

/*-----------------------------------------------------------------------------
 * 
 *  Following values are defined to send Warning header in 488 response for
 *  an INVITE received and MM can not establish channel.
 *
 *---------------------------------------------------------------------------*/
/*One or more network protocols contained in the session 
 *description are not available*/
#define ICF_WARNING_CODE_INCOMPATIBLE_NW_PROTOCOL            300

/*One or more network address formats contained in the session 
 *description are not available*/
#define ICF_WARNING_CODE_INCOMPATIBLE_NW_ADDR_FMT            301

/*One or more transport protocols described in the session 
 *description are not available*/
#define ICF_WARNING_CODE_INCOMPATIBLE_TRANSPORT_PROTOCOL     302

/*One or more bandwidth measurement units contained in the 
 *session description were not understood*/
#define ICF_WARNING_CODE_INCOMPATIBLE_BW_UNITS               303

/*One or more media types contained in the session 
 *description are not available*/
#define ICF_WARNING_CODE_MEDIA_TYPE_NOT_AVAILABLE            304

/*One or more media formats contained in the session 
 *description are not available*/
#define ICF_WARNING_CODE_INCOMPATIBLE_MEDIA_FMT              305

/*One or more of the media attributes in the session 
 *description are not supported*/
#define ICF_WARNING_CODE_INCOMPATIBLE_ATTRIBUTE              306

/*A parameter other than those listed above was not understood*/
#define ICF_WARNING_CODE_INCOMPATIBLE_SDP                    307

/*The site where the user is located does not support multicast*/
#define ICF_WARNING_CODE_MULTICAST_NOT_AVAILABLE             330

/*The site where the user is located does not support unicast 
 *communication (usually due to the presence of a firewall)*/
#define ICF_WARNING_CODE_UNICAST_NOT_AVAILABLE               331

/*The bandwidth specified in the session description or defined 
 *by the media exceeds that known to be available*/
#define ICF_WARNING_CODE_INSUFFICIENT_BANDWIDTH              370

/*The warning text can include arbitrary information to be 
 *presented to a human user or logged.  A system receiving this 
 *warning MUST NOT take any automated action*/
#define ICF_WARNING_CODE_MISCELLANEOUS                       399
