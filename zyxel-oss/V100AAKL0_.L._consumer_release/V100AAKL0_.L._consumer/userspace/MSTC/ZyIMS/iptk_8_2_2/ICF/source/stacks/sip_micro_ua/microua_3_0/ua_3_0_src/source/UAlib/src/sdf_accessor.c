 ...
 ** 22/06/09   Abhishek Dhammawat SPR 19769      Modified
 ...
 ** 09-Sep-2009 Anuradha Gupta    SPR 19999      Merged CSR 1-7432250
 **
 ******************************************************************************
...
/* klocworks: At Memory allocation locations, checks have been added so that in 
 * case of memory allocation failure, execution do not proceed further and return
 * with error cause as NoMemoryError.
 */
...
#endif
{
    ...
    pDestInfo->dReqUriPort = pSrcInfo->dReqUriPort;
...
#endif	
{
	...
	if (Sdf_mc_strstr(pTemp, ":") != Sdf_co_null)
	{
		...
		if(Sdf_co_null == pFinal)
		{
#ifdef SDF_ERROR
		sdf_fn_setError(Sdf_en_majorErrors,Sdf_en_noMemoryError, \
			(Sdf_ty_s8bit *)"sdf_ivk_uaAddheader():  "
			"memeory allocation failure",pErr);
#endif			
			pErr->errCode = Sdf_en_noMemoryError;
			return Sdf_co_fail;
		}
...
#endif
{
	...
	if(Sdf_co_null != pParam)
	{
		...
		(void)sip_listInsertAt(&(((SipReplacesHeader *)
					(pReplacesHeader->pHeader))->slParam),dcount,
							   pParam,(SipError *)&(pError->stkErrCode));
	}
	...
	*ppUrlString = Sdf_co_null;
	if (Sdf_co_null != pReplacesHdr)
	{
	   ...
	   if (Sdf_co_null != pReplacesStr)	
	   {
		...
		if(SipFail == sip_formSipParamList(/* CSR_1-6613911 Change Start*/SIP_NULL,
                    /* CSR_1-6613911 Change End */pReplacesStr,&(pRequestHdr \
...
#endif
{
    ...
    /* Fix for IPTK CSR 1-7329950: UATK CSR is 1-7332012 */
    Sdf_ty_u32bit dIndex=0, dLength = 0; 
    Sdf_ty_s8bit  *pTempStr2 = Sdf_co_null , offset = 0; 
	...
	*ppReplacesHdr = Sdf_co_null;
	...
	if( pSipUrl != Sdf_co_null )
	{
        ...
        {
            /* Fix for SPR:19886 (Merged CSR-1-7329950)*/
            /* Fix for CSR 1-7329950: UATK CSR 1-7332012 */
            dLength = Sdf_mc_strlen(pTempStr); 
            pTempStr2 = (Sdf_ty_s8bit  *)sdf_memget(0, (dLength+1), pErr); 
            if(pTempStr2 == Sdf_co_null) 
            { 
                pErr->errCode = Sdf_en_noMemoryError; 
#ifdef SDF_ERROR                   
                sdf_fn_setError(Sdf_en_majorErrors,pErr->errCode,
                        (Sdf_ty_s8bit*)"sdf_ivk_uaGetReplacesHeaderFromReferTo(): " 
                        "Failed to allocate memory",pErr); 
#endif              
                return Sdf_co_fail; 
            } 
            for (dIndex = 0; dIndex < dLength ; dIndex++) 
            {
                pTempStr2[dIndex] = Sdf_mc_toupper(pTempStr[dIndex]); 
            }
            pTempStr2[dIndex] = '\0';

            /* Change done for CSR 1-7432250 : UATK CSR 1-7754493*/ 
            pReplacesHdrStr = Sdf_mc_strstr(pTempStr2,"REPLACES=");          
        }
        ...
        {
            offset = pReplacesHdrStr - pTempStr2;
            (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTempStr2,pErr); 
            pReplacesHdrStr=pTempStr + offset;             

            /* Change done for CSR 1-7432250 : UATK CSR 1-7754493*/ 
            (void)Sdf_mc_strtok(pReplacesHdrStr, "&");
