...
/*------------------------------------------------------------------------------
 *
 * Macro Name       : ICF_RGM_INITIALIZE_REG_RESP_STRUCT
 * Description      : Macro for declaring and initializing icf_rgm_register_resp_st
 *
 *----------------------------------------------------------------------------*/
#define ICF_RGM_INITIALIZE_REG_RESP_STRUCT(register_response)                                \
{                                                                             \
    register_response.p_rgm_context = ICF_NULL; \
    register_response.p_crm_pdb = ICF_NULL; \
    register_response.app_id = 0; \
    register_response.p_request_payload = ICF_NULL;\
    register_response.options_flag = 0; \
    register_response.status = 0; \
    register_response.error_cause = 0; \
    register_response.request_type = 0; \
    register_response.response_code = 0; \
    register_response.line_id = ICF_INVALID_LINE_ID; \
}
