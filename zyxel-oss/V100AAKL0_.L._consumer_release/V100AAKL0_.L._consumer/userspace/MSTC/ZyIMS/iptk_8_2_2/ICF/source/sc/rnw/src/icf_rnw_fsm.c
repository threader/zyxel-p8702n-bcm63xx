        ...
        INOUT  icf_rnw_pdb_st    *p_pdb)
{
    ...
    if (ICF_FALSE == icf_rnw_validate_incoming_refer(p_pdb))
    {
        ...
        if (ICF_NULL != p_refer_req->p_replaces_hdr)
        {
            /*Free p_replaces_hdr in refer request after sending the response*/
            ICF_MEMFREE(p_glb_pdb, p_refer_req->p_replaces_hdr,
                    ICF_MEM_COMMON,
                    p_ecode, ret_val)
        }        
    ...
    else if (ICF_FAILURE == icf_rnw_fill_call_ctx(p_pdb, p_call_ctx_orig))
    {
        /*Klocwork warning removal*/
         p_glb_pdb->p_call_ctx = p_call_ctx_orig;
        /*Deallocate all the memory allocated for p_call_ctx*/
        ret_val = icf_rnw_clear_call_ctx(p_pdb, p_call_ctx);
        /*  CSR 1-6017450 Merge */
        /*  Now new Invite will not be send so the service
            should be ended normally */
        ICF_RNW_SET_STATUS(p_pdb, ICF_STATUS_SERVICE_ENDED_NORMAL)
    ...
    if (ICF_FAILURE == ret_val)
    {
        ...
        /*  CSR 1=6017450 Merge */
        /*  If ICF_STATUS_SERVICE_ENDED_NORMAL is set the return Success,
            so that SCM in the return path clear the service. */
        if(ICF_RNW_CHECK_STATUS(p_pdb, ICF_STATUS_SERVICE_ENDED_NORMAL))
        {
            ret_val = ICF_SUCCESS;
        }
        else
        ...
        INOUT   icf_rnw_pdb_st *p_pdb)
{
                ...
                (icf_void_t **)&p_config_data, &ecode))
    {
   	...
   	/* CSR_1_7129410 Merged SPR 19672 Start */
    if (ICF_FALSE == p_pdb->p_rnw_ctx->notify_sent)
    {
        if (ICF_NULL != 
                   (ICF_CALL_CONNECT_DONE & p_glb_pdb->p_call_ctx->common_bitmask))
        {
        ...
        INOUT  icf_rnw_pdb_st *p_pdb)
{
    ...
    /* CSR_1_7129410 Merged SPR 19672 End */
    else if (p_glb_pdb->p_call_ctx->call_id == 
            p_pdb->p_rnw_ctx->p_call_ctx_c->call_id)
    {
        ...
        /* CSR_1_7129410 Merged SPR 19672 Start */
        if (ICF_TRUE == p_config_data->conf_reqd_at_transferee && 
            ICF_FALSE == p_pdb->p_rnw_ctx->merge_media_sent)
        {
            ret_val = icf_rnw_check_med_for_merge_req(p_pdb);
        }
        /* CSR_1_7129410 Merged SPR 19672 End */
        ...
        INOUT   icf_rnw_pdb_st   *p_pdb)
{
	...
	/* CSR 1-6212448: Send Remote Transfer confirm trace to application */
#ifdef ICF_TRACE_ENABLE
...
#ifdef ICF_TRACE_ENABLE
       p_pdb->result=p_cta_init_cfm->result;
       p_pdb->error_cause=(p_pdb->p_internal_msg->msg_hdr.ecode);
       ICF_RNW_APP_TRACE(p_pdb, ICF_TRACE_REMOTE_CALL_TRANSFER_CFM)       
#endif 
