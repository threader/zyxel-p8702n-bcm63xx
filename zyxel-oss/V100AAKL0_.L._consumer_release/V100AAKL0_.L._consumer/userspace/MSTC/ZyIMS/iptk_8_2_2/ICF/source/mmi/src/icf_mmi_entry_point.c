 ...
 * 22-Apr-2008    Shruti Thakral    SPR 18455       Klocworks warning removal
 * 29-Apr-2008    Shruti Thakral    SPR 18455       Klocworks warning removal
 * 09-Feb-2009    Tarun Gupta       Rel 8.1 LLD     SDP Tunnelling Enhancements
 * 09-Feb-2009    Anurag Khare      ICF Rel 8.1     Options enhancements 
 * 17-Feb-2009    Tarun Gupta       Rel 8.1         Modified icf_mmi_process_mesg : 
        ...
        OUT     icf_error_t          *p_ecode)
{
    ...
    /* Tunnel mode specific event handler functions */
	/* Functions to handle events in State ICF_MMI_00_IDLE */

    p_mmi_data->mmi_fsm_table[ICF_MMI_00_IDLE]
        [ICF_MMI_TUNNELLED_CMS_REQ] = icf_mmi_00_tunnelled_cms_request;
   
	p_mmi_data->mmi_fsm_table[ICF_MMI_00_IDLE]
        [ICF_MMI_TUNNELLED_MMS_REQ] = icf_mmi_00_tunnelled_mms_request;
    
	p_mmi_data->mmi_fsm_table[ICF_MMI_00_IDLE]
        [ICF_MMI_TUNNELLED_DMS_REQ] = icf_mmi_xx_tunnelled_dms_request;
    
	p_mmi_data->mmi_fsm_table[ICF_MMI_00_IDLE]
        [ICF_MMI_CALL_CLEAR] = icf_mmi_call_clear;

    p_mmi_data->mmi_fsm_table[ICF_MMI_00_IDLE]
        [ICF_MMI_TUNNELLED_CMS_RESP] = icf_mmi_unexpected_event;

	p_mmi_data->mmi_fsm_table[ICF_MMI_00_IDLE]
        [ICF_MMI_TUNNELLED_MMS_RESP] = icf_mmi_unexpected_event;
    ...
    /* Tunnel mode specific event handler functions */
    /* Functions to handle events in State ICF_MMI_11_AWT_TUNNELLED_CMS_RESP */

	p_mmi_data->mmi_fsm_table[ICF_MMI_11_AWT_TUNNELLED_CMS_RESP]
		[ICF_MMI_TUNNELLED_CMS_REQ] = icf_mmi_unexpected_event;
    
	p_mmi_data->mmi_fsm_table[ICF_MMI_11_AWT_TUNNELLED_CMS_RESP]
		[ICF_MMI_TUNNELLED_MMS_REQ] = icf_mmi_unexpected_event;
    
	p_mmi_data->mmi_fsm_table[ICF_MMI_11_AWT_TUNNELLED_CMS_RESP]
		[ICF_MMI_TUNNELLED_MMS_RESP] = icf_mmi_unexpected_event;
    
	p_mmi_data->mmi_fsm_table[ICF_MMI_11_AWT_TUNNELLED_CMS_RESP]
		[ICF_MMI_TUNNELLED_CMS_RESP] = icf_mmi_11_tunnelled_cms_resp;
	...
	/* Functions to handle events in State ICF_MMI_12_AWT_TUNNELLED_MMS_RESP */
    
	p_mmi_data->mmi_fsm_table[ICF_MMI_12_AWT_TUNNELLED_MMS_RESP]
		[ICF_MMI_TUNNELLED_CMS_REQ] = icf_mmi_unexpected_event;
	...
	p_mmi_data->mmi_fsm_table[ICF_MMI_12_AWT_TUNNELLED_MMS_RESP]
		[ICF_MMI_TUNNELLED_MMS_RESP] = icf_mmi_12_tunnelled_mms_resp;
	...
	/* End tunnel mode specific handler functions */
        ...
        INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
    ...
    switch(p_mmi_pdb->p_internal_msg->msg_hdr.msg_id)
    {
        ...
        case ICF_CREATE_MEDIA_REQ:
             ...
             /* This will be mapped to a seperate event in tunnel mode of operation.*/
ICF_CHECK_TUNNEL_MODE_START(p_mmi_pdb->p_glb_pdb,ICF_TRUE)
            ICF_MMI_SET_EVENT(p_mmi_pdb,ICF_MMI_TUNNELLED_CMS_REQ)
...
ICF_CHECK_TUNNEL_MODE_START(p_mmi_pdb->p_glb_pdb,ICF_TRUE)
            ICF_MMI_SET_EVENT(p_mmi_pdb,ICF_MMI_TUNNELLED_MMS_REQ)
ICF_CHECK_TUNNEL_MODE_END
...
ICF_CHECK_TUNNEL_MODE_START(p_mmi_pdb->p_glb_pdb,ICF_TRUE)
            ICF_MMI_SET_EVENT(p_mmi_pdb,ICF_MMI_TUNNELLED_DMS_REQ)
ICF_CHECK_TUNNEL_MODE_END
...
ICF_CHECK_TUNNEL_MODE_END
        ...
        case ICF_FORCEFUL_CALL_CLEAR_IND:   
            /* We will set this event even in tunnel mode. In tunnel
               mode we will just return success from the handler 
               function.
            */   
        ...
        INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
    ...
    /* The following switch cases have been modified according to the 
       following logic:

       if non tunnel mode
          if tunnelled CMS or tunnelled MMS response
              set ecode as ICF_INVALID_API_ID_RECD
          else
              handle response as earlier

       if tunnel mode
          if any other API other than tunnelled CMS or
          tunnelled MMS response
              set ecode as ICF_INVALID_API_ID_RECD
          else
              handle tunnel mode specfic API responses
...
ICF_CHECK_TUNNEL_MODE_END                
        ...
        case ICF_NAT_STUN_PROC_STATUS_IND:
             ...
             /* Tunnelled CMS Response is recvd from MM */
        case ICF_TUNNELLED_CREATE_MEDIA_SESSION_RESP:
  
ICF_CHECK_TUNNEL_MODE_START(p_mmi_pdb->p_glb_pdb,ICF_TRUE)
             ICF_MMI_SET_EVENT(p_mmi_pdb,ICF_MMI_TUNNELLED_CMS_RESP)
ICF_CHECK_TUNNEL_MODE_END
...
ICF_CHECK_TUNNEL_MODE_END
             ...
             /* Tunnelled MMS response is recvd from MM */
        case ICF_TUNNELLED_MODIFY_MEDIA_SESSION_RESP:

ICF_CHECK_TUNNEL_MODE_START(p_mmi_pdb->p_glb_pdb,ICF_TRUE)
             ICF_MMI_SET_EVENT(p_mmi_pdb,ICF_MMI_TUNNELLED_MMS_RESP)
ICF_CHECK_TUNNEL_MODE_END
...
ICF_CHECK_TUNNEL_MODE_END
             break;
        /* End tunnel mode specific MM APIs */
        ...
        INOUT icf_internal_msg_st        *p_internal_msg)
{
    ...
    icf_boolean_t                      unexpected_msg = ICF_FALSE;
...
ICF_MM_ROUTING_END
    ...
    /*  Rel 8.1
     *  If tunnel mode is false then the API 
     *  ICF_TUNNELLED_MEDIA_CAPABILITES_RESP must be rejected 
     */
ICF_CHECK_TUNNEL_MODE_START(mmi_pdb.p_glb_pdb,ICF_FALSE)
    if((ICF_NULL != mmi_pdb.p_glb_pdb->p_recv_msg) &&
            ...
            mmi_pdb.p_glb_pdb->p_recv_msg->hdr.api_id))
    {
        ...
        unexpected_msg = ICF_TRUE;
    }
ICF_CHECK_TUNNEL_MODE_END

    /*  Rel 8.1
     *  If tunnel mode is true then the API
     *  ICF_MEDIA_CAPABILITES_RESP must be rejected 
     */
ICF_CHECK_TUNNEL_MODE_START(mmi_pdb.p_glb_pdb,ICF_TRUE)
    if((ICF_NULL != mmi_pdb.p_glb_pdb->p_recv_msg) &&
...
ICF_CHECK_TUNNEL_MODE_END
    ...
    /* Rel 8.1
     * If this was media capabilities response 
     * or ICF_tunnelled_media_capabilities_resp then
     * dont enter the FSM
     * Send media capabilities response to SIC
     */
    if(ICF_TRUE == unexpected_msg)
    {
         ICF_FUNCTION_EXIT(mmi_pdb.p_glb_pdb)
         ICF_RESET_MODULE(mmi_pdb.p_glb_pdb)
         return ret_val;
    ...
    if (ICF_NULL != mmi_pdb.p_call_ctx)
    {
	...
	/* In tunnel mode, the following code snippet is not required.
	   Whenever a response is received, MMI directly copies the local SDP 
	   received from the application to the call context.

	   Whenever a request has to be sent to the application, MMI directly
	   copies the remote received SDP from the call context.
	*/
ICF_CHECK_TUNNEL_MODE_START(mmi_pdb.p_glb_pdb,ICF_FALSE)
...
ICF_CHECK_TUNNEL_MODE_END
    ...
    /* Now invoking the MMI state machine */
    if((ICF_NULL != mmi_pdb.p_call_ctx) && (ICF_SUCCESS == ret_val))
    {
		if ( (mmi_pdb.p_call_ctx-> mmi_context.mmi_state < ICF_MMI_MAX_STATES) && 
			(mmi_pdb.p_call_ctx->mmi_context.mmi_event < ICF_MMI_MAX_EVENTS ))
			{
