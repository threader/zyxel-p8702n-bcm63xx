 ...
 * 27-May-2008  Tarun Gupta     SPR 18585           Modified icf_cc_13_start_proc_media and icf_cc_13_ssa_call_cancel_req
 * 20-Jun-2008   Jagmohan        CSR Merge 1-5945413 Modified icf_cc_13_start_proc_media
 * 08-Jun-2009   Kamal Ashraf   SPR 19590           Modified icf_cc_13_start_proc_media
 *                                                  and icf_cc_13_ssa_call_cancel_req
...
icf_return_t icf_cc_13_start_proc_media(INOUT icf_cc_pdb_st *p_cc_pdb)
{
    ...
    /* CSR Merge 1-5945413: If pending event is Call Connect then handle it
     * with specific fucntion */    
...
ICF_HANDLE_REINVITE_END
        ...
        {
            /* SPR 19590 : Extract the txn id from pending event list
               list corresponding to input bit so that we work on correct
               header list using the extracted txn-id */
            icf_cc_extract_txnid_from_pending_event_list(p_cc_pdb,
                                  ICF_PENDING_EVENT_ALERT_PROV_RESP_WITH_SDP); 
            /* SPR 19590 : End */
        ...
        /* T38 fax request from remote is a pending event */
        else if(ICF_NULL != ICF_CC_CHECK_PENDING_EVENT(p_cc_pdb,
            ICF_PENDING_EVENT_SSA_FAX_REQ))
        {
            /* SPR 19590 : Extract the txn id from pending event list
               corresponding to input bit so that we work on correct
               header list using the extracted txn-id */
            ...
            /* SPR 19590 : End */
             /* Set Offer Change as Ongoing event and remove it from pending list */
             ICF_CC_SET_ONGOING_EVENT(p_cc_pdb,ICF_CC_SSA_FAX_REQ)
    ...
    {
       if(ICF_CALL_CONNECT_DONE == ICF_CC_CHECK_COMMON_BITMASK(
...
icf_return_t icf_cc_13_ssa_call_cancel_req(INOUT icf_cc_pdb_st *p_cc_pdb)
{
    ...
    /* if the call is connected, cancel is received for reinvite transaction */
