...
* 29-Apr-2008     Alok Tiwari          SPR 18456      Klocwork warning resolved
* 11-Feb-2009     Ashutosh Mohan       IPTK Rel8.1    Added tunnelled APIs
* 10-Jul-2009     Anuradha Gupta       SPR 18948      Modified icf_encode_mm_api_payload
*
...
****************************************/
	...
	*p_encoded_buf_length = length;
        ...
        if(length >= ICF_API_HEADER_LENGTH)
        {
          *p_p_encoded_buf = (icf_uint8_t*)ICF_CLIB_MEMGET(length);
        }

	if (ICF_NULL == *p_p_encoded_buf) {
...
****************************************/
	...
	case ICF_MM_INIT_COMPLETE:
        ...
        case ICF_TUNNELLED_CREATE_MEDIA_SESSION_REQ:
                ret_val= (icf_uint8_t)icf_pack_icf_tunnelled_create_media_session_req_st(p_dest, p_src, &length);
                break;

        case ICF_TUNNELLED_CREATE_MEDIA_SESSION_RESP:
                ret_val= (icf_uint8_t)icf_pack_icf_tunnelled_create_media_session_resp_st(p_dest, p_src, &length);
                break;

        case ICF_TUNNELLED_MODIFY_MEDIA_SESSION_RESP:
                ret_val= (icf_uint8_t)icf_pack_icf_tunnelled_modify_media_session_resp_st(p_dest, p_src, &length);
                break;

        case ICF_TUNNELLED_MODIFY_MEDIA_SESSION_REQ:
                ret_val= (icf_uint8_t)icf_pack_icf_tunnelled_modify_media_session_req_st(p_dest, p_src, &length);
                break;

        case ICF_TUNNELLED_MEDIA_CAPABILITIES_RESP:
                ret_val= (icf_uint8_t)icf_pack_icf_tunnelled_media_capabilities_resp_st(p_dest, p_src, &length);
                break;
...
****************************************/
	...
	case ICF_MM_INIT_COMPLETE:
        ...
        case ICF_TUNNELLED_CREATE_MEDIA_SESSION_REQ:
                ret_val= (icf_uint8_t)icf_unpack_icf_tunnelled_create_media_session_req_st(p_dest, p_src, &length);
		break;


        case ICF_TUNNELLED_CREATE_MEDIA_SESSION_RESP:
                ret_val= (icf_uint8_t)icf_unpack_icf_tunnelled_create_media_session_resp_st(p_dest, p_src, &length);
		break;


        case ICF_TUNNELLED_MODIFY_MEDIA_SESSION_RESP:
                ret_val= (icf_uint8_t)icf_unpack_icf_tunnelled_modify_media_session_resp_st(p_dest, p_src, &length);
		break;


        case ICF_TUNNELLED_MODIFY_MEDIA_SESSION_REQ:
                ret_val= (icf_uint8_t)icf_unpack_icf_tunnelled_modify_media_session_req_st(p_dest, p_src, &length);
		break;


        case ICF_TUNNELLED_MEDIA_CAPABILITIES_RESP:
                ret_val= (icf_uint8_t)icf_unpack_icf_tunnelled_media_capabilities_resp_st(p_dest, p_src, &length);
		break;
...
****************************************/
	...
	case ICF_MM_INIT_COMPLETE:
        ...
        /*SPR 18948 : In case of MEDIA_CAPAB_RESP, instead of pack, unpack was called. the issue is corrected
         */ 
        case ICF_TUNNELLED_MEDIA_CAPABILITIES_RESP:
                ret_val= (icf_uint8_t)icf_pack_icf_tunnelled_media_capabilities_resp_st(p_dest, p_src, &length);
		break;
