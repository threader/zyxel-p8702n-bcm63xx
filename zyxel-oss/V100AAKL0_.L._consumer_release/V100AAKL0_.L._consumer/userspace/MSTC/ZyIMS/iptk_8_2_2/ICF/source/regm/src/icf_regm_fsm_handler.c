 ...
 *
 * 24-Jan-2008  Tarun Gupta    IPTK Rel 8.0 LLD     Changes done for AKA related
 *                                                  handling.
 ...
 * 27-May-2008  Tarun Gupta     SPR 18585           Modified icf_rgm_01_04_fail_no_resp
 * 9-Jul-2008 Abhishek Dhammawat SPR 18776          Modified
 *                                                  icf_rgm_01_reg_req_recvd
 *                                                  icf_rgm_04_reg_req_recvd
 * 18-Nov-2008 Rajiv Kumar       SPR 19188          Rel 7.1.2 SPR merging (SPR
 *                                                  18997) 
 * 20-Nov-2008 Rajiv Kumar       SPR 19188          Rel 7.1.2 SPR merging (SPR
 *                                                  19151)  
 * 03-Dec-2008 Kamal Ashraf      SPR 19218          CSR 1-6089701 Merge 
 * 15-Dec-2008 Anurag Khare      SPR 19218          CSR 1-6168806 Merge
 * 27-Mar-2009 Anuradha Gupta    ICF Rel 8.2        Changes for call clear trigger and
 *                                                  Reg Event Pckg Subs Handling
 * 27-Mar-2009 Ashutosh Mohan    Rel 8.2            Modified icf_rgm_03_notify_shortened
 *                                                           icf_rgm_01_04_success_resp
 *                                                           icf_rgm_01_04_failure_resp
 * 12-May-2009 Anuradha Gupta    SPR 19601          Modified icf_rgm_01_04_success_resp
 * 11-Jun-2009 Ashutosh Mohan     SPR 19737     Changes done for GCC 4.1 warnings removal
 * 19-Jun-2009 Anuradha Gupta    SPR 19746          Modified icf_rgm_01_04_fail_no_resp
 * 28-Jul-2009 Tarun Gupta       SPR 19886          Merged CSR 1-7233758   
 * 08-Sep-2009 Anuradha Gupta    SPR 19999          Merged CSR 1-7533733
 * 12-Sep-2009 Abhishek Dhammawat SPR 20041       Modified for icf_rgm_01_04_timeout
 * 16-Oct-2009 Abhishek Dhammawat SPR 20026       Modified
 *                                                icf_rgm_03_notify_terminated   
 * 28-Oct-2009 Abhishek Dhammawat SPR 20185       Modified
 *                                                icf_rgm_03_reg_req_recvd
 * 28-May-2010	Rohit Aggarwal	CSR 1-8652432	Populating 'to' header in App
 *												initiated Deregister response
 *
...
ICF_CHECK_IMS_END
    ...
    /* if reg event package is supported then IPTK will send Subscription
     * for the default public URI.
     */
     ...
     */ 
    if(ICF_TRUE == p_rgm_context->reg_event_pckg_subs_required)    
    {
	   p_reg_event_subs_ctx = p_rgm_context->p_reg_event_subs_ctx;
	   ...
	   if (ICF_NULL == p_reg_event_subs_ctx)
	   {
		  ...
		  {
		      ICF_PRINT(((icf_uint8_t *)"\nUnable to Send SUBSCRIBE request for reg event package.\n"))	 
		  }
 	   }
    }

    /* SPR 19601: In case the registration request is internally generated
     * then no need to issue response to application , response should be
     * sent only in case of error / failure conditions
     */ 
    if(ICF_RGM_INTERNAL_GENERATED_REG != 
            (ICF_RGM_INTERNAL_GENERATED_REG & p_rgm_context->indicator))
    {
  ...
  else
  {
    ...
    /* If reg_refresh_required is set to TRUE then on successful 
     * completion of the register,IPTK shall start the timer and on timer expiry,
     * refresh the registration on its own*/

    if (ICF_TRUE == p_crm_pdb->p_rgm_context->reg_refresh_required)
    {
        ret_val = icf_rgm_start_expires_timer(
                p_glb_pdb,
                p_nw_resp->expires_duration,
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
    ...
    else
    {
        ...
        /* If it is a dereg. req. given by the application to send dereg. request 
           on the network */ 
        else if(ICF_RGM_APP_INITIATED_DEREG & p_rgm_context->indicator) 
        {
        ...
        else
        {
            ...
            register_response.line_id = p_rgm_context->line_id;
            register_response.options_flag = ICF_NULL;
            /*Fix Merge for CSR 1- 6168806  In case this was dereg for all user
...

{
    ...
    */
    ...
    /*Rel8.2 Response code being set here to pass it to application in ICF_REGISTER_CFM or 
     *ICF_REGISTER_STATUS_IND on receiving failure response from the network*/
    if ( ICF_RGM_SSA_NW_FAILURE == p_nw_resp->status )
    {
        register_response.response_code = p_nw_resp->response_code;
    }
    ...
    {
            /* SPR 18064 Merge */
            case ICF_REGM_SSA_AUTH_PARAM_NOT_RCVD : 
		    register_response.error_cause = 
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
	...
	* to the applications that are kept in this rgm_context */
    ...
    /* SPR 19746: Populate the failure response code while sending
     * response to application for registration
     */ 
    if (ICF_RGM_SSA_NW_FAILURE == p_nw_resp->status)
    {
        register_response.response_code = p_nw_resp->response_code;
	...
	else if(ICF_REGM_SSA_AUTH_FAILURE ==  p_crm_pdb->error_cause)
	{
        ...
        /*  SPR 18064 Merge */
        else if(ICF_REGM_SSA_AUTH_PARAM_NOT_RCVD ==  p_crm_pdb->error_cause)
	{
            register_response.error_cause = 
                          ICF_CAUSE_AUTH_PARAM_NOT_RCVD;
            ...
            ICF_RGM_SET_ECODE(p_crm_pdb, ICF_ERROR_FAILURE_RESPONSE)
	...
	* to the applications that are kept in this rgm_context */
...
#else
        /* Whenever the function icf_rgm_update_auth_info returns failure,it
         * always sets a Non NULL error cause in p_crm_pdb->error_cause.
         * The error cause being checked here is used to send register_cfm failure
         * to application.
         */
...
#ifdef ICF_ERROR_ENABLE
                    /*Set error code for sending report_error_ind to application*/
                    ICF_RGM_SET_ECODE(p_crm_pdb, ICF_ERROR_AUTHENTICATION_FAILURE)
#endif
                   /*Merge for CSR_1-5685130*/
				/* Send failure responses/indications to applications */
	...
	{   
        ...
        else
        {
            ...
            /* Registration is to be retried in case port returns synchronous response 
             * for authentication */
            ...
            {
                /* Set the registration state to AWT auth resp */
                p_rgm_context->rgm_state = ICF_RGM_06_AWT_AUTH_RESP;
                ret_val = ICF_SUCCESS;
            }
#ifdef ICF_IPSEC_ENABLED
        } /* End else(ipsec_enabled) */
...

{
	...
	{/* less than 2 401s to handle: begins*/
	   ...
	   p_rgm_context->auth_retry_count++;
		...
		if (ICF_FAILURE == icf_rgm_update_auth_info(p_crm_pdb))
		{
            /* Whenever the function icf_rgm_update_auth_info returns failure,it
             * always sets a Non NULL error cause in p_crm_pdb->error_cause.
             * The error cause being checked here is used to send register_cfm
             * failure to application.
             */
    ...
    {/* 2 invalid challenges have been handled: begins */
        ...
        /* SPR 19886 : Merged fix for CSR 1-7233758: SPR 19673 */
#ifdef ICF_ERROR_ENABLE
	    ICF_RGM_SET_ECODE(p_crm_pdb, ICF_ERROR_AUTHENTICATION_FAILURE)
#endif
...
/* END: Multiple Public User ICF5.0*/
	...
	* send the CFM to only this application */
     ...
     * If this bitmask is set, then copy the flag value in the regm context.
	 * IPTK shall send status indications to the application for refresh
     * registration if this flag is enabled.
	 */
	if (ICF_REG_REQ_REFRESH_INDICATIONS_REQD & p_recv_payload->bit_mask)
	{
		p_crm_pdb->p_rgm_context->refresh_ind_reqd =\
				p_recv_payload->refresh_ind_reqd;
	}
			 ...
			 else
			 {
                    ...
                    p_crm_pdb->p_rgm_context->rgm_duration)
				{
                  ...
                  */
                    ...
                    ret_val = icf_rgm_register_user(
                            p_crm_pdb,p_crm_pdb->p_rgm_context);
                 /* SPR 18997: the statistics for register request are captured
                  * in the function icf_rgm_register_req so no need to capture
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
    ...
    if(ICF_SUCCESS == ret_val)
    {
        ...
        {
            /*SPR 18776: Set the error cause as ICF_CAUSE_ONGOING_REGISTRATION
              because for this user we are in registering state and another 
              REGISTER request is received */
            register_response.status = ICF_FAILURE;
            register_response.error_cause = ICF_CAUSE_ONGOING_REGISTRATION;
            register_response.request_type = ICF_REQUEST_TYPE_REGISTERING;
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
	...
	* send the CFM to only this application */
	...
	{/* Application sent a  non-dereg REGISTER request*/
        ...
        /* SPR 17671 : CSR 1-5397601 : Request type register instead of
        * Deregister
        */
        register_response.request_type = ICF_REQUEST_TYPE_REGISTERING;
        register_response.status = ICF_FAILURE;
	...
	ret_val = icf_rgm_send_response(p_crm_pdb->p_glb_pdb,
					&register_response, p_crm_pdb->p_ecode);
    /* SPR 17671 : CSR 1-5397601 : Clear the context to allow the user to be 
     * registered again.*/
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
    ...
    /* Coverity bug fix */
	icf_address_st       rgm_uri = {{0,"\0"},0};/* The URI that will be used to send REGISTER */
...
/*#else*/
    ...
    if(ICF_TRUE == is_ims_max_retry)
    {
        ...
        register_response.status = ICF_FAILURE;
        /*SSA can send error when procy/registrar/DNS server are not responding
         *send same error_cause as received from SSA*/
        if ((ICF_ERROR_DNS_QUERY_FAILURE == p_crm_pdb->error_cause) ||
            (ICF_CAUSE_DNS_SERVER_NOT_RESPONDING == p_crm_pdb->error_cause) ||
            (ICF_CAUSE_PROXY_SERVER_NOT_RESPONDING == p_crm_pdb->error_cause) ||
            (ICF_CAUSE_REGISTRAR_NOT_RESPONDING == p_crm_pdb->error_cause))
        {
           register_response.error_cause = p_crm_pdb->error_cause;
        ...
        /*SPR 20041 Problem was that register status indication was having the
         * request type as 0 when register refresh retransmission timeout
         * happens because request type value was being overwritten here.
         * As fix removed the null set of request_type as it is not a valid
         * value and it was overwriting the previously assigned value for
         * registering */
#ifdef ICF_NAT_RPORT_SUPPORT
...
ICF_CHECK_IMS_START(p_glb_pdb,ICF_TRUE)
    ...
    if (ICF_FALSE == is_ims_max_retry)
	{
		...
		if (ICF_FAILURE == icf_rgm_update_auth_info(p_crm_pdb))
		{
            /* The error cause being checked here is being set via the function
             * call to icf_rgm_update_auth_info and is used to send
             * register_cfm failure to application.
             */
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
    ...
    if (ICF_TRUE == p_crm_pdb->p_config_data->ipsec_enabled)
    {
       ...
       /* If it is application given  dereg. request */
        else if(ICF_RGM_APP_INITIATED_DEREG & p_rgm_context->indicator) 
        {
            p_rgm_context->indicator &= ~ICF_RGM_APP_INITIATED_DEREG;
            icf_rgm_form_and_send_dereg_resp(p_crm_pdb,p_rgm_context);
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
    ...
    {
        /* get cfg module global data, if it is NULL.*/
    ...
    { 
	  ...
	  if (ICF_APP_ID_INVALID == p_crm_pdb->recd_con_app_id)
      {
       ...
       ICF_STATS(p_crm_pdb->p_glb_pdb->p_stats_info->stat_register.stats_nw_dereg_done)
  	    ...
  	    /* Since AOR is a public user along with self contact received in
         * NOTIFY body set the bitmask for clearing registration context 
         * identifying that it is a Network Initiated deregistration*/
        /*SPR 20026 Besides AOR we should check the Associated URI presence
         in aor of xml for NOTIFY because if any of the public URI is
         de-registered all corresponding URI should be de-registered*/   
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
   ...
   if(ICF_FAILURE == icf_dbm_fetch_line_cntxt_blk(
    ...
    /* SPR 19151: Since the registration is once complete with success
     * therfore set the bitmask. This bitmask is used while sending the
     * deregister request whether to allow deregistration. For NAT cases,
     * this bitmaks should be set as and when first success response is 
     * received so that on attempt of deregistration, registartion context 
     * is not deallocated.
     */
    p_rgm_context->indicator |= ICF_RGM_ONCE_REGISTERED;    
		  ...
		  while (ICF_NULL != p_user_db)
		  {
				...
				p_crm_pdb->p_ecode);
                    ...
                    else if(ICF_RGM_03_REGISTERED == p_temp_rgm_context->rgm_state)
                    {
                        ...
                        /* SPR 19601: the following bitmask setting is required so that
                         * after completion of the registration procedure , no response 
                         * is sent to application.
                         */ 
                        p_temp_rgm_context->indicator |= ICF_RGM_INTERNAL_GENERATED_REG;
                        ...
                        /* This de-reg is sent by IPTK due to nat binding change */
                        p_temp_rgm_context->indicator |= ICF_RGM_INTERNAL_GENERATED_DEREG;
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
   ...
   /* (icf_rgm_context_st *)(p_crm_pdb->p_internal_msg->
    ...
    if(ICF_INITIAL_RPORT_COMPLETED == p_rgm_context->rport_status ||
       ICF_INITIAL_RPORT_INITIAL == p_rgm_context->rport_status)
    {
          ...
          {
              /* This de-reg is sent by IPTK due to nat binding change */
              p_rgm_context->indicator |= ICF_RGM_INTERNAL_GENERATED_DEREG;
...
)
{
	...
	/* Set the bitmask to fill user address in response API */
	register_response.options_flag |= ICF_RGM_CTXT_COPY;

    register_response.p_crm_pdb = p_crm_pdb;
    register_response.p_rgm_context = p_rgm_context;

    /* CSR 1-6089701: SPR 18721: If deregister is for all users then send
     * register status indication instead of register cfm
     */
    if(ICF_REGISTER_FOR_ALL_USER & p_rgm_context->indicator)
    {
		/* Fix for CSR 1-8652432 */
		/* Change from assignment to bitwise setting */
	    register_response.options_flag |= ICF_RGM_RESPONSE_IND; 
    }
    else
    {
 ...
 *
 * DESCRIPTION:         This function is invoked when IPTK is waiting for
 *              authentication response from application and application 
 *              sends Authentication response API to IPTK.
 *
 * RETURNS:             
 *
 ***************************************************************************/
icf_return_t icf_rgm_06_user_auth_resp(
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
    icf_aka_auth_resp_st		*p_recv_payload = ICF_NULL;
    icf_return_t                ret_val = ICF_SUCCESS;
    ...
    /* Update the response in registration context from API */
    p_recv_payload = p_crm_pdb->p_recv_payload;
    if(ICF_SUCCESS == p_recv_payload->result)
    {
        icf_cmn_auth_data_st        *p_auth_data_sent=
            &(p_crm_pdb->p_rgm_context->auth_data_sent);

        /* Successful AKA negotiation trace */
        ICF_RGM_APP_TRACE(p_crm_pdb, ICF_TRACE_REG_AKA_AUTH_SUCCESS)
            
        /*Colecting stats for authentication success from application */
        ICF_STATS(p_crm_pdb->p_glb_pdb->p_stats_info->stat_register.
         ...
         */
        if(ICF_AKA_AUTH_SEQ_NUM_PRESENT == (p_recv_payload->bit_mask & 
                ICF_AKA_AUTH_SEQ_NUM_PRESENT))
        {
            p_auth_data_sent->sequence_num = 
                p_recv_payload->sequence_num;
        }
        ...
        p_auth_data_sent->res.str_len = 
            p_recv_payload->auth_response.str_len;
        icf_port_strncpy((icf_int8_t*)&(p_auth_data_sent->res.str),
                (const icf_int8_t*)&(p_recv_payload->auth_response.str), ICF_MAX_STR_LEN - 2);
        p_auth_data_sent->res.str[ICF_MAX_STR_LEN - 1] = '\0';

        /* Copying Integrity key and Cipher key generated by AKA 
         * procedure. */
        if(ICF_AKA_AUTH_CK_PRESENT == (p_recv_payload->bit_mask & 
                ICF_AKA_AUTH_CK_PRESENT))
        {
            p_auth_data_sent->enc_key.str_len = 
                p_recv_payload->Ck.str_len;

            icf_port_strncpy((icf_int8_t*)&(p_auth_data_sent->enc_key.str),
                    (const icf_int8_t*)&(p_recv_payload->Ck.str), ICF_MAX_SHORT_STR_LEN - 2);
            p_auth_data_sent->enc_key.str[ICF_MAX_SHORT_STR_LEN - 1] = '\0';
        }

        if(ICF_AKA_AUTH_IK_PRESENT == (p_recv_payload->bit_mask & 
                ICF_AKA_AUTH_IK_PRESENT))
        {
            p_auth_data_sent->auth_passwd.str_len = 
                p_recv_payload->Ik.str_len;

            icf_port_strncpy((icf_int8_t*)&(p_auth_data_sent->auth_passwd.str),
                    (const icf_int8_t*)&(p_recv_payload->Ik.str), ICF_MAX_SHORT_STR_LEN - 2);
            p_auth_data_sent->auth_passwd.str[ICF_MAX_SHORT_STR_LEN - 1] = '\0';
        }
        
        if(ICF_RGM_APP_INITIATED_DEREG & p_crm_pdb->p_rgm_context->indicator)
        {
            /* DeRegistration is to be retried */
            ret_val = icf_rgm_deregister_user(p_crm_pdb,
                                  p_crm_pdb->p_rgm_context);
        ...
        { 
            /* Registration is to be retried */
            ret_val = icf_rgm_register_user(p_crm_pdb,
                                  p_crm_pdb->p_rgm_context);
        }
    }
    else
    {
       /* Failure AKA negotiation trace */
       ICF_RGM_APP_TRACE(p_crm_pdb, ICF_TRACE_AKA_AUTH_FAIL)

       /*Colecting stats for authentication failure from application */
       ICF_STATS(p_crm_pdb->p_glb_pdb->p_stats_info->stat_register.
                    stats_reg_auth_failure)

        if(ICF_CAUSE_HOME_NETWORK_NOT_AUTH == p_recv_payload->error_cause)
        {
            /* Do not fill AUTS and RES */
            /* Reset AUTH SET bit */
        ...
        else if(ICF_CAUSE_SQN_OUT_OF_RANGE == 
                p_recv_payload->error_cause)
        {
            p_crm_pdb->p_rgm_context->auth_data_sent.auth_data_present |=
                ICF_AUTH_AUTS_PRESENT;

            /* AKA procedures returned SUCCESS but AUTS to be used */
            p_crm_pdb->p_rgm_context->auth_data_sent.auts.str_len = 
                p_recv_payload->auts_token.str_len;
            
            icf_port_strncpy((icf_int8_t*)&(p_crm_pdb->p_rgm_context->auth_data_sent.auts.str),
                    (const icf_int8_t*)&(p_recv_payload->auts_token.str), ICF_MAX_STR_LEN - 2);
            p_crm_pdb->p_rgm_context->auth_data_sent.auts.str[ICF_MAX_STR_LEN - 1] = '\0';
        ...
        else
        {
            ...
            if (ICF_NULL != p_crm_pdb->p_rgm_context)
            {
            /* Send failure register response to application */
            /*  send CFM with FAILURE: */		
            register_response.p_crm_pdb = p_crm_pdb;
            register_response.p_rgm_context = p_crm_pdb->p_rgm_context;
            ...
            register_response.p_request_payload = ICF_NULL;
            register_response.request_type = ICF_REQUEST_TYPE_AWTING_AUTH_RESP;
            register_response.status = ICF_FAILURE;
            register_response.error_cause = p_recv_payload->error_cause;
 ...
 *
 * DESCRIPTION:         This function is invoked when ICF_REGISTER_REQ is
 *          received from application at an instant when IPTK is waiting for 
 *          authentication response from application.
 *
 * RETURNS:             
 *
 ***************************************************************************/
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
    ...
    /* Return failure response to application */
    register_response.status = ICF_FAILURE;
    register_response.error_cause = p_crm_pdb->error_cause;
    register_response.request_type = ICF_REQUEST_TYPE_AWTING_AUTH_RESP;
 ...
 *
 * DESCRIPTION:         This function is invoked when ICF_RGM_NOTIFY_TERMINATED
 *              event is received in the await authentication response state. 
 *              Check if the req event package is supported or not. 
 *              Notify can be received by IPTK only if it supports reg 
        ...
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
	icf_return_t		       ret_val = ICF_SUCCESS;
	icf_rgm_context_st		   *p_rgm_context = ICF_NULL;
	icf_rgm_register_resp_st   register_response = {ICF_NULL,ICF_NULL,0,ICF_NULL,0,0,0,0,0,0};
    ...
    { 
        ...
        if (ICF_APP_ID_INVALID == p_crm_pdb->recd_con_app_id)
        {
            ...
            /* Network initiated DEREG happened */
            /*SPR 20041 register_response.request_type was set as 0 which
              was not a valid value therefore assigning the ICF_REQUEST_TYPE_DEREGISTER
              as it corresponds to the assigned error cause.*/
            register_response.request_type = ICF_REQUEST_TYPE_DEREGISTER;
            register_response.status = ICF_FAILURE;
            register_response.error_cause = ICF_CAUSE_NETWORK_DEREGISTERED;
 ...
 *
 * DESCRIPTION:         This function is invoked when Registration Guard 
 *             timer at IPTK has expired.
 *     Current State: ICF_RGM_06_AWT_AUTH_RESP 
 *     Event Received: ICF_RGM_TIMER_EXPIRY 
 *             Once the guard timer has expired and IPTK does not receive any
 *             authentication response from application, IPTK clears the 
 *             registration context and sends error indication to application.
 *
 * RETURNS:             
 *
 ***************************************************************************/
icf_return_t icf_rgm_06_guard_timer_exp(
        INOUT icf_crm_pdb_st *p_crm_pdb)
{
	icf_timer_data_st     *p_buff_guard_timer = ICF_NULL;
    icf_timer_data_st     **p_p_buff_guard_timer;
	icf_rgm_context_st    *p_rgm_context = ICF_NULL;
    ...
    ICF_RGM_APP_TRACE(p_crm_pdb, ICF_TRACE_REG_GUARD_TIMER_EXPIRY)
    ...
    p_p_buff_guard_timer = (icf_timer_data_st **)(p_glb_pdb->p_recv_msg->
            payload);

    p_buff_guard_timer = (icf_timer_data_st *)(*p_p_buff_guard_timer);
	
    /* Get the pointer to registration context from received timer data */
	p_rgm_context = (icf_rgm_context_st *)(p_buff_guard_timer->
            p_timer_buffer);
    ICF_ASSERT(
            p_crm_pdb->p_internal_msg->msg_hdr.p_glb_pdb,
            (ICF_NULL != p_rgm_context))    
    ...
    /* collect the stats for gaurd timer expiry */
    ICF_STATS(p_crm_pdb->p_glb_pdb->p_stats_info->stat_register.
