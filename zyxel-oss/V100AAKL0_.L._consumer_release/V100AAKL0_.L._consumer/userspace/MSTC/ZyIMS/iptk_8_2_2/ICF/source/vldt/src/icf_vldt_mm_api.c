 ...
 * 13-JUN-2007  Jagmohan Chauhan   No              Initial
                Rishi Saxena
 * 11-Feb-2009  Ashutosh Mohan    Rel8.1        Added validation functions for tunnelled APIs
 *                                           
...
*
* DESCRIPTION:     This function is invoked to validate
*                  icf_vldt_icf_tunnelled_create_media_session_resp_st api.
*
**************************************************************************/
                                      ...
                                      INOUT icf_vldt_err_st *p_vldt_err)
{
    ...
    /* Initializing create media session resp local API structure */
    icf_tunnelled_create_media_session_resp_st *p_icf_tunnelled_create_media_session_resp = ICF_NULL;

    /* Assigning payload of create media session resp api to local api structure0 */
    p_icf_tunnelled_create_media_session_resp = (icf_tunnelled_create_media_session_resp_st *) p_payload;

    /* Check: result should be validated against icf_vldt_icf_result_t */

    ret_val = icf_vldt_icf_result(p_icf_tunnelled_create_media_session_resp->result,
                                                            p_vldt_err);
    if(ICF_FAILURE == ret_val)
    {
        /* Add error string to p_vldt_err structure */
    ...
    if (ICF_SUCCESS == ret_val)
    {
        if(p_icf_tunnelled_create_media_session_resp->bit_mask & ICF_TUNNELLED_CMS_RESP_SDP_INFO_PRESENT)
        {
            ret_val = icf_vldt_icf_tunnelled_sdp_st(&p_icf_tunnelled_create_media_session_resp->tunnelled_sdp_info, p_vldt_err);
            if(ICF_FAILURE == ret_val)
            {
                /* Add error string to p_vldt_err structure */
                icf_vldt_port_strcat(p_vldt_err->err_str,(icf_uint8_t *)"tunnelled_sdp_info:");
            }
        }
    }
...
*
* DESCRIPTION:     This function is invoked to validate
*                  icf_vldt_icf_tunnelled_modify_media_session_resp_st api.
*
**************************************************************************/
                                      ...
                                      INOUT icf_vldt_err_st *p_vldt_err)
{
    ...
    /* Initializing create media session resp local API structure */
    icf_tunnelled_modify_media_session_resp_st *p_icf_tunnelled_modify_media_session_resp = ICF_NULL;

    /* Assigning payload of create media session resp api to local api structure0 */
    p_icf_tunnelled_modify_media_session_resp = (icf_tunnelled_modify_media_session_resp_st *) p_payload;
    /* Check: result should be validated against icf_vldt_icf_result_t */

    ret_val = icf_vldt_icf_result(p_icf_tunnelled_modify_media_session_resp->result,
                                                            p_vldt_err);
    if(ICF_FAILURE == ret_val)
    {
        /* Add error string to p_vldt_err structure */
    ...
    if (ICF_SUCCESS == ret_val)
    {
        if(p_icf_tunnelled_modify_media_session_resp->bit_mask & ICF_TUNNELLED_MMS_RESP_SDP_INFO_PRESENT)
        {
            ret_val = icf_vldt_icf_tunnelled_sdp_st(&p_icf_tunnelled_modify_media_session_resp->tunnelled_sdp_info, p_vldt_err);
            if(ICF_FAILURE == ret_val)
            {
                /* Add error string to p_vldt_err structure */
                icf_vldt_port_strcat(p_vldt_err->err_str,(icf_uint8_t *)"tunnelled_sdp_info:");
...
*
* DESCRIPTION:     This function is invoked to validate
*                  icf_vldt_icf_tunnelled_media_capabilities_resp_st api.
*
**************************************************************************/
                                      ...
                                      INOUT icf_vldt_err_st *p_vldt_err)
{
    ...
    /* Initializing create media session resp local API structure */
    icf_tunnelled_media_capabilities_resp_st *p_icf_tunnelled_media_capabilities_resp = ICF_NULL;

    /* Assigning payload of create media session resp api to local api structure0 */
    p_icf_tunnelled_media_capabilities_resp = (icf_tunnelled_media_capabilities_resp_st *) p_payload;
    /* Check: result should be validated against icf_vldt_icf_result_t */

    ret_val = icf_vldt_icf_result(p_icf_tunnelled_media_capabilities_resp->result,
                                                            p_vldt_err);
    if(ICF_FAILURE == ret_val)
    {
        /* Add error string to p_vldt_err structure */
    ...
    if (ICF_SUCCESS == ret_val)
    {
        if(p_icf_tunnelled_media_capabilities_resp->bit_mask & ICF_TUNNELLED_MED_CAP_RESP_SDP_INFO_PRESENT)
        {
            ret_val = icf_vldt_icf_tunnelled_sdp_st(&p_icf_tunnelled_media_capabilities_resp->tunnelled_sdp_info, p_vldt_err);
            if(ICF_FAILURE == ret_val)
            {
                /* Add error string to p_vldt_err structure */
                icf_vldt_port_strcat(p_vldt_err->err_str,(icf_uint8_t *)"tunnelled_sdp_info:");
