 ...
 ** 27/02/09        Alok Tiwari    ICF Rel-8.1  Changes done to replace the 
 **                                          use of open source Digest with
 **                                          aDigest utility and removal of
 **                                          open source Base-64.
 ...
 ** 28-Jul-2009 Rajiv Kumar     SPR 19886    Merged CSR-1-7427320  
 **
 *****************************************************************************
...
/* klocowrks : NULL pointer checks, memory allocation failrue checks
 * have been added. Failure is returned in case of any issues
 */
...
#endif
{
    Sdf_ty_s8bit HA1[SDF_ADIGEST_HEX_CHECKSUM_LEN+1]="";
	Sdf_ty_s8bit HEntity[SDF_ADIGEST_HEX_CHECKSUM_LEN+1]="";
	Sdf_ty_s8bit dResponse[SDF_ADIGEST_HEX_CHECKSUM_LEN+1]="";
	...
	*ppGenericCredential=Sdf_co_null;
	...
	if (Sdf_mc_strcasecmp(pAuthenticationParams->pScheme, "Basic") == 0)
	{
         ...
         * was as below:
		  ...
		  icf_encodeBase64(dBasicEncodeInput, dBasicResponse);
	...
	else
	{
		 ...
		 * sdf_fn_uaDigestCalcHEntity to calculate the hash of the entity body.
		 */
		 ...
		 */ 
		sdf_fn_uaDigestCalcHEntity((Sdf_ty_s8bit *)"",0,HEntity);
			...
			(SipError *)&(pError->stkErrCode)) != SipFail)
		{
			if (dMsgBodyCount > 0)
			{
				 ...
				 */
				sdf_fn_uaDigestCalcHEntity(dEntityBody+2,dLength-2,HEntity);
			}	
		}
		/*
		 ...
		 * Calculate the H(A1)
		 */
		sdf_fn_uaDigestCalcHA1(pAuthenticationParams->pAlgorithm, \
		 ...
		 */
		/*ZyXEL porting*/ 
		if( (NULL == pUacAuthInfo->pNonceCount) || (NULL == pUacAuthInfo->pQop) || (NULL == pUacAuthInfo->pCnonce) || (NULL == HEntity)){
		    sdf_fn_uaDigestCalcResponse(HA1, dAuthNonce,NULL,\
			NULL, NULL, pMethod, pUri, NULL, \
			dResponse);
			
		}else{
		sdf_fn_uaDigestCalcResponse(HA1, dAuthNonce, pUacAuthInfo->pNonceCount,\
...
#endif
{
	...
	if(pOverlapTransInfo == Sdf_co_null)
	{
		...
		else if(Sdf_co_null != pTransaction)
		{
	...
	if(Sdf_co_null == pProcessedHeaderList)
	{
#ifdef SDF_ERROR
		sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_callObjectAccessError,\
				(Sdf_ty_s8bit *)"sdf_ivk_uaAuthorize(): "
				"Failed to get Process header list from Transaction",pErr);
#endif
		pErr->errCode=Sdf_en_callObjectAccessError;
		return Sdf_en_error;
	}
	...
	if (size != 0)
	{
		...
		if(SipCredBasic == credType)
		{
			...
			Sdf_ty_s8bit *pTrack = Sdf_co_null;
            ...
            /* The invocation of Base-64 API decodeBase64
             * has been removed from here in process of 
             * OPEN_SOURCE_REMOVAL.The earlier API invocation
             * was as below:

			    icf_decodeBase64(dTempBasic, dResponse);	
             */
		...
		else
		{
	  		...
	  		for (dIndex = 0; dIndex < dCount; dIndex++)
			{
				...
				if (Sdf_mc_strcasecmp(pName, "realm") == 0)
				{
					...
					if(Sdf_co_null != pUasAuthInfo->pRealm)
				...
				else if (Sdf_mc_strcasecmp(pName, "username") == 0)
				{
					...
					if(Sdf_co_null != pUasAuthInfo->pUserName)
				...
				else if (Sdf_mc_strcasecmp(pName, "algorithm") == 0)
				{
					...
					if(Sdf_co_null != pUasAuthInfo->pAlgorithm)
				...
				else if (Sdf_mc_strcasecmp(pName, "nonce") == 0)
				{
					...
					if(Sdf_co_null != pUasAuthInfo->pNonce)
				...
				else if (Sdf_mc_strcasecmp(pName, "cnonce") == 0)
				{
					...
					if(Sdf_co_null != pUasAuthInfo->pCnonce)
...
#endif
{
	...
	if(Sdf_mc_strcasecmp(pUasAuthInfo->pScheme,"Digest") == 0)
	{
...
#endif /*ICF_PORT_SYMBIAN*/
        Sdf_ty_s8bit dResponse[SDF_ADIGEST_HEX_CHECKSUM_LEN+1]="";
		Sdf_ty_s8bit dCalculatedResponse[SDF_ADIGEST_HEX_CHECKSUM_LEN+1]="";
		Sdf_ty_s8bit dReceivedResponse[SDF_ADIGEST_HEX_CHECKSUM_LEN+1]="";
		Sdf_ty_s8bit HEntity[SDF_ADIGEST_HEX_CHECKSUM_LEN+1]=" ";
		 ...
		 */
		sdf_fn_uaDigestCalcResponse(pUasAuthInfo->pSecret, dAuthNonce, pNonceCount, \
...
/*w534*/(void)sip_insertHeaderFromStringAtIndex(pOverlapTransInfo->pSipMsg,\
    ...
    if(Sdf_co_null == pOverlapTransInfo)
    {
        if (Sdf_co_null == pTransaction)
		{
			...
			sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_callObjectAccessError,\
					(Sdf_ty_s8bit *)"sdf_ivk_uaFormRequestWithAuthorization(): "
					"Neither transaction nor Overlap Transaction present",pError);
#endif
			pError->errCode=Sdf_en_callObjectAccessError;
			return Sdf_co_fail;
		}
...
#endif
{
    ...
    /* Fix for SPR: 19886 (Merged CSR 1-7427320)*/
...
**
**********************************************************/
Sdf_ty_retVal sdf_fn_uaDigestCalcHA1
#ifdef ANSI_PROTO 
  (Sdf_ty_s8bit *pszAlg,
...
#else
  (pszAlg,pszUserName,pszRealm,pszPassword,pszNonce,pszCNonce,SessionKey)
  Sdf_ty_s8bit *pszAlg;
  Sdf_ty_s8bit *pszUserName;
  Sdf_ty_s8bit *pszRealm;
  Sdf_ty_s8bit *pszPassword;
...
#endif
{
  util_string_st      dAlgorithm = {0,0};
  util_string_st      dUser_name = {0,0};
  util_string_st      dRealm = {0,0};
  util_string_st      dPassword = {0,0};
  util_string_st      dNonce = {0,0};
  util_string_st      dCNonce = {0,0};
  util_string_st      HA1 = {0,0};
  aDigest_ecode_et    error = 0;
  
  dAlgorithm.str = (util_uint8_t*)(pszAlg);
  dAlgorithm.str_len = Sdf_mc_strlen(pszAlg);
  dUser_name.str = (util_uint8_t*)(pszUserName);
  dUser_name.str_len = Sdf_mc_strlen(pszUserName);
  dRealm.str = (util_uint8_t*)(pszRealm);
  dRealm.str_len = Sdf_mc_strlen(pszRealm);
  dPassword.str = (util_uint8_t*)(pszPassword);
  dPassword.str_len = Sdf_mc_strlen(pszPassword);
  dNonce.str = (util_uint8_t*)(pszNonce);
  dNonce.str_len = Sdf_mc_strlen(pszNonce);
  dCNonce.str = (util_uint8_t*)(pszCNonce);
  dCNonce.str_len = Sdf_mc_strlen(pszCNonce);
  HA1.str = (util_uint8_t*)SessionKey;
  HA1.str_len = SDF_ADIGEST_HEX_CHECKSUM_LEN;
 
  aDigest_calc_HA1(dAlgorithm, dUser_name, dRealm,
                       dPassword, dNonce, dCNonce, HA1, &error);
  return  Sdf_co_success;
}
...
**
**********************************************************/
Sdf_ty_retVal sdf_fn_uaDigestCalcHEntity
#ifdef ANSI_PROTO 
  (Sdf_ty_s8bit *pszEntityBody,
   Sdf_ty_s16bit dLength,
   Sdf_ty_s8bit *HEntity)
#else
  (pszEntityBody, dLength, HEntity)
  Sdf_ty_s8bit *pszEntityBody;
  Sdf_ty_s16bit dLength;
  Sdf_ty_s8bit *HEntity;
#endif
{
  util_string_st      entity_body = {0,0};
  util_string_st      temp_HEntity = {0,0};
  aDigest_ecode_et    error = 0;

  entity_body.str = (util_uint8_t*)pszEntityBody;
  entity_body.str_len = dLength;
  temp_HEntity.str = (util_uint8_t*)HEntity;
  temp_HEntity.str_len = SDF_ADIGEST_HEX_CHECKSUM_LEN;

  aDigest_calc_HEntity(entity_body, temp_HEntity, &error);
  return  Sdf_co_success;
}
...
Sdf_ty_retVal sdf_fn_uaDigestCalcResponse
#ifdef ANSI_PROTO 
  (Sdf_ty_s8bit *HA1,
   Sdf_ty_s8bit *pszNonce,
...
#else
  (HA1, pszNonce, pszNonceCount, pszCNonce, pszQop,
        pszMethod, pszDigestUri, HEntity, Response)
  Sdf_ty_s8bit *HA1;
  Sdf_ty_s8bit *pszNonce;
  Sdf_ty_s8bit *pszNonceCount;
  Sdf_ty_s8bit *pszCNonce;
...
#endif
{
  util_string_st      temp_HA1 = {0,0};
  util_string_st      dNonce = {0,0};
  util_string_st      dNonceCount = {0,0};
  util_string_st      dCNonce = {0,0};
  util_string_st      qop_val = {0,0};
  util_string_st      req_method = {0,0};
  util_string_st      digest_uri = {0,0};    
  util_string_st      temp_HEntity = {0,0};
  util_string_st      dResponse = {0,0};
  aDigest_ecode_et    error = 0;

  temp_HA1.str = (util_uint8_t*)HA1;
  temp_HA1.str_len = SDF_ADIGEST_HEX_CHECKSUM_LEN;
  dNonce.str = (util_uint8_t*)pszNonce;
  dNonce.str_len = Sdf_mc_strlen(pszNonce);
  dNonceCount.str = (util_uint8_t*)pszNonceCount;
  dNonceCount.str_len = Sdf_mc_strlen(pszNonceCount);
  dCNonce.str = (util_uint8_t*)pszCNonce;
  dCNonce.str_len = Sdf_mc_strlen(pszCNonce);
  qop_val.str = (util_uint8_t*)pszQop;
  qop_val.str_len = Sdf_mc_strlen(pszQop);
  req_method.str = (util_uint8_t*)pszMethod;
  req_method.str_len = Sdf_mc_strlen(pszMethod);
  digest_uri.str = (util_uint8_t*)pszDigestUri;
  digest_uri.str_len = Sdf_mc_strlen(pszDigestUri);
  temp_HEntity.str = (util_uint8_t*)HEntity;
  temp_HEntity.str_len = SDF_ADIGEST_HEX_CHECKSUM_LEN;
  dResponse.str = (util_uint8_t*)Response;
  dResponse.str_len = Sdf_mc_strlen(Response);

  aDigest_calc_response(temp_HA1, dNonce, dNonceCount, dCNonce,
                     qop_val, req_method, digest_uri, temp_HEntity,
                                               dResponse, &error);
