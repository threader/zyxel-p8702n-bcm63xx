 ...
 ** 12-Feb-2009  Rajiv Kumar    Rel 8.1       New API added for handling
 **                                           icf_tunnelled_sdp_info_st in
 **                                           tunnelled mode.
 ** 17-Feb-2009 Abhishek Dhammawat IPTK Rel8.1 Modified 
 **                                            Sdf_ivk_uaSetMediaLevelAttrInSdpMsg
 ** 03-Mar-2009 Anurag Khare      Rel 8.1      Modified Function
 **                                            Sdf_ivk_uaInitAndFormCline
 ** 04-Mar-2009 Ashutosh Mohan     Rel8.1      Modified
 **                                            Sdf_ivk_uaInitAndFormCline
 **                                            Sdf_ivk_uaInitAndFormMline
 ** 05-Mar-2009 Ashutosh Mohan     Rel8.1      Modified
 **                                            Sdf_ivk_uaInitAndFormCline
 ** 22-Apr-2009 Abhishek Dhammawat SPR 19455   Modified 
...
#endif
{
	...
	*pSdpMedia=Sdf_co_null;
    ...
    Sdf_mc_strcpy(dTemp,"\0");	/* Initializing the payload list */
	if (pPayload != Sdf_co_null)
	{
        if(Sdf_co_smallBufferSize <= Sdf_mc_strlen(pPayload))
		{
#ifdef SDF_ERROR			
			sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_invalidParamError, \
				(Sdf_ty_s8bit*)"sdf_ivk_uaFormSdpMedia( ): "
				"pPayload is large length", pErr);
#endif			
			pErr->errCode = Sdf_en_invalidParamError;
			return Sdf_co_fail;
		}
...
#endif            
		{
            /* klocowrks : All the calls for STRCAT are replaced to STRNCAT 
             * with checking for remianing length of the destination buffer
             * these changes have been done to resolev ABR warnings
             */ 
			Sdf_ty_u32bit	remLen = 0;
			...
			if (!sdf_fn_uaIsNumber(pPayload))
			{
				...
				{
					if((Sdf_co_smallBufferSize -1) > 
							(Sdf_mc_strlen(dTemp) + Sdf_mc_strlen(pPayload)))
					{
						remLen = Sdf_co_smallBufferSize - 
							(Sdf_mc_strlen(dTemp) + Sdf_mc_strlen(pPayload)) - 2;

					    Sdf_mc_strncat(dTemp, pPayload,remLen);
					}
					else
					{
#ifdef SDF_ERROR			
				...
				{
					if((Sdf_co_smallBufferSize -1) > 
							(Sdf_mc_strlen(dTemp) + Sdf_mc_strlen(pLocalVal)))
					{
						remLen = Sdf_co_smallBufferSize - 
							(Sdf_mc_strlen(dTemp) + Sdf_mc_strlen(pLocalVal)) -2;

					    Sdf_mc_strncat(dTemp, pLocalVal,remLen);
					}
					else
					{
#ifdef SDF_ERROR			
						sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_invalidParamError, \
								(Sdf_ty_s8bit*)"sdf_ivk_uaFormSdpMedia( ): "
								"pLocalVal is large length", pErr);
#endif
			...
			if ((pPayload = Sdf_mc_strtokr(pLasts, " ",&pLasts))\
				!= Sdf_co_null)
			{
				if((Sdf_co_smallBufferSize  - 1) > (Sdf_mc_strlen(dTemp) + 1))
				{
					remLen = Sdf_co_smallBufferSize - Sdf_mc_strlen(dTemp) -3;
					Sdf_mc_strncat(dTemp, " ",remLen);
				}
				else
				{
					pErr->errCode = Sdf_en_invalidParamError;
...
#endif
{
	...
	if (pMediaFormats != Sdf_co_null)
    {
		...
		if(Sdf_co_null == pCodecStr)
		{
			...
			sdf_fn_setError(Sdf_en_majorErrors,Sdf_en_invalidParamError , \
					(Sdf_ty_s8bit*)"sdf_ivk_uaDeleteStreams(): "
					"Invalid Codec String passed",pError);
#endif
			pError->errCode=Sdf_en_invalidParamError ;
			return Sdf_co_fail;
		}
...
#endif
{
	...
	{
		Sdf_st_codec	*pCodec;
		Sdf_ty_s8bit	dTemp[Sdf_co_smallBufferSize] = {'\0',};
		Sdf_ty_s8bit	dRtpAttribute[Sdf_co_smallBufferSize] = {'\0',};
        SIP_U32bit	remLen = 0;

        /* klocowrks : All the calls for STRCAT are replaced to STRNCAT 
         * with checking for remianing length of the destination buffer
         * these changes have been done to resolev ABR warnings.
         * Also all the calls for SPRINTF are replaced with SNPRINTF
         * so as to ensure and prevent any ABR
         */
		...
		if(pCodec->dCodecNum !=-1)
			(void)sdf_fn_uaSnprintf(dTemp,Sdf_co_smallBufferSize,"%u",pCodec->dCodecNum);
		else
			(void)sdf_fn_uaSnprintf(dTemp,Sdf_co_smallBufferSize,"%s",pCodec->pOtherCodecName);

		if(Sdf_co_characterBufferSize > 
				(Sdf_mc_strlen(dPayLoad) + Sdf_mc_strlen(dTemp)))
		{
			remLen = Sdf_co_characterBufferSize - 
				(Sdf_mc_strlen(dPayLoad) + Sdf_mc_strlen(dTemp));

		    Sdf_mc_strncat(dPayLoad,dTemp,remLen);
			remLen = 0;
		}
		else
		{
		...
		if(Sdf_co_characterBufferSize > 
				(Sdf_mc_strlen(dPayLoad) + 1))
		{
			remLen = Sdf_co_characterBufferSize - 
				(Sdf_mc_strlen(dPayLoad) + 1);

		    Sdf_mc_strncat(dPayLoad," ",remLen);
			remLen = 0;
		}
		else
		{
		...
		{	
			...
			{
				Sdf_ty_s8bit	dTempCodecString[Sdf_co_smallBufferSize] = {'\0',}; 
				
				(void)sdf_fn_uaMapCodecEnumToString(pCodec->dCodecName,\
						dTempCodecString,pErr);
				(void)sdf_fn_uaSnprintf(dTemp,Sdf_co_smallBufferSize,"%u %s",pCodec->dCodecNum, \
								dTempCodecString);
			}
#ifdef SDF_UAB2B
			...
			{
                if(Sdf_co_smallBufferSize > 
						(Sdf_mc_strlen(dRtpAttribute) + Sdf_mc_strlen(dTemp)))
				{
					remLen = Sdf_co_smallBufferSize - 
						(Sdf_mc_strlen(dRtpAttribute) + Sdf_mc_strlen(dTemp));
				
					Sdf_mc_strncat(dRtpAttribute,dTemp,remLen);
					remLen = 0;
				}
				else
				{
				...
				{
					Sdf_ty_s8bit dEncode[Sdf_co_smallBufferSize] = {'\0',};

					if(Sdf_co_smallBufferSize > 
							(Sdf_mc_strlen(dRtpAttribute) + 1))
					{
						remLen = Sdf_co_smallBufferSize - 
							(Sdf_mc_strlen(dRtpAttribute) + 1);

					    Sdf_mc_strncat(dRtpAttribute,"/",remLen);
						remLen = 0;
					}
					else
					{
					...
					(void)sdf_fn_uaSnprintf(dEncode,Sdf_co_smallBufferSize,
											"%d",pCodec->dEncodingRate);

					if(Sdf_co_smallBufferSize > 
							(Sdf_mc_strlen(dRtpAttribute) + Sdf_mc_strlen(dEncode)))
					{
						remLen = Sdf_co_smallBufferSize - 
							(Sdf_mc_strlen(dRtpAttribute) + Sdf_mc_strlen(dEncode));

					    Sdf_mc_strncat(dRtpAttribute,dEncode,remLen);
						remLen = 0;
					}
					else
					{
				...
				{
					Sdf_ty_s8bit dChanel[Sdf_co_smallBufferSize] = {'\0',};
					...
					(void)sdf_fn_uaSnprintf(dChanel,Sdf_co_smallBufferSize,"%d",pCodec->dChannels);

					if(Sdf_co_smallBufferSize > 
							(Sdf_mc_strlen(dRtpAttribute) + Sdf_mc_strlen(dChanel)))
					{
						remLen = Sdf_co_smallBufferSize - 
							(Sdf_mc_strlen(dRtpAttribute) + Sdf_mc_strlen(dChanel));

					    Sdf_mc_strncat(dRtpAttribute,dChanel,remLen);
						remLen = 0;
					}
					else
					{
			...
			{
				Sdf_ty_s8bit	dTempFmtp[Sdf_co_smallBufferSize] = {'\0',};
				
    		    (void)sdf_fn_uaSnprintf(dTempFmtp,Sdf_co_smallBufferSize,"%s%s","a=fmtp:",\
							pCodec->pFmtpString);
...
#endif
{
	...
	if( pCodecName != Sdf_co_null )
	{
		...
		(void)sdf_fn_uaSnprintf(dTemp,Sdf_co_smallBufferSize,
							   "%s/%d/%d",pCodecName,dEncodingRate,dChannels);
...
#endif	
{
	...
	SdpOrigin *pOrigin=Sdf_co_null;
	SdpTime *pSdpTime=Sdf_co_null;
	Sdf_ty_u32bit i =0,j =0,dSize = 0,dCount = 0,dRand =0;
	Sdf_ty_s8bit dTemp[Sdf_co_smallBufferSize];
	...
	*ppCapabilities=Sdf_co_null;
	...
	for(i=0;i<dCount;i++)
	{
		...
		if( (Sdf_co_null != pSdpMedia) && (pSdpMedia->pPortNum != Sdf_co_null))
		{
	...
	(void)sdf_fn_uaSnprintf(dTemp,Sdf_co_smallBufferSize,"%d",dRand);
...
#endif
{
	...
	for (dIndex=0; dIndex<dCount; dIndex++)
	{
		...
		if ((Sdf_co_null != pMsgBody) && (pMsgBody->dType == SipAppSipBody))
		{
			/* 
...
Sdf_ty_retVal Sdf_ivk_uaInitAndFormCline
    ...
    (Sdf_st_cLineInSdp 	  *pCLineInSdp,
     ...
     Sdf_st_error         *pErr)
#else
    (pCLineInSdp, pConnection,pErr)
     Sdf_st_cLineInSdp    *pCLineInSdp;
     SdpConnection        **ppConnection;
     Sdf_st_error         *pErr;    
#endif
{
    Sdf_ty_u8bit	ip_type[4] = "\0"; 
    Sdf_ty_s8bit	*pTmpVal = Sdf_co_null;
    Sdf_ty_s8bit	*pTmpIpType = Sdf_co_null;
    Sdf_ty_s8bit	*pTmpNetAddr = Sdf_co_null;
     ...
     * connection line can be IP4 or IP6 so for other values other than
     * SDF_CLINE_ADDRESS_IPV6_ADDR we are
     * setting the value of Address type to IP4 */
     
	if (SDF_CLINE_ADDRESS_IPV6_ADDR == pCLineInSdp->dNetworkAddress.dAddrType)
	{
		strcpy((Sdf_ty_s8bit *)ip_type,"IP6");
	}
	else
	{
		strcpy((Sdf_ty_s8bit *)ip_type,"IP4");
	}    

    pTmpVal = Sdf_mc_strdupSdp((const Sdf_ty_s8bit *)pCLineInSdp->dNetType.dStr);
    pTmpIpType = Sdf_mc_strdupSdp((const Sdf_ty_s8bit *)ip_type);
    pTmpNetAddr = Sdf_mc_strdupSdp((const Sdf_ty_s8bit *)pCLineInSdp->dNetworkAddress.dAddrVal.dStr);
	if (SipFail == sip_initSdpConnection(\
                ppConnection,(SipError *)&(pErr->stkErrCode)))
    {
#ifdef SDF_ERROR
    sdf_fn_setError(Sdf_en_majorErrors,Sdf_en_memoryError, \
            (Sdf_ty_s8bit *) "Sdf_ivk_uaInitAndFormCline():"\
            "sip_initSdpConnection function failed",pErr);
#endif        
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpVal,pErr); 
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpIpType,pErr);
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpNetAddr,pErr);

        return Sdf_co_fail;
	}
	else if (SipFail == sdp_setNetTypeInConnection(*ppConnection,pTmpVal,
                (SipError *)&(pErr->stkErrCode)))
	{
#ifdef SDF_ERROR
    sdf_fn_setError(Sdf_en_majorErrors,Sdf_en_sdpError, \
            (Sdf_ty_s8bit *) "Sdf_ivk_uaInitAndFormCline():"\
            "sdp_setNetTypeInConnection function failed",pErr);
#endif   
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpVal,pErr); 
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpIpType,pErr);     
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpNetAddr,pErr);
...
#ifdef ANSI_PROTO 
    (Sdf_st_mLineInSdp 	  *pMLineInSdp,
     SdpMedia             **ppMedia,
     Sdf_st_error         *pErr)
#else
    (pMLineInSdp, ppMmedia,pErr)
     Sdf_st_mLineInSdp    *pMLineInSdp;
     SdpMedia             **ppMmedia;
     Sdf_st_error         *pErr;    
#endif
{
    Sdf_ty_s8bit 	dUserPayLoads[Sdf_co_smallBufferSize] = {'\0',};
    Sdf_ty_s8bit	*pConvertedPayLoadList = Sdf_co_null;
    Sdf_ty_s8bit	*pPayload = Sdf_co_null; 
    Sdf_ty_s8bit    *pLocalVal = Sdf_co_null;
	Sdf_ty_s8bit 	dTemp[Sdf_co_smallBufferSize] = {'\0',};
    ...
    SdpMedia           *pSdpMedia = Sdf_co_null;
    Sdf_ty_s8bit	   *pTmpVal1 = Sdf_co_null;    
    Sdf_ty_s8bit	   *pTmpVal2 = Sdf_co_null;    
...
#endif/*End of SDF_PARAMVALIDATION ifdef*/    
    
	sdf_fn_debug((Sdf_ty_s8bit *)"SDF_DEBUG - Entering Sdf_ivk_uaInitAndFormMline");    
...
#ifdef SDF_ERROR		
        sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_memoryError, \
                (Sdf_ty_s8bit*)"Sdf_ivk_uaInitAndFormMline( ): "
                "Failed to initialize a SdpMedia structure", pErr);
#endif		
        pErr->errCode=Sdf_en_memoryError;
        return Sdf_co_fail;
    ...
    pTmpVal1 = Sdf_mc_strdupCallHandler((const Sdf_ty_s8bit
                *)(pMLineInSdp->dMediaType.dStr));
    
    /*Set the value of media type in pSdpMedia*/
    if (sdp_setMvalueInMedia(pSdpMedia, pTmpVal1, \
                (SipError*)&(pErr->stkErrCode)) == SipFail)
    {
#ifdef SDF_ERROR			
        sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_sdpError, \
...
#endif			
        sip_freeSdpMedia(pSdpMedia);
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpVal1,pErr); 
        pErr->errCode=Sdf_en_sdpError;
        return Sdf_co_fail;
    }    

	if (sdp_setPortInMedia(pSdpMedia,(Sdf_ty_u16bit)pMLineInSdp->dPort, \
		(SipError*) &(pErr->stkErrCode)) == SipFail)
	{
#ifdef SDF_ERROR		
		sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_sdpError, \
	...
	if (pMLineInSdp->dNumOfPorts > 1)
	{	
		if (sdp_setNumportInMedia(pSdpMedia, pMLineInSdp->dNumOfPorts,\
			(SipError*) &(pErr->stkErrCode)) == SipFail)
		{
#ifdef SDF_ERROR			
			sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_sdpError, \
    ...
    pTmpVal2 = Sdf_mc_strdupCallHandler((const Sdf_ty_s8bit
                *)(pMLineInSdp->dProtocol.dStr));

    if (sdp_setProtoInMedia(pSdpMedia, pTmpVal2, \
                (SipError*)&(pErr->stkErrCode)) == SipFail)
    {
#ifdef SDF_ERROR			
        sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_sdpError, \
...
#endif			
        sip_freeSdpMedia(pSdpMedia);
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pTmpVal2,pErr); 
        pErr->errCode=Sdf_en_sdpError;
        return Sdf_co_fail;
    }    

    Sdf_mc_strcpy(dUserPayLoads,pMLineInSdp->dFormat.dStr);

    pPayload = Sdf_mc_strtokr(dUserPayLoads, " ",&pLasts);
    Sdf_mc_strcpy(dTemp,"\0");
    if (pPayload != Sdf_co_null)
	{
		Sdf_ty_u32bit	dLen = 0, remLen = 0;
        ...
        do
        {
            ...
            if (!sdf_fn_uaIsNumber(pPayload))
            {
                ...
                if(pLocalVal == Sdf_co_null)
				{
					dLen = Sdf_mc_strlen(dTemp);

					if(Sdf_co_smallBufferSize > (dLen + Sdf_mc_strlen(pPayload)))
					{
						remLen = Sdf_co_smallBufferSize - dLen - Sdf_mc_strlen(pPayload) -1;
						Sdf_mc_strncat(dTemp,pPayload ,remLen);
					}
					else
					{
                ...
                else
                {
					...
					if(Sdf_co_smallBufferSize > (dLen + Sdf_mc_strlen(pLocalVal)))
					{
						remLen = Sdf_co_smallBufferSize - dLen - Sdf_mc_strlen(pLocalVal) -1;
						Sdf_mc_strncat(dTemp,pLocalVal ,remLen);
					}
					else
					{
                    ...
                    != Sdf_co_null) 
			{
				...
				if(Sdf_co_smallBufferSize > (dLen+1))
				{
					remLen = Sdf_co_smallBufferSize - dLen - 1 -1;
                    Sdf_mc_strncat(dTemp, " ",remLen);
				}
				else
				{
...
#endif		
        sip_freeSdpMedia(pSdpMedia);
        (void)sdf_memfree(0,(Sdf_ty_pvoid *)&pConvertedPayLoadList,pErr); 
        pErr->errCode=Sdf_en_sdpError;
        return Sdf_co_fail;
    }

    pErr->errCode = Sdf_en_noUaError;
    sdf_fn_debug((Sdf_ty_s8bit *)"SDF_DEBUG - Exiting Sdf_ivk_uaInitAndFormMline");
...
Sdf_ty_retVal Sdf_ivk_uaSetSessionLevelAttrInSdpMsg
    ...
    (SdpMessage 	  *pSdpMessage,
     ...
     Sdf_st_error     *pErr)
#else
    (pSdpMessage, dAttr,pErr)
     SdpMessage       *pSdpMessage;
     Sdf_st_string    dAttr;
     Sdf_st_error     *pErr;    
#endif
{
    Sdf_ty_u8bit dStartVar = dAttr.dStr[0];
...
#ifdef SDF_ERROR	
		sdf_fn_dumpError((Sdf_ty_s8bit *)"SDF_ERROR - "
			"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg(): "
			"Invalid Error parameter passed to function");
#endif			
		return Sdf_co_fail;
	}    
	if (Sdf_co_null == pSdpMessage)
	{
#ifdef SDF_ERROR	
		sdf_fn_dumpError((Sdf_ty_s8bit *)"SDF_ERROR - "
			"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg(): "
    ...
    switch(dStartVar)
    {
        case 'o':
        {
            dRetVal = sdp_setSdpOriginFromString(pSdpMessage,
                    (Sdf_ty_s8bit *)dAttr.dStr,(SipError*)&(pErr->errCode));
            break;
        }
        case 'v':
        {
            SIP_S8bit *pDummyPos = SIP_NULL;
            SIP_S8bit *pDupStr   = SIP_NULL;

            SIP_U32bit	dLen;    
            dLen = sip_strlen((SIP_S8bit *)dAttr.dStr);
            pDupStr = sip_nStrdup(ACCESSOR_MEM_ID,(SIP_S8bit *)dAttr.dStr , dLen+1,(SipError*)&(pErr->errCode));

            if (SIP_NULL == pDupStr)
            {
#ifdef SDF_ERROR
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for version",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            else
            {
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseSdpVersion(pDupStr+2, pDupStr+dLen,
                            &pDummyPos,pSdpMessage, (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
        ...
        case 's':
        {
            ...
            SIP_U32bit	dLen;    
            dLen = sip_strlen((SIP_S8bit *)dAttr.dStr);
            pDupStr = sip_nStrdup(ACCESSOR_MEM_ID,(SIP_S8bit *)dAttr.dStr , dLen+1,
                    (SipError*)&(pErr->errCode));
            ...
            if (SIP_NULL == pDupStr)
            {
                ...
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for session name",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            ...
            {	
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseSdpSessionNameOrSessionInfo(pDupStr+2, 
                            pDupStr+dLen,
                            &pDummyPos,&(pSdpMessage->pSession),SipTrue,
                            (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
                                      (SipError*)&(pErr->errCode));
        ...
        case 'i':
        {
            ...
            SIP_U32bit	dLen;    
            dLen = sip_strlen((SIP_S8bit *)dAttr.dStr);
            pDupStr = sip_nStrdup(ACCESSOR_MEM_ID,(SIP_S8bit *)dAttr.dStr ,
                    dLen+1,(SipError*)&(pErr->errCode));
            if (SIP_NULL == pDupStr)
            {
#ifdef SDF_ERROR
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for information",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            ...
            {            
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseSdpSessionNameOrSessionInfo(pDupStr+2,
                            pDupStr+dLen, &pDummyPos, \
                            &(pSdpMessage->pInformation),SipFalse,
                            (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
                                      (SipError*)&(pErr->errCode));
        ...
        case 'u':
        {
            ...
            if (SIP_NULL == pDupStr)
            {
                ...
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for URI",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
        ...
        case 'k':
        {
            ...
            if (SIP_NULL == pDupStr)
            {
                ...
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for Key",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            ...
            {            
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseSdpKey(pDupStr+2, pDupStr+dLen, 
                            &pDummyPos,&(pSdpMessage->pKey), 
                            (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
                                      (SipError*)&(pErr->errCode));
        ...
        case 'a':
        {
            ...
            if( (sip_listSizeOf(&(pSdpMessage->slAttr), &dIndex, \
                            (SipError *)&(pErr->stkErrCode))) == SipFail)
            {
                pErr->errCode = Sdf_en_noExistError;
#ifdef SDF_ERROR			
            ...
            if(sdp_insertSdpAttrAtIndexFromString(pSdpMessage,(SIP_S8bit *)dAttr.dStr,dIndex,
                        (SipError*)&(pErr->stkErrCode))==SipFail)
            {
                pErr->errCode = Sdf_en_sdpError;
#ifdef SDF_ERROR			
        ...
        case 'e':
        {
            ...
            if (SIP_NULL == pDupStr)
            {
                ...
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for Email",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            ...
            {            
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseSdpEmailAddrOrPhoneNum(pDupStr+2,
                            pDupStr+dLen,&pDummyPos,&(pSdpMessage->slEmail),
                            (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
                                      (SipError*)&(pErr->errCode));
        ...
        case 'p':
        {
            ...
            if (SIP_NULL == pDupStr)
            {
                ...
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for Phone",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            ...
            {	
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseSdpEmailAddrOrPhoneNum(pDupStr+2,
                            pDupStr+dLen,&pDummyPos,&(pSdpMessage->slPhone),
                            (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
                                      (SipError*)&(pErr->errCode));
        ...
        case 'b':
        {
            ...
            if (SIP_NULL == pDupStr)
            {
                ...
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for bandwidth",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            ...
            {	
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseSdpBandwidthInfo(pDupStr+2,
                            pDupStr+dLen,&pDummyPos,&(pSdpMessage->pBandwidth),
                            (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
                                      (SipError*)&(pErr->errCode));
        ...
        case 't':
        {
            ...
            if (SIP_NULL == pDupStr)
            {
                ...
                sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
                    (Sdf_ty_s8bit *)"Sdf_ivk_uaSetSessionLevelAttrInSdpMsg():"
                    "Failed is returned while doing strdup for time",pErr);
#endif
                pErr->errCode=Sdf_en_noMemoryError;
                return Sdf_co_fail;
            }           
            ...
            {            
                ...
                /*Invoke the parser function and set the value in SDP message*/
                if (SipFail == sip_parseTunnelledSdpTime(pDupStr+2,
                            pDupStr+dLen,&pDummyPos,&(pSdpMessage->slTime),
                            (SipError*)&(pErr->errCode)))
                {
                    (void)sip_memfree(ACCESSOR_MEM_ID, (SIP_Pvoid *)(&pDupStr),
                                      (SipError*)&(pErr->errCode));
        ...
        default:
        {
            ...
            sdf_fn_dumpError((Sdf_ty_s8bit *)"SDF_ERROR - "
                    "Sdf_ivk_uaSetSessionLevelAttrInSdpMsg(): "
                    "Invalid value for parameter dAttr is passed to function");
#endif			
            return Sdf_co_fail;            
        }
    }
...
Sdf_ty_retVal Sdf_ivk_uaSetMediaLevelAttrInSdpMsg
    ...
    (SdpMessage 	  *pSdpMessage,
     ...
     Sdf_st_error     *pErr)
#else
    (pSdpMessage,dAttr,dMediaIndex,pErr)
     SdpMessage       *pSdpMessage;
     Sdf_st_string    dAttr;
     Sdf_ty_u8bit     dMediaIndex;      
     Sdf_st_error     *pErr;    
#endif
{
    ...
    switch(dStartVar)
    {
        case 'a':
        {
            ...
            /*Get the pMedia at index denoted by dMediaIndex*/
            if ((sdp_getMediaAtIndex(pSdpMessage, &pMedia, dMediaIndex, \
                            (SipError *)&(pErr->stkErrCode))) == SipFail)
                return Sdf_co_fail;            

            /*Get the attribue count which are already set in pMedia.This is
             * done so that the attribute can be added at the end of list*/
            if(sdp_getAttrCountFromMedia(pMedia,&dCount,\
                        (SipError*)&(pErr->stkErrCode))==SipFail)
            {
                pErr->errCode = Sdf_en_sdpError;
#ifdef SDF_ERROR			
                sdf_fn_setError(Sdf_en_minorErrors,pErr->errCode,  \
                        (Sdf_ty_s8bit*)"Sdf_ivk_uaSetMediaLevelAttrInSdpMsg(): "
                        "Failed to get Attr Count",pErr);
#endif			
                (void)sip_freeSdpMedia(pMedia);
                return Sdf_co_fail;
            }            
            /*Inset the attribute at last node*/
			if(sdp_insertAttrFromStringAtIndexInMedia(pMedia,(SIP_S8bit *)dAttr.dStr,dCount,\
								(SipError*)&(pErr->stkErrCode))==SipFail)
			{
				pErr->errCode = Sdf_en_sdpError;
#ifdef SDF_ERROR			
        ...
        case 'b':
        {
            ...
            if ((sdp_getMediaAtIndex(pSdpMessage, &pMedia, dMediaIndex, \
                            (SipError *)&(pErr->stkErrCode))) == SipFail)
                return Sdf_co_fail;            
            /*Get the Bandwidth count which are already set in pMedia.This is
             * done so that the Bandwidth can be added at the end of list*/            
            if(sdp_getBandwidthCountFromMedia(pMedia,&dCount,\
                        (SipError*)&(pErr->stkErrCode))==SipFail)
            {
                pErr->errCode = Sdf_en_sdpError;
#ifdef SDF_ERROR			
            ...
            {   
               Sdf_ty_s8bit	*pTmpVal = Sdf_co_null;
		       pTmpVal = Sdf_mc_strdupCallHandler((SIP_S8bit *)dAttr.dStr);
               /* This increment of two is done to remove b: which was used till
                * now to identify the b line */
               Sdf_mc_strcpy(pTmpVal, pTmpVal + 2);

               /*Insert the attribute at last node*/
		       if(SipFail==sdp_insertBandwidthAtIndexInMedia(pMedia,pTmpVal, \
					(Sdf_ty_u32bit) dCount, (SipError*)&(pErr->stkErrCode)))
		       {
#ifdef SDF_ERROR			
		        	sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_sdpError, \
				      (Sdf_ty_s8bit*)"Sdf_ivk_uaSetMediaLevelAttrInSdpMsg( ): "
		...
		sdf_fn_dumpError((Sdf_ty_s8bit *)"SDF_ERROR - "
			"Sdf_ivk_uaSetMediaLevelAttrInSdpMsg(): "
			"Invalid value for parameter dAttr passed to function");
#endif			
		return Sdf_co_fail;
...
#endif
{
...
#ifdef SDF_ERROR	
		sdf_fn_dumpError((Sdf_ty_s8bit *)"SDF_ERROR - "
			"sdf_ivk_uaGetClineFromSdpMessage(): "
			"Invalid Error parameter passed to function");
#endif			
		return Sdf_co_fail;
	} 
    ...
    /*Get the session level c line from SDP message*/ 
    if ((sdp_getConnection(pSdpMessage, &pConnection, \
                    (SipError*)&(pErr->stkErrCode)))==SipFail)    
    {
        pErr->errCode = pErr->stkErrCode ;
#ifdef SDF_ERROR				
    ...
    {
        Sdf_mc_strcpy(pCLineInSdp->dNetType.dStr,pConnection->pNetType);

        pCLineInSdp->dNetType.dStrLen = Sdf_mc_strlen(pConnection->pNetType);
        ...
        {
            pCLineInSdp->dNetworkAddress.dAddrType = SDF_CLINE_ADDRESS_IPV4_ADDR;
        }
        ...
        {
            pCLineInSdp->dNetworkAddress.dAddrType = SDF_CLINE_ADDRESS_IPV6_ADDR;
        }
        /*As currently rfc:4566 states that the value of address type in
        ...
        Sdf_mc_strcpy(pCLineInSdp->dNetworkAddress.dAddrVal.dStr,pConnection->pAddr);

        pCLineInSdp->dNetworkAddress.dAddrVal.dStrLen =
            Sdf_mc_strlen(pConnection->pAddr);
    }
...
Sdf_ty_retVal sdf_ivk_uaGetElementFromSessionAttr
    ...
    (SdpMessage 	        *pSdpMessage,
     ...
     Sdf_st_error           *pErr)
#else
    (pSdpMessage,pAttrStr,dType,pErr)
     SdpMessage             *pSdpMessage;
     Sdf_ty_u8bit           *pAttrStr,
     Sdf_ty_sessionAttrType dType,
     Sdf_st_error           *pErr;    
#endif
{
    ...
    switch(dType)
    {
        /*dType value Sdf_en_Origin specify that extract the origin from sdp message and
         * return the same as string*/
        case Sdf_en_Origin:
        {
            Sdf_ty_s8bit        *pTmpStr = Sdf_co_null;
            sdp_getSdpOriginAsString(pSdpMessage,&pTmpStr,
                    (SipError*)&(pErr->stkErrCode));
            if (Sdf_co_null != pTmpStr)
            {
                /*SPR 19455 Copy only 127 bytes from the received line
                  to avoid memory corruption */
                Sdf_mc_strncpy(pAttrStr,pTmpStr,SDF_MAXIMUM_STR_LEN - 1);
                /*Deallocate the memory occupied by pTmpStr*/
                /*Here MemID is passed as ACCESSOR_MEM_ID since at the time of memory
                 * allocation MemId is passed as ACCESSOR_MEM_ID in fast_memget*/
                (void)sdf_memfree(ACCESSOR_MEM_ID,(Sdf_ty_pvoid *)&pTmpStr, \
                                  pErr);                
            }
            else
            {
        ...
        /*dType value Sdf_en_Version specify that extract the Version from sdp message and
         * return the same as string */
        case Sdf_en_Version:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            sdp_getVersion(pSdpMessage,&pTmpStr,
                    (SipError*)&(pErr->stkErrCode));
            if (Sdf_co_null != pTmpStr)
            {
                /*SPR 19455 Copy only 125 bytes from the received line
                  to avoid memory corruption */
                Sdf_mc_strncpy(pAttrStr,pTmpStr,SDF_MAXIMUM_STR_LEN - 3);
            }
            else
            {
                pErr->errCode = Sdf_en_sdpError;
        ...
        /*dType value Sdf_en_Session specify that extract the Session from sdp message and
         * return the same as string */
        case Sdf_en_Session:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            sdp_getSession(pSdpMessage,&pTmpStr,
                    (SipError*)&(pErr->stkErrCode));
            if (Sdf_co_null != pTmpStr)
            {
                /*SPR 19455 Copy only 125 bytes from the received line
        ...
        /*dType value Sdf_en_Info specify that extract the Info from sdp message and
         * return the same as string */
        case Sdf_en_Info:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            sdp_getInfo(pSdpMessage,&pTmpStr,
                    (SipError*)&(pErr->stkErrCode));
            if (Sdf_co_null != pTmpStr)
            {
                /*SPR 19455 Copy only 125 bytes from the received line
        ...
        /*dType value Sdf_en_Uri specify that extract the Uri from sdp message and
         * return the same as string */
        case Sdf_en_Uri:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            sdp_getUri(pSdpMessage,&pTmpStr,
                    (SipError*)&(pErr->stkErrCode));
            if (Sdf_co_null != pTmpStr)
            {
                /*SPR 19455 Copy only 125 bytes from the received line
        ...
        /*dType value Sdf_en_Key specify that extract the Key from sdp message and
         * return the same as string */
        case Sdf_en_Key:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            sdp_getKey(pSdpMessage,&pTmpStr,
                    (SipError*)&(pErr->stkErrCode));
            if (Sdf_co_null != pTmpStr)
            {
                /*SPR 19455 Copy only 125 bytes from the received line
        ...
        default:
        {
            ...
            sdf_fn_dumpError((Sdf_ty_s8bit *)"SDF_ERROR - "
                    "sdf_ivk_uaGetElementFromSessionAttr(): "
                    "Invalid value for parameter dType passed to function");
#endif			
            return Sdf_co_fail;
        }
    }
...
#endif
{
    ...
    switch(dType)
    {
        /*dType value equal to 0 specify that size of email list is to be
          returned.*/
        case Sdf_en_Email:
        {
            (void)sip_listSizeOf(&(pSdpMessage->slEmail),(Sdf_ty_u32bit *)pSize, 
                                 (SipError*)&(pErr->stkErrCode));
            break;
        }
        /*dType value equal to 1 specify that size of phone list is to be
          returned.*/
        case Sdf_en_Phone:
        {
            (void)sip_listSizeOf(&(pSdpMessage->slPhone),(Sdf_ty_u32bit *)pSize,
                                 (SipError*)&(pErr->stkErrCode));
            break;
        }
        /*dType value equal to 2 specify that size of time list is to be 
          returned.*/
        case Sdf_en_Time:
        {
            (void)sip_listSizeOf(&(pSdpMessage->slTime),(Sdf_ty_u32bit *)pSize,
                                 (SipError*)&(pErr->stkErrCode));
            break;
        }
        /*dType value equal to 3 specify that size of bandwidth is to be
          returned.*/
        case Sdf_en_Bandwidth:
        {
            (void)sip_listSizeOf(&(pSdpMessage->pBandwidth),(Sdf_ty_u32bit *)pSize,
                                 (SipError*)&(pErr->stkErrCode));
            break;
        }
        /*dType value equal to 4 specify that size of attribute list is to
          be returned.*/
        case Sdf_en_Attribute:
        {
            (void)sip_listSizeOf(&(pSdpMessage->slAttr),(Sdf_ty_u32bit *)pSize,
                                 (SipError*)&(pErr->stkErrCode));
            break;
        }
        default:
...
#endif
{
     Sdf_ty_u8bit     tempStrLen = 0;
#ifdef SDF_PARAMVALIDATION
	if (Sdf_co_null == pErr)
	{
#ifdef SDF_ERROR	
		sdf_fn_dumpError((Sdf_ty_s8bit *)"SDF_ERROR - "
			"sdf_ivk_uaGetElementFromSessionAttrList(): "
			"Invalid Error parameter passed to function");
#endif			
		return Sdf_co_fail;
	} 
    ...
    switch(dType)
    {
        /*dType value equal to 0 specify that dIndex element of email list is to be
          returned.*/
        case Sdf_en_Email:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            (void)sip_listGetAt(&(pSdpMessage->slEmail),dIndex,(void *)&pTmpStr,
                                (SipError*)&(pErr->stkErrCode));
            Sdf_mc_strcpy(pAttrStr,(Sdf_ty_u8bit *)"e=");
            Sdf_mc_strncpy(pAttrStr + 2, pTmpStr, SDF_MAXIMUM_STR_LEN - 3);
            Sdf_mc_strcat(pAttrStr,"\0");
            break;
        }  
        /*dType value equal to 1 specify that dIndex element of phone list is to be
          returned.*/
        case Sdf_en_Phone:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            (void)sip_listGetAt(&(pSdpMessage->slPhone),dIndex,(void *)&pTmpStr,
                                (SipError*)&(pErr->stkErrCode));
            Sdf_mc_strcpy(pAttrStr,(Sdf_ty_u8bit *)"p=");
            Sdf_mc_strncpy(pAttrStr + 2, pTmpStr, SDF_MAXIMUM_STR_LEN - 3);
            Sdf_mc_strcat(pAttrStr,"\0");

            break;
        }  
        /*dType value equal to 2 specify that dIndex element of time list is to be
          returned.*/
        case Sdf_en_Time:
        {
            SdpTime     *pSdpTime = Sdf_co_null;
            ...
            (void)sip_listGetAt(&(pSdpMessage->slTime),dIndex,(void *)&pSdpTime,
                                (SipError*)&(pErr->stkErrCode));
           
            Sdf_mc_strcpy(pAttrStr,(Sdf_ty_u8bit *)"t=");
            /*Copy the "start" component uptil max limit - 3 because 3 bytes
             * are reqd for t= and null char.*/
            Sdf_mc_strncpy(pAttrStr + 2,pSdpTime->pStart,SDF_MAXIMUM_STR_LEN - 3);
            
            /*If there is memory available in pAttrStr for two bytes one for
             * null and another for space then concatenate space*/
            tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);
            if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 2))
            {
               Sdf_mc_strcat(pAttrStr," ");
            }
            tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);
            if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 1))
            {   
               /*Copy "stop" parameter values for available bytes*/ 
               Sdf_mc_strncpy(pAttrStr + tempStrLen,pSdpTime->pStop,((SDF_MAXIMUM_STR_LEN -
                           1) - tempStrLen));
            }   

            /*Since the repeat time attribute in SDP ie r= are always
             * associated with particular t attribute, append the repeat time
             * field with comma seperating the two*/
            (void)sip_listSizeOf(&(pSdpTime->slRepeat), &tsize,
                                 (SipError*)&(pErr->stkErrCode));
            if (tsize > 0)
            {
                tindex = 0;
                /* Fix For SPR:20154*/
                /* Traverse all the r values. After the UATK fix shared by us
                 * for CSR-1-7909949 stack will store all the r values received
                 * in sip message.*/
                while (tindex < tsize)
                {
                    Sdf_ty_s8bit   *pRepeat = Sdf_co_null;         

                    tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);   
                    if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 2))
                    {   
                       Sdf_mc_strcat(pAttrStr,(Sdf_ty_u8bit *)",");
                    }   
                    tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);   
                    (void)sip_listGetAt(&(pSdpTime->slRepeat), tindex, (void *)&pRepeat,
                                    (SipError*)&(pErr->stkErrCode));
                    /*Copy SDF_MAXIMUM_STR_LEN - 3 - tempStrLen because
                      3 bytes are reserved for r= and null */
                    if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 3))
                    {   
                       Sdf_mc_strcat(pAttrStr,(Sdf_ty_u8bit *)"r=");
                       Sdf_mc_strncpy(pAttrStr + tempStrLen + 2,pRepeat,((SDF_MAXIMUM_STR_LEN -
                           3) - tempStrLen));
                    }   
                    tindex++;
                }
            ...
            /*Fix For SPR:20154*/
            if(Sdf_co_null != pSdpTime->pZone)
            {
                tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);   
                /*If there is memory available in pAttrStr for two bytes one for
                 * null and another for "," then concatenate space*/                
                if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 2))
                {   
                    Sdf_mc_strcat(pAttrStr,(Sdf_ty_u8bit *)",");
                }   
                tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);
                /*Copy SDF_MAXIMUM_STR_LEN - 3 - tempStrLen because
                  3 bytes are reserved for z= and null */
                if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 3))
                {   
                    Sdf_mc_strcat(pAttrStr,(Sdf_ty_u8bit *)"z=");
                    Sdf_mc_strncpy(pAttrStr + tempStrLen + 2,pSdpTime->pZone,
                            ((SDF_MAXIMUM_STR_LEN - 3) - tempStrLen));
                }   
            }
            tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);
            if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 1))
        ...
        /*dType value equal to 3 specify that dIndex element of attribute list is to be
          returned.*/
        case Sdf_en_Attribute:
        {
            SdpAttr *pTmpAttr = Sdf_co_null;
            (void)sip_listGetAt(&(pSdpMessage->slAttr),dIndex,
                                (Sdf_ty_pvoid *)&pTmpAttr,
                                (SipError*)&(pErr->stkErrCode));
            Sdf_mc_strcpy(pAttrStr,(Sdf_ty_u8bit *)"a=");
            if (Sdf_co_null != pTmpAttr->pName)
            {
                Sdf_mc_strncpy(pAttrStr + 2, pTmpAttr->pName, SDF_MAXIMUM_STR_LEN -
                        3);
            }
            if (Sdf_co_null != pTmpAttr->pValue)
            /*Since attribute can be <attribute:value> or <attribute> itself */
            {
               ...
               {
                   Sdf_mc_strcat(pAttrStr,":");
                   Sdf_mc_strncpy(pAttrStr + 1 + tempStrLen, pTmpAttr->pValue,
                        (SDF_MAXIMUM_STR_LEN - 2) - tempStrLen);
               }
            }	            
            tempStrLen = Sdf_mc_strlen((Sdf_ty_s8bit *)pAttrStr);   
            if(tempStrLen < (SDF_MAXIMUM_STR_LEN - 1))
        ...
        /*dType value equal to 0 specify that dIndex element of bandwidth list is to be
          returned.*/
        case Sdf_en_Bandwidth:
        {
            Sdf_ty_u8bit        *pTmpStr = Sdf_co_null;
            (void)sip_listGetAt(&(pSdpMessage->pBandwidth),dIndex,(void *)&pTmpStr,
                                (SipError*)&(pErr->stkErrCode));
            Sdf_mc_strcpy(pAttrStr,(Sdf_ty_u8bit *)"b=");
            Sdf_mc_strncpy(pAttrStr + 2, pTmpStr, SDF_MAXIMUM_STR_LEN - 3);
            Sdf_mc_strcat(pAttrStr,"\0");
