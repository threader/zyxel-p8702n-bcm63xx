 ...
 * 13-JUN-2007  Jagmohan Chauhan   No              Initial
                Rishi Saxena
 * 26-MAR-2008  Jagmohan Chauhan   No             SPR FIX 18296
 *                                                Failure in vldt module when media 
 *                                                transport mode is given as RTP/SAVP 
 *                                                in cms resp
                                         ...
                                         INOUT icf_vldt_err_st *p_vldt_err)
{
    ...
    */
        ...
        (p_sdp_stream->bit_mask & ICF_SDP_STREAM_MEDIA_STREAM_PRESENT))
     {
         ...
         if(ICF_FAILURE != ret_val)
         {
            /* Fix for SPR 18296.Previously it contained the check for transport mode in 
               sdp stream,which was wrong.Like it checked for mode as TLS,UDP,TCP or both.  
               Now the check has been done for media  transport type present in sdp stream 
               (UDP,TCP,RTP_AVP,UDPTL or SAVP)  by calling the function 
               icf_vldt_icf_media_transport_type_et.This function checks for media transport
               type*/

             ret_val= icf_vldt_icf_media_transport_type_et(
                               p_sdp_stream->trans_mode,p_vldt_err);
             if(ICF_FAILURE == ret_val)
             {
                 /* Add error string to p_vldt_err structure */
                 icf_vldt_port_strcat(p_vldt_err->err_str,(icf_uint8_t *)"video_profile:" );
                                      ...
                                      INOUT icf_vldt_err_st *p_vldt_err)
{
   ...
   */
    ...
    if(ICF_SUCCESS == ret_val)
    {
        ...
        {
           /*Merged CSR 1-7973336. Fix for CSR 1-8561947.*/
           if(p_codec_id->codec_id > 128)
           {
               ret_val = ICF_FAILURE;
