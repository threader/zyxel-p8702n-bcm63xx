 ...
 * 24-Jan-2008      Kamini Gangwani Rel 8.0     Modified Function icf_mmi_handle_mms_failure_resp
 * 24-Jan-2008      Abhishek Dhammawat Rel8.0   Modified
 *                                              icf_mmi_04_pending_req
...
)
{
	...
	p_mms_resp->error_cause = p_recv_msg_payload->error_cause;
        ...
        /*Rel 8.0 :Check if the error cause is present print the trace
	...
	for (count =0; (count < p_mms_resp->mod_stream_count) && (count < ICF_MAX_STREAM_PER_CALL); count++)
	{	
	...
	if (p_mmi_ctxt->ongoing_event == ICF_MMI_MMS_MOD_ONGOING)
	{
		...
		for (resp_count =0; 
				 (resp_count < p_mms_resp->add_stream_count) && (resp_count < ICF_MAX_STREAM_PER_CALL);
				 resp_count++)
		{
...
)
{
					...
					ICF_MODIFY_MEDIA_RESP_REJECT_STREAM_PRESENT)
	{
		...
		for(stream_count =0; 
			(stream_count <p_recv_msg_payload->rejected_stream_list.count) && (stream_count < ICF_MAX_STREAM_PER_CALL);
			stream_count++)
		{
			...
			while ((resp_count < p_mms_resp->mod_stream_count) && (resp_count < ICF_MAX_STREAM_PER_CALL))
			{
	...
	for(resp_count = 0; (resp_count < p_mms_resp->mod_stream_count) && (resp_count < ICF_MAX_STREAM_PER_CALL);
		resp_count++)
	{
		accpt_stream_id = 
	...
	while ((resp_count < p_mms_resp->mod_stream_count) && 
			(p_accpt_list_hdr) && (resp_count < ICF_MAX_STREAM_PER_CALL))
	{
		p_stream_node = ICF_CMN_GET_DATA_NODE_FOR_LIST(p_accpt_list_hdr);
