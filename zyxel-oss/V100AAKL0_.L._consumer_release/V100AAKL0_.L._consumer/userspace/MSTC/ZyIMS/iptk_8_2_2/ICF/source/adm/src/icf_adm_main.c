...
* 03-Apr-2008    Sumant Gupta                              Windows Porting Merging
* 25-Aug-2009    Anuradha Gupta   SPR 19969              Modified icf_adm_process_mesg
*
    ...
    if (ICF_EXTERNAL_API_END >= p_internal_msg->msg_hdr.msg_id)
    {
		...
		{			
			...
			if(ICF_GENERIC_APP_ID == app_id)
			{
                ...
                /* SPR 19969: Added NUL check for p_config_data so that
                 * in case it is not present then there is no NULL pointer 
                 * access.
                 */ 
                ...
                else if(ICF_NULL != p_config_data->p_generic_routing_info)
                {
            ...
            else
			{
                ...
                if(ICF_FALSE == p_glb_pdb->p_glb_data->recvd_restart_reinit_event)
                {
                    ...
                    /* SPR 19969: In case config data is not present then 
                     * no trace is issued as it requires information from config data
                     */ 
                   if((ICF_FAILURE == ret_val) && (ICF_NULL != p_config_data))
		    	    {
#ifdef ICF_TRACE_ENABLE
