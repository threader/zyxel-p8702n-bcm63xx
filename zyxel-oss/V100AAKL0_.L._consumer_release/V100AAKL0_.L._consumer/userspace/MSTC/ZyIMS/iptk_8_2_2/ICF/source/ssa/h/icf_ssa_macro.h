 ...
 * 14-Mar-2008  Divij Agarwal  SPR 18122            Support for Asyn AKA Enh
 * 04-Mar-2009  Saurabh Gupta  SPR 19349            Klocworks fix
 * 19-Mar-2009  Tarun Gupta    SPR 19400            Modified ICF_SSA_ADD_MEM_CHUNK_TO_PTR_LIST 
 * 02-Jun-2009  Rajiv Kumar    SPR 19666            Modified ICF_SSA_RESET_SSA_TIMER_BLK 
...
{\
    icf_uint8_t  count = 0, max = p_ssa_pdb->alloc_index;\
        ...
        ICF_NULL;\
        for ( count =0 ; (count < max) && (count < ICF_SSA_MAX_MMRY_CHUNK_LST);)\
        {\
...
/* Fix for SPR 19400 : Changed the condition from >= to <=. Otherwise
   the else condition of the below macro would never get hit and 
   the p_ssa_pdb->alloc_index would never get incremented.
   As a result failure would be always returned to the calling
   function.
*/   
...
{\
    icf_uint8_t  state = p_ssa_pdb->\
		...
		if ((state < ICF_SSA_TCPM_MAX_STATES) && ((event) < ICF_SSA_TCPM_MAX_EVENTS))\
		{\
...
/*------------------------------------------------------------------------------
 *
 * Macro Name       : ICF_SSA_GET_METHOD_NAME
 * Description      : Converts SIP method name from string format to corresponding
 *                      # define
 *---------------------------------------------------------------------------*/
#define ICF_SSA_GET_METHOD_NAME(p_ssa_pdb,sip_method_name)\
{\
    if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"REGISTER"))\
    {\
        sip_method_name = ICF_METHOD_REGISTER;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"INVITE"))\
    {\
        sip_method_name = ICF_METHOD_INVITE;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"SUBSCRIBE"))\
    {\
        sip_method_name = ICF_METHOD_SUBSCRIBE;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"UPDATE"))\
    {\
        sip_method_name = ICF_METHOD_UPDATE;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"BYE"))\
    {\
        sip_method_name = ICF_METHOD_BYE;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"MESSAGE"))\
    {\
        sip_method_name = ICF_METHOD_MESSAGE;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"PUBLISH"))\
    {\
        sip_method_name = ICF_METHOD_PUBLISH;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"REFER"))\
    {\
        sip_method_name = ICF_METHOD_REFER;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"INFO"))\
    {\
        sip_method_name = ICF_METHOD_INFO;\
    }\
    else if(ICF_NULL == icf_port_strcmp((icf_uint8_t*)p_ssa_pdb->p_method,(icf_uint8_t*)"OPTIONS"))\
    {\
        sip_method_name = ICF_METHOD_OPTIONS;\
    }\
    else\
    {\
        *(p_ssa_pdb->p_ecode) = ICF_CAUSE_INVALID_METHOD_TYPE;\
        return ICF_FAILURE;\
    }\
}
