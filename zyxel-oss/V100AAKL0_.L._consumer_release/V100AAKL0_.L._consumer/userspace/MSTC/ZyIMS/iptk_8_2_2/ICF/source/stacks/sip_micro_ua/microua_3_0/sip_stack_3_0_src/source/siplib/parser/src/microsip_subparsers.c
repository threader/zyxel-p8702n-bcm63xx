		...
		SIP_S8bit **ppHost, SIP_U16bit **ppPort, SipError *pErr)
{
	...
	SIP_S8bit *tmp = SIP_NULL;
    /*CSR 1-7527851 Change Start*/
    if(pEnd < pStart)
    {
        sip_error(SIP_Minor,"Host/port not present\n");
        return SipFail;
    }
    /* CSR 1-7527851 Change Start */	
	...
	if (*pStart == '[')
	{
		...
		*/
                ...
                /* CSR 1-7002108 : Validate host, if ASCII value is between 
                   65 & 90 or 97 & 122 then return success fix start */ 
                SIP_S8bit *validateHost = SIP_NULL;
                SIP_U16bit dotCount = 0;
                en_SipBoolean IsAlphabet = SipFail; 
                validateHost = pStart;
                /* CSR 1-7527851 Change Start */
                while (validateHost <= pEnd)
                {
                /* CSR 1-7527851 Change End */
                    if (!IsAlphabet)
                    {
                        if (((65 <= *validateHost) && (*validateHost <= 90)) || 
                            ((97 <= *validateHost) && (*validateHost <= 122)))
                            IsAlphabet = SipSuccess;
                    }  
                    if (*validateHost == '.')
                        dotCount++;
                ...
                if ((SipSuccess != IsAlphabet) && (dotCount != 3))
                {  
                    sip_error(SIP_Minor,"Error in Parsing HostName/IPv4\n");
                    goto error;
                }
                /* CSR 1-7002108 fix end */
...

{
	...
	SIP_S8bit		*pCurrentPos = SIP_NULL;
        ...
        /* CSR 1-7002108 fix start */ 
        SIP_S8bit   *pSemiColonCurrentPos =SIP_NULL;
        SIP_S8bit   *pATPos = SIP_NULL;
        /* CSR 1-7002108 fix end */
	...
	if (tmp == SIP_NULL)
	{
		 ...
		 */
               ...
               /* CSR 1-7002108: if pATPos is not NULL then set pCurrentPos to 
                  sip_tokenize(pATPos,pEnd,';') else set it to pSemiColonCurrentPos */
               pSemiColonCurrentPos = sip_tokenize(pStart, pEnd, ';');
               pATPos = sip_tokenize(pStart,pEnd,'@');
               if (SIP_NULL != pATPos)
                   pCurrentPos = pSemiColonCurrentPos < pATPos ? sip_tokenize(pATPos,\
                   pEnd,';'):pSemiColonCurrentPos;
               else
                   pCurrentPos = sip_tokenize(pStart, pEnd, ';');
               /* CSR 1-7002108 fix end */
	 ...
	 */
		...
		if (SIP_NULL == tmpUrl->pHeader)
		{
			...
			return SipFail;
               ...
               pSemiColonCurrentPos = sip_tokenize(pStart, tmp-1, ';');
               pATPos = sip_tokenize(pStart,tmp-1,'@');
               if (SIP_NULL != pATPos)
                   pCurrentPos = pSemiColonCurrentPos < pATPos ? sip_tokenize(pATPos,\
                       tmp-1, ';'):pSemiColonCurrentPos;
               else
