 ...
 **									   decodeMessage()
 **
 ** 31-Mar-2009     Kamal Ashraf       SPR 19310         Fix For SPR 19310
 ** 22-May-2009     Kamal Ashraf       SPR 19672         CSR_1_7113244 Merged
 ** 05-Jun-2009     Anuradha Gupta     SPR 19737         Klocworks Warning Removal
 ** 07-Sep-2009     Preksha            SPR 19999         SPR 19813 Merged
 ** 08-Sep-2009     Rajiv Kumar        SPR 19999         Merged CSR 1-7586127 
 ** 09-Sep-2009     Preksha            SPR 19999         SPR 19812 Merged
 ** 14-Sep-2009     Preksha            SPR 19999         SPR 19988 Merged
 ** 22-Sep-2009     Preksha            SPR 19999         SPR 20017 Merged
 ** 10-Oct-2009     Preksha            SPR 20115         CSR 1-7856222
 ** 25-Oct-2009     Rajiv Kumar        SPR 20151         Fix for SPR:20151 
 ** 27-Oct-2009     Rajiv Kumar        SPR 20169         Fix for SPR:20169 
 ** 31-Oct-2009     Rajiv Kumar        SPR 20169         Previous changes
 **                                                      reverted for SPR:20169
 **                                                      and merged UATK fix 
 **                                                      for CSR-1-7813536 
 **
 ****************************************************************************
	...
	en_HeaderType dHdrType, SipHeader *pHeader,  SipError *pError)
{
    ...
    /* Coverity bug fix */
    if( dHdrType >= (sizeof(glbSipParser)/sizeof(*glbSipParser)))
    {
		return SipFail;
    }
...
/*	SIP_S8bit *pTmpString 					= SIP_NULL ;*/
	...
	SIP_S8bit *pHeaderBodyEndIndicator = SIP_NULL ;
	/* CSR No.:1-5139236 Enhancement Change End */
    /*CSR_1-5624614 Start*/
     /* CSR_1-6665946 changes start */
  /* CSR_1-5603443 changes start */
#ifdef SIP_DATE
        en_SipBoolean dateFlag					= SipFalse;
#endif
    /*CSR_1-5624614 End*/	
    /* CSR_1-5603443 changes end */
/* CSR_1-6665946 changes end */
 /* Fix Start CSR 1-7663570 */
	en_SipBoolean exitCondition1 		 	= SipFalse;
 /* Fix End CSR 1-7663570 */
#ifdef SIP_TRACE
	SIP_S8bit *tracetemp;
	SipError traceError;
#endif
   ...
   (void)fast_memfree(DECODE_MEM_ID,tracetemp,SIP_NULL);
    ...
    /* klocworks : NULL checks have been added throughout the function for
     * pCurrentPos, pBeginIndex, pHeaderNameBeginIndex , pMessageBodyBegin
     * pSipMessage , whenever it is accessed
     * and there is a possibility of dereferencing
     */ 

     /* klocworks : On failure legs the pSipHeaderOrderInfo is freed if not 
      * already appended in the SIP Message. The boolean flag check as
      * orderAppended as False and toBeappended as True is added before
      * freeing the memory
      */
	...
	*
	*/
   if ((SIP_NULL == *ppDecodedMsg) && (SIP_NULL != pCurrentPos))
   {
		...
		while ((SIP_NULL != pCurrentPos) && (*pCurrentPos != '\r') && (*(pCurrentPos) != '\n'))
		{
		...
		while((SIP_NULL != pCurrentPos) && (pCurrentPos < (pMessage+dMessageLength)))
		{
			if((*pCurrentPos == '\n')||(*pCurrentPos == '\r'))
				if((*(pCurrentPos+1) != '\n') && (*(pCurrentPos+1) != '\r') &&
		...
		if((SIP_NULL != pCurrentPos) && (pCurrentPos == (pMessage+dMessageLength-1)) && 
		   ((*pCurrentPos == '\n')||(*pCurrentPos == '\r')))
		{
        ...
        else if(SIP_NULL == (pCurrentPos+1))
		{
			*pErr =  E_MAYBE_INCOMPLETE;
			return SipFail;
		}
		...
		if ((SIP_NULL == pCurrentPos) || (*(pCurrentPos+1) == '\t') || (*(pCurrentPos+1) == ' '))
			return SipFail;
	...
	do
	{
		...
		{   
            ...
            while ((SIP_NULL != pCurrentPos) && (pCurrentPos <= (pMessage + dMessageLength)))
			{
				 ...
				 * so break
				 */ 
                 /* Codenomicon issue: Fix Start CSR 1-7663570 */
				if (*pCurrentPos == ':')
                {   
                    SIP_U32bit dLength=0;
                    SIP_S8bit *pLimitaion=SIP_NULL;
                    SIP_S8bit *pTemp=SIP_NULL,*pTemp_Name=SIP_NULL,*pLimitaion1=SIP_NULL;
                    if(pCurrentPos!=SIP_NULL && pBodyEndIndex!=SIP_NULL)
                        dLength=pCurrentPos-pBodyEndIndex;
                    pTemp = (SIP_S8bit *) fast_memget(DECODE_MEM_ID,\
                            dLength+1, pErr);
                    memcpy(pTemp,pBodyEndIndex,dLength);
                    pTemp[dLength]='\0';
                    if(pTemp!=SIP_NULL)
                        pLimitaion = strstr(pTemp,"v=");
                    pTemp_Name = (SIP_S8bit *) fast_memget(DECODE_MEM_ID,\
                            pCurrentPos -pHeaderNameBeginIndex +1, pErr);
                    memcpy(pTemp_Name,pHeaderNameBeginIndex,pCurrentPos -pHeaderNameBeginIndex);
                    pTemp_Name[pCurrentPos -pHeaderNameBeginIndex]='\0';
                    pLimitaion1	= strstr(pTemp_Name,"=");
                    if(pLimitaion!=SIP_NULL && pLimitaion1!=SIP_NULL )
                    {
                        SIP_U32bit dLength1=pLimitaion - pTemp;
                        pBodyEndIndex=pBodyEndIndex+dLength1 -1;
                        pCurrentPos=pBodyEndIndex;
                        exitCondition1 =SipTrue;
                        headerParsingState	= Sip_en_headerBodyExpected ;
                    }
                    (void)fast_memfree(DECODE_MEM_ID, pTemp,pErr);
                    (void)fast_memfree(DECODE_MEM_ID, pTemp_Name,pErr);
                    break;
                }
				/*
                ...
                /* Codenomicon issue: Fix Start CSR 1-7663570 */
                /* If the header name encounters with '\r' and '\n' before
                 * ':' then it is ignored and the header name parsing is
                 * started again with current pos.
                 */ 
                if (((*pCurrentPos == '\r') &&
                        (*(pCurrentPos+1) == '\n')) &&
                         ((*(pCurrentPos+2) != ' ') || (*(pCurrentPos+2) != '\t')))
                ...
                /*Codenomicon issue: Fix End CSR 1-7663570 */
			   ...
			   { 
				   *ppDecodedMsg = pSipMessage ;
				   *pErr = E_SIPINCOMPLETE;
				   /*CSR-1-5567418 fix-Inlined with Rel 7.0 cert changes*/
				   /*
					  if (orderAppended == SipFalse)
					  __sip_freeSipHeaderOrderInfo(pSipHeaderOrderInfo);
					*/
				   /*CSR-1-5567418 fix*/
				   /* klocworks warning fixing, the additional
					* check has been added so that CSR issue
				    * does not resurface again
				    */	
				   if (orderAppended == SipFalse)
				   {
            ...
            if(SIP_NULL != pHeaderNameBeginIndex)
...
/* CSR_1-6665946 changes end */
           ...
           /* CSR_1-6170138 Change Start */
           if(pCurrentPos < (pMessage + dMessageLength))
             /* CSR_1-6170138 Change End */
             pCurrentPos++ ; 
             /* CSR_1-6170138 Change Start */
            else
            {
                /* CSR_1-6241083 Change Start */
                *ppDecodedMsg = pSipMessage ;	
                if (orderAppended == SipFalse)
                {
	                if (toBeappended == SipTrue)
		...
		{  
			...
			if (hdrChkFlag)
			{
				 ...
				 */	
					wrongClass = SipTrue;
					__sip_freeSipHeaderOrderInfo(pSipHeaderOrderInfo);
                    /* CSR_1-7556819 Fix Start */
                    pSipHeaderOrderInfo = SIP_NULL;
                    /*CSR_1-7556819 Fix End */
				}	
				hdrChkFlag = SipFalse;	
			}	
...
/*Fix from UATK 2.1 End*/
                ...
                *ppDecodedMsg = pSipMessage ;
                ...
                /* CSR_1-7556819 Fix Start */
                if (orderAppended == SipFalse && wrongClass != SipTrue)
                /*CSR_1-7556819 Fix End */
                /* CSR_1-7813536 START */
                {	
                /* CSR_1-7813536 END */
        ...
        /*SPR- 19314 fixed*/
       /*CSR No.:1-5139236 Enhancement Change Start */
           pHeaderBodyEndIndicator = pBodyEndIndex;
		   while((*pHeaderBodyEndIndicator != '\n') && \
                  ((*pHeaderBodyEndIndicator != '\r') && \
                   (*(pHeaderBodyEndIndicator+1) != '\n')))
           {
       	       pHeaderBodyEndIndicator++;
       	       if(pHeaderBodyEndIndicator >= (pMessage+dMessageLength))
       	       {
         	     *ppDecodedMsg = pSipMessage;
         	     *pErr = E_SIPINCOMPLETE;
                 /* CSR_1-7059757 Changes Starts */
                 if (orderAppended == SipFalse && wrongClass != SipTrue)
                 /* CSR_1-7059757 Changes Ends */
                 /* CSR_1-7813536 START */
                 {	
                 /* CSR_1-7813536 END */
                     __sip_freeSipHeaderOrderInfo(pSipHeaderOrderInfo);
...
/*Fix from UATK 2.1 End*/
                    ...
                    *ppDecodedMsg = pSipMessage ;
                    ...
                    /* CSR_1_7113244 Mreged SPR 19672 Start */
                    if (orderAppended == SipFalse && wrongClass != SipTrue)
                    /* CSR_1_7113244 Mreged SPR 19672 End */
                    /* CSR_1-7813536 START */
                    {	
                    /* CSR_1-7813536 END */
                        __sip_freeSipHeaderOrderInfo(pSipHeaderOrderInfo);
...
/*Fix from UATK 2.1 End*/
            ...
            /*CSR 1-6665946 changs Start*/
            /*CSR 1-6089510: CSR_1-5603443 */
	    /* CSR_1-6089510 Change Start */
            if((dateFlag == SipTrue) && (((*pBodyEndIndex) == '\r' && (*(pBodyEndIndex+1)) == '\n') || ((*pBodyEndIndex) == '\n')))
            {
              dateFlag = SipFalse;
              continue;
            }
            /*CSR 1-6089510: CSR_1-5603443 End*/
            /* CSR_1-6665946 changes end */

            /* Fix for CSR-1-5624614 Start*/
            /* CSR_1-6665946 changes start */
            /*CSR_1-5603443 changes Start */
            if ((dateFlag ==  SipTrue)&& ((*pBodyEndIndex) == ','))
            {
                pBodyEndIndex++;
                dateFlag = SipFalse;
                continue ;
            }
            /* Fix for CSR-1-5624614 End */
            /*CSR_1-5603443 changes end */
            /* CSR_1-6665946 changes end */
#endif /* SIP_DATE */
			/*
            ...
            if ((isAuth == SipFalse)  && (*pBodyEndIndex) == ',')	
			{
				...
				else if(SIP_NULL != pHeaderNameBeginIndex)
				{
#ifdef SIP_MIME_PARSING
				...
				if (wrongClass)
				{
					...
					else
					{
						...
						pCurrentPos = pBodyEndIndex;
                                  ...
                                  /*End of handling Wrong Class here*/
				else if(SIP_NULL != pHeaderNameBeginIndex)
				{	
#ifdef SIP_MIME_PARSING
					if (SipFail == sip_handleIncParser \
...
THis is the duplicate code this code is alredy written few lines above */
    ...
    /*CSR-1-5654540 changes start*/
	if ((SIP_NULL != pSipMessage) && (SIP_NULL != pSipMessage->pGeneralHdr->pContentTypeHdr))
	{
		SIP_S8bit *pMediaType;

		pMediaType = pSipMessage->pGeneralHdr->pContentTypeHdr->pStr1;
		if(SIP_NULL != pMediaType ) 
		{
			if ((0 == sip_strcmp(pMediaType, (SIP_S8bit *) "application/sdp")) ||
			    (0 == sip_strcmp(pMediaType, (SIP_S8bit *) "multipart/mixed")) ||
			    (0 == sip_strcmp(pMediaType, (SIP_S8bit *) "message/sipfrag")) ||
			    (0 == sip_strcmp(pMediaType, (SIP_S8bit *) "application/isup")))
			{
      /*CSR-1-5654540 changes end*/
				while((SIP_NULL != pMessageBodyBegin ) && 
                     ((*pMessageBodyBegin == ' ' )||
				          (*pMessageBodyBegin == '\t')))	
	...
	if ((SIP_NULL != pSipMessage) && (pSipMessage->pGeneralHdr->pContentLengthHdr == SIP_NULL))
	{
		/*
...
/*CSR 1-4735601*/
	...
	if (SipTrue == mimeHdrParsing)
	{
		...
		/* free the previously allocated memeory if any
		 * before assigning new pointer to ppNextMsg
	     */	 
		if(*ppNextMsg != SIP_NULL)
        {
	...
	if (ParseBody)
	{
       ...
       /*CSR-1-5654540 changes end*/	
       /*TBD */
                     if ( (SIP_NULL != pMessageBodyBegin) && (*pMessageBodyBegin == '\0' )&&\
		...
		else
		{
			...
			if ((SIP_NULL == pMsgBody->u.pUnknownMessage->pBuffer) || (SIP_NULL == pMessageBodyBegin))
			{
				sip_freeSipMsgBody(pMsgBody);
                /* CSR_1-7734248 START*/
                /*sip_freeSipMessage(pSipMessage);*/
	...
	{
        /* CSR_1-7734248 START*/
        /* IPTK SPR 20085 fix start */
        sip_freeSipMessage(pSipMessage);
        /* *ppDecodedMsg = pSipMessage ; */
        /* IPTK SPR 20085 fix end */
        /* CSR_1-7734248 END*/
    	...
    	switch (*pErr)
        {
		   ...
		   if ((pOpt->dOption & SIP_OPT_BADMESSAGE) == SIP_OPT_BADMESSAGE)
...
#endif
			   {
            	...
            	break;
            case E_NO_MEM:
                /* CSR_1-7566960 Change Start */
#ifdef SIP_STATISTICS
                INC_ERROR_MEM
#endif
                if ( SIP_NULL != pTempMessage )
                {   
                    /* CSR_1-7734248 START*/
                    /*sip_freeSipMessage(pTempMessage);*/
                    *ppDecodedMsg = pTempMessage ;
...
SipBool Sip_parseMsrpUrl(SIP_S8bit  *pBuf, MsrpUrl *pMsrpUrl, SipError *pErr)
{
...
#endif
	{
		...
		{
			sip_error(SIP_Minor,"pErr buffer is NULL\n"); 
		}
		return SipFail;
	}
