 ...
 ** 27/02/09    Alok Tiwari    ICF Rel8.1    Changes done to replace the 
 **                                          use of open source Digest with
 **                                          aDigest utility.
 ** 22/04/09    Rajiv Kumar    ICF Rel8.2    RFC 4320 Implementation added. 
 ** 30/04/09    Rajiv Kumar    ICF Rel8.2    Fix for SPR 19531.
 ** 12/06/09    Anuradha Gupta SPR 19737     klocoworks warning removal 
 **
 *****************************************************************************
...
/* klocworks: In case of function call failures , dynamic memeory allocated within
 * the function is freed before returning failure from the function
 */
...
/*CSR No: 1-7013841 fix -commented the ifdef SDF_UAB2B*/        
/*#ifdef SDF_UAB2B*/
...
#endif
{
    ...
    /* Implemented for RFC 4320 */
    /* Chek whether it's a non-invite transaction */
    if(Sdf_mc_strcmp(pMethod,"INVITE")!=0)
    {
        if(180 == dRespCode)
        {
            /* Don't send 180 response for a non-invite transaction 
             * skip the function */
#ifdef SDF_ERROR			
            sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_formResponseError, \
                    (Sdf_ty_s8bit*)"sdf_ivk_uaFormResponse( ): "
                    "RFC 4320 implementation: " "180 response for a non-invite transaction is not allowed", pError);
#endif			
            pError->errCode=Sdf_en_responseForNITNotAllowed;
            return Sdf_co_fail;
        }
        else if((100 == dRespCode) && (Sdf_co_null != pOverlapTransInfo) &&
                (Sdf_co_null != pOverlapTransInfo->pDestInfo) &&
                (Sdf_en_protoUdp == pOverlapTransInfo->pDestInfo->dScheme))
        {
            /* Not send 100 ringing response for non-invite txn in unreliable transport*/
#ifdef SDF_ERROR			
            sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_formResponseError, \
                    (Sdf_ty_s8bit*)"sdf_ivk_uaFormResponse( ): "
                    "RFC 4320 implementation: " "100 response(on UDP) for a non-invite transaction is not allowed", pError);
#endif			
            pError->errCode=Sdf_en_responseForNITNotAllowed;
            return Sdf_co_fail;
        }
        else if(408 == dRespCode)
        {
            /* Don't send 408 response for a non-invite transaction 
             * skip this function */
#ifdef SDF_ERROR			
			sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_formResponseError, \
					(Sdf_ty_s8bit*)"sdf_ivk_uaFormResponse( ): "
				"RFC 4320 implementation: " "408response for a non-invite transaction is not allowed", pError);
#endif			
			pError->errCode=Sdf_en_responseForNITNotAllowed;
			return Sdf_co_fail;
        }
...
Sdf_ty_retVal sdf_ivk_uaUpdateDestinationInfo
	...
	Sdf_st_error *pError)
{
    /* klocworks: Since event context field was passed as NULL
     * so a warning of NULL pointer dereferencing was reported.
     * As a fix a local Event context is declared and initialised
     * and passed to the funciton. After the return from the 
     * function sdf_fn_uaHandleLookup, the event context has been
     * freed.
     */ 
	...
	if(Sdf_co_fail == (sdf_fn_uaHandleLookup (pObject, pOverlapTransaction,
			pMessage, pEventContext ,pError)))
	{
...
#endif
{
	...
	*ppDestDetails = (Sdf_st_destDetails *) sdf_memget (Sdf_mc_initMemId, \
	...
	if(Sdf_co_null == *ppDestDetails)
	{
#ifdef SDF_ERROR
		sdf_fn_setError(Sdf_en_majorErrors, Sdf_en_noMemoryError,\
			(Sdf_ty_s8bit *)"sdf_ivk_uaGetDestinationDetails(): "
			"ppDestDetails - memory allocation failure", pError);
#endif
		pError->errCode = Sdf_en_noMemoryError;
		return Sdf_co_fail;
	}
	...
	if (dType == SipMessageRequest)
    {
	  ...
	  if(dIsPresentLr == Sdf_co_true)
	   ...
	   if(Sdf_co_null == (*ppDestDetails)->u.pAddrSpec)
	   {
		   sip_freeSipAddrSpec(pAddrSpec);
		   sdf_memfree(Sdf_mc_callHandlerMemId, \
				   (Sdf_ty_pvoid*)&pHost, pError);
...
#endif	
#ifdef SDF_AUTHENTICATE
	sdf_fn_uaDigestCalcHEntity((Sdf_ty_s8bit *)pInString, dLength, *ppOutString);
...
#endif
{
		...
		(SipError*)&(pError->errCode))) == SipFail)
	{
		...
		(void)sdf_ivk_uaFreeHeaderList(pHeaderList);
		(void)sdf_memfree(Sdf_mc_callHandlerMemId,(Sdf_ty_pvoid*)&pIpAddr,&dLocalErr);
		pError->errCode = pError->stkErrCode;
#ifdef SDF_ERROR		
		sdf_fn_setError(Sdf_en_majorErrors, pError->errCode, \
			(Sdf_ty_s8bit*)"sdf_ivk_uaFormWarningHeader( ): "
