 ...
 * 26-Mar-2008   Shruti Thakral  SPR 18283   Klocworks warning removal   
 *                                 & 18303
 * 8-Apr-2008    Sumant Gupta    SPR:18355   Rel7.1 Windows porting Merging
 * 2-May-2008    Amit Sharma     SPR:18453   Reset serv_ended_err if 
 *                                           serv_end_w_payload is set
 * 20-Jun-2008   Anurag Khare    SPR 18684/CSR 1-6032268 Merge
 * 15-Jul-2008   Anuradha Gupta  SPR 18779   Take the backup of message length as well.
 * 04-Mar-2009   Saurabh Gupta   SPR 19349      Klocworks fix
 * 17-Mar-2009  Abhishek Dhammawat Rel 8.1 To correct the SPR 19349 fix
 * 01-Apr-2009   Kamal Ashraf    Rel 8.2     Klocworks Fix 
 * 30-Jul-2009   Tarun Gupta     SPR 19886   Merged CSR 1-7268001
 * 01-Aug-2009   Rajiv Kumar     SPR 19886   Merged CSR 1-7442376
 * 24-Oct-2009  Abhishek Dhammawat SPR 20181 Modified icf_scm_route_message
        ...
        while (ICF_NULL != p_list)
        {
            ...
            if (ICF_MEDIA_STREAM_ACTIVE & p_stream_sdp->stream_status)
            {
                        ...
                        (1 < count))
                {
                    ...
                    /* Fix for SPR: 19886 (Merged CSR 1-7442376)*/    
...
)
{
    ...
    ICF_FUNCTION_ENTER(p_glb_pdb)
	if (( (service_index-1) < ICF_SCM_MAX_SL ) && ( (service_index-1) >= 0) )
	{
		service_bitmask = ((p_pdb->p_scm_data->
    ...
    else
    {
        if (ICF_NULL != p_second_service_ctx)
        {
			/*Fix for CSR 7268001 SPR 19700*/
			if (ICF_INT_MODULE_CC != 
					p_second_service_ctx->p_call_ctx->signalling_ongoing)
			{
            	p_second_service_ctx->p_call_ctx->signalling_ongoing = 
                	ICF_INT_MODULE_INVALID; 
		...
		if (ICF_INT_MODULE_CC != p_pdb->p_service_ctx->p_call_ctx->signalling_ongoing)
		{
        	p_pdb->p_service_ctx->p_call_ctx->signalling_ongoing = 
            ICF_INT_MODULE_INVALID;
		}
		if (ICF_INT_MODULE_CC != p_pdb->p_service_ctx->p_call_ctx->media_ongoing)
		{
       		 p_pdb->p_service_ctx->p_call_ctx->media_ongoing = 
            ICF_INT_MODULE_INVALID;
		}
                ...
                (ICF_STATUS_SERVICE_REQ_FOR_OTHER_CALL & status)))
        {
			...
			if (ICF_INT_MODULE_CC != p_second_service_ctx->p_call_ctx->signalling_ongoing)
			{
             p_second_service_ctx->p_call_ctx->signalling_ongoing = 
                 ICF_INT_MODULE_INVALID;
			}
			if (ICF_INT_MODULE_CC != p_second_service_ctx->p_call_ctx->media_ongoing)
			{
             p_second_service_ctx->p_call_ctx->media_ongoing = 
                 ICF_INT_MODULE_INVALID;
			}
        ...
        {
            if (ICF_NULL != p_pdb->p_msg->p_msg_data)
            {
                p_second_service_ctx= ((icf_service_call_data_st *)
                   (p_pdb->p_msg->p_msg_data))->p_other_call_ctx->p_service_ctx;
            }
                ...
                (ICF_NULL != p_glb_pdb->p_call_ctx->p_service_ctx))
        {
                            ...
                            p_sl_ctx[service_index - 1])
            {
                ...
                { 
                     ...
                     *
                     * No need for NULL check for 
                     * p_pdb->p_service_ctx, as it cannot
                     * be NULL here */
                    if(ICF_NULL == 
                            ((icf_service_ctx_st *)p_glb_pdb->p_call_ctx->
                             p_service_ctx)->active_service_list.\
    ...
    /*If ICF_STATUS_SERVICE_ENDED_ERR_WITH_PAYLOAD and ICF_STATUS_SERVICE_ENDED_ERROR
     *both set in status, then reset ICF_STATUS_SERVICE_ENDED_ERROR, as CM in that
     *case ignores ICF_STATUS_SERVICE_ENDED_ERR_WITH_PAYLOAD, which is required to clear
     *both the calls*/
    if (ICF_STATUS_SERVICE_ENDED_ERR_WITH_PAYLOAD & p_pdb->p_msg->msg_hdr.status)
    {
    ...
    if(ICF_TRUE == send_to_pa)
    {
        ...
        switch (*p_ecode)
        {
            ...
            case ICF_CAUSE_INVALID_CALL_ID:
                ...
                break;
           /*  CSR 1-6032268 Merge */ 
            case ICF_CAUSE_CALL_MERGED_DUE_TO_JOIN:
                ecode = ICF_CAUSE_CALL_MERGED_DUE_TO_JOIN; 
                break;
            default:
                break;                
        } /* switch (*p_ecode) */
        ...
        )
{
    ...
    ICF_SCM_GET_INDEX_FROM_SERVICE_MODULE_ID(handler_module,
            handler_module_index);
    /*If the message id is valid then corresponding service handler module shall
      be returned and below check hold valid but incase the handler module
      id is invalid it's not required to go further but the value returned
      should be success and not failure. This was to fix issue of sending BYE
      towards network when CHR is ongoing and reinit API is received*/
	  #if 0
	if ( ( (handler_module_index - 1 ) >= ICF_SCM_MAX_SL ) || ( (handler_module_index - 1 ) < 0))
	{
		return ICF_SUCCESS; 
	}
	#endif
    ...
    switch(p_pdb->msg_id)
    {
        ...
        case ICF_INTERNAL_REFER_REQ:
                ...
                if (ICF_TRUE == is_valid)
                {
                    ...
                    if ( ( ICF_SCM_MAX_SL <= (handler_module_index - 1 ) ) || ( (handler_module_index - 1 ) < 0))
                    {
                        ret_val = ICF_FAILURE; 
                        break;
                    }
        ...
        case ICF_CC_CALL_RELEASED:
             {                    
                ...
                icf_void_t    *p_data = ICF_NULL;/*Variable is used to save p_msg_data*/
                icf_uint32_t  msg_len = 0; /* variable is used to save the length of
                                             p_msg_data */  
                ...
                if (ICF_NULL != p_pdb->p_service_ctx)
                {
                    for(iterator=0 ; iterator< ICF_SCM_MAX_SL; iterator++)
                    {
                                ...
                                service_bitmask)
                        {
                            ...
                            ret_val = 
                                (p_scm_data->scm_sl_info[iterator]).
                                msg_handler(p_pdb->p_msg);

                            /*Service ended error and both calls or other call is set and 
                             *p_data is not already set by some other service module*/
                            if  (((ICF_STATUS_SERVICE_APPLY_BOTH_CALLS & p_pdb->p_msg->msg_hdr.status)||
                                   (ICF_STATUS_SERVICE_REQ_FOR_OTHER_CALL & p_pdb->p_msg->msg_hdr.status)) && 
                                  (ICF_STATUS_SERVICE_ENDED_ERROR & p_pdb->p_msg->msg_hdr.status) && 
                                  (ICF_NULL != p_pdb->p_msg->p_msg_data) && (ICF_NULL == p_data))
                            {
                                p_data = p_pdb->p_msg->p_msg_data;
                                /* save the message length as well */
                                msg_len = p_pdb->p_msg->msg_hdr.payload_length;
                            }
                            ...
                            /*This loop should be break when return failure
                             *or Service context is freed by process_return_status
                             *function, which indicates that no more services are
                             *pending which should be sent call released*/
                            if ((ICF_FAILURE == ret_val) ||
                ...
                }/* end if NULL == p_pdb->p_service_ctx */

                /*If service ended w payload is set then reset service ended error, as CM do not invoke
                 *generic error for both calls if ICF_STATUS_SERVICE_ENDED_ERROR is set.*/
                if (ICF_STATUS_SERVICE_ENDED_ERR_WITH_PAYLOAD & status)
                {
                    status &= ~ICF_STATUS_SERVICE_ENDED_ERROR;
                }
                /*Copy p_data into p_msg_data  and message length 
                 * so that CM can send generic error bot both calls*/
                if (ICF_NULL != p_data)
                {
                    p_pdb->p_msg->p_msg_data = p_data;
                    p_pdb->p_msg->msg_hdr.payload_length = msg_len;
                }
                p_pdb->p_msg->msg_hdr.status = status;
                route_message_to_handler_module = ICF_FALSE;
	...
	/*If msg is external conference req then call xconf module*/
	    if (ICF_USER_INITIATE_EXTERNAL_CONF_REQ == p_pdb->msg_id) 
	    {
            /*SPR 20181 Moved the klocworks warning spr 19349 fix
              below so that the second else if part is not bypassed. 
              Additionally this was creating the forceful call clear
              when after establishing the xconf call call modification 
              was happening. This function was returning failure due to below
              check and therefore causing the issue.*/
		    if ( ( (handler_module_index - 1 ) >= ICF_SCM_MAX_SL ) || ( (handler_module_index - 1 ) < 0))
		    {
			    return ICF_FAILURE; 
		    }
        ...
        /* Check if service and service logic context exists */
                 ...
                 && (ICF_SCM_MAX_SL >= handler_module_index)
                 && (ICF_NULL != p_pdb->p_service_ctx->\
