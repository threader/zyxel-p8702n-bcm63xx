...
* 01-Sep-2009    Anuradha Gupta  SPR 19999       Merged CSR 1-7489962
*
        ...
        INOUT   icf_cta_pdb_st   *p_pdb)
{
    ...
    icf_glb_pdb_st            *p_glb_pdb = ICF_NULL;
    icf_error_t               error_cause = ICF_ERROR_NONE;
    ...
    {
        /*  SPR - 19999: Fix for CSR 1-7489962*/
        ...
        {
            /* CSR1-5634872: SPR 18160: Handling of 401/407 enhancement
	     * Send the error cause as filled by SSA as a result of 401/407
	     * failure
	     */
            if((401 == p_refer_resp->response) || (407 == p_refer_resp->response))
            {
                error_cause = p_refer_resp->error_cause;
            }
            else
            {
                error_cause = ICF_CAUSE_REMOTE_REJECT_CALL_TRANFER_REQ;
            }
            /*Send failure CTA response to PA*/
            ret_val = icf_cta_send_cta_resp(p_pdb,
                    (icf_boolean_t)(ICF_FAILURE),
        ...
        INOUT   icf_cta_pdb_st    *p_pdb)
{
    ...
    {
        /*If call with party B is already released then return service_ended_error
