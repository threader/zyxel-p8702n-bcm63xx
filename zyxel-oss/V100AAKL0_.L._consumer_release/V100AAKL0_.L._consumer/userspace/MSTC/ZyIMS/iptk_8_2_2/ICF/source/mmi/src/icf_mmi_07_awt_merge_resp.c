 ...
 * 21-Jun-2008      Jagmohan Chauhan               CSR merge 1-5945413
 * 26-May-2008      Anuradha Gupta   SPR 19672     CSR merge 1-6667668
 *
        ...
        INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
    ...
    /* 
     * This piece of code would ensure the following
     * 1) The other call in conference may also be in releasing phase and if cc
     ...
     * 2) If BYE was received for the call on which REFER was received from
     *    network CC would have initiated DMS request for the call which 
     *    would have been ignored so send DMS request now.
     */
	if((ICF_CC_07_AWT_DMS_RESP == p_dbm_call_ctx->call_state) && 
             (ICF_SUCCESS == p_recv_msg_payload->result))
	{
		/* Set the call context in mmi pdb to the call context of the other
		 * call in conference */
		p_mmi_pdb->p_call_ctx = p_dbm_call_ctx;

		p_mmi_pdb->p_mmi_ctx = &(p_mmi_pdb->p_call_ctx->mmi_context);
		p_mmi_pdb->p_mmi_ctx->ongoing_event = ICF_MMI_DMS_ONGOING;
        ...
        else if(ICF_CC_13_WAIT_START_PROC_MEDIA == p_dbm_call_ctx->call_state)
        {
             /* Merge CSR 1-5945413 : SPR 18684 : If there is any pending 
              * request on other call in conference due to media ongoing 
              * then send  the trigger to the other call to start 
              * processing the media */

              if (p_cm_internal_msg) 
			  { 
              p_mmi_pdb->p_glb_pdb->p_call_ctx = p_dbm_call_ctx;
			  p_cm_internal_msg->msg_hdr.msg_id = ICF_START_PROC_MEDIA; 
              ret_val = icf_cm_process_mesg(p_cm_internal_msg);
			  }
        }
