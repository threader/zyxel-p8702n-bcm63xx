...
** 5-Mar-2008          Alok tiwari        Rel 8.0    Memory in use fixed 
**                                                   allocated to 
**                                                   p_peer_name on TLS
** 03-Apr-2008         Tarun Gupta        SPR 18355  Merged Windows Porting Changes
** 06-May-2008         Alok tiwari        SPR 18503  Modified function
**                                                   icf_ssa_init_conn_db 
** 11-May-2008         Amit Sharma        SPR:18512  Fix For Vxwork TCP Load
** 14-May-2008         Amit Sharma        SPR:18546  Check for self_addr_per_line
**                                                   and fetch address from proper data 
** 17-May-2008         Amit Sharma        SPR:18577  Assigning APP ctxt to glb_pdb
** 29-May-2008         Tarun Gupta        SPR 18585  Modified icf_ssa_tcpm_01_fail_open_conn_resp
** 24-June-2008        Jagmohan Chauhan   SPR 18684  CSR Merge 1-6048221
** 27-JUn-2008         Anurag Khare       SPR 18743  Fix is done in 
**                                                   icf_ssa_tcpm_01_succ_open_conn_resp
** 02-july-2008        Anurag Khare       SPR 19756  Fix is done in sdf_cbk_uaSendMsgOnNetwork
**                                                   icf_ssa_tcpm_00_connect_req
** 08-July-2008        Anuradha Gupta     SPR 18782  Klocwork warning removal
** 17-July-2008        Anuradha Gupta     SPR 18767  UMR removal
** 18-Jul-2008      Abhishek Dhammawat    SPR 18753 Modified
**                                                  icf_ssa_handle_network_send_failure
** 04-Nov-2008      Abhishek Dhammawat    SPR 19189 SPR 18846 merge
** 18-Nov-2008      Rajiv Kumar           SPR 19188 Rel 7.1.2 SPR merging (SPR
**                                                  18897) 
** 19-Nov-2008      Abhishek Dhammawat    SPR 19189 SPR 18882 merge
** 24-Nov-2008      Abhishek Dhammawat    SPR 19189 SPR 18882 merge cycle2
** 01-Dec-2008      Rajiv Kumar           SPR 19215 CSR-1-6212448 Merged
** 17-Dec-2008      Rajiv Kumar           SPR 19218 CSR-1-6102712 Merged
** 04-Mar-2009      Saurabh Gupta	      SPR 19349 Klocworks fix		
** 01-May-2009      Anuradha Gupta	      SPR 19535 Modified fn icf_ssa_handle_network_send_failure
** 29-May-2009      Tarun Gupta           Rel 8.2       Async NOTIFY Support
** 11-Jun-2009      Ashutosh Mohan        SPR 19737     Changes done for GCC 4.1 warnings removal
** 14-Jul-2009      Tarun Gupta           SPR 19824     Modified icf_ssa_tcpm_00_connect_req
** 24-Jul-2009      Abhishek Dhammawat    SPR 19791 Modified icf_ssa_handle_network_send_failure
** 20-Aug-2009      Anuradha Gupta        SPR 19927    GCC warning removal
** 14-Sep-2009      Preksha Gupta         SPR 19999    Merged CSR 1-7694021
** 17-Sep-2009      Rajiv Kumar           SPR 19999    Merged for CSR
**                                                     1-7694021 Modified
** 15-Oct-2009      Anuradha Gupta        SPR 20121    Vxworks compilation issue resolved
** 28-Oct-2009      Abhishek Dhammawat    SPR 20185    Coverity bug fix merge
** 30-Oct-2009      Anuradha Gupta        SPR 20205    Modified sdf_cbk_uaSendMsgOnNetwork
**
*****************************************************************************
...
#endif
/* Added function under CSR Merge 1-60482218*/
extern icf_return_t icf_ssa_handle_network_send_failure(
...
#endif
{
    ...
    icf_ssa_ctxt_st       *p_ssa_ctxt = ICF_NULL; 
     ...
     icf_ssa_pdb_st       ssa_pdb, *p_ssa_pdb = ICF_NULL , *p_old_ssa_pdb = ICF_NULL;
    ...
    /* SPR 18767: the ecode should be assigned to p_ecode
     * otherwise p_ecode will have junk value of NULL value
     */
    ssa_pdb.p_ecode = &ecode;

    /*  Fix for SPR 18756 */
    /*  Store the ssa_pdb that was already present
        in ssa_ctxt */
    p_old_ssa_pdb = p_ssa_ctxt->p_ssa_pdb;
    ...
    if ( ICF_NULL != p_ssa_ctxt)
    {
        ...
        if (dTransptype == Sdf_en_protoUdp)
        {
            ...
            /* Fix for CSR 1-6212448 */
			ICF_MEMGET(p_persistent_glb_pdb, buffLenSipMsg + 1, ICF_MEM_COMMON,\
					p_sip_pdu_start,ICF_RET_ON_FAILURE,\
					&ecode, ret_val)
			icf_port_memcpy(p_sip_pdu_start, 
								  p_buffer_to_send_on_nw, buffLenSipMsg);
            p_sip_pdu_start[buffLenSipMsg] = '\0';
		...
		else
		{
			...
			/*Boolean conn_not_to_close is set to TRUE if call_ctxt is
              NOT NULL and call is not connected corresponding to FD on which
              connecton is established such that connection should not
              be closed before recieving the final response*/
            p_conn = icf_ssa_get_conn_db( pTranspaddr->dSockFd, 
                    p_ssa_pdb->p_ssa_glb_data->p_ssa_tcpm_conn_db);
            if((p_conn!=ICF_NULL) &&  (p_ssa_ctxt->p_call_ctxt !=NULL) && \
                    (!(ICF_CALL_CONNECT_DONE & \
                       p_ssa_ctxt->p_call_ctxt->common_bitmask)))
            {
                p_ssa_pdb->p_ssa_glb_data->p_ssa_tcpm_conn_db->conn_not_to_close = ICF_TRUE;
            }
            /* SPR 20205: In case call is connected then explicitly set the boolean
             * to be false.
             */
            else
            {
                p_ssa_pdb->p_ssa_glb_data->p_ssa_tcpm_conn_db->conn_not_to_close = ICF_FALSE;
            }
                ...
                { 
                    p_ssa_ctxt->p_conn_table_req_entry = p_conn;
                        ...
                        ((ICF_CONN_TYPE_RESPONSE & p_conn->prot_conn_bitmask)||
                         (ICF_CONN_TYPE_REQUEST & p_conn->prot_conn_bitmask)))
                { 
                  /*precondition: The condition is modified for checking 
                    the presence of request connection also when message is
                    response. This change resolves the cleaning of 180response  
                    from pending pdu list after dispatch to NW*/
            ...
            if (ICF_NULL != p_conn)
            {
                             ...
                             (ICF_CONN_TYPE_REQUEST & p_conn->prot_conn_bitmask))
                {
                    ...
                    if (p_ssa_pdb->p_conn_table_entry) p_ssa_pdb->p_conn_table_entry->prot_conn_bitmask |= ICF_CONN_TYPE_REQUEST;
                }
                if ((dMsgType == SipMessageResponse)  &&
                             (ICF_CONN_TYPE_RESPONSE & p_conn->prot_conn_bitmask))
                {
                    p_ssa_pdb->p_conn_table_entry = p_ssa_ctxt->p_conn_table_resp_entry;
                    if (p_ssa_pdb->p_conn_table_entry) p_ssa_pdb->p_conn_table_entry->prot_conn_bitmask |= ICF_CONN_TYPE_RESPONSE;
                }
            }
            else
            {
			...
			if ( p_ssa_pdb->p_conn_table_entry && ( ICF_SSA_TCPM_AWT_CON_RESP != p_ssa_pdb->
			 p_conn_table_entry->tcpm_sm_state))
			{ 
                /* Fix for CSR 1-6212448 */
				ICF_MEMGET(p_glb_pdb, buffLenSipMsg + 1, \
						ICF_MEM_COMMON,\
				p_sip_pdu_start,ICF_RET_ON_FAILURE,\
				&ecode, ret_val)
				icf_port_memcpy(p_sip_pdu_start, 
	...
	{
        /* Fix for CSR 1-6212448 */
        ICF_MEMGET(p_persistent_glb_pdb, dBuflen + 1, ICF_MEM_COMMON,\
                p_sip_pdu_start,ICF_RET_ON_FAILURE,\
                &ecode, ret_val)
        icf_port_memcpy(p_sip_pdu_start, pBuffer, dBuflen);
        p_sip_pdu_start[dBuflen] = '\0';
        if (ICF_FAILURE == 
                icf_ssa_dispatch_pdu_to_es(p_glb_pdb, p_ssa_ctxt, pTranspaddr, 
    ...
    /*  Fix for SPR 18756 */
    /*  Restore the ssa_pdb that was previously present in ssa_ctxt*/
    if(ICF_NULL != p_ssa_ctxt->p_ssa_pdb)
    { 
       p_ssa_ctxt->p_ssa_pdb = p_old_ssa_pdb; 
    }
    /*pBuffer[dBuflen] = '\0';*/
   /* icf_print_pdu(p_glb_pdb, pBuffer, pTranspaddr->pHost, pTranspaddr->dPort, dBuflen);*/
    if (ICF_NULL != p_ssa_ctxt)
    {
        /* Indication should be sent to corresponding module in case 
         * re-transmission of a message has failed on network */
        if((Sdf_co_fail == sdf_ret_val) &&
                (p_ssa_ctxt->bitmask_1 & ICF_SSA_UATK_TIMER_TYPE_EXPIRE))
        {
            /* Function to set dest module ID for corresponding module and fill the 
             * internal message structure */
            ret_val = icf_ssa_handle_network_send_failure(pCallObj,
                    (Sdf_ty_pvoid)p_ssa_ctxt,dTransptype,pError);
            ICF_SSA_RESET_COMMON_BITMASK(\
                    p_ssa_ctxt->bitmask_1,\
                    ICF_SSA_UATK_TIMER_TYPE_EXPIRE)
...
#endif
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /*Fix for CSR 7694021 - SPR 20007*/
                /* If Boolean conn_not_to_close is TRUE then
                   connection should not be closed*/

                if(ICF_FALSE == (p_ssa_pdb->p_conn_table_entry->conn_not_to_close))
                {
...
#endif
{
    ...
    /* Coverity bug fix */
    Sdf_ty_socketId sockfd = ICF_INVALID_SOCKFD;
#ifdef SDF_ERROR    
    Sdf_st_error dError;
#endif
    (void)dDomain;
...
#endif
{
    ...
    else
    {
	   ...
	   if( ICF_NULL == ssa_pdb.p_conn_table_entry )
        ...
        /*Set Connection as persistent if PURGE_TIMER is defined as
         *non-Zero value*/
        ...
        icf_ssa_pdb_st   *p_ssa_pdb)
{
    ...
    /*Assign result for sending connection status in TRACE*/
    ...
    for (p_temp = p_root; ICF_NULL != p_temp; p_temp = p_temp->p_next)
    {
                ...
                p_ssa_list = p_ssa_list->p_next)
        {
            ...
            /*Allocate APP context to p_glb_pdb*/
            ...
            {
                /*  Fix for spr 18743*/
                /*  THis time the ssa_ctxt contains the ssa_pdb 
                    that has been freed  so populate it with
                    the current ssa_pdb */ 
                p_ssa_pdb->p_ssa_ctxt->p_ssa_pdb = p_ssa_pdb;
                if ( ICF_NULL != 
                         ...
                         & (p_ssa_ctxt->bitmask)))
                {
                            ...
                            icf_ssa_send_dialog_initiator_msg(p_ssa_pdb))
                    {
                        ...
                        /* Since we are returning Failure from here free the 
                         * the p_root list structure allocated before for loop
                         * above.The change is done while merging CSR_1_5397601
                         * (when TCP case is run).*/ 
                ...
                for (pdu_ctr = 0; (pdu_ctr < p_ssa_ctxt->no_of_pending_pdu) && (pdu_ctr < ICF_MAX_PENDING_PDU_PER_CONN) ; pdu_ctr++)
                {
                    pdu_len = 
    ...
    /*Stat collection for TCP/TLS connection*/ 
    if( ret_val != ICF_FAILURE)
    {
        if(p_ssa_pdb->common_bitmask & ICF_SSA_SECURE_CONN)
        {
        ...
        icf_ssa_pdb_st   *p_ssa_pdb)
{
    ...
    icf_boolean_t        call_app_mod = ICF_TRUE;
    ...
    /*Iterate thru all SSA context in hash table and search SSA Context
     * for which this connection failure is received*/
    for (p_temp = p_root; ICF_NULL != p_temp; p_temp = p_temp->p_next)
    {
                ...
                ICF_NULL != p_ssa_list ;)
        {
            ...
            if (ICF_NULL != p_ssa_ctxt)
            {
                ...
                /*If connection entry received is same as request or response
                 * connection in SSA Context, it means connection matches*/
                if ((p_ssa_ctxt->p_conn_table_req_entry != p_conn_db_node) &&
                        (p_ssa_ctxt->p_conn_table_resp_entry != p_conn_db_node))
                {
                    continue;
...
#endif
                if (ICF_SSA_SECURE_CONN == 
                        (p_ssa_pdb->common_bitmask & ICF_SSA_SECURE_CONN))
                {
                    /* Collect stats of connection failure of TLS*/
                    ICF_STATS(p_ssa_pdb->p_glb_pdb->p_stats_info->
...
#ifdef ICF_DNS_LOOKUP_ENABLED
                /* this code is added here to make the retries on the different
                 * IPs in case of TCP, we will check if the Transaction related
                 * informationis stored here and then will fetch the next IP 
                 * and retry on the same */
                if((ICF_NULL != p_ssa_pdb->p_ssa_ctxt) && 
                   (p_ssa_pdb->p_ssa_ctxt->bitmask & 
                    ICF_SSA_DNS_XSACTION_PRESENT) &&
                   ((ICF_NULL != 
                     p_ssa_pdb->p_ssa_ctxt->p_call_obj->pUacTransaction) ||
                    (ICF_NULL != p_ssa_pdb->p_ssa_ctxt->p_overlap_transaction)))
                {
                    icf_boolean_t    ready_to_send = ICF_TRUE;

                    ICF_SSA_SET_TXN_DNS_STATE(
                            ((Sdf_st_transaction*)p_ssa_ctxt->p_transaction), 
                            ((Sdf_st_overlapTransInfo*)
                             p_ssa_ctxt->p_overlap_transaction), 
                            Sdf_en_dnsIpUsed)

                    /*Connect has failed to the current IP, so we attempt to 
                     * move to the next IP*/
                    icf_ssa_remove_conn_entry(p_ssa_pdb ,
                            p_conn_db_node->conn_fd);
                    p_ssa_pdb->p_conn_table_entry = ICF_NULL;
                    /* This pointer has been set to NULL to avoid a crash in the
                     * following scenario:
                     * OPTIONS sent on TCP and connect fails,failure response
                     * being sent to application.
                     */
                                ...
                                p_ssa_ctxt->p_overlap_transaction))
                    {
                        ...
                        /*If non-persistent then break loop as required SSA
                         * context is found, in case of persistent more than
                         * one SSA context can be pending on this connection*/
                        if(0 == ICF_PURGE_TIMER_DURATION)
                        {
                            non_persistent_break = ICF_TRUE;
                            break;
                    ...
                    /*If no more IP address is available for first FQDN then 
                     * try on second FQDN or secondary server address*/
                    else if (ICF_SUCCESS == 
                    ...
                    /*If Failure ecode is not proxy or registrar, means above 
                     * function could not send request on secondary server
                     * and failure occured, then do not do revert processing
                     * and return failure*/
                    else if ((ICF_CAUSE_REGISTRAR_NOT_RESPONDING !=
                            *(p_ssa_pdb->p_ecode)) &&
                            (ICF_CAUSE_PROXY_SERVER_NOT_RESPONDING !=
                             *(p_ssa_pdb->p_ecode)))
                    {
                        call_app_mod = ICF_FALSE;
                        ret_val = ICF_FAILURE;
                    }
                }
                else
                {
#endif
                    /*This leg will hit when either DNS_LOOKUP is not enabled or
                     * remote server address is not domain name, This part will
                     * check if secondary address is present then send request 
                     * on that address*/
                    if(ICF_NULL != p_ssa_pdb->p_ssa_ctxt && 
                       ((ICF_NULL != 
                        p_ssa_pdb->p_ssa_ctxt->p_call_obj->pUacTransaction) ||
                        (ICF_NULL != 
                         p_ssa_pdb->p_ssa_ctxt->p_overlap_transaction)))
                    {
                        /*Connect has failed to the current IP, so we attempt 
                         * to move to the next IP*/
                        icf_ssa_remove_conn_entry(p_ssa_pdb ,
                                p_conn_db_node->conn_fd);
                        p_ssa_pdb->p_conn_table_entry = ICF_NULL;
                        /* This pointer has been set to NULL to avoid a crash
                        ...
                        /*If Failure ecode is not proxy or registrar, 
                         * means above function could not send request on 
                         * secondary server and failure occured, 
                         * then do not do revert processing
                         * and return failure*/
                        else if ((ICF_CAUSE_REGISTRAR_NOT_RESPONDING !=
                            *(p_ssa_pdb->p_ecode)) &&
                            (ICF_CAUSE_PROXY_SERVER_NOT_RESPONDING !=
                ...
                if (ICF_FAILURE == ret_val)
                {
                    /*If ret_val is failure then ignore*/
                }
            ...
            if ( ICF_FAILURE != ret_val)
            {
                if ((ICF_TRANSPORT_MODE_BOTH == mode) && 
                        (ICF_SUCCESS == ret_val))
                {
                    if (p_ssa_pdb->p_ssa_ctxt) p_ssa_pdb->p_ssa_ctxt->revert_mode = ICF_TRUE;
                }
                else if (ICF_SUCCESS == ret_val)
                {
                   if (p_ssa_pdb->p_ssa_ctxt) p_ssa_pdb->p_ssa_ctxt->revert_mode = ICF_FALSE;
                }
                if ( p_ssa_pdb->p_ssa_ctxt && ( ICF_TRUE == p_ssa_pdb->p_ssa_ctxt->revert_mode))
                {
                    ICF_SSA_RESET_COMMON_BITMASK(\
                            p_ssa_ctxt->bitmask,\
            ...
            /*If failure is because of resend_on_sec_server then do not
             * call failure_to_app_module as SSA context is already deleted,
             * and there is not much information left to send to APP module
             * Now calling module will be cleared with its gaurd timer*/
            if ((ICF_FAILURE == ret_val) && (ICF_TRUE == call_app_mod))
            {
                /*Some failure, be it failure in getting line data, 
                 * in sending message to peer or transport mode not
                 * supporting a  revert to UDP, we will send failure
        ...
        icf_ssa_pdb_st   *p_ssa_pdb)
{
   ...
   if (nw_interface_info.bitmap == ICF_INTF_INFO_PROTOCOL_TCP)
    ...
    /* Asynchronus NOTIFY Support : Fixed issue wherein
       stats for number of active TCP connections were 
       coming negative.
    */   
    if(ICF_NULL < p_ssa_pdb->p_glb_pdb-> \
            p_stats_info->stat_tcp_connections.stats_active_conn)
    {
    ...
    if(ICF_NULL < p_ssa_pdb->p_glb_pdb-> \
            p_stats_info->stat_tls_connections.stats_active_conn)
    {
    ...
    if (ICF_NULL != p_conn_db_node)
    {
        ...
        nw_interface_info.receiver_fd = p_conn_db_node->conn_fd;
                    ...
                    (0 == ICF_PURGE_TIMER_DURATION))
            {
             ...
             {
                 /*Decrement ref_count so that this connection
                  *can be closed with purge timer expiry*/
                 if ((ICF_TRUE == non_persistent_break) &&
                     (0 < p_conn_db_node->ref_count))
                 {
                     p_conn_db_node->ref_count--;
                 }
             }
		}
		else
        ...
        icf_ssa_pdb_st   *p_ssa_pdb)
{
    ...
    icf_port_memset((icf_void_t*)nw_interface_info.self_ip_addr,
      ...
      */
     /*If self address is configured as per line then fetch it from 
      *corresponding line_data, otherwise fetch self_addr from config_data*/
     if (ICF_CFG_SELF_ADDR_PER_LINE ==
                       p_ssa_pdb->p_glb_cfg_data->self_addr_per_line)
     {
          /*Get line data to fetch self_addr for this line*/
          if ( ICF_FAILURE ==
                    icf_dbm_fetch_line_cntxt_blk(
                        p_ssa_pdb->p_glb_pdb,
          ...
          else
          {
              icf_cmn_convert_transport_addr_to_ip_string(
                 &(p_ssa_pdb->p_line_data->self_addr.addr),
    ...
    else if( ICF_FAILURE == icf_es_open_tcp_conn(
    ...
    ICF_FUNCTION_EXIT(p_ssa_pdb->p_glb_pdb)
    /* 
        Fix for SPR 19824 : The macro should be passed a pointer as the second 
        argument.
    */
    ICF_KLOCWORK_FALSE_POSITIVE(p_ssa_pdb->p_glb_pdb, &nw_interface_info.receiver_fd)
...
#endif
    p_conn->bitmap = ICF_NULL;
    p_conn->p_pending_pdu = ICF_NULL;
    p_conn->pending_pdu_length = 0;
}
/**************************************************************************************
...
icf_void_t      icf_ssa_print_conn_db (const icf_ssa_tcpm_conn_data_st *p_conn)
{
    ...
    {
        ICF_PRINT(((icf_uint8_t *)"Connection type = TLS\n"))
    }
#endif
#ifdef ICF_SECURE_SUPPORT
...
/* The memory for p_peer_name is allocated and used for printing
   purpose only in TLS mode.The allocated memory to p_peer_name 
   is freed here.*/ 

#ifdef ICF_SECURE_SUPPORT
            ICF_MEMFREE(p_ssa_pdb->p_glb_pdb,
                        p_ssa_pdb->p_ssa_glb_data->
                           p_ssa_tcpm_conn_db[i].p_peer_name,
                        ICF_MEM_COMMON,
...
/*****************************************************************************
 ** FUNCTION:      icf_ssa_handle_network_send_failure
 **
 ** DESCRIPTION:  This function is used to find the message, its type and once
 **               the message is identified, this function fills the detination module ID
 **               and message structure so that it can be sent as internal messaeg to other
 **               modules
 *****************************************************************************/
icf_return_t icf_ssa_handle_network_send_failure(
        Sdf_st_callObject                      *pCallObj,
        Sdf_ty_pvoid                           pContext,
        Sdf_ty_protocol                        dTransptype,
        Sdf_st_error                           *pError)
{
    ...
    /*Fix for spr 18882 */
    ICF_CHECK_IF_ANY_OF_THREE_VAR_IS_NULL(p_glb_pdb,p_ssa_ctxt->p_ssa_pdb->p_ssa_ctxt,
            pCallObj, pError, return_val)
    ...
    if(dTransptype == Sdf_en_protoTcp)
    {
        ecode  = ICF_ERROR_TCP_SEND;
    }
    else if(dTransptype == Sdf_en_protoUdp)
    {
        ecode  = ICF_ERROR_UDP_SEND;
    }
    
    /* Find the overlap transaction to fetch sip message */
    sdf_listInitIterator(&(pCallObj->slOverlapTransInfo), \
    ...
    /*SPR 18753:Start Previously the fix was no handling for UAS transaction
      If p_sip_msg is not fetched from overlap txn then it was obtained from
      UAC txn. However in scenario when responses get retranmitted and there
      is failure in icf_send then the UAS txn should be used.*/ 
    if(Sdf_co_null == p_sip_msg)
    {
        if((ICF_NULL != pCallObj->pUasTransaction) &&
           (ICF_NULL != pCallObj->pUasTransaction->pSipMsg))
        {
            p_sip_msg = pCallObj->pUasTransaction->pSipMsg;
        }
        else if((ICF_NULL != pCallObj->pUacTransaction) &&
                (ICF_NULL != pCallObj->pUacTransaction->pSipMsg))
        {
            p_sip_msg = pCallObj->pUacTransaction->pSipMsg;
        }
    }/*End Fix for SPR18753*/    
    if (Sdf_co_null != p_sip_msg)
    {
    ...
    /* Send failed for requested transaction */ 
            ...
            (p_ssa_ctxt->p_conn_table_req_entry != ICF_NULL))
    {
        ...
        /* Fix for spr 18882*/
        ICF_CHECK_IF_VAR_IS_NULL(p_glb_pdb,p_ssa_ctxt->p_ssa_pdb->p_glb_pdb->p_glb_data,return_val)
        if(ICF_FAILURE == return_val)
        {
            return Sdf_co_fail;
        }
    ...
    else if (dMsgType == Sdf_en_bye)
    {
        ...
        /*SPR 19791 The scenario was that after call establishment 
         PA has terminated the call and instead of 200OK for BYE peer sends
         BYE request, now IPTK tries to send the 200OK for the BYE and in
         function icf_port_udp_send/icf_port_tcp_send failure occurs then
         icf_ssa_handle_network_send_failure is invoked leading to transaction
         free because of invocation of sdf_ivk_uaFreeCallObject in BYE case.
         In return path the txn buffer was getting accessed leading to FMW.
         Removed the unnecessary invocation of sdf_ivk_uaFreeCallObject*/
    ...
    {
        /*Fix for spr 18882 */
        ICF_CHECK_IF_ANY_OF_TWO_VAR_IS_NULL(p_glb_pdb,p_internal_msg,
                p_internal_msg->p_msg_data, return_val)
        ...
        else if (ICF_INT_MODULE_SIC == dest_module_id)
        {
            ...
            if( Sdf_en_refer == dMsgType)
            {
                ...
                {
                    p_refer_resp->p_sic_ctx->error_code = ecode;
            ...
            else if( Sdf_en_subscribe == dMsgType)
            {
                ...
                else
                {
                    ...
                    {
                        p_subs_resp->p_sic_ctx->error_code = ecode;
            ...
            else if( Sdf_en_options == dMsgType)
            {
                ...
                ICF_CHECK_IF_VAR_IS_NULL(p_glb_pdb, p_opt_resp->p_sic_ctx, return_val)
                if (ICF_FAILURE == return_val)
                {
                      return return_val;
                }
                p_opt_resp->p_sic_ctx->error_code = ecode;
            }
            else if(p_method)
            {
                if( icf_port_strcmp( (icf_uint8_t *)p_method,(icf_uint8_t *)"MESSAGE" ) == 0 )
                {
                    ...
                    {
                        p_msg_resp->p_sic_ctx->error_code = ecode;
                        ICF_ASSERT(p_glb_pdb,
                ...
                else if( icf_port_strcmp((icf_uint8_t *) p_method, (icf_uint8_t *)"PUBLISH") == 0)
                {
                    ...
                    {
                        p_publish_resp->p_sic_ctx->error_code = ecode;
                        ICF_ASSERT(p_glb_pdb,
    ...
    {
        ICF_SSA_SET_ECODE(p_ssa_ctxt->p_ssa_pdb,p_method,ecode)
    }
