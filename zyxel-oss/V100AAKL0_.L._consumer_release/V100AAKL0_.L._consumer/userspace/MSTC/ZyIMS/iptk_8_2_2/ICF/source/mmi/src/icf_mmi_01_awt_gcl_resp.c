 ...
 * 09-Jun-2008      Tarun Gupta    SPR 18585      Modified icf_mmi_01_dms_request
 * 26-Jun-2008      Amit Sharma    SPR 18725      Populated warning codes
 * 04-Jul-2008      Amit Sharma    SPR 18725      Review comments incorporated
 * 05-Dec-2008      Tarun Gupta    SPR 19218      Merged CSR 1-6367281
 * 04-Mar-2009      Saurabh Gupta  SPR 19349      Klocworks fix
 * 20-May-2009      Ashutosh Mohan CSR-1-6794762  Modified icf_mmi_clone_sdp_stream_params
	...
	if(ICF_MMI_MMS_MOD_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
	{
		...
		else
		{
			for(stream_count = 0; (stream_count < 
					p_mms_req->modify_stream_count) && (stream_count < ICF_MAX_STREAM_PER_CALL); stream_count++)
			{
		...
		for(stream_count = 0; (stream_count < p_mms_req->modify_stream_count) && (stream_count < ICF_MAX_STREAM_PER_CALL); 
				stream_count++)
		{
			if((p_mms_req->modify_media[stream_count].bitmask & 
        ...
        INOUT icf_mmi_pdb_st             *p_mmi_pdb)
{
    ...
    icf_uint8_t							non_special_codec_count = ICF_NULL;
	...
	else if(ICF_MMI_CMS_ONGOING == p_mmi_pdb->p_mmi_ctx->ongoing_event)
	{
        ...
        /* Check if gcl_resp contains only "telephone-event/fec/red/CN" codec */
        while(p_src_list)
        {
            p_src_data = ((icf_stream_capab_st *)p_src_list->p_data);

            /* SPR 19187, 20073 : Special handling for fec/red/CN */
			/* Keep this as this may be required for traces */

            icf_cmn_get_list_count(p_src_data->p_codec_info_list,
                                            &p_mmi_pdb->num_of_codecs);		
            
            /* Get the no. of codecs in this list that are not special codecs
			 * like telephone-event, fec, red, CN.
			 */

            icf_cmn_get_codec_attrib_list_count(p_src_data->p_codec_info_list,
								&non_special_codec_count);

            if (ICF_NULL == non_special_codec_count)
            {
                ICF_PRINT(((icf_uint8_t *)"\n[MMI]: Only special payload types received in GCL response"));    
                icf_mmi_send_rel_res_ind(p_mmi_pdb);
							...
							&p_internal_sdp))
	{
		...
		if(p_sdp)
		{
			...
			icf_mmi_clone_sdp_stream_params(
							...
							p_sdp,
							p_src_sdp);
            ICF_KLOCWORK_FALSE_POSITIVE(p_glb_pdb,p_sdp)
...
)
{
	...
	if (ICF_SDP_STREAM_ENCPARAMS_PRESENT & p_src_sdp_stream->bit_mask)
	{
        ...
        /*CSR 1-6794762: In case th enc params is absent in GCL RESP,
          remove the same from the SDP*/	
