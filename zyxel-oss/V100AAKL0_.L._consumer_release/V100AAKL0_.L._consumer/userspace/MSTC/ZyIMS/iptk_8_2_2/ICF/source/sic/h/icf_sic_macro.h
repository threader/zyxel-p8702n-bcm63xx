...
/* New macro definitions for asynchronous NOTIFY support */

/* ----------------------------------------------------------------------------
 * Macro to set the pending event received in the SIC context
 * --------------------------------------------------------------------------*/
#define ICF_SIC_SET_PENDING_EVENT(p_sic_pdb, event_recd)\
{\
    if(ICF_NULL != p_sic_pdb->p_sic_ctx)\
    {\
        p_sic_pdb->p_sic_ctx->subs_fsm_pending_event = event_recd;\
    }\
}/* End of macro */

/* ----------------------------------------------------------------------------
 * Macro to set the previous state state in the SIC context
 * --------------------------------------------------------------------------*/
#define ICF_SIC_SET_PREV_STATE(p_sic_pdb,state_recd)\
{\
    if(ICF_NULL != p_sic_pdb->p_sic_ctx)\
    {\
        p_sic_pdb->p_sic_ctx->prev_state = state_recd;\
    }\
}/* End of macro */

/* ----------------------------------------------------------------------------
 * Macro to set the previous event received in the SIC context
 * --------------------------------------------------------------------------*/
#define ICF_SIC_SET_PREV_EVENT(p_sic_pdb, event_recd)\
{\
    if(ICF_NULL != p_sic_pdb->p_sic_ctx)\
    {\
        p_sic_pdb->p_sic_ctx->prev_event = event_recd;\
    }\
}/* End of macro */
