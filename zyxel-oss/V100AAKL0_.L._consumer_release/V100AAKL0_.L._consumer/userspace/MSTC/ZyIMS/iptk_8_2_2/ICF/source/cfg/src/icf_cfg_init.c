 ...
 * 17-Jan-2008 Abhishek Dhammawat   Rel8.0      Modified icf_cfg_init for 
 *                                              precondition_required handling
 * 18-Jan-2008  Kamini Gangwani     Rel 8.0     Set the default media transport 
 *                                              type as RTP/AVP in
 *                                              icf_cfg_init.
 * 24-Jan-2008  Kamini Gangwani     Rel 8.0     Initialize the p_access_network_info 
 *                                              in config data.
 * 07-Jun-2008  Amit Sharma       IPTK 8.0 SRTP LLD      SRTP Enhancements
 * 13-Jun-2008  Tarun Gupta       SPR 18585     Modified icf_cfg_init
 ...
 * 13-Mar-2009  Rajiv Kumar       Rel 8.1 LLD   Replacing ARES with DNS CLIENT. 
 * 18-May-2009  Anuradha Gupta    SPR 19672     modified icf_cfg_init 
 * 21-May-2009  Anuradha Gupta    SPR 19672     modified icf_cfg_init 
 * 26-May-2009  Anuradha Gupta    SPR 19672     modified icf_cfg_init 
 * 26-May-2009  Anuradha Gupta    SPR 19672     modified icf_cfg_init
 * 28-May-2009  Anuradha Gupta    SPR 19672     modified icf_cfg_init 
 * 28-May-2009  Tarun Gupta       Rel 8.2       Async NOTIFY Support 
 * 28-May-2009  Ashutosh Mohan    SPR 19672     Modified icf_cfg_init
        ...
        OUT     icf_error_t              *p_ecode)
{
    ...
    /* Initialize primary and secondary DNS server address to NULL */
    p_config_data->p_dns_server_addr        = ICF_NULL;
    p_config_data->p_sec_dns_server_addr    = ICF_NULL;
...
ICF_INIT_END
	...
	p_config_data->nw_res_reservation_supported = ICF_FALSE;
        ...
        /*Initialize the field as NULL in config data*/
        p_config_data->p_access_network_info = ICF_NULL;
...
ICF_HANDLE_REINVITE_END
  ...
  p_config_data->media_transport_type = ICF_TRANSPORT_MODE_RTP_AVP;
  ...
  for (i = 0; i <= ICF_WIN_TLS_SIP_SERVER; i++)
  {
    ...
    /* initialize the maximum active calls per line  */
    p_config_data->max_active_calls_per_line = ICF_ACTIVE_CALLS_PER_LINE_DEFAULT;  
    
    /*  Initialise the clent port for DHCP */
    p_config_data->dhcp_client_port = \
                ICF_DHCP_CLIENT_PORT;

    /*  Initialise the DHCP Response server */
    p_config_data->dhcp_response_timer = \
                ICF_DHCP_RESPONSE_TIMER;

    /*  Initialise the dhcp REsponse */
    p_config_data->p_dhcp_resp = ICF_NULL;
    
    /*  Initialise the dhcp server address */
    p_config_data->p_dhcp_server_addr = ICF_NULL;

   /*Start: Rel8.0 Added for precondition support */
#ifdef ICF_QOS_SUPPORT
    p_config_data->precondition_required = ICF_FALSE;
#endif     

    /*Initialize srtp fallback_reqd flag and srtp failure resp code list*/
    p_config_data->srtp_fallback_reqd  = ICF_FALSE;
    p_config_data->p_srtp_fallback_resp_codes = ICF_NULL;

  /*End: Rel8.0 Added for precondition support */    
    p_config_data->ignore_stale_param = ICF_FALSE;
#ifdef ICF_SESSION_TIMER
    p_config_data->session_refresh_method = ICF_CFG_SESSION_REFRESH_METHOD_INVITE;
#endif

	p_config_data->app_port_config = ICF_NULL;
    ...
    /* SDP Tunnelling Enhancements */
     p_config_data->early_incoming_session_ind_reqd = ICF_FALSE;

#ifdef ICF_DNS_LOOKUP_ENABLED
     /*variable added for open source replacement(IPTK REL 8.1)------START---*/
     p_config_data->p_arDnsClient_addr = ICF_NULL;
     p_config_data->p_arDnsClib_addr = ICF_NULL;  
     /*variable added for open source replacement(IPTK REL 8.1)------END----*/
#endif    
   
    /* CSR_1-6347417:initialize the field retry_codec_commit_req which
     * is by default FALSE.If it will be true,IPTK will retry if the 491
     * received from network of auto reinvite/update.
     */
     p_config_data->retry_codec_commit_req = ICF_FALSE;

     /* SPR 19321- By default, update for offer answer after 
	* connect  will be disabled at run time.*/
    p_config_data->update_for_offeranswer_after_connect = ICF_FALSE;
    ...
    /* By default we send INVITE for a Start / Stop fax requests */
	p_config_data->fax_req_method = ICF_CFG_CALL_MODIFY_METHOD_INVITE;

    /* Asynchronous NOTIFY support:
    
       Initialise the field async_notify_support_reqd in config data to its
       default value ICF_FALSE.
    */
    p_config_data->async_notify_support_reqd = ICF_FALSE; 

    /* Fix for CSR-1-6976670: By default, check_remote_codec_list
       will be disabled at run time */

    p_config_data->check_remote_codec_list = ICF_FALSE;

    /* CSR_1_7113139 Merged SPR 19672 Start */
    /* SPR 19462- By default, inc_sdp_version_when_no_change
    will be disabled at run time.*/
    p_config_data->inc_sdp_version_when_no_change = ICF_FALSE;
    /* CSR_1_7113139 Merged SPR 19672 End */

    /* Asynchronous MESSAGE support:

       Initialise the field async_message_support_reqd in config data to its
       default value ICF_FALSE.
     */
    p_config_data->async_message_support_reqd = ICF_FALSE; 
        ...
        OUT     icf_error_t              *p_ecode)
{
    ...
    if (ICF_NULL != p_config_data->p_dns_server_addr)
    {
        ICF_STATIC_MEMFREE(
            p_glb_pdb,
            p_config_data->p_dns_server_addr,
            p_ecode,
            ret_val)
    }
    ...
    if (ICF_NULL != p_config_data->p_sec_dns_server_addr)
    {
        ICF_STATIC_MEMFREE(
            p_glb_pdb,
            p_config_data->p_sec_dns_server_addr,
            p_ecode,
            ret_val)
    }
    ...
    if (ICF_NULL != p_config_data->p_dhcp_server_addr)
    {
        ICF_STATIC_MEMFREE(
                p_glb_pdb,
                p_config_data->p_dhcp_server_addr,
                p_ecode,
                ret_val)
    }
