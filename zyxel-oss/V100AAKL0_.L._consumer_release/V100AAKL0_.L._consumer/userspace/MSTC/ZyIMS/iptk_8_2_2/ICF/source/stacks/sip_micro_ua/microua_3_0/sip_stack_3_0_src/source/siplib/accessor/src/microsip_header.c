...
** 28-01-2009   Rajiv Kumar         IPTK Rel 8.1   Prototype for 
**                                                 API sip_insertMultipleHeadersFromStringAtIndex
** 29/01/2009   Kamal Ashraf        IPTK Rel 8.1   API 
**                                                 sip_getTypeFromContactHdr  
**                                                 sdp_getSdpConnectionAsString
**                                                 sdp_getSdpOriginAsString added 
** 13/02/2009   Kamal Ashraf        IPTK Rel 8.1   UT Bug Fixes 
** 05/05/2009   Rajiv Kumar         IPTK Rel 8.2   Fix for SPR:19515    
** 05-Jun-2009  Anuradha Gupta      SPR 19737       Klocworks Warning Removal 
** 03-Jul-2009  Abhishek Dhammawat  SPR 19706     Modified
**                                                sdp_getOriginAsString
** 28-Jul-2009  Tarun Gupta         SPR 19886     Merged CSR 1-7233780
** 17-Aug-2009  Anuradha Gupta      SPR 19927     klocworks warning fixing
** 08-Oct-2009  Rajiv Kumar         SPR 20105     Fix For SPR: 20105
**
...
/* Klocoworks : NULL checks have been added so that
 * dereferencing does not happen
 */
      ...
      SipError          *pErr )
{
	...
	if (SIP_NULL == pNameValuePair->pName)
	{
		*pErr = E_NO_MEM;
		sip_freeSipParam(pTempParam);
                sip_freeSipNameValuePair(pNameValuePair);
		return SipFail;
	}
	 ...
	 SipError *pError)
{
	...
	if(sip_strcasecmp(pAuth->pStr1,(SIP_S8bit*)"Basic") == 0)
	{
		 ...
		 */ 
	/* NOTE: When we do a listGetAt, we dont need to init the param first.
	 * This is giving memleaks while running accessor test for MicroSip.So
	 * commented the code here */
		...
		if(SIP_NULL != pParam->pName)
		{	
			/* klocwokrs warning fixing: strcat have been changed to strncat so as
			 * to avoid possible ABR warnings
			 */
			(void)sip_strncpy((*pCredintial)->u.pBasic, pParam->pName,(tmpSize -1));

			if (pString != SIP_NULL)
			{
				(void)sip_strncat((*pCredintial)->u.pBasic,(SIP_S8bit *) "=",
								  (tmpSize - sip_strlen(pParam->pName) - 1));
				(void)sip_strncat((*pCredintial)->u.pBasic, pString,
								  (tmpSize - sip_strlen(pParam->pName) -1 - 1));
			}
		}
	}	
	else
...
{	
	...
	else if(SIP_NULL == pSipParam->pName)
	{
		return SipFail;
	}
	...
	else if (sip_strcasecmp(pSipParam->pName, (SIP_S8bit *)"q") == 0)	
	{
		...
		if (SipFail == sip_listGetAt(&(pSipParam->slValue), 0, \
			(SIP_Pvoid*)&pTmpString, pError))
		{
            /* CSR 1-7195117 Change Start*/ 
            /*	sip_freeSipParam(pSipParam);*/
            /* CSR 1-7195117 Change End*/             
            sip_freeSipContactParam(pSipContactParam);
			return SipFail;
		}
	...
	switch (contact_param->dType)
	{
		...
		case SipCParamExpires:
		{
			...
			if (contact_param->u.pExpire->dIntVar1 == SipExpDate)
			{
				...
				if (SipFail == sip_formDateStruct(/* CSR_1-6613911 Change Start */ SIP_NULL, /* CSR_1-6613911 Change End */&pArr,contact_param->\
...
*/
	...
	if(dType == SipHdrTypeUnknown)
	{
                if((SipCommonHeader *)(pHeader->pHeader) == SIP_NULL)
		{
			*pErr = E_PARSER_ERROR;
			return SipFail;
		}
...
(SipMessage *s, en_HeaderType dType,  SIP_S8bit **hdr, SIP_U32bit dIndex, SipError *err)
{
	...
	*hdr = (SIP_S8bit *)fast_memget(ACCESSOR_MEM_ID,SIP_MAX_HDR_SIZE,err);
	...
	if (sip_formSingleHeader(/*CSR 1-6613911 Change Start*/(*hdr+SIP_MAX_HDR_SIZE)/*CSR 1-6613911 Change End*/, \
                dType, NULL, dIndex, SipModeNew, s, 0, *hdr, err) == SipFail)
...
/******************************************************************************
**
** FUNCTION:     sip_insertMultipleHeadersFromStringAtIndex
**
** DESCRIPTION:  This function inserts a SipHeader of en_HeaderType "dType" in 
**               exactly the same way as sip_insertHeaderAtIndex; the input 
**               pHeader here is in the form of a string "hdr" with multiple 
**               header values separated by comma.
**
*******************************************************************************/
SipBool sip_insertMultipleHeadersFromStringAtIndex
#ifdef ANSI_PROTO
	(SipMessage *pMsg, en_HeaderType dType, SIP_S8bit *hdr, SIP_U32bit dIndex, 
     SipError *err)
#else
	(pMsg, dType, hdr, dIndex, err)
	SipMessage *pMsg;
	en_HeaderType dType;
	SIP_S8bit *hdr;
	SIP_U32bit dIndex;
...
#endif
{
    ...
    SIP_S8bit *pStart_of_str_to_parse = SIP_NULL ;
    SIP_S8bit *pCur_comma_pos_in_str = SIP_NULL ;
    SIP_S8bit *pEnd_of_str_to_parse = SIP_NULL ;

    SIP_U32bit lengthOfHeader = 0 ;
    SIP_U32bit len = 0 ;
    en_SipBoolean colon_present = SipFalse;
    ...
    SIP_S8bit  *pBody_begin  = SIP_NULL ;
    SIP_S8bit  *pBody_end    = SIP_NULL;
    SIP_U32bit length_of_body = 0;
    SIP_U32bit header_count = 0;
	...
	if(SIP_NULL == pMsg)
	{
        /*Set the value of err as invalid parameter ie E_INV_PARM*/
		*err = E_INV_PARAM;
		return SipFail;
	}
    ...
    /*Check if header type is unknown. If so dont use the parser */
    /*Stack does not perform the parsing of Unknown header ; now if
      the application uses this API to insert an unknown header 
      for example thhe input string is 
      "Header1 : user1@aricent.com,user2@aricent.com ,
      user3@aricent.com,user4@aricent.com" then same header
      will be added by stack without any validation */
    
    if(SipHdrTypeUnknown == dType)
    {
        /*Use the sip_insertHeaderFromStringAtIndex  API to insert
          the unknown header*/      
        if (SipFail == sip_insertHeaderFromStringAtIndex(pMsg,SipHdrTypeUnknown,
                        hdr,dIndex,err))
        {
            return SipFail;
        }
        /* Set the value of err as E_NO_ERROR indicating unknown header has
           been successfully insert by stack in SIP message*/
        *err = E_NO_ERROR;
        return SipSuccess;                
    }
    
    /*Get message type from SIP message*/
    if(SipFail == sip_getMessageType(pMsg,&dReqRespType,err))
    {
        return SipFail;
    }

    /*Initilised a temporary sip message.With the help of this
      temporary sip meaasge we will parse each of the comma
      seperated header value and populate the given header type in 
      temporary sip message*/
    ...
    { 
        if (SipFail == sip_initSipMessage (&pSipMessage,SipMessageRequest,err))
        {
            return SipFail;
        }
    }
    ...
    { 
        if (SipFail == sip_initSipMessage (&pSipMessage,SipMessageResponse,err))
        {
            return SipFail;
        }
    }
    ...
    /*Set current position to starting of input string*/
    pCurrentPos = hdr;
    /*Initialised pHeaderNameBeginIndex with startling of hdr position*/
    pHeaderNameBeginIndex = hdr;
    
    pEnd_of_str_to_parse = hdr + sip_strlen(hdr) - 1;

    /*Here trying to find the end of the header name */
    while (pCurrentPos <= pEnd_of_str_to_parse)
    {
        /* If we find a ':' , this means the end of the header name.
           so break	 */ 

        if (*pCurrentPos == ':')
        {
            colon_present = SipTrue;
            break;
        }
    ...
    if (SipFalse == colon_present) 
        
    {
        /*Set the value of err as invalid header indicating invalid string 
          passed in variable hdr ie E_INV_HEADER*/
	 *err = E_INV_HEADER;
         sip_freeSipMessage(pSipMessage);
	 return SipFail;        
    }
    /*Now find the header name from the received string : For example if
      the input string is "Contact : <sip :u1@127.0.0.1:33287>,
      <sip :u2@127.0.0.1:33289>" then header name ie "Contact" will 
      be extracted in pName*/   

    pHeaderNameEndIndex = pCurrentPos - 1;

    /*Remove the blank spaces from begining of header name*/
    pHeaderNameBeginIndex = sip_stripLeadingLWS\
                            (pHeaderNameBeginIndex, pHeaderNameEndIndex);   

    /*Remove the blank spaces from end of header name*/
    pHeaderNameEndIndex = sip_stripTrailingLWS\
                          (pHeaderNameBeginIndex, pHeaderNameEndIndex);    
    lengthOfHeader =\
                    pHeaderNameEndIndex-pHeaderNameBeginIndex+1;    
    pName = sip_nStrdup\
				(DECODE_MEM_ID,pHeaderNameBeginIndex,
                 (SIP_S32bit)lengthOfHeader, err);   
    ...
    /*In the following while loop we are parsing each of the comma 
      seperated string received as a input hdr to this API*/
    /*Here pCurrentPos currently indicating the position of ':' in the input
     * string*/  
    pStart_of_str_to_parse = pCurrentPos + 1;
    
    do
    {
        SIP_S8bit  temp_header_str[SIP_MAX_HDR_SIZE] = "";
        SIP_S8bit  *pTemp_str = SIP_NULL;
        SIP_U32bit slen1 = 0 ;
        SIP_U32bit slen2 = 0 ;
        SIP_U32bit remLen = 0 ;
        SIP_U32bit slen_temp_str = 0 ;

        /*Get the current position of ',' from the string*/
        pCur_comma_pos_in_str = sip_tokenize(pStart_of_str_to_parse
                ,pEnd_of_str_to_parse , ',');

        if (SIP_NULL == pCur_comma_pos_in_str)
        {
            /* If the value of pCur_comma_pos_in_str is null then it means that
             * this is the last loop for the parsing of the string so setting
             * the value of exitcondition to true */
            exitcondition = SipTrue;
        }
        /* Extration of the comma seperated header values is done in two ways
         * depending upon the value of pCur_comma_pos_in_str;
         * If pCur_comma_pos_in_str is not NULL we are extracting the header
         * value with the help of pCur_comma_pos_in_str and 
         * pStart_of_str_to_parse variables.
         * If pCur_comma_pos_in_str is NULL we are extracting the header
         * value with the help of pEnd_of_str_to_parse and 
         * pStart_of_str_to_parse variables*/
        if (SIP_NULL != pCur_comma_pos_in_str)
        {
            pBody_begin = sip_stripLeadingLWS(pStart_of_str_to_parse,
                    pCur_comma_pos_in_str - 1);

            pBody_end = sip_stripTrailingLWS(pStart_of_str_to_parse,
                pCur_comma_pos_in_str - 1);

            length_of_body = (SIP_S32bit)(pBody_end - pBody_begin) + 1;

            pTemp_str = sip_nStrdup(DECODE_MEM_ID,pBody_begin,
                    (SIP_S32bit)length_of_body,err);

            pStart_of_str_to_parse = pCur_comma_pos_in_str + 1 ;

            pEnd_of_str_to_parse = pStart_of_str_to_parse + 
                sip_strlen(pStart_of_str_to_parse) - 1 ;
        }
        else
        {
            pBody_begin = sip_stripLeadingLWS\
                          (pStart_of_str_to_parse,pEnd_of_str_to_parse );

            pBody_end = sip_stripTrailingLWS\
                        (pStart_of_str_to_parse,pEnd_of_str_to_parse );
            
            length_of_body = (SIP_S32bit)(pBody_end - pBody_begin) + 1;
            
            pTemp_str = sip_nStrdup\
                   (DECODE_MEM_ID,pBody_begin,(SIP_S32bit)length_of_body, err); 
        }

        if (SIP_NULL == pTemp_str )
        {
            /*Free the pName variable*/
            (void)fast_memfree(DECODE_MEM_ID, pName, SIP_NULL);
            /*Free the temporary SIP message*/
            sip_freeSipMessage(pSipMessage);
            return SipFail; 
        }
        
        /*Generate a single header from the hdr string*/
        /*For example if the hdr string is "Contact : <sip :u1@127.0.0.1 
          :33287>,<sip :u2@127.0.0.1:33289>" then while executing the
          while loop for the first time, single header which is specified in
          the following code by temp_header_str will be "Contact : <sip :u1@
          127.0.0.1:33287> */

        /* klocworks: Adjusting the length so that ABR do not happen
         */ 
        slen1 = sip_strlen(pName);
        slen2 = sip_strlen(pTemp_str);

        (void)sip_strncpy(temp_header_str,pName,SIP_MAX_HDR_SIZE);

		slen_temp_str = sip_strlen(temp_header_str);
     
        /* klocwokrs warning fixing: strcat have been changed to strncat so as
		 * to avoid possible ABR warnings
		 */
        if((SIP_MAX_HDR_SIZE  -1)> (slen_temp_str + 2))
		{
			remLen = SIP_MAX_HDR_SIZE - slen_temp_str -3;
            (void)sip_strncat(temp_header_str,(SIP_S8bit*)": ",remLen);
		}

		slen_temp_str = sip_strlen(temp_header_str);

        if((SIP_MAX_HDR_SIZE -1) > (slen_temp_str + slen2))
		{
			remLen = SIP_MAX_HDR_SIZE - slen_temp_str - slen2 - 3;
           (void)sip_strncat(temp_header_str,pTemp_str,remLen); 
		}

		slen_temp_str = sip_strlen(temp_header_str);

        temp_header_str[slen_temp_str] = '\0';
 
        /* Insert the individual string ie "Contact : <sip :u1@
           127.0.0.1:33287>in the temporary sip message 
           Later on we will fetch the header values from temporary
           sip messages and insert the header at particular index in 
           original sip message*/

        if (sip_insertHeaderFromStringAtIndex(pSipMessage,dType,temp_header_str,
                    count,err) == 0)
        {
            /*Free the pName variable*/
            (void)fast_memfree(DECODE_MEM_ID, pName, SIP_NULL);
            /*Free the pTemp_str variable*/
            (void)fast_memfree(DECODE_MEM_ID, pTemp_str, SIP_NULL);
            /*Free the temporary SIP message*/
            sip_freeSipMessage(pSipMessage);
            return SipFail ; 
        }
        ...
        /* Increment the value of count which is used for inserting the header
         * at particular index specified by count in temporary sip message*/ 
        count ++;
                
    }while (exitcondition != SipTrue);

    /*Free the pName variable*/
    (void)fast_memfree(DECODE_MEM_ID, pName, SIP_NULL);

    if (SipFail != sip_getHeaderCount(pSipMessage, dType, &header_count,
                err))
    {
        while(header_count != 0) 
        {
            SipHeader dParsedHeader;
            
            if(SipFail == sip_getHeaderAtIndex(pSipMessage, dType,&dParsedHeader,
                        header_count - 1, err))
            {
                /*Free the temporary SIP message*/  
                sip_freeSipMessage(pSipMessage);
            ...
            if(SipFail == sip_insertHeaderAtIndex(pMsg,&dParsedHeader,dIndex,
                        err))
            {
                sip_freeSipHeader(&dParsedHeader);
                /*Free the temporary SIP message*/
                sip_freeSipMessage(pSipMessage);
                return SipFail;
            }
...
#endif
{
    SIPDEBUGFN( "Entering sip_getTypeFromContactHeader");
#ifndef SIP_NO_CHECK
    if ( SIP_NULL == pErr )
        return SipFail;
    if ( SIP_NULL == pHdr )
...
#endif
    *pValue = ((SipCommonHeader *) (pHdr->pHeader))->dIntVar1; 	
    SIPDEBUGFN ( "Exiting sip_getTypeFromContactHdr");
    *pErr = E_NO_ERROR;
    return SipSuccess;
}
...
#endif
{
    ...
    if ( SIP_NULL == ppStrConn )
    {
        /*Set the value of pErr as invalid parameter*/
        *pErr = E_INV_PARAM;
        return SipFail;
    }
    if ( SIP_NULL == pSdpMsg->slConnection )
    {
        *pErr = E_INV_PARAM;
        return SipFail;
    }
#endif
    /*Allocate the memory equal to SIP_MAX_HDR_SIZE for pOut*/
    pOut = (SIP_S8bit *)fast_memget(ACCESSOR_MEM_ID, SIP_MAX_HDR_SIZE, pErr);

    if ( SIP_NULL == pOut )
    {
        /*Set the value of pErr as no memory availiable*/
        *pErr = E_NO_MEM;
        return SipFail;
    }	
    ...
    /*Making c-line string*/
    STRCPY ((char *)pOut, "c=");
    /*SPR 19706 Check that the pUser is non null and then copy bytes as per the
     * avaliable pOut memory*/ 
    if(SIP_NULL != pSdpMsg->slConnection->pNetType)
    {    
        (void)sip_strncat(pOut, pSdpMsg->slConnection->pNetType, SIP_MAX_HDR_SIZE - 3);
    }  
    /*SPR 19706 extract the strlen of pOut and then check if we still have two
     * bytes space then copy space*/
    tmpSize = sip_strlen(pOut);
    if((SIP_MAX_HDR_SIZE - tmpSize) > 1) 
    {    
         STR_CAT (pOut," ");
    }  
    /*SPR 19706 extract the strlen of pOut and then check if we still have two
    ...
    tmpSize = sip_strlen(pOut);
    if(((SIP_MAX_HDR_SIZE - tmpSize) > 1) &&
       (SIP_NULL != pSdpMsg->slConnection->pAddrType)) 
    {    
         (void)sip_strncat(pOut, pSdpMsg->slConnection->pAddrType, (SIP_MAX_HDR_SIZE -
            tmpSize - 1));
    }
    /*SPR 19706 extract the strlen of pOut and then check if we still have two
     * bytes space then copy space*/
    ...
    tmpSize = sip_strlen(pOut);
    if(((SIP_MAX_HDR_SIZE - tmpSize) > 1) &&
       (SIP_NULL != pSdpMsg->slConnection->pAddr)) 
    {    
         (void)sip_strncat(pOut, pSdpMsg->slConnection->pAddr, (SIP_MAX_HDR_SIZE -
            tmpSize - 1));
    }
	
    *ppStrConn = (SIP_S8bit *) STRDUPACCESSOR (pTempOut);
    if ( SIP_NULL == *ppStrConn )
    {
        /*Deallocate the memory occupied by pOut*/
        (void)fast_memfree(ACCESSOR_MEM_ID, pTempOut, pErr);
        *pErr = E_NO_MEM;
        return SipFail;
    }
...
#endif
{
    ...
    if ( SIP_NULL == ppStrOrigin )
    {
        /*Set the value of pErr as invalid parameter*/
        *pErr = E_INV_PARAM;
        return SipFail;
    }	
    if ( SIP_NULL == pSdpMsg->pOrigin )
    {
        *pErr = E_INV_PARAM;
        return SipFail;
    }
    ...
    /*Making o-line string*/
    STRCPY ((char *)pOut,"o=");
    /*SPR 19706 Check that the pUser is non null and then copy bytes as per the
     * avaliable pOut memory*/ 
    if(SIP_NULL != pSdpMsg->pOrigin->pUser)
    {    
        (void)sip_strncat(pOut, pSdpMsg->pOrigin->pUser, SIP_MAX_HDR_SIZE - 3);
    }  
    ...
    tmpSize = sip_strlen(pOut);
    if(((SIP_MAX_HDR_SIZE - tmpSize) > 1) &&
       (SIP_NULL != pSdpMsg->pOrigin->pSessionid)) 
    {    
         (void)sip_strncat(pOut, pSdpMsg->pOrigin->pSessionid, (SIP_MAX_HDR_SIZE -
            tmpSize - 1));
    }
    ...
    tmpSize = sip_strlen(pOut);
    if(((SIP_MAX_HDR_SIZE - tmpSize) > 1) &&
       (SIP_NULL != pSdpMsg->pOrigin->pVersion))
    {    
         (void)sip_strncat(pOut, pSdpMsg->pOrigin->pVersion, (SIP_MAX_HDR_SIZE -
            tmpSize - 1));
    }         
    tmpSize = sip_strlen(pOut);
    if((SIP_MAX_HDR_SIZE - tmpSize) > 1)
    ...
    tmpSize = sip_strlen(pOut);
    if(((SIP_MAX_HDR_SIZE - tmpSize) > 1) &&
       (SIP_NULL != pSdpMsg->pOrigin->pNetType))
    {    
         (void)sip_strncat(pOut, pSdpMsg->pOrigin->pNetType, (SIP_MAX_HDR_SIZE -
            tmpSize - 1));
    }         
    tmpSize = sip_strlen(pOut);
    if((SIP_MAX_HDR_SIZE - tmpSize) > 1)
    ...
    tmpSize = sip_strlen(pOut);
    if(((SIP_MAX_HDR_SIZE - tmpSize) > 1) &&
       (SIP_NULL != pSdpMsg->pOrigin->pAddrType))     
    {    
         (void)sip_strncat(pOut, pSdpMsg->pOrigin->pAddrType, (SIP_MAX_HDR_SIZE -
            tmpSize - 1));
    }         
    tmpSize = sip_strlen(pOut);
    if((SIP_MAX_HDR_SIZE - tmpSize) > 1)
    ...
    tmpSize = sip_strlen(pOut);
    if(((SIP_MAX_HDR_SIZE - tmpSize) > 1) &&
       (SIP_NULL != pSdpMsg->pOrigin->pAddr))
    {    
         (void)sip_strncat(pOut, pSdpMsg->pOrigin->pAddr, (SIP_MAX_HDR_SIZE -
            tmpSize - 1));
    }         
    *ppStrOrigin = (SIP_S8bit *) STRDUPACCESSOR (pTempOut);
...
/********************************************************************
**
** FUNCTION:  sip_getIPTKHeaderAsStringAtIndex
**
** DESCRIPTION:  This function retrieves a SipHeader of en_HeaderType
** "dType" in exactly the same way as sip_getHeaderAtIndex;
** the output pHeader here is in the form of a string "hdr".
...
**
*********************************************************************/
SipBool sip_getIPTKHeaderAsStringAtIndex
(SipMessage *s, en_HeaderType dType,  SIP_S8bit **hdr, SIP_U32bit dIndex, SipError *err)
{
    SipError pTempErr;
    if(validateSipHeaderType(dType,err)==SipFail)
    ...
    *hdr = (SIP_S8bit *)fast_memget(ACCESSOR_MEM_ID,SIP_MAX_HDR_SIZE,err);
    ...
    (void)sip_memset(*hdr, 0, SIP_MAX_HDR_SIZE);
    if (sip_formSingleIPTKHeader((*hdr+SIP_MAX_HDR_SIZE),dType, dIndex, SipModeNew, s, 
				0, *hdr, err) == SipFail)
    {
        (void)fast_memfree(ACCESSOR_MEM_ID, *hdr, &pTempErr);
        *hdr=SIP_NULL;
