 ...
 * 15-Jan-2008  Shruti Thakral                  Rel 8.0 changes for Sec Server
 *                                              Support
    ...
    switch (trace_id)
    {
        ...
        case ICF_CFG_TRACE_SEC_DNS_SERVER_ADDR_RECD:
            if(ICF_FOUR_INT_PRESENT == 
                    p_trace_data->num_of_integer_values)
            {
                ICF_PRINT(((icf_uint8_t*)"\n>>   Secondary DNS server address = %d", 
                            p_trace_data->int_val[0]));
                ICF_PRINT(((icf_uint8_t*)".%d", 
                            p_trace_data->int_val[1]));
                ICF_PRINT(((icf_uint8_t*)".%d", 
