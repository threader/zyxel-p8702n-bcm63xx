 ...
 * 16-JAN-2008  Anurag Khare       ICF Rel 8.0  Chnges for DHCP Server
...
ICF_4_0_CRM_END
		case ICF_CFG_MMI_MM_INIT_COMPLETE:
			...
			break;
/*  Now this feature is out of scope */
#if 0
	/*  Case added for ICF_REDISCOVER_SIP_SERVER API introduced 
	    for clearing the calls on the proxy*/
		case ICF_REDISCOVER_SIP_SERVER:
	    	    ret_val = icf_cfg_rediscover_proxy_server(&crm_pdb);
    		break;
#endif
        default:
...
ICF_CHECK_IMS_END
#endif
/*Removing the ICF_CHECK_IMS_START macro(run time check for mode)so as to make 
  ICF_CFG_CC_SET_TRANSPORT_PARAMS_REQ work for both IMS and NON_IMS mode*/    
...
#endif
/*Removing the ICF_CHECK_IMS_START macro(run time check for mode)so as to make 
  ICF_CFG_CC_SET_SELF_ID_PARAMS_REQ work for both IMS and NON_IMS mode*/      
#ifdef NON_IMS_CLIENT
...
#endif
/*Removing the ICF_CHECK_IMS_START macro(run time check for mode)so as to make 
  ICF_CFG_CC_SET_REGISTRATION_PARAMS_REQ work for both IMS and NON_IMS mode*/      
#ifdef NON_IMS_CLIENT
				ICF_STAT(p_crm_pdb->p_glb_pdb,
                        ICF_STAT_CFG_CC_SET_TRANS_PARAMS_REQ)
...
/*                ICF_STAT(p_crm_pdb->p_glb_pdb,
            ...
            case ICF_AKA_AUTH_RESP:
                 ret_val = icf_rgm_aka_auth_resp_hdlr(p_crm_pdb);
                 p_crm_pdb->resp_to_be_sent = ICF_FALSE;
                 break;                
