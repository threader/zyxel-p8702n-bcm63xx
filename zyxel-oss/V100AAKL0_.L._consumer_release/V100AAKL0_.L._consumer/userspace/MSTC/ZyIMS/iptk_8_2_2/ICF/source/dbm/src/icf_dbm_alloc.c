 ...
 *  17-May-2008   Amit Sharma      SPR:18577  Assigning sic_ctx and rgm_ctx in
 *                                           p_glb_pdb
 ...
 *  18-Feb-2009  Rajiv Kumar      IPTK Rel   Klockwork warning resolved
 *                                8.1  
 * 04-Mar-2009 Saurabh Gupta	   SPR 19349       Klocworks fix
 * 01-Apr-2009   Kamal Ashraf     Rel 8.2    Klicwork Fix
        ...
        OUT   icf_error_t                      *p_ecode)
{
        ...
        p_ecode))    
    {
        ...
        p_glb_pdb->p_rgm_ctx = *p_p_registration_data;
    }
        ...
        OUT   icf_error_t                       *p_ecode)
{
    ...
    else
    {
        ...
        *p_p_sic_context_data   = 
        ...
        p_glb_pdb->p_sic_ctx = (*p_p_sic_context_data);
...
/*Allocate memory for the Address block and each member of it */
    ...
    /* SPR 18880 : Call the following function to do memory
       allocations from the pool. This is a utility function
       which takes care of allocations of blocks from the pool
        ...
        IN    icf_rgm_context_st   *p_rgn_data)
{
    ...
    while((line_count < p_rgn_data->num_of_lines) && (line_count < ICF_MAX_NUM_OF_LINES))
    {
        ...
        icf_rgm_context_st   *p_rgn_data)
{
    ...
    else
    {
         ...
         * left*/
        while((line_count + 1 < p_rgn_data->num_of_lines) && ((line_count +1 ) < ICF_MAX_NUM_OF_LINES))
        {
            ...
            rgn_cntxt_blk_next_used_id)
    {
                    ...
                    p_user_addr))
        {
                        ...
                        &p_p_line_data[line_id]->registrar_addr))
            {
               ...
               if (p_rgn_blk_addr->num_of_lines < ICF_MAX_NUM_OF_LINES) 
			   {
        ...
        IN icf_address_st *p_adress2)
{
    ...
    {
        /* Fix for CSR 1-6421580 */	
        /* Compare user1@ip1 with user2@ip2, or e164-1 with e164-2 */
        if (((ICF_NULL != icf_port_strchr(p_adress1->addr_val.str, '@')) &&
			 (ICF_NULL != icf_port_strchr(p_adress2->addr_val.str, '@'))) ||
			((ICF_NULL == icf_port_strchr(p_adress1->addr_val.str, '@')) &&
			 (ICF_NULL == icf_port_strchr(p_adress2->addr_val.str, '@'))))
		{
		...
		/* Compare user1@ip with e164-2 */
		else if ((ICF_NULL != icf_port_strchr(p_adress1->addr_val.str, '@')) &&
				 (ICF_NULL == icf_port_strchr(p_adress2->addr_val.str, '@')))
		{
			icf_port_memcpy(&temp_addr, p_adress1, sizeof(icf_address_st));
			p_temp_str = (icf_uint8_t *)icf_port_strtok((icf_int8_t *)temp_addr.addr_val.str,"@");
		...
		/* Compare e164-1 with user2@ip */
		else if ((ICF_NULL == icf_port_strchr(p_adress1->addr_val.str, '@')) &&
				 (ICF_NULL != icf_port_strchr(p_adress2->addr_val.str, '@')))
		{
			icf_port_memcpy(&temp_addr, p_adress2, sizeof(icf_address_st));
			p_temp_str = (icf_uint8_t *)icf_port_strtok((icf_int8_t *)temp_addr.addr_val.str,"@");
 ...
 *              argument. For pool-id ICF_MEM_TIMER, it calls 
 *              icf_dbm_create_timer_block to allocate a pool.
 *
 * RETURNS: icf_return_t
 *
************************************************************************/
       ...
       OUT   icf_error_t                               *p_ecode)
{
	...
	switch(pool_id)
    {
        ...
        case ICF_MEM_TIMER:
            /* SPR 18880 : Call function icf_dbm_create_timer_block
               to allocate memory from the pool created at init time
            */   
            ret_val = icf_dbm_create_timer_block(p_glb_pdb,
                          p_p_mem_blk,p_ecode);
            /* Setting size to NULL to avoid the direct memget below */
            size = ICF_NULL;
            break;
        ...
        IN icf_address_st *p_adress2)
{
    ...
    {
        ICF_FUNCTION_EXIT(p_glb_pdb)
        /* Some input variable is NULL. Return ICF_SUCCESS so 
           that the calling function fails to process the
           redirect request.
        */   
        return ICF_SUCCESS;
    }

    /* Proceed to compare the two addresses */
    if((p_adress1->addr_type != p_adress2->addr_type) || 
       (p_adress1->addr_val.str_len != p_adress2->addr_val.str_len) ||
       (ICF_NULL != icf_port_strcmp(p_adress1->addr_val.str, 
                                    p_adress2->addr_val.str)))
    {
        ret_val = ICF_FAILURE;
    }
...
{
    icf_boolean_t   ret_val = ICF_FALSE;
    icf_address_st  temp_addr1 = {{0,"\0"},0},
                    temp_addr2 = {{0,"\0"},0};
    ...
    if((p_adress1->addr_type == p_adress2->addr_type) ||
       ((ICF_ADDRESS_PLAN_SIP_URI == p_adress1->addr_type) &&
        (ICF_ADDRESS_PLAN_E164 == p_adress2->addr_type)) ||
       ((ICF_ADDRESS_PLAN_E164 == p_adress1->addr_type) &&
        (ICF_ADDRESS_PLAN_SIP_URI == p_adress2->addr_type)))
    {
        icf_port_memcpy(&temp_addr1,p_adress1, 
                        ICF_PORT_SIZEOF(icf_address_st));
        icf_port_memcpy(&temp_addr2,p_adress2,
                        ICF_PORT_SIZEOF(icf_address_st));
        ...
        if((ICF_NULL != p_temp_str1) && (ICF_NULL != p_temp_str2))
        {
