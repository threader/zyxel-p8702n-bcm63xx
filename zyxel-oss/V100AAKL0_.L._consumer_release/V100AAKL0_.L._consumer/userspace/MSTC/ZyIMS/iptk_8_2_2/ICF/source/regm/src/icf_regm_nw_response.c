 ...
 * 11-Mar-2008  Divij Agarwal   SPR 18122           Support for async AKA negotiation
...
ICF_4_0_CRM_END
    ...
    {
        ICF_PRINT(((icf_uint8_t *)"\n[ICF][REGM]:p_rgm_context is NULL"))
        return ICF_FAILURE;
    }
...
ICF_CHECK_IMS_END
    ...
    {
        ICF_PRINT(((icf_uint8_t *)"\n[ICF][REGM]:p_crm_pdb->p_config_data is NULL"))
        return ICF_FAILURE;
    }
...
#endif
    /*This is for the guard timer that was started before
      sending out any reg/dereg request on n/w*/
    
    /* The registration guard timer will be stopped when any
     * network response (other than valid 401 or 407) is 
     * received from network. */
    if((ICF_RGM_SSA_NW_FAILURE == p_nw_resp->status) &&
       (ICF_RGM_SSA_AUTHENTICATION_REQ == p_nw_resp->ecode))
    {
        /* Do not stop guard timer in this case */
        /* increment the stats for challenge received */
        ICF_STATS(p_crm_pdb->p_glb_pdb->p_stats_info->stat_register.
                                               stats_reg_auth_received)
    }
    else
    {
    ...
    else if (p_nw_resp->status == ICF_RGM_SSA_NW_FAILURE)
    {
        switch (p_nw_resp->ecode)
        {
            ...
            case ICF_REGM_SSA_AUTH_PARAM_NOT_RCVD:
				...
				p_crm_pdb->error_cause = ICF_REGM_SSA_AUTH_PARAM_NOT_RCVD;
                /* Trigger FSM */
				if ( p_rgm_context->rgm_state < ICF_RGM_MAX_STATES)
				{
                ret_val = p_rgm_glb_data->icf_rgm_state_machine
...
ICF_4_0_CRM_START
           ...
           case ICF_CAUSE_DNS_SERVER_NOT_RESPONDING:
           case ICF_CAUSE_PROXY_SERVER_NOT_RESPONDING:
           case ICF_CAUSE_REGISTRAR_NOT_RESPONDING:

                ICF_RGM_TRACE(p_crm_pdb,
					ICF_TRACE_REGISTRATION_FSM_TRIGGERED)
               
                p_crm_pdb->error_cause = p_nw_resp->ecode;
