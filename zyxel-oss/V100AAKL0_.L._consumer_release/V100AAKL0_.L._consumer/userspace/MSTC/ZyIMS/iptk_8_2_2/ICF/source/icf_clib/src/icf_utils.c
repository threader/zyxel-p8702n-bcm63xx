...
* 17-Apr-2008  Abhishek Dhammawat SPR 18459 Modified icf_free_msg
* 28-Apr-2008   Alok Tiwari        SPR 18456    Klocwork warning removal
* 30-May-2008   Tarun Gupta        SPR 18585    Modified icf_get_api_length
...
* 11-Feb-2009   Ashutosh Mohan     IPTK Rel8.1  Added code for following APIs
*                                               ICF_TUNNELLED_CREATE_MEDIA_SESSION_REQ
*                                               ICF_TUNNELLED_CREATE_MEDIA_SESSION_RESP
*                                               ICF_TUNNELLED_MODIFY_MEDIA_SESSION_REQ
*                                               ICF_TUNNELLED_MODIFY_MEDIA_SESSION_RESP
*                                               ICF_TUNNELLED_MEDIA_CAPABILITIES_RESP
*                                               ICF_UPDATE_REQ_SEND
*                                               ICF_EARLY_SESSION_IND
*                                               ICF_EARLY_SESSION_CFM
*                                               ICF_TUNNELLED_OPTIONS_RESP
* 27-Feb-2009  Tarun Gupta         Rel 8.1      Removed handling of ICF_UPDATE_REQ_SEND
* 30-Mar-2009 Abhishek Dhammawat   Rel 8.2      Modified icf_get_api_length
* 30-Mar-2009 Abhishek Dhammawat   Rel 8.2      Modified icf_free_msg
* 31-Mar-2009 Preksha              Rel 8.2      Modified icf_get_api_length
* 28-May-2009  Tarun Gupta         Rel 8.2      Async NOTIFY Support
* 02-Jun-2009 Preksha              Rel 8.2      Asynchronus MESSAGE Enh
* 22-Jun-2009 Anuradha Gupta       SPR 19752    Modified icf_free_msg
* 03-Sep-2009 Abhishek Dhammawat   SPR 20016    Modified
*                                               icf_make_msg_body_node
...
icf_return_t icf_make_msg_body_node(icf_msg_body_st** start, icf_uint8_t count)
{
    ...
    while (count--) {
        ...
        /*SPR 20016 As the msg body can be of any size within 
          ICF_MAX_API_PAYLOAD_LENGTH so instead of 40 * 128 use the 
          payload length*/
        p_body->value =
            (icf_uint8_t*)ICF_CLIB_MEMGET(ICF_MAX_API_PAYLOAD_LENGTH);

        if (!p_body->value) {
            ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_make_msg_body_node - ICF_CLIB_MEMGET Failed for Body Value node (p_body->value)\n"));
            ICF_CLIB_MEMFREE(p_body);
            return ICF_FAILURE;
        }
...
icf_return_t icf_free_msg(icf_msg_st* p_msg)
{
	...
	case ICF_CONNECT_REQ:
		...
		break;
        /*Change start for release 8.2*/
	case ICF_GENERIC_MSG_REQ:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_GENERIC_MSG_REQ\n"));

		if(((icf_generic_msg_req_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_generic_msg_req_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_generic_msg_req_st*)p_msg->payload)->header_list.count);
        }    
		if(((icf_generic_msg_req_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
        {    
			ret_val = icf_free_msg_body_node(&((icf_generic_msg_req_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_generic_msg_req_st*)p_msg->payload)->body_list.count);
        }   
		if(((icf_generic_msg_req_st*)p_msg->payload)->bit_mask & ICF_TAG_LIST_PRESENT)
        {    
			ret_val = icf_free_tag_node(&((icf_generic_msg_req_st*)p_msg->payload)->tag_list.tag, 
            (icf_uint8_t)((icf_generic_msg_req_st*)p_msg->payload)->tag_list.count);
        }    
		break;
	case ICF_GENERIC_MSG_RESP:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_GENERIC_MSG_RESP\n"));

		if(((icf_generic_msg_resp_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_generic_msg_resp_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_generic_msg_resp_st*)p_msg->payload)->header_list.count);
        }    

		if(((icf_generic_msg_resp_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
        {    
			ret_val = icf_free_msg_body_node(&((icf_generic_msg_resp_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_generic_msg_resp_st*)p_msg->payload)->body_list.count);
        }   
		break;
	case ICF_GENERIC_MSG_CFM:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_GENERIC_MSG_CFM\n"));

		if(((icf_generic_msg_cfm_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_generic_msg_cfm_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_generic_msg_cfm_st*)p_msg->payload)->header_list.count);
        }    

		if(((icf_generic_msg_cfm_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
        {    
			ret_val = icf_free_msg_body_node(&((icf_generic_msg_cfm_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_generic_msg_cfm_st*)p_msg->payload)->body_list.count);
        }   
		
        if(((icf_generic_msg_cfm_st*)p_msg->payload)->bit_mask & ICF_TAG_LIST_PRESENT)
        {    
			ret_val = icf_free_tag_node(&((icf_generic_msg_cfm_st*)p_msg->payload)->tag_list.tag, 
            (icf_uint8_t)((icf_generic_msg_cfm_st*)p_msg->payload)->tag_list.count);
        }    
		break;
	case ICF_GENERIC_MSG_IND:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_GENERIC_MSG_IND\n"));

		if(((icf_generic_msg_ind_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_generic_msg_ind_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_generic_msg_ind_st*)p_msg->payload)->header_list.count);
        }    

		if(((icf_generic_msg_ind_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
        {    
			ret_val = icf_free_msg_body_node(&((icf_generic_msg_ind_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_generic_msg_ind_st*)p_msg->payload)->body_list.count);
        }   
		break;

    case ICF_PRACK_REQ_RECV_IND:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_PRACK_REQ_RECV_IND\n"));

        if(((icf_prack_req_recv_ind_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_prack_req_recv_ind_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_prack_req_recv_ind_st*)p_msg->payload)->header_list.count);
        }    
		if(((icf_prack_req_recv_ind_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
        {    
			ret_val = icf_free_msg_body_node(&((icf_prack_req_recv_ind_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_prack_req_recv_ind_st*)p_msg->payload)->body_list.count);
        }   
		    
		break;
	case ICF_PRACK_RESP_SENT_IND:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_PRACK_RESP_SENT_IND\n"));

		if(((icf_prack_resp_sent_ind_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_prack_resp_sent_ind_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_prack_resp_sent_ind_st*)p_msg->payload)->header_list.count);
        }    

		break;
	case ICF_PRACK_REQ_SENT_IND:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_PRACK_REQ_SENT_IND\n"));

		if(((icf_prack_req_sent_ind_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_prack_req_sent_ind_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_prack_req_sent_ind_st*)p_msg->payload)->header_list.count);
        }    

        break;
    case ICF_PRACK_RESP_RECV_IND:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_PRACK_RESP_RECV_IND\n"));

		if(((icf_prack_resp_recv_ind_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
			ret_val = icf_free_hdr_node(&((icf_prack_resp_recv_ind_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_prack_resp_recv_ind_st*)p_msg->payload)->header_list.count);
        }    

		if(((icf_prack_resp_recv_ind_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
        {    
			ret_val = icf_free_msg_body_node(&((icf_prack_resp_recv_ind_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_prack_resp_recv_ind_st*)p_msg->payload)->body_list.count);
        }   
		break;
...
****************************************/
	...
	case ICF_INCOMING_CALL_MODIFY_IND:
		...
		break;
    /*SPR18459 Added handling to free lists in INFO_CFM API*/    
	case ICF_INFO_CFM:
		ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_INFO_CFM\n"));
        if(((icf_info_cfm_st*)p_msg->payload)->bit_mask & ICF_INFO_CFM_HEADER_LIST_PRESENT)
        {
		  ret_val = icf_free_hdr_node(&((icf_info_cfm_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_info_cfm_st*)p_msg->payload)->header_list.count);
        }    
		if(((icf_info_cfm_st*)p_msg->payload)->bit_mask & ICF_INFO_CFM_TAG_LIST_PRESENT)
	    {
		  ret_val = icf_free_tag_node(&((icf_info_cfm_st*)p_msg->payload)->tag_list.tag, 
            (icf_uint8_t)((icf_info_cfm_st*)p_msg->payload)->tag_list.count);
        }
        if(((icf_info_cfm_st*)p_msg->payload)->bit_mask & ICF_INFO_CFM_BODY_LIST_PRESENT)
        {
		  ret_val = icf_free_msg_body_node(&((icf_info_cfm_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_info_cfm_st*)p_msg->payload)->body_list.count);
        }
        break;
...
/* RN*/
    ...
    case ICF_REMOTE_CALL_TRANSFER_INITIATED_CFM:
         ...
         if (((icf_remote_call_transfer_initiated_cfm_st*)p_msg->payload)-> \
               bit_mask & ICF_HDR_LIST_PRESENT)
         {
             ret_val = icf_free_hdr_node(
                 &((icf_remote_call_transfer_initiated_cfm_st*)p_msg->payload)-> \
                 header_list.hdr,
                 (icf_uint8_t)((icf_remote_call_transfer_initiated_cfm_st*)p_msg-> \
                 payload)->header_list.count);
         }
         break;
    /* End of SPR 19218 : Merged CSR 1-5478785 */


case ICF_EARLY_SESSION_IND:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_CREATE_CALL_REQ\n"));

		if (((icf_early_session_ind_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
			ret_val = icf_free_hdr_node(&((icf_early_session_ind_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_early_session_ind_st*)p_msg->payload)->header_list.count);

case ICF_TUNNELLED_OPTIONS_RESP:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_CREATE_CALL_REQ\n"));

		if (((icf_tunnelled_options_resp_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
			ret_val = icf_free_hdr_node(&((icf_tunnelled_options_resp_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_tunnelled_options_resp_st*)p_msg->payload)->header_list.count);


		if (((icf_tunnelled_options_resp_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
			ret_val = icf_free_msg_body_node(&((icf_tunnelled_options_resp_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_tunnelled_options_resp_st*)p_msg->payload)->body_list.count);
		
		break;

        case ICF_TERMINATE_CALL_REQ:
            ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_TERMINATE_CALL_REQ\n"));

		    if (((icf_terminate_call_req_st *)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
			    ret_val = icf_free_hdr_node(&((icf_terminate_call_req_st *)p_msg->payload)->header_list.hdr, 
                   (icf_uint8_t)((icf_terminate_call_req_st *)p_msg->payload)->header_list.count);

            if (((icf_terminate_call_req_st *)p_msg->payload)->bit_mask & ICF_TAG_LIST_PRESENT)
			    ret_val = icf_free_tag_node(&((icf_terminate_call_req_st *)p_msg->payload)->tag_list.tag, 
                    (icf_uint8_t)((icf_terminate_call_req_st *)p_msg->payload)->tag_list.count);

		    if (((icf_terminate_call_req_st *)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
			    ret_val = icf_free_msg_body_node(&((icf_terminate_call_req_st *)p_msg->payload)->body_list.body, 
                    (icf_uint8_t)((icf_terminate_call_req_st *)p_msg->payload)->body_list.count);
		
		break;

       case ICF_TERMINATE_CALL_IND:
            ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_TERMINATE_CALL_IND\n"));

		    if (((icf_terminate_call_ind_st *)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
			    ret_val = icf_free_hdr_node(&((icf_terminate_call_ind_st *)p_msg->payload)->header_list.hdr, 
                   (icf_uint8_t)((icf_terminate_call_ind_st *)p_msg->payload)->header_list.count);


		    if (((icf_terminate_call_ind_st *)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
			    ret_val = icf_free_msg_body_node(&((icf_terminate_call_ind_st *)p_msg->payload)->body_list.body, 
                    (icf_uint8_t)((icf_terminate_call_ind_st *)p_msg->payload)->body_list.count);
		...
		ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_NOTIFY_CFM\n"));
        if(((icf_notify_cfm_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {
		  ret_val = icf_free_hdr_node(&((icf_notify_cfm_st*)p_msg->payload)->header_list.hdr, 
            (icf_uint8_t)((icf_notify_cfm_st*)p_msg->payload)->header_list.count);
        }    
		if(((icf_notify_cfm_st*)p_msg->payload)->bit_mask & ICF_TAG_LIST_PRESENT)
	    {
		  ret_val = icf_free_tag_node(&((icf_notify_cfm_st*)p_msg->payload)->tag_list.tag, 
            (icf_uint8_t)((icf_notify_cfm_st*)p_msg->payload)->tag_list.count);
        }
        if(((icf_notify_cfm_st*)p_msg->payload)->bit_mask & ICF_BODY_LIST_PRESENT)
        {
		  ret_val = icf_free_msg_body_node(&((icf_notify_cfm_st*)p_msg->payload)->body_list.body, 
            (icf_uint8_t)((icf_notify_cfm_st*)p_msg->payload)->body_list.count);
        }
        break;

        /* Added for asynchronous MESSAGE support */
        case ICF_MESSAGE_CFM:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_MESSAGE_CFM\n"));

        if(((icf_message_cfm_st*)p_msg->payload)->bit_mask & ICF_HDR_LIST_PRESENT)
        {   
            ret_val = icf_free_hdr_node(&((icf_message_cfm_st*)p_msg->payload)->header_list.hdr, 
                    (icf_uint8_t)((icf_message_cfm_st*)p_msg->payload)->header_list.count);
        }    

        if(((icf_message_cfm_st*)p_msg->payload)->bit_mask & ICF_TAG_LIST_PRESENT)
        {    
            ret_val = icf_free_tag_node(&((icf_message_cfm_st*)p_msg->payload)->tag_list.tag, 
                    (icf_uint8_t)((icf_message_cfm_st*)p_msg->payload)->tag_list.count);
        }    
        break;

        /* SPR 19752 : The srtp fallback resp code has to be freed
         * in case not absorbed by IPTK
         * In case the p_srtp_fallback_resp_codes list is not NULL then
         * free the list here
         */ 
        case ICF_SET_CALL_PARAM_REQ:
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_free_msg - ICF_SET_CALL_PARAM_REQ\n"));

        if(((icf_set_call_params_req_st *)p_msg->payload)->bit_mask & 
                ICF_SET_CALL_PRM_SRTP_FALLBACK_FLAG)
        {
            if(ICF_NULL != ((icf_set_call_params_req_st *)p_msg->payload)-> \
                    p_srtp_fallback_resp_codes)
            { 
                icf_free_list(((icf_set_call_params_req_st *)p_msg->payload)-> \
                    p_srtp_fallback_resp_codes);
            }
        }
        break;
...
****************************************/
	...
	case ICF_TUNNELLED_CREATE_MEDIA_SESSION_REQ:
		api_length = sizeof(icf_tunnelled_create_media_session_req_st);
		break;

	case ICF_TUNNELLED_CREATE_MEDIA_SESSION_RESP:
		api_length = sizeof(icf_tunnelled_create_media_session_resp_st);
		break;

	case ICF_TUNNELLED_MODIFY_MEDIA_SESSION_REQ:
		api_length = sizeof(icf_tunnelled_modify_media_session_req_st);
		break;

	case ICF_TUNNELLED_MODIFY_MEDIA_SESSION_RESP:
		api_length = sizeof(icf_tunnelled_modify_media_session_resp_st);
		break;

	case ICF_TUNNELLED_MEDIA_CAPABILITIES_RESP:
		api_length = sizeof(icf_tunnelled_media_capabilities_resp_st);
		break;
...
/*NOTIFY_OUT_OF_DIALOG_END*/
   ...
   case ICF_TRACE_NW_BUFF_IND :
    ...
    case ICF_REMOTE_USER_TERMINATE_CALL_RESP:
        api_length = sizeof(icf_remote_user_term_call_resp_st);
        break;
	...
	break;
     ...
     case ICF_CALL_MODIFY_CANCEL_REQ:
        ...
        case ICF_AKA_AUTH_RESP:										
        	api_length = sizeof(icf_aka_auth_resp_st);
        	break;

        case ICF_DISPLAY_IND:
               api_length = sizeof(icf_display_ind_st);
        	break;

	case ICF_EARLY_SESSION_IND:
               api_length = sizeof(icf_early_session_ind_st);
        	break;

	case ICF_EARLY_SESSION_CFM:
               api_length = sizeof(icf_early_session_cfm_st);
        	break;

	case ICF_TUNNELLED_OPTIONS_RESP:
               api_length = sizeof(icf_tunnelled_options_resp_st);
        	break;


#ifdef ICF_UT_TEST
        case ICF_UT_SIMULATE:
            api_length = sizeof(icf_ut_simulate_st); 
            break;
        case ICF_UATK_API_TEST:
            api_length = sizeof(icf_uatk_api_test_st); 
            break;
#endif
#ifdef ICF_DNS_LOOKUP_ENABLED
	case ICF_DNS_MAPPING_CHANGE_IND:
    	api_length = sizeof(icf_dns_mapping_change_ind_st);
        break;


#endif /* ICF_DNS_LOOKUP_ENABLED */   
    case ICF_SESSION_TIMER_TOGGLE_REQ:
    	api_length = sizeof(icf_session_timer_toggle_req_st);
        break;

    case ICF_SESSION_TIMER_TOGGLE_RESP:
    	api_length = sizeof(icf_session_timer_toggle_resp_st);
        break;
    /*Change Start for Rel 8.2*/
    case ICF_GENERIC_MSG_REQ:
    	api_length = sizeof(icf_generic_msg_req_st);
        break;
    case ICF_GENERIC_MSG_RESP:
    	api_length = sizeof(icf_generic_msg_resp_st);
        break;
    case ICF_GENERIC_MSG_IND:
    	api_length = sizeof(icf_generic_msg_ind_st);
        break;
    case ICF_GENERIC_MSG_CFM:
    	api_length = sizeof(icf_generic_msg_cfm_st);
        break;

    case ICF_PRACK_RESP_SENT_IND:
        api_length = sizeof(icf_prack_resp_sent_ind_st);
        break;
    case ICF_PRACK_REQ_SENT_IND:
        api_length = sizeof(icf_prack_req_sent_ind_st);
        break;
    case ICF_PRACK_RESP_RECV_IND:
        api_length = sizeof(icf_prack_resp_recv_ind_st);
        break;
    case ICF_PRACK_REQ_RECV_IND:
        api_length = sizeof(icf_prack_req_recv_ind_st);
        break;

    /*Change End for Rel 8.2*/

    case ICF_MESSAGE_CFM:
        api_length = sizeof(icf_message_cfm_st);
        break;
...
#ifndef ICF_KLOCWORK_SHOW_FALSE_POSITIVE
   	p_junk_temp->p_data = (*p_p_curr_node)->p_data;
