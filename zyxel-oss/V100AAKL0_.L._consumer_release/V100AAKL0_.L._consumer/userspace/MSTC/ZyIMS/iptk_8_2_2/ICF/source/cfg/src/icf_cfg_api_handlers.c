 ...
 * 24-Jan-2008  Kamini Gangwani    ICF Rel 8.0     Modified function icf_cfg_internal_init
 * 28-Jan-2008  Amit Mahajan       ICF Rel 8.0     reg event package support changes.
 * 30-Jan-2008  Abhishek Dhammawat Rel8.0          removed handling for
 *                                                 ICF_QOS_RES_RESERV_SUPPORTED
 *                                                 present in icf.cfg
 * 3-Apr-2008   Sumant Gupta    ICF 8.0            Rel7.1 Windows porting Merging
 *
 * 15-Apr-2008  Alok Tiwari     SPR-18359          Klocwork Warning resolved.
 * 28-Apr-2008  Kamini Gangwani SPR-18468          changes for icf.cfg cleanup
 * 30-Apr-2008  Amit Sharma     SPR-18474          Reinit sec_dns_server_active
 * 01-May-2008  Amit Sharma     SPR:18469          REinit config params on receiving
 *                                                 reinit_req from application
 * 07-Jun-2008  Amit Sharma       IPTK 8.0 SRTP LLD      SRTP Enhancements
 * 13-Jun-2008  Tarun Gupta     SPR 18585          Modified icf_cfg_set_line_params_hdlr
 * 25-Jun-2008  Abhishek Dhammawat  IPTK Rel8.0    Modified 
 *                                                 icf_cfg_set_line_params_hdlr
 * 03-July-2008 Anurag Khare     SPR 18684         CSR 1-6068965
 * 05-Jul-2008  Amit Sharma     SPR 18762          Code removed to initialise transport
 *                                                 modes on receving reinit
 * 08-July-2008 Anurag Khare    SPR 18798          Modified function icf_cfg_delete_config_hdlr
 * 04-Nov-2008  Rajiv Kumar     SPR 19188          Rel 7.1.2 SPR merging (SPR
 *                                                 19001) 
 * 07-Nov-2008  Rajiv Kumar     SPR 19188          Rel 7.1.2 SPR merging (SPR
 *                                                 19012)
 * 12-Nov-2008  Anuradha gupta  SPR 19188          Rel 7.1.2 SPR Merging (SPR 18972)
 * 14-Nov-2008  Rajiv Kumar     SPR 19188          Rel 7.1.2 SPR merging (SPR
 *                                                 19120)
 * 17-Nov-2008  Rajiv Kumar     SPR 19188          Rel 7.1.2 SPR merging (SPR
 *                                                 19134)
 * 03-Dec-2008 Abhishek Dhammawat SPR 19218        Merge the CSR 1-6246508
 * 11-Dec-2008  Anurag Khare    SPR 19218          CSR 1-6456839 Merged
 * 06-Feb-2009  Tarun Gupta     Rel 8.1 LLD        SDP Tunnelling Enhancements
 * 02-Mar-2009 Kamal Ashraf     Rel 8.1            code added in icf_cfg_internal_init
 *                                                 function for open source replacement  
 * 04-Mar-2009 Saurabh Gupta	   SPR 19349       Klocworks fix
 * 20-Mar-2009  Abhishek Dhammawat REL 8.1     Modified icf_cfg_delete_config_hdlr
 * 27-Mar-2009  Anuradha Gupta     REL 8.2         Changes for Call Clear Trigger 
 * 2-May-2009  Abhishek Dhammawat  SPR19571    Modified
 *                                             icf_cfg_application_add_hdlr
 ...
 * 02-Aug-2009 Rajiv Kumar        SPR 19886    Merged SPR 19425  
 ...
 * 24-May-2010 Preksha            SPR 20412    Merged CSR 1-8047728 
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
   ...
   /* Get DBM data to fetch application context */
    ...
    /*Fix for SPR : 19188 (Rel 7.1.2 SPR merging for SPR 19012)*/
    /* SPR 19012:
        ...
        INOUT   icf_crm_pdb_st           *p_crm_pdb)
{
    ...
    ICF_FUNCTION_ENTER(p_crm_pdb->p_glb_pdb)
    /*Change Start Rel8.1*/
    /*Previously the thread for arDnsClient was restarted at end of function.
      In the scenario when DNS query in done for BYE and response is awaited
      we were restarting the thread and when response received we were
      starting the DNS response timer. It is better to have restart of thread
      at start of the reinit and then post query for BYE so that when response
      for DNS query (BYE) is received we stop the DNS response timer*/ 
#ifdef ICF_DNS_LOOKUP_ENABLED    
    /* Free the memory allocated for DNS client fds allocated at the time of
     * initilisation*/
    if (ICF_NULL != p_crm_pdb->p_glb_pdb->p_glb_data->p_glb_port_info) 
    {
        icf_global_port_info_st     *p_glb_port_info = ICF_NULL;
        p_glb_port_info = p_crm_pdb->p_glb_pdb->p_glb_data->p_glb_port_info;
    ...
    if (ICF_FAILURE == icf_port_init_dns_client(\
                p_crm_pdb->p_config_data->p_arDnsClient_addr,\
                p_crm_pdb->p_config_data->p_arDnsClib_addr,\
                p_crm_pdb->p_glb_pdb,
                ICF_TRUE))
    {
        ret_val = ICF_FAILURE;
    ...
    /*The remaining handling should be done for both the case 
     *either init_complete happens or not 
     */
    /* Reset line data */
						...
						(line_index < ICF_MAX_NUM_OF_LINES)); line_index++)
    {
       ...
       /* SPR 18972: In case codec profile is present for audio and video then
        * free the memory
        */ 
       if(ICF_NULL != p_crm_pdb->p_line_data[line_index]->p_codec_attributes_audio)
       {
            icf_cmn_delete_codec_attrib_list(p_crm_pdb->p_glb_pdb,
					&p_crm_pdb->p_line_data[line_index]->p_codec_attributes_audio,
					p_crm_pdb->p_ecode);
       }


       if(ICF_NULL != p_crm_pdb->p_line_data[line_index]->p_codec_attributes_video)
       {
            icf_cmn_delete_codec_attrib_list(p_crm_pdb->p_glb_pdb,
					&p_crm_pdb->p_line_data[line_index]->p_codec_attributes_video,
					p_crm_pdb->p_ecode);
       }

       if (ICF_NULL != p_crm_pdb->p_line_data[line_index]->p_sec_registrar_addr)
       {
           ICF_MEMFREE(
                   p_crm_pdb->p_glb_pdb,
                   p_crm_pdb->p_line_data[line_index]->p_sec_registrar_addr,
                   ICF_MEM_COMMON,
                   p_crm_pdb->p_ecode,
                   ret_val)
       } 
       if (ICF_NULL != p_crm_pdb->p_line_data[line_index]->p_sec_proxy_addr)
       {
           ICF_MEMFREE(
                   p_crm_pdb->p_glb_pdb,
                   p_crm_pdb->p_line_data[line_index]->p_sec_proxy_addr,
                   ICF_MEM_COMMON,
                   p_crm_pdb->p_ecode,
                   ret_val)
       }
       /*Free the memory for the access_network_info header configuration*/
       if (ICF_NULL != p_crm_pdb->p_line_data[line_index]->p_access_network_info)
       {
           ICF_MEMFREE(p_crm_pdb->p_glb_pdb,
                  p_crm_pdb->p_line_data[line_index]->p_access_network_info,
                  ICF_MEM_COMMON,
                  p_crm_pdb->p_ecode, 
                  ret_val)
       }
        ...
        /* SPR 19886 : Additional fix done while merging Changes for CSR 1-7334184 Start */
        p_crm_pdb->p_line_data[line_index]->remove_route_hdr = ICF_FALSE;
        ...
        if(dType == ICF_PORT_IPV4_FAMILY){
            ...
            /*  Make the address type INVALID because the address is not valid now */
        ...
        /*Fix for SPR 19134 */
        p_crm_pdb->p_config_data->xconf_subs_duration = 0;
        p_crm_pdb->p_config_data->xconf_subs_reqd = ICF_FALSE;        
        ...
        p_crm_pdb->p_config_data->check_remote_codec_list = ICF_FALSE;

        /* CSR_1-7113139 Merged SPR 19672 Start */
        /*SPR 19462*/
        p_crm_pdb->p_config_data->inc_sdp_version_when_no_change = ICF_FALSE;
        /* CSR_1-7113139 Merged SPR 19672 End */
    ...
    /* Check for IMS/Non IMS mode of operation, and set the Default values 
       in config data.In case of IMS Mode by default it is set to TRUE and
       in NON-IMS mode by default it is set to FALSE.*/
    if (ICF_OPERATION_MODE_IMS == p_crm_pdb->p_config_data->ims_oprn_flag)
    {
        p_crm_pdb->p_config_data->reg_event_pkg_support_reqd = ICF_TRUE;
    }
    else
    {
        p_crm_pdb->p_config_data->reg_event_pkg_support_reqd = ICF_FALSE;
    }
    
    p_crm_pdb->p_config_data->is_reg_triggered = ICF_FALSE;
...
/* Get DBM data to fetch application context */
    ...
    if (ICF_NULL != p_crm_pdb->p_config_data->p_dhcp_resp)
    {
        ICF_MEMFREE(
                    p_crm_pdb->p_glb_pdb,
    ...
    p_crm_pdb->p_config_data->dhcp_client_port = ICF_DHCP_CLIENT_PORT;
    p_crm_pdb->p_config_data->dhcp_response_timer = ICF_DHCP_RESPONSE_TIMER;
    ...
    if (ICF_NULL != p_crm_pdb->p_config_data->p_sec_dns_server_addr)
    {
        ICF_MEMFREE(
                    p_crm_pdb->p_glb_pdb,
    ...
    /*Reinitialize sec_dns_server_active and sec_proxy_server_active flag*/
    p_crm_pdb->p_config_data->sec_dns_server_active = ICF_FALSE;
    p_crm_pdb->p_config_data->sec_proxy_server_active = ICF_FALSE;

    p_crm_pdb->p_config_data->max_active_calls_per_line = ICF_ACTIVE_CALLS_PER_LINE_DEFAULT;

    /*Free p_access_nw_info configured by application*/
    ...
    /*media_transport_type is set to RTP/AVP by default*/
    p_crm_pdb->p_config_data->media_transport_type = ICF_TRANSPORT_MODE_RTP_AVP;

#ifdef ICF_QOS_SUPPORT
    p_crm_pdb->p_config_data->precondition_required = ICF_FALSE;
#endif

    /*Reset srtp_fallback_reqd flag*/
    p_crm_pdb->p_config_data->srtp_fallback_reqd = ICF_FALSE;
    
    /*Delete SRTP failure response codes if already configured*/
    ...
    */
    ...
    /* Reset the field early_incoming_session_ind_reqd in the config data */
    p_crm_pdb->p_config_data->early_incoming_session_ind_reqd = ICF_FALSE;
    /* reset the flag*/
	p_crm_pdb->p_config_data->update_for_offeranswer_after_connect = ICF_FALSE;
    p_crm_pdb->p_config_data->retry_codec_commit_req = ICF_FALSE;
    p_crm_pdb->p_config_data->app_connect_reqd_for_replaces = ICF_FALSE;
    p_crm_pdb->p_config_data->check_remote_codec_list = ICF_FALSE;
    p_crm_pdb->p_config_data->fax_req_method = ICF_CFG_CALL_MODIFY_METHOD_INVITE;

    /* Asynchronous NOTIFY support */
    /* Reset the field async_notify_support_reqd in config data to its default value */
    p_crm_pdb->p_config_data->async_notify_support_reqd = ICF_FALSE;
    /* Asynchronous MESSAGE support */
    /* Reset the field async_message_support_reqd in config data to its default value */
    p_crm_pdb->p_config_data->async_message_support_reqd = ICF_FALSE;
...
/*			ICF_SET_BIT(p_crm_pdb->p_config_data->opt_parms_bitmask,
    ...
    /*This following field ie.is_reg_triggered indicates if any registration
      request has been triggered by application.Initially set to FALSE*/
    p_crm_pdb->p_config_data->is_reg_triggered = ICF_FALSE;
...
ICF_CRM_START
    ...
    p_crm_pdb->p_config_data->update_reqd = 
                  ICF_UPDATE_WHN_OFFER_SDP_WITH_MULTIPLE_CODEC_COMMIT;

    p_crm_pdb->p_config_data->early_inc_ind_for_all_users = ICF_FALSE;

    /*Rel 8.0:Check if the operation mode in config data is IMS then set
     *the bit ICF_CFG_CC_CALL_THRU_SIP_PROXY in default settings.
     */ 
    if(ICF_OPERATION_MODE_IMS == p_crm_pdb->p_config_data->ims_oprn_flag)
    {
	p_crm_pdb->p_config_data->default_settings |= 
    ...
    /* checking NULL value for p_arDnsClient_addr */
    if (ICF_NULL == p_crm_pdb->p_config_data->p_arDnsClient_addr)
    {
        /* Alocating memory for p_arDnsClient_addr of type
        ...
        ICF_MEMGET(p_crm_pdb->p_glb_pdb,
            ...
            &(p_crm_pdb->p_internal_msg->msg_hdr.ecode),
            ret_val); 
        /* copying structure into config_data */ 
        icf_port_memcpy(p_crm_pdb->p_config_data->p_arDnsClient_addr,
                    p_recv_msg_payload->p_arDnsClient_addr,
                    ICF_PORT_SIZEOF(icf_transport_address_st));

    }

    /* checking NULL value for p_arDnsClib_addr */    
    if (ICF_NULL == p_crm_pdb->p_config_data->p_arDnsClib_addr)
    {
        /* Alocating memory for p_arDnsClib_addr of type
           icf_transport_address_st */
        ICF_MEMGET(p_crm_pdb->p_glb_pdb,
            ICF_PORT_SIZEOF(icf_transport_address_st),
            ICF_MEM_COMMON,
        ...
        {
            /*Free the memory allocated to p_arDnsClient_addr*/
            ICF_MEMFREE(
                    p_crm_pdb->p_glb_pdb,
                    p_crm_pdb->p_config_data->p_arDnsClient_addr,
                    ICF_MEM_COMMON,
        ...
        {
            /* copying structure into config_data */
            icf_port_memcpy(p_crm_pdb->p_config_data->p_arDnsClib_addr,
                    p_recv_msg_payload->p_arDnsClib_addr,
                    ICF_PORT_SIZEOF(icf_transport_address_st));
        }
    }
        ...
        IN      icf_app_id_t app_id)
{
    ...
    if(ICF_NULL != p_msg)
    {
        ...
        {
             p_payload->bit_mask = p_payload->bit_mask | ICF_APP_ADD_CFM_ERROR_CAUSE_PRESENT;
        }
 ...
 ******************************************************************************/
     ...
     INOUT   icf_crm_pdb_st *p_crm_pdb)
 {
	 ...
	 else if(ICF_FAILURE != p_crm_pdb->result)
	 {
         ...
         /* Fix for SPR: 19886 (Merged CSR-1-7423906)*/
         /* Fix for CSR 1-7423906: SPR 19772 */         
         /* Fixes for Debian on big endian platform */
		 ...
		 else
		 {
			...
			*/
			 ...
			 */
              ...
              * p_payload->p_media_profile will be
              * freed under the code changes for Fix for SPR : 19001*/         
/*ICF REL 6.0*/
		    if((ICF_FAILURE != p_crm_pdb->result) &&
            ...
            &(p_crm_pdb->p_internal_msg->msg_hdr.ecode)))
        {
          ...
          /* ########## TIMERS ##################  */
                            ...
                            p_recv_msg_payload->bit_mask))    
                {
                    ...
                    to the response API's payload instead of hardcoded this to
                    ICF_FALSE.system_settings_resp must specify the value of
                    sip_compact_header as specified in icf.cfg **/
                                                         
                    p_payload->sip_compact_header =
                        p_crm_pdb->p_config_data->sip_compact_hdrs;
        ...
        INOUT   icf_crm_pdb_st *p_crm_pdb)
{
   ...
   if( (p_recv_msg_payload->line_id < ICF_MAX_NUM_OF_LINES) && (ICF_SUCCESS == icf_cfg_check_line_configured(p_crm_pdb,
        ...
        INOUT   icf_crm_pdb_st                  *p_crm_pdb)
{
    ...
    ICF_MEMGET(p_crm_pdb->p_glb_pdb,api_len,
            ...
            &(p_crm_pdb->p_internal_msg->msg_hdr.ecode)))
        {
            ...
            if((ICF_SUCCESS == p_crm_pdb->result) && 
				(p_recv_msg_payload->line_id < ICF_MAX_NUM_OF_LINES)) 
            {
               ...
               /* ######## Reg Duration ######## */
                ...
                /* ####### SIP T4 TIMER ####### */
                if (ICF_NULL != (ICF_REQD_SIP_T2_TIMER & \
                            p_recv_msg_payload->bit_mask))
                {
                    /** Copy the timer to the response API's payload. **/
                    p_payload->sip_T2_timer =\
                             p_crm_pdb->p_line_data\
                             [p_recv_msg_payload->line_id]->sip_timer_t2;

                    p_payload->bit_mask |= ICF_REQD_SIP_T2_TIMER;
                }

                /* ####### SIP T4 TIMER ####### */
                if (ICF_NULL != (ICF_REQD_SIP_T4_TIMER & \
                            p_recv_msg_payload->bit_mask))
                {
                    /** Copy the timer to the response API's payload. **/
                    p_payload->sip_T4_timer =\
                             p_crm_pdb->p_line_data\
                             [p_recv_msg_payload->line_id]->sip_timer_t4;

                    p_payload->bit_mask |= ICF_REQD_SIP_T4_TIMER;
                }
                ...
                if (ICF_NULL != (ICF_REQD_REG_ROUTE_ADDR & \
                            p_recv_msg_payload->bit_mask))    
                { 
                    /* Copy the proxy address structure to the response API's
                       payload. **/

                    if(p_crm_pdb->p_line_data[p_recv_msg_payload->line_id]->\
                            line_data_received & ICF_CFG_LINE_DATA_REG_ROUTE_ADDR_RECD)
                    {                    
                        icf_port_memcpy( 
                                (icf_void_t*)&(p_payload->\
                                               reg_route_addr), 
         ...
         (ICF_NULL != p_crm_pdb->p_config_data))  
     {
		for (line_index = 0; line_index < (p_crm_pdb->p_config_data->max_lines); line_index++)
		{
 					...
 					p_crm_pdb->p_ecode))
        	{
                ...
                if ( ICF_SET_LINE_PRM_T4_TIMER & 
                        p_recv_msg_payload->bitmask) 
                {
                    p_line_data->sip_timer_t4 = 
                        p_recv_msg_payload->sip_timer_t4;
                }
...
/*Changes for CSR_1-5671948*/
            if (p_recv_msg_payload->bitmask & ICF_STALE_PARAM_HANDLING_PRESENT)
            {
                p_line_data->ignore_stale_param =        
                    p_recv_msg_payload->ignore_stale_param;
            }
#ifdef ICF_SESSION_TIMER
            if (p_recv_msg_payload->bitmask & ICF_LINE_CONFIG_SESSION_REFRESH_METHOD_PRESENT)
            {
                p_line_data->session_refresh_method =
                    p_recv_msg_payload->session_refresh_method;
            }
#endif
                /*Changes for CSR_1-5671948*/
                if(ICF_SET_LINE_PRM_ACCESS_NW_INFO &
                        p_recv_msg_payload->bitmask)
                {
                    /*If Application has set the bitmask for the presence
                      of the access_network_info but string is null this 
                      means Application wants to delete the already configured
                      value for the header*/
                    if(0 == icf_port_strcmp(
                                (icf_uint8_t *)p_recv_msg_payload->access_network_info.str,
                                (icf_uint8_t *)""))
                    {
                        if(ICF_NULL != p_line_data->p_access_network_info)
                        {
                            ICF_MEMFREE(p_crm_pdb->p_glb_pdb,
                                    p_line_data->p_access_network_info,
                                    ICF_MEM_COMMON,
                                    p_crm_pdb->p_ecode, 
                                    ret_val)
                        }
                    ...
                    {
                        /*In case when the received access_network_info header
                          is non-null then check if already memory is allocated 
                          to access_network_info header or not. If yes then
                        ...
                        if(ICF_NULL != p_line_data->p_access_network_info)
                        {
                            icf_port_strcpy(p_line_data->p_access_network_info->str,
                                    p_recv_msg_payload->access_network_info.str);
                        }
                    }
                }/*End of if condition for ICF_SET_LINE_PRM_ACCESS_NW_INFO*/
                /*Fix for SPR: 19999 (Merged CSR 1-7547597 : SPR 19898)*/
                if (p_recv_msg_payload->bitmask &
                        ICF_SET_LINE_SESSION_REFRESH_METHOD_STRICT_UPDATE)
                {
                    p_line_data->session_refresh_method_strict_update = 
                        p_recv_msg_payload->session_refresh_method_strict_update;
                }              
                ...
                {
                    p_line_data->strip_req_uri_param = 
                        p_recv_msg_payload->strip_req_uri_param;
                } 
            }
        }
	}
              ...
              (ICF_TRUE == p_crm_pdb->p_config_data->session_timer_enable))
          {
           ...
           if (p_recv_msg_payload->bitmask &
            ...
            {  
                ...
                /*Klocwork Warning Removal*/
                ICF_KLOCWORK_FALSE_POSITIVE(p_crm_pdb->p_glb_pdb,
                      p_recv_msg_payload->p_codec_attributes_audio);
            }                    
            if (p_recv_msg_payload->bitmask &
                ICF_SET_LINE_VIDEO_CODEC)
            {  
                ...
                /*Klocwork Warning Removal*/
                ICF_KLOCWORK_FALSE_POSITIVE(p_crm_pdb->p_glb_pdb,
                        p_recv_msg_payload->p_codec_attributes_video);
...
)
{
    ...
    else if ( ICF_SET_LINE_PRM_T4_TIMER & p_recv_msg_payload->bitmask &&
              0 == p_recv_msg_payload->sip_timer_t4)
    {
        ICF_CFG_SET_VALID_PARAM(p_crm_pdb, ICF_FALSE)
        ICF_CFG_SET_RESPONSE(
                p_crm_pdb,
      ...
      (ICF_TRUE == p_crm_pdb->p_config_data->session_timer_enable))
 {
                    ...
                    p_recv_msg_payload->bitmask))
    {
        ...
        {
            /*SPR 19120 In case the SET_LINE_PARAM API does have only
 ...
 *                  from input string.
 *					p_str to be null terminated string
 *                  
...
icf_uint16_t icf_cfg_trim_string(icf_uint8_t   *p_str)
{
 ...
 if (ICF_NULL == p_str) return 0; 
 
 icf_port_memset(&temp_str.str,ICF_NULL,ICF_MAX_STR_LEN);
 temp_str.str_len = 0;
 
 while( (index < ICF_MAX_STR_LEN) && (*(p_str + index)))
 {
  ...
  if ( temp_index ) /* is not zero */
  {
  	icf_port_strncpy((icf_int8_t *)p_str, (icf_int8_t *)temp_str.str, temp_index );
  	temp_str.str_len = temp_index; /* (icf_uint16_t)icf_port_strlen(p_str);*/
  }
  
  if ( temp_index < index ) p_str[temp_index]=ICF_NULL; 
  /* note in case p_str is >= ICF_MAX_STR_LEN; then it is not initialised with null */
...
icf_return_t icf_cfg_get_register_status_req_hdlr(INOUT  icf_crm_pdb_st  *p_crm_pdb)
{
    ...
    icf_user_reg_status_st                 *temp = ICF_NULL;
        ...
        ICF_MEMGET(
                   ...
                   ret_val)
       /*Klocwork Warning Removal*/
       ICF_KLOCWORK_FALSE_POSITIVE(p_crm_pdb->p_glb_pdb,p_msg)
           ...
           while(ICF_NULL != p_user_db)
            {
                ...
                if (( ICF_FAILURE !=
                     ...
                     (ICF_RGM_00_IDLE == registration_state))
                 {
                   ...
                   ret_val)
                   /*Klocwork Warning Removal*/
                   ICF_KLOCWORK_FALSE_POSITIVE(p_crm_pdb->p_glb_pdb,temp)
...
icf_uint16_t icf_cfg_trim_string_for_filter_list(icf_uint8_t   *p_str)
{
 ...
 while((index < ICF_MAX_STR_LEN) && (*(p_str + index)))
 {
   ...
   {
     if (temp_index < ICF_MAX_STR_LEN) temp_str.str[temp_index] = *(p_str + index);
     temp_index++;
     index++;
   }
  }
        ...
        )
{
    ...
    ICF_CFG_APP_TRACE(p_crm_pdb, ICF_TRACE_PCSCF_UNREACHABLE)

    /*If proxy address was fetched through DHCP address then only do re-register
     * and clear calls*/
    if(ICF_NULL != p_crm_pdb->p_config_data->p_dhcp_resp)
    {
        /*Clear all calls through this proxy server */
        if (ICF_FAILURE == icf_cfg_cm_forceful_call_clear(p_crm_pdb))
        {
            ret_val = ICF_FAILURE;
        }
        /*Rediscover P-CSCF address thru DHCP query*/
        else if (ICF_FAILURE == icf_cfg_handle_dhcp_query(p_crm_pdb))
        {
            ret_val = ICF_FAILURE;
        }
        /*In case of IMS, re-register will be done again*/
        else if(ICF_TRUE == p_crm_pdb->p_config_data->ims_oprn_flag)
        {
            /*re-register all users, registered through this proxy server */
