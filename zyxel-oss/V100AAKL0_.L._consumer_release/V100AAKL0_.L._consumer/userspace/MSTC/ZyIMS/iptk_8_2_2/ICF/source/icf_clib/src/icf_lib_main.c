...
* 17-Apr-2008 Abhishek Dhammawat SPR 18459 Modified icf_free_decoded_msg
...
* 11-Feb-2009  Ashutosh Mohan  IPTK Rel8.1      Added following APIs
...
* 30-Mar-2009  Abhishek Dhammawat Rel 8.2       Additions for REFER
*                                               Enhancements
* 31-Mar-2009  Preksha            Rel 8.2       Additions for PRACK
*                                               Enhancements
* 26-May-2009  Preksha            Rel 8.2       Asynchronus Message 
*                                               enhancements
* 22-Jun-2009  Anuradha Gupta     SPR 19752     Modified icf_free_decoded_msg
* 09-Jul-2009  Anuradha Gupta     SPR 18948     Modified icf_encode_mesg, 
...

{
    ...
    {
        /* SPR 18948 : Call VLDT module to validate the message.
         * In case of error, return from the function and do not
         * encode the message
         */ 
    ...
    *p_encoded_buf_length = length;
    ...
    /* SPR 18948: In case length is non zero ie API has payload information , only then
     * copy the message in the OUT buffer else return as success with length as zero
     * This issue is encountered when the APIs like ICF_STOP_FAX_REQ, ICF_REINITIALISE_REQ
     * are issued which has no payload then no attempt should be made to copy the payload
     * buffer.
     */ 
    if(0 != length)
    {
...

{
    ...
    /* SPR 18948: Allocate the header buffer only, message buffer will be allocated
     * through the function icf_encode_api_payload. Here in this function, firstly 
     * the header part is encoded and then the function icf_encode_api_payload 
     * encodes the message payload part. The both the buffers will be copied into the
     * OUT buffer p_encoded_buf
     */ 
    p_hdr_buff = (icf_uint8_t*)ICF_CLIB_MEMGET(ICF_API_HEADER_LENGTH)

	if (!p_hdr_buff) {
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: ICF_CLIB_MEMGET Failure!! (p_hdr_buff)\n"));
		*p_ecode = ICF_ERR_MEMGET_FAILED;
		return ICF_FAILURE;
	}
	...
	ret_val = (icf_uint8_t)icf_pack_icf_api_header_st(p_hdr_buff, p_src, &length, &api_length_index);

	hdr_length = length;
		
    if (ret_val == ICF_FAILURE) {
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_encode_msg - ENCODING Failed While Packing API Header!!!\n"));
	    ICF_CLIB_MEMFREE(p_hdr_buff)
		*p_ecode = ICF_ERR_ENCODE_FAILED;
        return ret_val;
    }
...
/* ZyXEL porting end */
    ...
    ret_val = icf_encode_api_payload(p_msg, /* Input*/
							&p_msg_buff, /* Output*/
							&msg_buff_length,
							p_ecode,
                            p_vldt_err);
	...
	api_length_value = (icf_uint16_t)(msg_buff_length + ICF_API_HEADER_LENGTH);

	icf_pack_U16(p_hdr_buff + api_length_index, (icf_uint8_t*)&api_length_value, &hdr_length);

	*p_encoded_buf_length = api_length_value;
    ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_encode_msg - Getting Memory for p_p_encoded_buf\n"));
	*p_p_encoded_buf = (icf_uint8_t*)ICF_CLIB_MEMGET(api_length_value);

	if (!*p_p_encoded_buf) {
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: ICF_CLIB_MEMGET Failure! (*p_p_encoded_buf)\n"));
	    ICF_CLIB_MEMFREE(p_hdr_buff)
	    ICF_CLIB_MEMFREE(p_msg_buff)
		*p_ecode = ICF_ERR_MEMGET_FAILED;
		return ICF_FAILURE;
	}
	...
	ICF_CLIB_MEMCPY(*p_p_encoded_buf, p_hdr_buff, ICF_API_HEADER_LENGTH)
    ...
    {    
        /* Copy the rest of the encoded payload, move past the
           encoded API Header*/
        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_encode_msg - MEMCopying Payload to encoded buffer\n"));
        ICF_CLIB_MEMCPY(*p_p_encoded_buf + ICF_API_HEADER_LENGTH, p_msg_buff, msg_buff_length)

        ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_encode_msg - FREEing p_msg_buff\n"));
        ICF_CLIB_MEMFREE(p_msg_buff)
    }

    ICF_CLIB_TRACE(("\n[ICF_CLIB]: icf_encode_msg - FREEing p_hdr_buff\n"));
	ICF_CLIB_MEMFREE(p_hdr_buff)
...
/* ICF_6_1_OPTIONS_SUPPORT end */
   ...
   /*  Changes for REL 7.0 XCONF */
    ...
    case ICF_SESSION_TIMER_TOGGLE_REQ:			case ICF_SESSION_TIMER_TOGGLE_RESP:
#endif
		ICF_CLIB_MEMFREE(p_msg);
		break;
...
/*ICF_HANDLE_REINVITE_END*/
    ...
    case ICF_AKA_AUTH_RESP:										
        ret_val = (icf_uint8_t)icf_pack_icf_aka_auth_resp_st(p_dest, p_src, &length);
        break;
...
/*	case ICF_REGISTER_IND:										*/
	...
	case ICF_REFER_RESP:
		...
		ret_val = (icf_uint8_t)icf_pack_icf_display_ind_st(p_dest, p_src, &length);
		break;
...
/*call_modify_cancel*/
		...
		case ICF_REMOTE_USER_TERMINATE_CALL_RESP:
                /*  SPR 18064 Merge */
                     ret_val = (icf_uint8_t)icf_pack_icf_remote_user_terminate_call_resp_st(
                                               p_dest, p_src, &length);
...
/* ICF_6_1_OPTIONS_SUPPORT end */
	...
	case ICF_REINITIALISE_REQ:
        ...
        case ICF_UT_SIMULATE:
             ret_val = (icf_uint8_t)icf_pack_icf_ut_simulate_st(p_dest, p_src, 
			&length); 
             break;
        
        case ICF_UATK_API_TEST:
             ret_val = (icf_uint8_t)icf_pack_icf_uatk_api_test_st(p_dest, p_src, 
                     &length); 
             break;
...
#ifdef ICF_DNS_LOOKUP_ENABLED
		case ICF_DNS_MAPPING_CHANGE_IND:
			ret_val = (icf_uint8_t)icf_pack_icf_dns_mapping_change_ind_st(
										p_dest, p_src, &length);
            break;
#endif /* ICF_DNS_LOOKUP_ENABLED */ 

        case ICF_EARLY_SESSION_IND:
                ret_val = (icf_uint8_t)icf_pack_icf_early_session_ind_st(p_dest, p_src, &length);
                break;


        case ICF_EARLY_SESSION_CFM:
                ret_val = (icf_uint8_t)icf_pack_icf_early_session_cfm_st(p_dest, p_src, &length);
                break;


        case ICF_TUNNELLED_OPTIONS_RESP:
                ret_val = (icf_uint8_t)icf_pack_icf_tunnelled_options_resp_st(p_dest, p_src, &length);
                break;
#ifdef ICF_SESSION_TIMER
        case ICF_SESSION_TIMER_TOGGLE_REQ:
                ret_val = (icf_uint8_t)icf_pack_icf_session_timer_toggle_req_st(p_dest, p_src, &length);
                break;
            
        case ICF_SESSION_TIMER_TOGGLE_RESP:
                ret_val = (icf_uint8_t)icf_pack_icf_session_timer_toggle_resp_st(p_dest, p_src, &length);
                break;
#endif
                /*Change Start for Rel 8.2*/
        case ICF_GENERIC_MSG_REQ:
                ret_val = (icf_uint8_t)icf_pack_icf_generic_msg_req_st(p_dest, p_src, &length);
                break;
        case ICF_GENERIC_MSG_RESP:
                ret_val = (icf_uint8_t)icf_pack_icf_generic_msg_resp_st(p_dest, p_src, &length);
                break;
        case ICF_GENERIC_MSG_IND:
                ret_val = (icf_uint8_t)icf_pack_icf_generic_msg_ind_st(p_dest, p_src, &length);
                break;
        case ICF_GENERIC_MSG_CFM:
                ret_val = (icf_uint8_t)icf_pack_icf_generic_msg_cfm_st(p_dest, p_src, &length);
                break;
        case ICF_PRACK_REQ_RECV_IND:
                ret_val = (icf_uint8_t)icf_pack_icf_prack_req_recv_ind_st(p_dest, p_src, &length);
                break;
        case ICF_PRACK_RESP_RECV_IND:
                ret_val = (icf_uint8_t)icf_pack_icf_prack_resp_recv_ind_st(p_dest, p_src, &length);
                break;
        case ICF_PRACK_REQ_SENT_IND:
                ret_val = (icf_uint8_t)icf_pack_icf_prack_req_sent_ind_st(p_dest, p_src, &length);
                break;
        case ICF_PRACK_RESP_SENT_IND:
                ret_val = (icf_uint8_t)icf_pack_icf_prack_resp_sent_ind_st(p_dest, p_src, &length);
                break;
                /*Change End For Rel 8.2 */

        case ICF_MESSAGE_CFM:
                ret_val = (icf_uint8_t)icf_pack_icf_app_confirmation_st(p_dest, p_src, &length);
                break;

        /* SPR 18948: Added case for Stop Fax Req as it is without payload therefore in case
         * of absence of switch case statement, it will go to default rule and then will
         * be treated as MM API subsequently.
         */         
...
)
{
	...
	case ICF_TERMINATE_CALL_REQ:
        ...
        /*  SPR 18064 Merge */
        case ICF_REMOTE_USER_TERMINATE_CALL_RESP:
                ret_val = (icf_uint8_t)icf_unpack_icf_remote_user_terminate_call_resp_st(p_dest, 
                          p_src, &length);
            break;
	case ICF_CONNECT_REQ:
...
/*ICF_HANDLE_REINVITE_END*/
	...
	case ICF_AKA_AUTH_RESP:										
        ret_val = (icf_uint8_t)icf_unpack_icf_aka_auth_resp_st(p_dest, p_src, &length);
        break;
...
#endif
        case ICF_DISPLAY_IND:								
		ret_val = (icf_uint8_t)icf_unpack_icf_display_ind_st(p_dest, p_src, &length);
		break;
...
#ifdef ICF_UT_TEST
        case ICF_UT_SIMULATE:
             ret_val = (icf_uint8_t)icf_unpack_icf_ut_simulate_st (p_dest,
                     p_src, &length);
             break;
        
        case ICF_UATK_API_TEST:
             ret_val = (icf_uint8_t)icf_unpack_icf_uatk_api_test_st (p_dest,
                     p_src, &length);
             break;
#endif
...
/* ICF_6_1_OPTIONS_SUPPORT end */
...
#ifdef ICF_DNS_LOOKUP_ENABLED
		case ICF_DNS_MAPPING_CHANGE_IND:
        	ret_val = (icf_uint8_t)icf_unpack_icf_dns_mapping_change_ind_st(
									p_dest, p_src, &length);
            break;
#endif /* ICF_DNS_LOOKUP_ENABLED */
        ...
        case ICF_EARLY_SESSION_IND:
                ret_val = (icf_uint8_t)icf_unpack_icf_early_session_ind_st(p_dest, p_src, &length);
                break;


        case ICF_EARLY_SESSION_CFM:
                ret_val = (icf_uint8_t)icf_unpack_icf_early_session_cfm_st(p_dest, p_src, &length);
                break;


        case ICF_TUNNELLED_OPTIONS_RESP:
                ret_val = (icf_uint8_t)icf_unpack_icf_tunnelled_options_resp_st(p_dest, p_src, &length);
                break;

       case ICF_SESSION_TIMER_TOGGLE_REQ:
                ret_val = (icf_uint8_t)icf_unpack_icf_session_timer_toggle_req_st(p_dest, p_src, &length);
                break;
            
        case ICF_SESSION_TIMER_TOGGLE_RESP:
                ret_val = (icf_uint8_t)icf_unpack_icf_session_timer_toggle_resp_st(p_dest, p_src, &length);
                break;
                /*Change Start For Rel 8.2*/
       case ICF_GENERIC_MSG_REQ:
                ret_val = (icf_uint8_t)icf_unpack_icf_generic_msg_req_st(p_dest, p_src, &length);
                break;
       case ICF_GENERIC_MSG_RESP:
                ret_val = (icf_uint8_t)icf_unpack_icf_generic_msg_resp_st(p_dest, p_src, &length);
                break;
       case ICF_GENERIC_MSG_IND:
                ret_val = (icf_uint8_t)icf_unpack_icf_generic_msg_ind_st(p_dest, p_src, &length);
                break;
       case ICF_GENERIC_MSG_CFM:
                ret_val = (icf_uint8_t)icf_unpack_icf_generic_msg_cfm_st(p_dest, p_src, &length);
                break;

       case ICF_PRACK_REQ_RECV_IND:
                ret_val = (icf_uint8_t)icf_unpack_icf_prack_req_recv_ind_st(p_dest, p_src, &length);
                break;
       case ICF_PRACK_RESP_RECV_IND:
                ret_val = (icf_uint8_t)icf_unpack_icf_prack_resp_recv_ind_st(p_dest, p_src, &length);
                break;
       case ICF_PRACK_REQ_SENT_IND:
                ret_val = (icf_uint8_t)icf_unpack_icf_prack_req_sent_ind_st(p_dest, p_src, &length);
                break;
       case ICF_PRACK_RESP_SENT_IND:
                ret_val = (icf_uint8_t)icf_unpack_icf_prack_resp_sent_ind_st(p_dest, p_src, &length);
                break;

                /*Change End for Rel 8.2*/

       case ICF_MESSAGE_CFM:
                ret_val = (icf_uint8_t)icf_unpack_icf_app_confirmation_st(p_dest, p_src, &length);
                break;
