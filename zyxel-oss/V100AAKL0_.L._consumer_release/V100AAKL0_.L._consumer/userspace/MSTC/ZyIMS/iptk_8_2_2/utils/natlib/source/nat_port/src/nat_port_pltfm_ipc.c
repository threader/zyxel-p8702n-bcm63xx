...
* 21-Aug-2009  	Rajiv Kumar                     SPR 19944   Fix for SPR: 19944
* 21-Aug-2009   Anuradha Gupta                 SPR 19935    GCC warning removal
*
                     ...
                     nat_error_t    *p_ecode)
{
    ...
    nat_uint32_t              source_addr_len = sizeof (struct sockaddr_in);
#else
	struct sockaddr_storage source_addr;
	nat_uint32_t source_addr_len = sizeof(struct sockaddr_storage);
#endif
        ...
        IN      nat_uint32_t         fd)
{
    ...
    if (0 != getpeername(fd, (struct sockaddr*)&address, (nat_int32_t *)&addr_len))
#else
        ...
        INOUT   nat_uint8_t*    p_addr)
{
    ...
    /*Fix for SPR: 19944*/
#if defined (NAT_PORT_VXWORKS)
    /* As Vxwork 5.4 doesnt support gethostbyname; following code is added for 
     * gethostbyname support on Vxwork 5.4 */
    struct in_addr  inaddr;
    struct in_addr  inetAddress;
    int internet_addr = 0, network_addr = 0, host_addr = 0;
    
    inet_aton((nat_uint8_t *)p_hostname,&inetAddress);
    internet_addr = hostGetByName((nat_uint8_t *)p_hostname);
    host_addr = inet_lnaof(internet_addr);
    network_addr = inet_netof(inetAddress);
    inet_makeaddr_b(network_addr, host_addr,&inaddr);

    nat_port_strcpy(p_addr,(nat_uint8_t*) inet_ntoa(inaddr));
...
#ifndef NAT_IPV6_ENABLED
    struct sockaddr_in  source_addr;
    nat_uint32_t      source_addr_len = sizeof (struct sockaddr);
#else
	struct sockaddr_storage  source_addr;
    nat_uint32_t      source_addr_len = sizeof (struct sockaddr_storage);
#endif
