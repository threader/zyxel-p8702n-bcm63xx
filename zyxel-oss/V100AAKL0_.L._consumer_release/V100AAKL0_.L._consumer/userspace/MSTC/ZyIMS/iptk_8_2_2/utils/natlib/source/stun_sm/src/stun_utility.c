 ...
 *  02-Mar-2009     Alok Tiwari         IPTK Rel8.0   IPTK open source replacement
 *                                                    from IPTK:invokation of
 *                                                    stun_enc_dec APIs replaced     
 *                                                    with arStunEncDec APIs.
 *  15-Jul-2009     Tarun Gupta         SPR 19854     Modified stun_stop_current_timers,
 *                                                    stun_start_timer and stun_stop_timer.
...
{
    nat_return_t        ret_val = NAT_RETURN_SUCCESS;
    nat_timer_id_t      timer_id = NAT_INVALID_TIMER_ID;
    nat_uint8_t         ctr = 0;
		...
		INOUT stun_pdb_st             *p_stun_pdb)
{
	...
	arStun_msg_st          stun_message;
    arStun_ecode_et        error = ARSTUN_ERROR_START;
    ...
    if(UTIL_FAILURE == arStun_create_binding_req(&stun_message,
                      (arStun_string_st *)
                      ...
                      (arStun_ip_port_address_st *) 
                        &(p_public_ip_port_req->response_addr), &error))
    {
        ret_val  = NAT_RETURN_FAILURE;
        return ret_val;
    }
    else
    {
        ...
        if(UTIL_FAILURE == arStun_encode_message(&stun_message,
                           (arStun_string_st*)
                             &(p_stun_pdb->p_stun_context->shared_secret_password),
                           (arStun_msg_buffer_st*)p_buffer, &error))
        {
		...
		INOUT stun_pdb_st             *p_stun_pdb)
{
   ...
   if(UTIL_FAILURE == arStun_create_binding_req(&stun_message,
              (arStun_string_st *)
                &(p_stun_pdb->p_stun_context->shared_secret_username),
              NAT_FALSE,NAT_FALSE,
              (arStun_ip_port_address_st *)&response_addr, &error))
    {
        ret_val  = NAT_RETURN_FAILURE;
        return ret_val;
    }
