 ...
 * ****************************************************************************/
#ifndef __STUN_SHSEC_MACRO_H__
#define __STUN_SHSEC_MACRO_H__
...
{\
    nat_port_memset(p_stun_shsec_context,0,sizeof(stun_shsec_context_st));\
...
/*------------------------------------------------------------------------------
 * 
 * Macro name: NAT_STUN_SHSEC_SET_STATE
 * 
 * Description: This is used to set the current state in the mmi context.
 * 
 *----------------------------------------------------------------------------*/ 
#define NAT_STUN_SET_STATE(p_stun_shsec_pdb,state)\
{\
    p_stun_shsec_pdb->p_stun_shsec_context->current_state = state;\
}  

/*------------------------------------------------------------------------------
 * 
 * Macro name: NAT_STUN_SHSEC_SET_EVENT
 * 
 * Description: This is used to set the event in the mmi context.
 * 
 *----------------------------------------------------------------------------*/

#define NAT_STUN_SHSEC_SET_EVENT(p_stun_shsec_pdb,event)\
{\
p_stun_shsec_pdb->p_stun_shsec_context->current_event = event;\
}        
    
/*------------------------------------------------------------------------------
 * 
 * Macro name: NAT_STUN_SHSEC_SET_ECODE
 * 
 * Description: This is the macro used for setting ecodes.
 * 
 *----------------------------------------------------------------------------*/
#define NAT_STUN_SHSEC_SET_ECODE(p_stun_shsec_pdb,error)\
{\
    *(p_stun_shsec_pdb->->p_ecode) = (nat_error_t)(error);\
}
