 ...
 ** 29-DEC-09      Anurag Khare         Fix for SPR 20204
 **
 *****************************************************************************
...
iptkapp_core_remove_type_from_addr(icf_uint8_t *p_addr)
{
    ...
    /*  Fix for SPR 20204 */
    /*  Code is removed because when sip:abc@aricent.com is received as called
     *  party address then this logic is not settinf the pointer p_mem_pointer
     *  at appropriate location. idealy this pointer must be located at ':'
     *  but due to length 3 logic the ponter was not reaching to ':' that is
     *  wrong due to that Sample application tries to send the called partye
     *  address string with colon and vldt fails.
     */
#if 0
    {
        len ++;
