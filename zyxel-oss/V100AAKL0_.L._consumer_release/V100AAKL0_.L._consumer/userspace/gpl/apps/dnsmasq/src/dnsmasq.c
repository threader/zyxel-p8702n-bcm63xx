...
#ifdef MSTC_SUPPORT_DNSROUTE /*__MSTC__, DNS ROUTE*/
#define MAX_DNS_ROUTE_ENTRY 20
DNSRoute_Entry_t DNSRoute[MAX_DNS_ROUTE_ENTRY];
int num_entry = 0, num_wan = 0;
#define DNS_GW_FILE "/var/dnsmasq/dnsgw"
#define DNS_ROUTE_ENTRY_FILE "/var/dnsmasq/dnsRouteTable"

#define MAX_WAN_IFC_NUM 8
#define IFC_STR_LEN 32
#define IP_ADDR_STR_LEN 32
#define DNS_SERVERS_NUM 8
#define BUFLEN_256 256
/* Load Domain Name table and related WAN Interface*/
//#define DNSROUTE_DEBUG
int load_domain_name_table(void){

	FILE * fp;
	char line[BUFLEN_256];
	int i=0, k=0, j=0;	
	char *token = NULL, *tmp = NULL, *tmp2 = NULL;
	FILE *fp2;
	char *token2=NULL, *second = NULL;
	char dns_primary[MAX_WAN_IFC_NUM][IP_ADDR_STR_LEN];	
	char dns_secondary[MAX_WAN_IFC_NUM][IP_ADDR_STR_LEN];
	char dns_ifc[MAX_WAN_IFC_NUM][IFC_STR_LEN];
	char gw[MAX_WAN_IFC_NUM][IP_ADDR_STR_LEN];

	if((fp = fopen(DNS_GW_FILE, "r")) == NULL){		
		//printf("DNSMASQ: fail to read %s\n", DNS_GW_FILE);
		return 0;
	}
	while(fgets(line, sizeof(line), fp)!=NULL){ /*format: WANIfName:Gateway:dnserver1,dnserver2*/			
			/*trim newline*/
			if(line[strlen(line)-1]=='\n'){
				line[strlen(line)-1] = '\0';
			}
			#ifdef DNSROUTE_DEBUG
			fprintf(stderr, "DNSROUTE: reading wan info %d\n", i);
			#endif
			token = strtok_r(line, ":", &tmp);
			if(NULL!=token){
				snprintf(dns_ifc[i], IFC_STR_LEN-1, "%s", token);			
				if((token = strtok_r(NULL, ":", &tmp))!=NULL){
					snprintf(gw[i], IP_ADDR_STR_LEN-1, "%s", token);
					if( (token = strtok_r(NULL, ":", &tmp)) != NULL){
						for(token2 = strtok_r(token, ",", &tmp2); token2; token2 = strtok_r(NULL, ",", &tmp2), j++){
							if(j==0){
								snprintf(dns_primary[i], IP_ADDR_STR_LEN-1, "%s", token2);
							}
							else if(j==2){
								snprintf(dns_secondary[i], IP_ADDR_STR_LEN-1, "%s", token2);
							}
							else{
								break;
							}
			...
			memset(line, 0, BUFLEN_256);
			
			i++;
			if(i>=MAX_WAN_IFC_NUM){
				fprintf(stderr, "exceed max %d WANs\n", MAX_WAN_IFC_NUM);
				break;
			}
	}	
	...
	i=0;
	if((fp2 = fopen(DNS_ROUTE_ENTRY_FILE, "r")) == NULL){
		#ifdef DNSROUTE_DEBUG
		printf("fail to read %s\n", DNS_ROUTE_ENTRY_FILE);
		#endif
		return 0;
	}
	...
	while(fgets(line, sizeof(line), fp2)!=NULL){		
		...
		tmp2 = NULL;
		if((token2 = strtok_r(line, ":", &tmp2))!=NULL){
			#ifdef DNSROUTE_DEBUG
			fprintf(stderr, "DNSROUTE: reading dns route entry %d\n", i);
			#endif
			snprintf(DNSRoute[i].domainName, DNSROUTE_DOMAIN_NAME_LEN-1, "%s", token2);
			if((token2 = strtok_r(NULL, ":", &tmp2))!=NULL){
				snprintf(DNSRoute[i].ifcName, DNSROUTE_IFCNAME_LEN-1, "%s", token2);
				if((token2 = strtok_r(NULL, ":", &tmp2))!=NULL){
					snprintf(DNSRoute[i].subnetMask, DNSROUTE_SUBNETMASK_LEN-1, "%s", token2);
				}
			}	
			
			i++;
			if(i>=MAX_DNS_ROUTE_ENTRY){
				fprintf(stderr, "exceed max %d entries\n", MAX_DNS_ROUTE_ENTRY);
				break;
			}
		}
		memset(line, 0, BUFLEN_256);
	...
	for(i=0;i<num_entry;i++){
		for(k=0;k<num_wan;k++){
			if( !strcmp(DNSRoute[i].ifcName, dns_ifc[k])){
				snprintf(DNSRoute[i].primary,DNSROUTE_DNS_LEN-1 , dns_primary[k]);
				snprintf(DNSRoute[i].secondary,DNSROUTE_DNS_LEN-1, dns_secondary[k]);
				snprintf(DNSRoute[i].gateway, DNSROUTE_GW_LEN-1, gw[k]);
				break;
			}
		}		
	}
...
#if 0
int lookup_domain_name_table(char *cname){
	int i=0;
	int found=0;
	...
	if(found==1){		
		my_syslog(LOG_DEBUG, "###match table entry %d = [%s] !!\n", i,  DNSRoute[i].domainName);
		return 1;
	}else{
		my_syslog(LOG_DEBUG, "###No match table entry [%s] in Domain Name Table!!\n##", cname);
		return 0;
	}
}
#endif
...
void addDnsRoute(uint32_t s_addr, const char* cname){
	int i=0;
	int found=0;
	char cmd[128] = {0};
	char *p1=NULL, *p2=NULL, *ptmp = NULL;
	int mask=0;
	...
	if(s_addr == 0){
		#ifdef DNSROUTE_DEBUG
		fprintf(stderr, "DNSROUTE(%s): no A record is found for this cname %s\n", __FUNCTION__, cname);
		#endif
		return;
	}
	...
	for(i=0;i<num_entry;i++){
		#ifdef DNSROUTE_DEBUG
		fprintf(stderr, "DNSROUTE(%s): query %s, DNSRoute %s\n", __FUNCTION__, cname, DNSRoute[i].domainName);
		#endif
		if((p1=strstr(DNSRoute[i].domainName, "*.")) != NULL){
			p1+=1; //move to after *
			#ifdef DNSROUTE_DEBUG
			fprintf(stderr, "DNSROUTE: find pattern %s\n", p1);
			#endif
			if(strlen(p1)>0){
				ptmp = cname;
				while( (p2 = strstr(ptmp, p1)) != NULL){
		...
		else if(strcmp(cname, DNSRoute[i].domainName) == 0){
			found = 1;
			break;
		}
	}
	...
	if(found==1){
		my_syslog(LOG_DEBUG, "###match table entry %d = [%s] !!\n", i,	DNSRoute[i].domainName);
		fprintf(stderr, "###match table entry %d = [%s] !!\n", i,	DNSRoute[i].domainName);
		...
		if(strlen(DNSRoute[i].gateway)){
			sprintf(cmd, "ip route replace %s/%d via %s dev %s", inet_ntoa(srcIP), mask, DNSRoute[i].gateway, DNSRoute[i].ifcName);				
			fprintf(stderr, "ip route replace %s/%d via %s dev %s\n", inet_ntoa(srcIP), mask, DNSRoute[i].gateway, DNSRoute[i].ifcName);
			my_syslog(LOG_DEBUG, "###cmd> %s\n", cmd);
			system(cmd);
		}
		else{
			my_syslog(LOG_DEBUG, "###gateway not exist for interface %s\n", DNSRoute[i].ifcName);
			fprintf(stderr, "###gateway not exist for interface %s\n", DNSRoute[i].ifcName);
		}
	}
}
...
int main (int argc, char **argv)
{
...
#ifdef MSTC_SUPPORT_DNSROUTE
  load_domain_name_table();
#endif
  ...
  if (!(daemon->options & OPT_DEBUG) && getuid() == 0)   
	...
	{     
	  ...
	  /* finally drop root */
	  #ifndef MSTC_SUPPORT_DNSROUTE //__MSTC__, need permission to add route, so prevent setuid
