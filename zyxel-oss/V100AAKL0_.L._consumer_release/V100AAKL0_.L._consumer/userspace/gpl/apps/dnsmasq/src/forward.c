			 ...
			 HEADER *header, size_t plen, time_t now, struct frec *forward)
{
  ...
  int forwardAllDnsServer = 1;
#ifdef MSTC_DNS_ROUND_ROBIN
  time_t now_time;
  char diff_buf[8];  
  int diff=0;
#endif
  ...
  if (!daemon->servers)
      ...
      if (!(daemon->options & OPT_ORDER))
	{
	  ...
	  sprintf(diff_buf, "%f", difftime(now_time, forward->record_time ) );
	  diff = atoi(diff_buf);  

	  if ( diff >= 4 || diff <= -4  ){	 
		  forward->record_time = dnsmasq_time();
...
#if 1 /* Fix route via WAN Interface fail that there are more than 1 DNS assigned, ZyXEL Vincent */
          forward->forwardall = forwardAllDnsServer;  
#endif
	  }
  ...
  if (!flags && forward)
    {
      ...
      int forwarded = 0;
#ifdef HAVE_IPV6 //DingRuei, if ipv6 don't be found, check ipv4
      int firstRound = 1;
#endif
	...
	{ 
	  /* only send to servers dealing with our domain.
	     ...
	     must be NULL also. */
	  #if 0 /*Eason, AAAA type forward to ipv6 dns server*/
#ifdef HAVE_IPV6 //DingRuei, if ipv6 don't be found, check ipv4
	  if( (gotname == F_IPV6) && (start->addr.sa.sa_family != AF_INET6) && firstRound){
#else
	  if( (gotname == F_IPV6) && (start->addr.sa.sa_family != AF_INET6)){
#endif
	  	//my_syslog(LOG_DEBUG, "%s, v6 query skip v4 server", __FUNCTION__);	  	
	  	goto nextSrv;	  	
	  }
	  #endif
...
#if 1 /*Eason, AAAA type forward to ipv6 dns server*/ 
nextSrv:	  
#endif
	  if (!(start = start->next))
 	    start = daemon->servers;
